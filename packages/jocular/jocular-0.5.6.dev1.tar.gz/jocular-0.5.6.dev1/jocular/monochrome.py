...
class Monochrome(Component, Settings):
    ...
    unsharp_radius = NumericProperty(0)
    ...
    configurables = [
        ...
        # ('unsharp_radius', {
        #     'name': 'unsharp radius in pixels', 'float': (0, 20, .5),
        #     'help': 'radius of Gaussian used for unsharp masking (pixels)'}),
        # ('unsharp_amount', {
        #     'name': 'unsharp amount', 'float': (0, 3, .1),
        #     'help': 'original + amount * (original - blurred)'})
        ]

    save_settings = [
        'gradient', 'white', 'black', 'p1', 'lift', 'fracbin', 'TNR_param', 'unsharp_radius', 
    ...
    def on_new_object(self):
        ...
        self.gui.set('unsharp_radius', -0.1, update_property=True)
        self.gui.set('unsharp_amount', 0.5, update_property=True)

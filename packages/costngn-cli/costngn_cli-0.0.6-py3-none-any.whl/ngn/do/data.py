...
def do_main(nick, ce_enable:bool):
    ...
    for droplet in droplets['droplets']:
        ...
        do_drop['avzone']= droplet['size']['regions']
        ...
        do_reg['inst'].append(do_drop)
        do_rep['regions'].append(do_reg)
    ...
    report_file=company.upper()+ date.today().strftime(" %Y %m %d.json") 
    aws_json = json.dumps(do_rep,indent=2)
    with open(report_file, 'w') as outfile:
        outfile.write(aws_json)
    
    print(f'Saved to {report_file} in current folder, good bye!')

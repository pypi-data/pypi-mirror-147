...
GPR_ATTRIBUTE_SUFFIX_TRANSLATES = {
    ...
    GPRAttributeSuffix.IS_AVERAGE: ' is Average'
}
...
class GPR:
    ...
    def _shorten_terminals(terminals: List[str]):
        ...
        for attr, suffixes in attr_suffixes.items():
            ...
            if GPRAttributeSuffix.IS_LOW in suffixes and GPRAttributeSuffix.IS_HIGH in suffixes:
                shortened_terminals.append(f'{attr}{GPRAttributeSuffix.IS_AVERAGE}')
            elif len(suffixes) > 1:
    ...
    def rules(self) -> List[str]:
        ...
        for g in self._best_fit:
            ...
            accuracy = accuracy_score(self.sample_labels, (prediction_values > self.threshold).astype(int))
            ...
            rule = f'IF {joined} THEN {then_class_name} | Support: {support:.4f}, Accuracy: {accuracy:.4f}'
            rules.append(rule)

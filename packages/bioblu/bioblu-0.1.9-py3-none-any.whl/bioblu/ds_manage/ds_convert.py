...
def create_coco_json_from_yolo_set(yolo_root_dir: str, coco_root_dir: str, ds_source_set: str, target_save_file: str, materials_dict: dict = None):
    """
    ...
    img_src_paths = [os.path.join(img_dir, fname) for fname in sorted(os.listdir(img_dir))]
    img_target_paths = [os.path.join(coco_root_dir, ds_source_set, fname) for fname in sorted(os.listdir(img_dir))]
    labels_paths = [os.path.join(labels_dir, fname) for fname in os.listdir(labels_dir)]
    assert len(img_src_paths) == len(labels_paths)
    ...
    box_id_counter = 0
    for i, (img_src_path, label_path, img_target_path) in enumerate(zip(img_src_paths, labels_paths, img_target_paths)):
        coco_dict["images"].append({"file_name": img_target_path,
...
def convert_yolo_to_coco(yolo_root_dir: str, coco_target_dir: str = None, materials_dict: dict = None) -> None:
    """
...
if __name__ == "__main__":
    ...
    convert_yolo_to_coco(fdir_mini_gnejna, materials_dict={0: "trash"})

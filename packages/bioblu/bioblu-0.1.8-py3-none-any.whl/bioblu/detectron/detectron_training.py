...
if __name__ == "__main__":
    ...
    yolo_ds_root_dir = "/opt/nfs/shared/scratch/bioblu/datasets/dataset_04_gnejna_with_duplicates"
    ...
    ds_convert.convert_yolo_to_coco(yolo_ds_root_dir, coco_ds_root_dir, materials_dict)
    ...
    MetadataCatalog.get("trash_valid").set(thing_classes=classes)
    # MetadataCatalog.get("trash_test").set(thing_classes=classes)
    metadata_train = MetadataCatalog.get("trash_train")
    metadata_valid = MetadataCatalog.get("trash_valid")
    logging.info("Metadata registered.")

    cfg.DATASETS.TRAIN = ("trash_train",)
    cfg.DATASETS.TEST = ("trash_valid",)
    logging.info("Added TRAIN and TEST cfg.DATASET attributres.")

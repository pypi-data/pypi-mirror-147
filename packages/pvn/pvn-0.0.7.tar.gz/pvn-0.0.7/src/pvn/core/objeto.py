...
class Objeto():
  ...
  def gravarArquivo(self, **atributos):
    def acao(arquivo, dados):
      ...
      try:
        ...
        retorno = self.mensagemErro(mensagem = 'Falha ao gravar o arquivo {0}.'.format(arquivo), detalhe = traceback.format_exc(), codigo = 'ARQ-0007')
    ...
    if atributos.get('sincronizado', False):
      ...
      try:
        l.acquire()
        retorno = acao(atributos.get('arquivo'), atributos.get('dados'))
      except Exception as e:
        retorno = self.mensagemErro(mensagem = 'Falha ao solicitar bloqueio do arquivo.', codigo = 'ARQ-0013')
      finally:
        if l.locked():
          l.release()
    else:
  ...
  def removerArquivo(self, **atributos):
    def acao(arquivo):
      retorno = None
      ...
      try:
        ...
        retorno = self.mensagemSucesso(mensagem = 'Arquivo removido com sucesso.')
      except Exception as e:
        retorno = self.mensagemErro(mensagem = 'Falha ao remover o arquivo {0}.'.format(arquivo), detalhe = traceback.format_exc(), codigo = 'ARQ-0010')
    ...
    if arquivo:
      if os.path.exists(arquivo):
        if atributos.get('sincronizado', False):
          ...
          try:
            l.acquire()
            retorno = acao(arquivo)
          except Exception as e:
            retorno = self.mensagemErro(mensagem = 'Falha ao solicitar lock para remover o arquivo.', detalhe = traceback.format_exc(), codigo = 'ARQ-0011')
          finally:
            if l.locked():
              l.release()
        else:
        ...
        if atributos.get('ignorarArquivoInexistente', True):
          retorno = self.mensagemSucesso(mensagem = 'Arquivo {0} nao encontrado.'.format(arquivo))
        else:
          retorno = self.mensagemErro(mensagem = 'Arquivo {0} nao encontrado.'.format(arquivo), codigo = 'ARQ-0008')
    else:
      retorno = self.mensagemErro(mensagem = 'Nao informado o arquivo a ser removido.', codigo = 'ARQ-0009')

    return retorno

  def formatarData(self, **atributos):
    data = atributos.get('data', datetime.datetime.now())
    formato = atributos.get('formato', '%Y-%m-%d %H:%M:%S')

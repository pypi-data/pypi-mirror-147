...
class Opt(Vars):
    ...
    def __init__(self, 
        ...
        self.occu_min = occu_min
        self.gc_min = gc_per_min
        self.gc_max = gc_per_max
    ...
    def is_small(self, N, base_fasta):
        ...
        if N < self.spps_min:
            sys.stdout.write(f"Finding subset for {base_fasta}. Not found: low number of species ({N})\n")
            sys.stdout.flush()
            return True
    ...
    def select(self, file):
        ...
        sys.stdout.write('Finding subset for %s\r' % base_fasta)
        sys.stdout.flush()
        ...
        while True:
            ...
            constraints = [
                cp.sum(x) == n_spps,
                c_l  >= self.occu_min * n_spps * L,
                c_gc >= self.gc_min * n_spps,
                c_gc <= self.gc_max * n_spps,
                x >= 0,
                x <= 1
            ]
        ...
        if subsel.status != 'optimal':
            sys.stdout.write(f"Finding subset for {base_fasta}. Not found: problem {subsel.status}\n")
            sys.stdout.flush()
            return None


        n_iter = self.spps_max - n_spps
        sys.stdout.write(f"Finding subset for {base_fasta}, found after {n_iter} iterations\n")
        sys.stdout.flush()

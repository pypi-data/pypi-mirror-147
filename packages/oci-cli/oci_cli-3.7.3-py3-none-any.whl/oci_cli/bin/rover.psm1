...
function GetOciSubcommands_rover() {
    $ociSubcommands = @{
        'rover' = 'node standalone-cluster station-cluster'
...
function GetOciCommandsToLongParams_rover() {
    $ociCommandsToLongParams = @{
        ...
        'rover node list' = 'all compartment-id display-name from-json help lifecycle-state limit node-type page page-size sort-by sort-order'
        ...
        'rover standalone-cluster create' = 'address1 address2 address3 address4 addressee care-of city-or-locality cluster-size cluster-type compartment-id country defined-tags display-name email enclosure-type freeform-tags from-json help lifecycle-state lifecycle-state-details max-wait-seconds phone-number point-of-contact point-of-contact-phone-number shipping-preference state-province-region subscription-id system-tags wait-for-state wait-interval-seconds zip-postal-code'

...
class Results_Clsf():
    ...
    def best(self, n=8, **kwargs): return np.argsort(self.probs)[::-1][:n]
    def worst(self, n=8, **kwargs): return np.argsort(self.probs)[:n]

# y2,y = learner.logger.y2_ep.cpu().numpy(), learner.logger.y_ep.cpu().numpy()
# c = len(np.unique(actuals))
# probs = get_probs_bin(y2) if c==2 else get_probs(y2)
# actuals = y

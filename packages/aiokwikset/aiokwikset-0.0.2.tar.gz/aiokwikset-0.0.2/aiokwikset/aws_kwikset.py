...
class AWSKWIKSET(object):
    ...
    def process_custom_challenge(self):
        response = {'ANSWER': 'answerType:generateCode,medium:phone,codeType:login',"USERNAME": self.username}

        return response

    def process_custom_challenge_code(self):
        code = input('Enter Code:')

        response = {'ANSWER': 'answerType:verifyCode,medium:phone,codeType:login,code:{}'.format(code),"USERNAME": self.username}
        ...
        async with boto_client as client:
            ...
            if response['ChallengeName'] == self.PASSWORD_VERIFIER_CHALLENGE:
                ...
                if custom_challenge['ChallengeName'] == self.CUSTOM_VERIFIER_CHALLENGE:
                    ...
                    custom_challenge_response_code = self.process_custom_challenge_code()

                    tokens = await client.respond_to_auth_challenge(
                        ClientId=self.client_id,
                        ChallengeName=self.CUSTOM_VERIFIER_CHALLENGE,
                        Session=custom_challenge_code['Session'],
                        ChallengeResponses=custom_challenge_response_code)
    ...
    async def refresh_auth_token(self, refreshToken, client=None):
        boto_client = self.client or client
        auth_params = {'SECRET_HASH': '', 'REFRESH_TOKEN': refreshToken}

        async with boto_client as client:
            response = await client.initiate_auth(
                AuthFlow='REFRESH_AUTH_TOKEN',
                AuthParameters=auth_params,
                ClientId=self.client_id
            )

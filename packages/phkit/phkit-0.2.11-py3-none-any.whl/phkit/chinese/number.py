...
cellphone_pattern = r'((13[0-9])|(14[0-9])|(15[0-9])|(17[0-9])|(18[0-9]))\d{8}'
...
def convert_number_simple(text):
...
class TelePhone:
    ...
    def telephone2chntext(self):
        sp_parts = re.split(r'\D', self.telephone.strip('+'))
        self.raw_chntext = '<SP>'.join([
            num2chn(part, alt_two=False, use_units=False) for part in sp_parts if part
        ])
...
class Decimal:
    ...
    def __init__(self, decimal=None, chntext=None):
        self.decimal = decimal
        self.chntext = chntext

    def chntext2decimal(self):
        parts = self.chntext.split('点')
        if len(parts) == 2:
            a, b = parts
            return chn2num(a) + '.' + Digit(chntext=b).chntext2digit()
        elif parts == 1:
            return chn2num(parts[0])
        else:
            return self.chntext

    def decimal2chntext(self):
        parts = self.decimal.split('.')
        if len(parts) == 2:
            a, b = parts
            return num2chn(a) + '.' + Digit(digit=b).digit2chntext()
        elif parts == 1:
            return num2chn(parts[0])
        else:
            return self.decimal
...
class NSWNormalizer:
    def __init__(self):
        self.date_pattern = re.compile(r"((([089]\d|(19|20)\d{2})年)?(\d{1,2}月(\d{1,2}[日号])?)?)")
        self.money_pattern = re.compile(r"((\d+(\.\d+)?)[多余几]?" + CURRENCY_UNITS + r"(\d" + CURRENCY_UNITS + r"?)?)")
        # pattern = re.compile(r"\D((\+?86 ?)?1([38]\d|5[0-35-9]|7[678]|9[89])\d{8})\D")
        self.cellphone_pattern = re.compile(
            r'((\+?86[ \-]?)?((13[0-9])|(14[0-9])|(15[0-9])|(17[0-9])|(18[0-9])|(19[0-9]))\d{8})')
        self.IDCards_pattern = re.compile(r'([1-9]\d{5}[12]\d{3}(0[1-9]|1[012])(0[1-9]|[12][0-9]|3[01])\d{3}[0-9xX])')
        ...
        self.telephone_pattern = re.compile(r"((0(10|2[1-3]|[3-9]\d{2})-?)?[1-9]\d{6,7})")
        self.fraction_pattern = re.compile(r"(\d+/\d+)")
        self.percentage_pattern = re.compile(r"(\d+(\.\d+)?%)")
        self.cardinal_quantifier_pattern = re.compile(r"\D(\d{1,8}0*(\.\d+)?)[多余几]?" + COM_QUANTIFIERS)
        self.cardinal_pattern = re.compile(r"(\d{1,8}0*(\.\d+)?)[多余几]?")
        self.digit_pattern = re.compile(r"(\d{4,32})")
        self.particular_pattern = re.compile(r"(([a-zA-Z]+)二([a-zA-Z]+))")

    def _particular(self):
        text = self.norm_text

        matchers = self.particular_pattern.findall(text)
        if matchers:
            # print('particular')
            for matcher in matchers:
    ...
    def normalize(self, text):
        ...
        matchers = self.date_pattern.findall(text)
        if matchers:
            # print('date')
            for matcher in matchers:
        ...
        for pattern in [self.cellphone_pattern, self.IDCards_pattern, self.IP_pattern, self.telephone_pattern]:
        ...
        matchers = self.fraction_pattern.findall(text)
        if matchers:
            # print('fraction')
            for matcher in matchers:
        ...
        # 规范化百分数
        text = text.replace('％', '%')
        matchers = self.percentage_pattern.findall(text)
        if matchers:
            # print('percentage')
            for matcher in matchers:
        ...
        matchers = self.cardinal_quantifier_pattern.findall(text)
        if matchers:
            # print('cardinal+quantifier')
            for matcher in matchers:
                text = text.replace(matcher[0], Cardinal(cardinal=matcher[0]).cardinal2chntext(), 1)
                # text = text.replace(matcher[0], Decimal(decimal=matcher[0]).decimal2chntext(), 1)

        # 规范化纯数（小数+整数）
        matchers = self.cardinal_pattern.findall(text)
        if matchers:
            # print('cardinal')
            for matcher in matchers:
                text = text.replace(matcher[0], Cardinal(cardinal=matcher[0]).cardinal2chntext(), 1)
        ...
        # 规范化数字编号
        matchers = self.digit_pattern.findall(text)
        if matchers:
            # print('digit')
            for matcher in matchers:
...
def convert_number(text):
    return nsw_normalizer.normalize(text)


def nsw_test_case(raw_text):
    print('I:' + raw_text)
    print('O:' + convert_number(raw_text))
    print('')


def nsw_test():
    """
I:固话：0595-23865596或23880880。
O:固话：零五九五二三八六五五九六或二三八八零八八零。

I:固话：0595-23865596或23880880。
O:固话：零五九五二三八六五五九六或二三八八零八八零。

I:手机：+86 19859213959或15659451527。
O:手机：八六一九八五九二一三九五九或一五六五九四五一五二七。

I:分数：32477/76391。
O:分数：七万六千三百九十一分之三万两千四百七十七。
...
I:纯数：2983.07克或12345.60米。
O:纯数：二九八三.零七克或一二三四五.六十米。

I:日期：1999年2月20日或09年3月15号。
O:日期：一九九九年二月二十日或零九年三月十五号。

I:金钱：12块5，34.5元，20.1万
O:金钱：十二块五，三十四点五元，二十点一万
...
O:三千四百五十六万吨
    ...
    nsw_test_case('读数字58593200000.24吧')

...
def initialize():
    # 导入数据
    inpath = Path(__file__).absolute().parent.joinpath('phrase_pinyin_txt.py')
    _phrases_dict = parse_phrase_txt(inpath)
    load_phrases_dict(_phrases_dict)  # big:398815 small:36776

    inpath = Path(__file__).absolute().parent.joinpath('single_pinyin_txt.py')
...
class Pinyin(object):
    ...
    def lazy_pinyin(self, hans, style=Style.TONE3,
                    errors='default', strict=True, **kwargs):
        ...
        :param style: 指定拼音风格，默认是 :py:attr:`~pypinyin.Style.TONE3` 风格。
                      更多拼音风格详见 :class:`~pypinyin.Style`。
...
def pinyin(hans, style=Style.TONE, heteronym=False,
    ...
    Usage::
      ...
      [['чжун1'], ['синь1']]
      >>> pinyin('战略', v_to_u=True, style=Style.TONE3)
      [['zhan'], ['lüe']]
...
def slug(hans, style=Style.TONE3, heteronym=False, separator='-',
         errors='default', strict=True):
...
def lazy_pinyin(hans, style=Style.TONE3, errors='default', strict=True,
                v_to_u=False, neutral_tone_with_five=True):
    """将汉字转换为拼音，返回不包含多音字结果的拼音列表.

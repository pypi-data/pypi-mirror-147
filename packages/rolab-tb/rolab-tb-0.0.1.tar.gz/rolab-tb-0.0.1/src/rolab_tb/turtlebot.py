...
def check_control_limits(lin_vel, ang_vel,
...
class Turtlebot():
    ...
    def __init__(self, d_user_config=None):
        ...
        if d_user_config:
            self.config = {**self.config, **d_user_config}

        self.config['sub-msgs'] = \
                get_json_msgs(self.config['msg-sub-dir'])
        self.config['pub-msgs'] = \
                get_json_msgs(self.config['msg-pub-dir'])

        self.config['ws_uri'] = "ws://" + self.config['ws_host'] + ":" + \
...
INSERT INTO status VALUES (?, ?, ?);""",
    ...
    def __create_control_thread(self):
        ...
        self.db_con_control = sqlite3.connect(
                        self.config['sqlite-control-uri'],
                        uri=True, isolation_level=None)
...
CREATE TABLE control_inputs (linear_vel REAL, angular_vel REAL);""")
    ...
    def __await_sensor_stream(self, sleep=0.05, timeout=5):
        ...
        for topic in self.config['sub-msgs']:
            t_start = time.time()
            while True:
...
ORDER BY rowid DESC LIMIT 1;"""
    ...
    def set_control_inputs(self, lin_vel, ang_vel):
        ...
        lin_vel, ang_vel = check_control_limits(lin_vel, ang_vel)

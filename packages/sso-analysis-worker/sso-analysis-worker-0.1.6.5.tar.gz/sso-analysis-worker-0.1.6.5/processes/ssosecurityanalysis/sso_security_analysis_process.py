...
def get_auth_resp(chromedriver, tuple_url_and_counter):
    ...
    for request in chromedriver.requests:
        ...
        if postmessage_req and postmessage_res:
            ...
            if request.url.startswith(auth_req.params['redirect_uri']):
                u = request.url
                if u.__contains__("code") or u.__contains__("access_token") or u.__contains__("id_token"):
                    return request
                if form_post_request and len(request.body) > 0:
                    try:
                        decoded_body = request.body.decode("utf-8")
                        if decoded_body.__contains__("code=") or decoded_body.__contains__(
                                "access_token=") or decoded_body.__contains__("id_token="):
                            return request
                    except Exception:
                        # Nothing to do --> wrong message format
                        pass
            ...
            try:
                # Fallback if redirect uri did contain unencoded url characters or contained :443 port
                parsed = urlparse(auth_req.params['redirect_uri'])
                parsed = parsed._replace(query=urlencode(parse_qsl(parsed.query)))
                parsed = parsed._replace(netloc=parsed.netloc.replace(":443", "").replace(":80", ""))
                parse_url = parsed.geturl()
                if request.url.startswith(parse_url):
                    if request.url.__contains__("code") or parse_url.__contains__(
                            "access_token") or parse_url.__contains__("id_token"):
                        return request
            except Exception:
                pass
...
class SSOSecurityProcess(GenericAnalysisProcess):
    ...
    def get_next_object_to_analyse(self):
        res = self.rest_client.get_next_sso_detection_for_security_analysis_for_run(self.analysis_run_id)
        while not res['page']['basePage'].__contains__("financialpost.com"):
            res = self.rest_client.get_next_sso_detection_for_security_analysis_for_run(self.analysis_run_id)
        return res

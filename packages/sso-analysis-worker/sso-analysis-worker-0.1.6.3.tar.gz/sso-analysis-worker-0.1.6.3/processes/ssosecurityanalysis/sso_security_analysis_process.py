        ...
        if postmessage_req and postmessage_res:
            ...
            try:
                if json_obj["origin"] == auth_req_json_obj["documentLocation"]["origin"] and "type" in json_obj[
                    "data"] and \
                        json_obj["data"]["type"].lower() == "response":
                    return request
            except Exception:
            ...
            try:
                # Fallback if redirect uri did contain unencoded url characters
                ...
                if request.url.startswith(parse_url):
                    if parse_url.__contains__("code") or parse_url.__contains__(
...
def process_function(sso_detection, backend_info: BackendInformation, analysis_run_id: int,
    ...
    try:
        ...
        if element_width is not None and element_height is not None:
            logger.info("Pressing at coordinates did not get results. Trying to press in center of element")

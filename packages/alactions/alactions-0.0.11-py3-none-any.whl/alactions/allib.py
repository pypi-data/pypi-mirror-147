...
import glob
import logging
from alactions.utils import copy
import shutil
...
def transform_pipeline(pipeline_file, target_folder, run_id=None, debug=False, meta={'extract_upstream': False}):
    ...
    runs_on = phases.get('runs-on', 'localhost')
    if 'runs_on' in phases:
        del phases['runs-on']

    if phases is None:
        raise Exception('No tasks fown in pipeline')

    phase_keys = [x.strip() for x in 'steps, needs, strategy, name'.split(',')]
...
class HandleTarget(Thread):
    def __init__(self, cmd, target):
        ...
        self._target = target
    def run(self):
        proc = subprocess.Popen(self._cmd, stdout=subprocess.PIPE, stderr=subprocess.PIPE, shell=True, cwd=self._target)
        for line in proc.stdout:
            print(line.strip().decode('utf-8'))

def run(pipeline, target_folder, environ=None, runs_on='localhost'):
    pipeline_absolute = str(Path(pipeline).absolute())
    target_folder_absolute = str(Path(target_folder).absolute())

    if runs_on != 'localhost':
        target_pipeline = str(Path(target_folder_absolute) / "pipeline.yaml")
        
        shutil.copy(pipeline_absolute, target_pipeline)
        # os.chdir(target_folder_absolute)
        if platform == "win32":
            docker_cmd = f'docker run -t --rm -v %cd%:/home {runs_on} sh -c "pip install alactions && cd /home && python -m alactions.alcli run && exit"'
        else:
            docker_cmd = f'docker run -t --rm -v $(pwd):/home {runs_on} sh -c "pip install alactions && cd /home && python -m alactions.alcli run && exit"'
        # docker_cmd = f'docker run -it -v %cd%:/home {runs_on} sh -c "cd /home"'
        # subprocess.check_output(docker_cmd, shell=True, cwd=target_folder_absolute)
        thread_1 = HandleTarget(docker_cmd, target_folder_absolute)
        thread_1.start()
        thread_1.join()
        os.remove(target_pipeline)
        return

...
def transform_pipeline(pipeline_file='pipeline.yaml', run_id=None, target_file=None, debug=False, meta={'extract_upstream': True}):
        ...
        for k in phase:
            ...
            if k == 'steps':
                ...
                for step in phase_steps:
                    ...
                    step_name = step.get('name', f'_step_{stepid}')
                    ...
                    if uses_function is not None:
                        ...
                        code_fn = pipeline_path.parent / f'.run-{run_id}-{name}.sh'
                        code_lines=script_setup.split('\n') + [x.rstrip() for x in run_code.split('\n')] + ['touch {{product}}']
                        code_fn.write_text('\n'.join(code_lines))
                        ploomber_step = {
                            'source': str(code_fn.name),
                            'name': name,
                            'product': product_complete
                        }
...
#             'executor':{'dotted_path': 'ploomber.executors.Parallel','processes': 2 },
    ...
    Path(pipeline_path.parent / pp_file).write_text(pipeline_text)

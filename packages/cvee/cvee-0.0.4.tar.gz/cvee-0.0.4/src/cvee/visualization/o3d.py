...
class VizO3D:
    ...
    def _init_ui(self):
        ...
        self.window = app.create_window("Viz - O3D", 1200, 800)
    ...
    def get_scene(self, scene_idx):
        raise NotImplementedError("Please implement this function if the viz data is not loaded from files")

    def load_scene(self, scene_idx):
        geometries = {}
        scene_dir = os.path.join(self.viz_dir, f"{scene_idx:05d}")
    ...
    def visualize(self, load_from_file=False, no_cache=False):
        ...
        if self.load_from_file:
            scene_dirs = os.listdir(self.viz_dir)
    ...
    def set_scene_idx(self, idx):
        ...
        if self.viz_dir is None:
            raise RuntimeError("Please set viz_dir if you want to save data to files")
        os.makedirs(os.path.join(self.viz_dir, f"{self.scene_idx:05d}"), exist_ok=True)
    ...
    def get_output_path(self, type, name=None):
        ...
        if self.viz_dir is None:
            raise RuntimeError("Please set viz_dir if you want to save data to files")
        output_path = os.path.join(self.viz_dir, f"{self.scene_idx:05d}", type, f"{name}.{file_exts[type]}")
        return output_path
...
@click.command()
@click.option("--viz_dir", default=".", help="Visualize directory.")
def cli(viz_dir):
    viz = VizO3D(viz_dir=viz_dir)
    viz.visualize(load_from_file=True)

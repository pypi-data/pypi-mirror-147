...
from shine import dry_run as dry_run_size
from shine.utilities import is_contiguous
...
class _Communicator:
    ...
    def get_c_object(self):
        ...
        assert isinstance(c_obj, _shine.Communicator)
        return c_obj
...
libmpi = os.environ.get('SHINE_MPI')
if libmpi:
    import ctypes
    ctypes.CDLL(libmpi, ctypes.RTLD_GLOBAL)
    world = _shine.Communicator()
    if world.size == 1:
        world = serial_comm
else:
    try:
...
def cleanup():
    ...
    if error is not None:  # else: Python script completed or raise SystemExit
        if parallel and not (dry_run_size > 1):
            sys.stdout.flush()
            sys.stderr.write(('SHINE CLEANUP (node %d): %s occurred.  '
                              'Calling MPI_Abort!\n') % (world.rank, error))
            sys.stderr.flush()
...
def exit(error='Manual exit'):
    ...
    sys.exit()  # quit for serial case, return to _shine.c for parallel case

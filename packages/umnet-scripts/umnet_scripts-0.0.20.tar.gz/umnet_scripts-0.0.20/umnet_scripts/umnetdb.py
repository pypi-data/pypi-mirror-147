...
class UMnetdb:
    ...
    def _set_instantclient_path(self):
        """
        Sets the oracle instantclient path to files within this repo based on the system platform.
        Call this in the initializer of your child class if you're connecting to an oracle db.
        """

        module_path = os.path.abspath(__file__).replace("umnetdb.py","")

        p = platform.system()
        if p == "Darwin":
            lib_dir = f"{module_path}oracle_instantclient/mac"     
        elif p == "Linux":
            lib_dir = f"{module_path}oracle_instantclient/linux"
        else:
            raise UnsupportedOperation(f"This function only supported on Mac and Linux!")

        logger.debug(f"setting instant oracle client lib path to {lib_dir}")

...
class MVNMixtureModel():
    # def __init__(self, K, data_file=None, lineage=[], timepoint_pattern="cov",\
    #         data_tensor=None, IS=None, lineages_list=None):
    def __init__(self, K, data_tensor, IS=[], columns=[]):
        # if data_file is not None:
        #     data = pd.read_csv(data_file, sep=",")
        #     data = data[data["IS"].str.contains(r'\d+')]  # keep IS with more than one digit
        #     self.timepoints = [col for col in data if col.startswith(timepoint_pattern)]

        #     if len(lineage) > 0: # if we want to select specific lineages
        #         data = data.loc[(data["lineage"].isin(lineage))]
        
        #     data = data.pivot(index=["IS"], values=self.timepoints, columns="lineage")

        #     self.IS = data.index.values  # rownames are now the IS
        #     self.lineages = np.array([".".join(f) for f in data.columns.values])  # colnames are now the timepoints.lineage
        #     self.dataset = torch.tensor(data.fillna(0).values)
        
        self.dataset = torch.tensor(data_tensor.values)
        self.IS = IS
        # `columns` will be a list of type ["early.MNC", "mid.MNC", "early.Myeloid", "mid.Myeloid"]
        # corresponding to the columns of the input dataset
    ...
    def _initialize_attributes(self):
        self.params = {"N":self._N, "K":self.K}
        self.init_params = {"N":self._N, "K":self.K, "is_computed":False,\
    ...
    def filter_dataset(self, thr=None, n=None, ccf=True, k_interval=(2,25), perc=0.05,
            random_state=25, metric="calinski_harabasz_score"):
        '''
        Function used to filter the input dataset. \\
        - `thr` -> hard threshold for each observation. Only the observations with at least a 
        coverage of `thr` across the timepoints are kept. \\
        - `perc` -> percentage of the cumulative distribution that will be used as a threshold 
        ...
        - `ccf` -> if set to `True`, it will perform a K-means, for `k` in `k_interval`, to search 
        for the best `K` (optimizing the input metric), and performing K-means to discard 
        observations belonging to clusters with the centroid of all timepoints below `5%` of the 
        sum of centroids for all the timepoints. \\
        Either `thr` or `perc` is used for filtering.
        '''
        if thr is not None:
            try:
                self.IS = self.IS[self.dataset.sum(dim=1) >= thr]
            finally:
                self.dataset = self.dataset[self.dataset.sum(dim=1) >= thr,]
        ...
        if ccf:
            self.dataset, self.IS = self._filter_dataframe_init(k_interval, metric=metric, \
                random_state=random_state, perc=perc)
    ...
    def _filter_dataframe_init(self, k_interval=(2,25), metric="calinski_harabasz_score", perc=0.05, random_state=25):
        ...
        keep = torch.where((ctrs / ctrs.sum(dim=0) > perc).sum(dim=1) > 0)[0]
        try:
            ii = self.IS[np.in1d(np.array(clusters), keep)]
        except:
            ii = self.IS
        finally:
            return self.dataset[np.in1d(np.array(clusters), keep)], ii
    ...
    def _get_learned_parameters(self) -> Dict:
        ...
        p = {}
        p["N"], p["K"] = self.params["N"], self.params["K"]
    ...
    def _reduce_assignments(self, t=10, p=1, params=None, min_prob=.20):
        ...
        for par in params.keys():
            if (self.cov_type=="full" and par in ["N", "K", None]) or \
                (self.cov_type=="diag" and par in ["N", "K", "sigma_chol", None]):
                continue

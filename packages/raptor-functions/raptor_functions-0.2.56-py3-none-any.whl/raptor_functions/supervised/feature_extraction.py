...
def reduce_dimension(X, y, tree_model=xgb):
...
def get_training_features(df, offset=False, gradient=False):


    X_raw = df.drop(label, axis=1)
    # X_raw = X_raw.add_suffix(f'raw')
    X_all = []
    X_all.append(X_raw)
    # X_dict = {'raw': X_raw}

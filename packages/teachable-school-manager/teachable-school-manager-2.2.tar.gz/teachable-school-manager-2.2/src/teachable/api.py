...
class Teachable:
    ...
    def get_all_users(self) -> list[User]:
        ...
        :rtype: [User]
        """
        responses = self._get_paginated_json_at(self.URL_GET_ALL_USERS)
    ...
    def get_course_users(self, course_id):
        ...
        responses = self._get_paginated_json_at(path)
        # the API returns at least one page, at most N pages
        result_list = []
        for response in responses:
            user_list = response.get('users')
    ...
    def _get_paginated_json_at(self, path):
        json_data = self._get_json_at(path)
        yield json_data
        pages = json_data.get('meta')['number_of_pages']
        ...
        if pages > 1:
            ...
            for page_no in range(2, pages + 1):
                ...
                if path.find('?') > 0:
                    next_path = path + '&page=' + str(page_no)
                else:
                    next_path = path + '?page=' + str(page_no)
                response = self._get_json_at(next_path)
                yield response

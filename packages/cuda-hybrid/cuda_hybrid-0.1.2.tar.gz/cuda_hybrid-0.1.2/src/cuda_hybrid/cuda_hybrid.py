...
class HybridModel:
    ...
    def __init__(self, ABM: nx.Graph):
        ...
        self.neighbors) = HybridModel.transformNetwork(ABM)
    ...
    def transformNetwork(
        ...
        :return: adjacency matrix, fcm_labels for each node in each agent's FCM, numpy matrix with FCM values for each agent, numpy matrix with future FCM values for each agent, numpy adjacency matrix with FCM edge weights
        :rtype: Tuple[ NDArray[Float64], NDArray[Float64], Dict, NDArray[Float64], NDArray[Float64], Dict
        """
        ...
        return ABM_adj, FCM_adj, fcm_labels, node_val, node_future_val, neighbors
    ...
    def run_parallel(self, focus, threshold, iters, func, args, steps = 20):
...
def generate_model(agent_count, filename, graph_type, community_algorithm=None):
    ...
    :type graph_type: string
    :param community_algorithm: community algorithm defaults to None
    :type community_algorithm: function, optional

...
import shutil
import tempfile
from typing import Callable, Optional, Pattern, Tuple, TypeVar
...
class Builder(metaclass=ABCMeta):  # pylint: disable=too-few-public-methods
    ...
    def _fill_target(self, fill: Callable[[str], None]) -> None:
        ...
        if not os.path.exists(target):
            print("fill in place")
            shutil.copytree(self.template_path, target)
            fill(target)
            return
        ...
        # an items already in the target
        with tempfile.TemporaryDirectory() as stage_dir:
            print(f"fill in {stage_dir}")
            shutil.copytree(self.template_path, stage_dir, dirs_exist_ok=True)
            fill(stage_dir)

...
class SFPTypeListView(generic.ObjectListView):
    queryset = SFPType.objects.annotate(
        unassigned_sfps=Count('sfps', filter=Q(sfps__assigned=True)),
        assigned_sfps=Count('sfps', filter=Q(sfps__assigned=False)),
    ...
    def get_extra_context(self, request, instance):
        ...
        sfps_by_tenant = SFPByTenantTable(
            SFP.objects.prefetch_related('tenant').values("tenant__name").annotate(
                total_count=Count("tenant__name"),
                used_count=Count("tenant__name", filter=Q(assigned=True)),
                unused_count=Count("tenant__name", filter=Q(assigned=False)),
            ).order_by("tenant__name")
        )

        sfps_by_supplier = SFPBySupplierTable(
            SFP.objects.prefetch_related('supplier').values("supplier__name").annotate(
                total_count=Count("supplier__name"),
                used_count=Count("supplier__name", filter=Q(assigned=True)),
                unused_count=Count("supplier__name", filter=Q(assigned=False)),

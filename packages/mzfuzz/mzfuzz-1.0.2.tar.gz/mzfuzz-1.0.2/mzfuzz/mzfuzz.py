...
# 对excel表进行手机号快速筛选出归属地并重新覆盖

def excel_phone(filename,sheet):
	call=Phone()
	...
	df['省']=df.loc[:, sheet].apply(query_pro)
	df['市']=df.loc[:, sheet].apply(query_city)
...
def multi_thread(single_thread,args,number):
    ...
    return [i.result() for i in tqdm(r)]

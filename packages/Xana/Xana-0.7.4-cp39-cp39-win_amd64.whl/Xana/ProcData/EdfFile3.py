...
class EdfFile:
    ...
    def __init__(self, FileNameOrFileDescriptor, fastedf=None):
        ...
        self.__checkEdfFile = re.compile(r"^dim_1", re.IGNORECASE | re.MULTILINE)
        ...
        while 1:
            ...
            try:
                ...
                raise KeyError("EdfFile: Image doesn't have size information")
            except ValueError:
                raise ValueError("EdfFile: Image size information is not an integer")
            # Check DataType
            try:
                img.DataType = StaticHeader["DATATYPE"]
            except KeyError:
                raise KeyError("EdfFile: Image doesn't have datatype information")
            # Check ByteOrder
            try:
                img.ByteOrder = StaticHeader["BYTEORDER"]
            except KeyError:
                raise KeyError("EdfFile: Image doesn't have byteorder information")
            # Check DIM1
            try:
                ...
                raise KeyError("EdfFile: Image doesn't have dimension information")
            # DIM2
    ...
    def GetData(self, Index, DataType="", Pos=None, Size=None):
        ...
        if str.upper(self.SysByteOrder) != str.upper(self.Images[Index].ByteOrder):
            Data = Data.byteswap()
        if DataType != "":
    ...
    def WriteImage(self, Header, Data, Append=1, DataType="", ByteOrder=""):
        ...
        StrHeader = str.ljust(StrHeader, newsize)
        StrHeader = StrHeader + "}\n"
        ...
        if str.upper(self.Images[Index].ByteOrder) != str.upper(self.SysByteOrder):
...
def SetDictCase(Dict, Case, Flag):
    ...
    for i in list(Dict.keys()):
        ...
        if Flag & KEYS:
            if Case == LOWER_CASE:
                newkey = str.lower(newkey)
            else:
                newkey = str.upper(newkey)
        if Flag & VALUES:
            if Case == LOWER_CASE:
                newvalue = str.lower(newvalue)
            else:
                newvalue = str.upper(newvalue)

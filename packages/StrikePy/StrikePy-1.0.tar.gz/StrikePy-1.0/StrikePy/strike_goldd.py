...
from StrikePy.functions.rationalize import rationalize_all_numbers
...
def strike_goldd(*args):
    ...
    import sympy as sym
    import importlib
    from StrikePy.functions.elim_and_recalc import elim_and_recalc
    import symbtools as st

    # If no parameters are passed when calling the function the default options file is used
    if len(args) == 0:
        import StrikePy.options as options
        model = importlib.import_module('StrikePy.models.{}'.format(options.modelname))
    ...
    if len(args) == 1:
        options = importlib.import_module('StrikePy.custom_options.{}'.format(args[0]))
        model = importlib.import_module('StrikePy.models.{}'.format(options.modelname))
        print(' Analyzing the {} model... \n'.format(options.modelname))
    ...
    if ind == nd: #If I have done all the minimum derivatives to build onx (I have not exceeded the time limit)....
        ...
        while increaseLie == 1:  # while increaseLie is 1 I will increase the size of onx
            ...
            file = open("{}/results/obs_ident_matrix_{}_{}_Lie_deriv.txt".format(dir,options.modelname, nd), "w")
    ...
    file = open("{}/results/id_results_{}_{}.txt".format(dir,options.modelname, datetime.today().strftime('%d-%m-%Y')), "w")
    file.write('\n RESULTS SUMMARY:')

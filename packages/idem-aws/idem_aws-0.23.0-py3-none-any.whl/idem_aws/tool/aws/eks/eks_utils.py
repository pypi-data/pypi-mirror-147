...
def get_resource_vpc_config_changes(
    ...
    """
    final_vpc_config = {}
    if new_vpc_config.get("endpointPrivateAccess") and not old_vpc_config.get(
        "endpointPrivateAccess"
    ) == new_vpc_config.get("endpointPrivateAccess"):
        final_vpc_config["endpointPrivateAccess"] = new_vpc_config.get(
            "endpointPrivateAccess"
        )

    if new_vpc_config.get("endpointPublicAccess") and not old_vpc_config.get(
        "endpointPublicAccess"
    ) == new_vpc_config.get("endpointPublicAccess"):

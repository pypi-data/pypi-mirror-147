...
async def update(
    ...
    assert user_name and access_key_id and status

    if ctx.get("test"):
        result["comment"] += (
            f"Would update access key {access_key_id} for {user_name} to {status}",
        )
        return result
    try:
...
async def delete(hub, ctx, user_name: str, access_key_id: str) -> Dict[str, Any]:
    ...
    if ctx.get("test"):
        result["comment"] += (
            f"Would delete access key {access_key_id} for {user_name}",
        )
        return result
    try:
...
async def create(hub, ctx, user_name: str, pgp_key: str = None) -> Dict[str, Any]:
    ...
    if ctx.get("test"):
        result["ret"] = {
            "access_key_id": "GENERATED_KEY",
            "user_name": user_name,
            "secret_access_key": "SECRET_ACCESS_KEY",
            "status": "Active",
        }
        result["comment"] += (f"Would create access key for {user_name}",)
        return result
    ...
    if not ret["result"]:
        ...
        if pgp_key:
            decoded_pgp_key = _b64_decode(pgp_key)
            ...
            access_key["secret_access_key"] = _b64_encode(secret_access_key)
...
def _b64_decode(data):
    b64_data_bytes = bytes(data, "utf-8")
    decoded_data = base64.b64decode(b64_data_bytes)
    return decoded_data


def _b64_encode(data):
    data_bytes = str(data).encode("utf-8")
    b64_data_bytes = base64.b64encode(data_bytes)
    b64_data_str = str(b64_data_bytes, "utf-8")
    return b64_data_str

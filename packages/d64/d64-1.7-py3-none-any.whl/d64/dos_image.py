...
class DOSImage(object):
    ...
    def alloc_first_block(self):
        ...
        track = None
        low_range = list(range(self.DIR_TRACK-1, self.MIN_TRACK-1, -1))
        high_range = list(range(self.DIR_TRACK+1, self.MAX_TRACK+1))
        range_len = max(len(low_range), len(high_range))
        low_range += [None]*range_len
        high_range += [None]*range_len
        for low, high in zip(low_range[:range_len], high_range[:range_len]):
            if low is not None:

    ...
    while True:
        try:
            ...
            if d(text="选择送达时间").exists:
                print("选择送达时间")
                d(text="选择送达时间").click()
                time_count = d(resourceId="com.yaya.zone:id/rv_selected_hour").info.get(
                    "childCount", 0)
                for i in range(time_count):
                    info = d.xpath(
                        '//*[@resource-id="com.yaya.zone:id/rv_selected_hour"]'
                        '/android.view.ViewGroup[%s]' % str(i + 1)).info
                    if info.get("enabled", "") != "false":
                        d.xpath(
                            '//*[@resource-id="com.yaya.zone:id/rv_selected_hour"]'
                            '/android.view.ViewGroup[%s]' % str(i + 1)).click_exists(timeout=3)

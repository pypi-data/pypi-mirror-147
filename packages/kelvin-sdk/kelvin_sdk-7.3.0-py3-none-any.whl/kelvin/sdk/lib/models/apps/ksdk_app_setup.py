...
class ProjectCreationParametersObject(KSDKModel):
    ...
    def get_entrypoint(self) -> str:
        ...
        if self.app_flavour is ApplicationFlavour.pubsub:
            ...
            if self.app_type is ProjectType.kelvin:
                ...
                class_name = f"{camel_name(self.app_name)}Bridge"

            return f"{standard_app_name}:{class_name}"
    ...
    def get_language_block(self, requirements: Optional[str] = None) -> dict:
        ...
        if self.kelvin_app_lang == ApplicationLanguage.python:
            ...
            app_lang = {"entry_point": entrypoint, "requirements": requirements}
...
class ProjectEmulationObject(KSDKModel):
    ...
    def process_resources(resources: Optional[SystemResources]) -> Tuple[Optional[str], Optional[str]]:
        ...
        """
        memory = resources.memory if resources and resources.memory else None
        cpu = resources.cpu if resources and resources.cpu else None
        return memory, cpu

...
class _tpl_curve_generator:
  ...
  def __create_table(self):
    ...
    if self.curve.xerror is None and self.curve.yerror is None:
      ...
      table += f", {'dx':>8}\n"
      table += "\n".join([f"  {cx:8}, {cy:8}, {cdx:8}" for cx, cy, cdx in zip(self.curve.x, self.curve.y, self.curve.xerror)])
    elif self.curve.xerror is None and self.curve.yerror is not None:
      table += f", {'dy':>8}\n"
      table += "\n".join([f"  {cx:8}, {cy:8}, {cdy:8}" for cx, cy, cdy in zip(self.curve.x, self.curve.y, self.curve.yerror)])
  ...
  def _get_type(self):
    return f"%type (ybar, ybar interval)"
  
  def _get_marks(self):
    return f"only marks,"
  
  def _get_legend_type(self):
    return f"%legend type (ybar legend)"

  def __get_errorstr(self, draw: bool = False):
  ...
  def _get_addplot_code(self):
    return f"""\\addplot+[
  {self.curve.color if self.curve.color.value() is not None else f"%color"}
  {self._get_fill()}
  {self._get_draw()}
  {self._get_marks()}

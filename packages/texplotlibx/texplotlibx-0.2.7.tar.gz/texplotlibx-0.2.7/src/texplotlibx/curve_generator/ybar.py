...
class _tpl_curve_generator_ybar(_tpl_curve_generator):
  def _get_fill(self):
    color = "=%s"  % self.curve.color.value() if self.curve.color.value() is not None else ""
    return "fill%s,\n  fill opacity=0.3," % color

  def _get_draw(self):
    color = "=%s"  % self.ccurve.olor.value() if self.curve.color.value() is not None else ""

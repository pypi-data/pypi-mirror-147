...
def create_app_openshift(args):
    ...
    for app in apps:
        if app in LOCAL_OPENSHIFT_APPS:
            ...
            name, source, channel, csv, description, target_namespace, channels, crd = common.olm_app(app)
            if name is None:
            ...
            if 'namespace' in overrides:
                target_namespace = overrides['namespace']
            app_data = {'name': name, 'source': source, 'channel': channel, 'target_namespace': target_namespace,
                        'crd': crd}

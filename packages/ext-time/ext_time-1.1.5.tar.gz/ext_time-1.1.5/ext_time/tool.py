...
def publish():
    ...
    os.chdir(path)
    cmd="python  setup.py sdist &  twine upload dist/* & rd /s /q dist & rd /s /q zlparse.egg-info & python -m pip install ext_time==%d.%d.%d"%(arr[0],arr[1],arr[2])
    print(cmd)
    pkg="zlparse==%d.%d.%d"%(arr[0],arr[1],arr[2])
    os.system(cmd)

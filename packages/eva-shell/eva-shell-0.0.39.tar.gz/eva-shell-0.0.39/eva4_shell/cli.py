...
from .tools import get_node_svc_info, edit_file, get_term_size, exec_cmd
...
class CLI:
    ...
    def __init__(self, *args, **kwargs):
        ...
        for vc in [
            ...
            setattr(self, f'venv_{vc}', partial(self.venv_cmd, vc))
    ...
    def cloud_deploy(self, file, test, config_var):
        args = (f'--timeout {current_command.timeout} "{common.elbus_path}" '
                f'cloud deploy "{file}"')
        if config_var:
            for c in config_var:
                args += f' --config-var "{c}"'
    ...
    def cloud_undeploy(self, file, test, config_var):
        args = (f'--timeout {current_command.timeout} "{common.elbus_path}" '
                f'cloud undeploy "{file}"')
        if config_var:
            for c in config_var:
                args += f' --config-var "{c}"'
        if test:
    ...
    def registry_manage(self):
        ...
        try:
            ...
            os.system(
                f'env "YEDB_PS=registry:{system_name}" "{cmd}" "{yedb_socket}"')
        except:
            ...
            os.system(
                f'env YEDB_PS=registry "{cmd}" "{common.dir_eva}/runtime/registry"'
            )

...
class Oracle:
    def __init__(self, loop_list_path: str, hic_file: str, models_dir: str, norm: str, width: int = 56,chr_list:list=['All']):
    ...
    def makePredictions(self,chr):
        passed, failed = [], []
        for reso,sublist in self.__resoSubLists.items():
          ...
          mzd = self.__hic.getMatrixZoomData(chr, chr, "observed", 'SCALE' , "BP", reso)
          for index, row in sublist.iterrows():   
            x1,x2,y1,y2 = self.makeRegion((row[CHROM_KEY],row[X1_KEY],row[X2_KEY],row[Y1_KEY],row[Y2_KEY]),self.__width-1,reso)

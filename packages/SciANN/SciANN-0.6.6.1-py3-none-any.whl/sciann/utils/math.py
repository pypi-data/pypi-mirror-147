...
class GradLayer(Layer):
    def __init__(self, y, x, order=1, name=None):
        super(GradLayer, self).__init__(name=name)
        assert order > 0, \
            "Differentiation order should be greater than 0."
        self.model = tf.keras.Model(x, y)
        self.order = order

    def eval_grad(self, inputs, order=1):
        if order==1:
            with tf.GradientTape() as t1:
                t1.watch(inputs)
                outputs = self.model(inputs)
            return t1.gradient(outputs, inputs, unconnected_gradients='zero')
        else:
            with tf.GradientTape() as ti:
                ti.watch(inputs)
                grad = self.eval_grad(inputs, order=order-1)
            return ti.gradient(grad, inputs, unconnected_gradients='zero')

    def call(self, inputs):
        return self.eval_grad(inputs, self.order)

    # @tf.function
    # def eval_grad(self, inputs):
    #     outputs = self.model(inputs)
    #     for i in range(self.order):
    #         outputs = tf.gradients(outputs, inputs, unconnected_gradients='zero')
    #     return outputs

    # def call(self, inputs):
    #     return self.eval_grad(inputs)

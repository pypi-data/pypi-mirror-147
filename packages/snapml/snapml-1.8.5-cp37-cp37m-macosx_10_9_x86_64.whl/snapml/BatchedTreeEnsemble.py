...
class BatchedTreeEnsemble(BaseEstimator):
    ...
    def _validate_parameters(self):
        ...
        self.base_ensemble_reg_first = clone(self.base_ensemble_reg)
    ...
    def build_ensemble(self, X, target, weights, first_batch):
        ...
        if first_batch:
            new_ensemble = clone(self.base_ensemble_reg_first)
        else:
    ...
    def train_on_batch(self, X, y, sample_weight=None):
        ...
        if self.n_classes_ > 2:
            pred = np.zeros(shape=(X.shape[0], self.n_classes_))
            for i, te in enumerate(self.ensembles_):
                cls_ind = i % self.n_classes_
                pred[:, cls_ind] += self.learning_rate * te.predict(X)
        else:
            pred = np.zeros(shape=(X.shape[0],))
            for i, te in enumerate(self.ensembles_):
                pred += self.learning_rate * te.predict(X)
        ...
        if self.n_classes_ > 2:
            ...
            for c in range(self.n_classes_):
                new_ensembles.append(
                    self.build_ensemble(X, target[:, c], weights[:, c], first_batch)
                )

            if len(self.ensembles_) < self.n_classes_ * (1 + self.max_sub_ensembles):
                for e in new_ensembles:
                    self.ensembles_.append(e)
            else:
                offset = self.max_sub_ensembles * self.n_classes_
                for i, e in enumerate(new_ensembles):
                    self.ensembles_[offset + i] = e
        else:

            new_ensemble = self.build_ensemble(X, target, weights, first_batch)

            if len(self.ensembles_) < (1 + self.max_sub_ensembles):
...
class BatchedTreeEnsembleClassifier(BatchedTreeEnsemble, ClassifierMixin):
    ...
    def fit(self, X, y, sample_weight=None):
        ...
        """
        Fit the ensemble on a batch of data.
        Equivalent to calling partial_fit with first batch.
        The classes are automatically extracted from the batch of labels provided.
        ...
        return self.partial_fit(X, y, sample_weight, np.unique(y))
    ...
    def predict(self, X):
        ...
        margin = self.__predict_margin(X)
    ...
    def __predict_margin(self, X):
        ...
        if self.n_classes_ > 2:
            ...
            for i, ensemble in enumerate(self.ensembles_):
                cls_ind = i % self.n_classes_
                preds[:, cls_ind] += self.learning_rate * ensemble.predict(X)
        else:
            preds = np.zeros(shape=(X.shape[0],))
            for ensemble in self.ensembles_:
                preds += self.learning_rate * ensemble.predict(X)

...
class DataObject(ABC):
    def __init__(self, source, graph_based = None, replace_missing="Unassigned",
        ...
        if sharing_history is None:
            self.sharing_history = []
        else:
            self.sharing_history = sharing_history.copy()

        history_obj = generate_history_object()
        history_obj["hash_id"] = self.study_id
        self.sharing_history.append(history_obj)
...
class ScanpyData(DataObject):
    def __init__(self, source, graph_based = None, raw_key="counts", pca_key="X_pca",
        ...
        """
        if not isinstance(source, AnnData):
            source = scanpy.read_h5ad(source, "r")
        self.object = source

        if "sharing_history" in source.uns:
            sharing_history = source.uns["sharing_history"].to_dict(orient="records")
...
class SpringData(SubclusterData):
    def __init__(self, source, graph_based = None, **kwargs):
...
class LoomData(DataObject):
    def __init__(self, source, graph_based=None, raw_key="counts",

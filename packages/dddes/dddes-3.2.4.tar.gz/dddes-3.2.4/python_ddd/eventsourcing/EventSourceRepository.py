...
class MongoEventSourceRepository(EventPublisherRepository, metaclass=abc.ABCMeta):
    ...
    def __create_indexes(self):
        existing_memento_indexes = list(
            self.__collection.index_information().keys()
        )
        existing_memento_indexes = list(
            self.__memento_collection.index_information().keys()
        )

        if "eventid_unique" not in existing_memento_indexes:
            self.__collection.create_index(
                [("event_id", ASCENDING)], name="eventid_unique", unique=True
            )

        if "objectid_version_unique" not in existing_memento_indexes:
            self.__collection.create_index(
        ...
        if "event_timestamp" not in existing_memento_indexes:
            self.__collection.create_index(
                [("event_timestamp", DESCENDING)], name="timestamp_ascending"
            )

        if "memento_objectid_unique" not in existing_memento_indexes:

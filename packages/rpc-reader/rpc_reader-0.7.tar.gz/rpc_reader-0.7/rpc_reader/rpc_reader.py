...
class ReadRPC(object):
    ...
    def __read_data__(self, file_handle):
        ...
        print(f"\tHeader end at {self.headers['NUM_HEADER_BLOCKS'] * 512} bytes\n"
              f"\tFile end at {self.__file_size__}\n"
              f"\tBytes to read: {self.__file_size__ - self.headers['NUM_HEADER_BLOCKS'] * 512}")
        ...
        data_order = list()
        frame_no = 1
        print(f' frames: {frames}, ppg: {point_per_group}, ppf: {point_per_frame},'
              f' frame per group: {frames_per_group}, number of groups: {number_of_groups}')
        ...
        if self.debug:
            print(f' Frame grouping array {data_order}')
        ...
        for frame_group in data_order:
            ...
            for channel in range(channels):
                for frame in frame_group:
                    ...
                    # print(channel+1, frame, r1, r2, len(self.data[r1:r2, channel]), len(data), file_handle.tell())

...
if not hasattr(sys, "version_info") or sys.version_info < (3, 5):
    print("Sorry, you aren't running on Python 3.5 or later\n")
    print("Please upgrade to 3.5 or later.\n")
    sys.exit(1)

aiui = None

if platform.system() == "Windows":
    arch, os_ = platform.architecture()
    filepath = site.USER_BASE + "\\pyaiui\\libs\\" + os_ +  "\\" + arch + "\\libaiui.so"

    if not os.path.isfile(filepath):
        filepath = sys.prefix + "\\pyaiui\\libs\\" + os_ +  "\\" + arch + "\\libaiui.so"

    aiui = ctypes.windll.LoadLibrary(filepath)
else :
    arch, os_ = platform.architecture()
    filepath = site.USER_BASE + "/pyaiui/libs/" + os_ +  "/" + arch + "/libaiui.so"
    ...
    aiui = ctypes.cdll.LoadLibrary(filepath)
...
class IDataBundle:
    ...
    def getString(self, key: str, defaultVal: str):
        ...
        len = self.aiui_strlen(a)
        
        arrayType = ctypes.c_byte * len
        pa = ctypes.cast(s, ctypes.POINTER(arrayType))
        
        return str(pa.contents, encoding="utf-8")
    ...
    def getBinaryAsStr(self, key: str):
        ...
        arrayType = ctypes.c_byte * (datalen.value - 1)
        pa = ctypes.cast(s, ctypes.POINTER(arrayType))

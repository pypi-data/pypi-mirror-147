...
def downloadGraph(gClient,
    ...
    uploadFolderName="tmp_{}".format(int(time.time()*1000))
    if uploadFolderName not in os.listdir("./downloadGraph"):
        os.mkdir("./downloadGraph/"+uploadFolderName)
    for srcVertexTypeItem in srcVertexKeynameDict:
        ...
        vertexDfItem.to_csv("./downloadGraph/{}/{}.csv".format(uploadFolderName,srcVertexTypeItem),index=None)
        vertexSchemaItem={
    ...
    for edgeTypeItem in edgeTypeList:
        for srcVertexTypeItem_head in srcVertexKeynameDict:
            for tgtVertexTypeItem_tail in srcVertexKeynameDict:
                ...
                edgeDfItem.to_csv("./downloadGraph/{}/{}.csv".format(uploadFolderName,edgeTypeItem),index=None)
                edgeSchemaItem={
    ...
    with open("./downloadGraph/{}/rawSchema.json".format(uploadFolderName),"w+") as rawSchemaFile:

...
def get_tags_df(df, graph, content_label):
    tag_df = DataFrame()
    for index, row in df.iterrows():
        try:
            tags = graph.custom_query(f'''
            g.V().hasLabel('{content_label}').has('{CONTENT_ID}',{row[CONTENT_ID]}).out('{HAS_TAG}').valueMap().by(unfold()).toList()
            ''', payload={
                content_label: content_label,
                CONTENT_ID: row[CONTENT_ID],
                HAS_TAG: HAS_TAG
            ...
            print(row[CONTENT_ID])
            df_tag_data = pd.DataFrame([{CONTENT_ID: row[CONTENT_ID], TAGS_DESCRIPTION: ''}])
            tag_df = pd.concat([tag_df, df_tag_data], axis=0)

        for tag in tags:
            for idx, x in enumerate(tag):
                df_tag_data = pd.DataFrame([{CONTENT_ID: row[CONTENT_ID], TAGS_DESCRIPTION: x["tags_description"]}])
                tag_df = pd.concat([tag_df, df_tag_data], axis=0)
        tags = []
...
def combine_features(df, graph, content_label):
    final_df = df
    tag_df = get_tags_df(df, graph, content_label)

...
class SimilarityUtils:
        ...
        for df in list_dataframe_homepage:
            for idx, values in df.iterrows():
                try:
                    ...
                    content_similarity_property = {content_homepage_id: list_dict_content_similarities[0]}
                    updated_cc_similarity = {**literal_eval(existing_cc_similarity), **content_similarity_property}
                    print("updated cc_similarity_score : ", updated_cc_similarity)
                    print("updating cc_similarity for content ID : ", values[CONTENT_ID])
    ...
    def add_all_content_similarity_property(all_content_new_df, all_content_dict_cos_sim, content_label, graph):
        for idx, values in all_content_new_df.iterrows():
            try:
                node_to_update = Node(**{LABEL: content_label, PROPERTIES: {CONTENT_ID: int(values[CONTENT_ID])}})
                query_content_node = graph.find_node(node_to_update)
                print("updated all_similarity_score : ", str(all_content_dict_cos_sim))
                print("updating all_similarity_score for content ID : ", values[CONTENT_ID])
                graph.update_node_property(query_content_node[0], {ALL_SIMILARITY_SCORE: str(all_content_dict_cos_sim)})
            except Exception as e:
                print(e)
    ...
    @staticmethod
    def add_content_similarity_all_content(content_label, graph):
    ...
    @staticmethod
    def add_content_similarity_property(content_label, content_homepage_id, graph):
        try:
            try:
            ...
            try:
                SimilarityUtils.add_content_similarity_all_content(content_label, graph)
            except Exception as e:
                print(e)
                print('Not able to add content similarity for all contents')

        except Exception as e:
            print(e)
            print('Not able to add content similarity for all contents and home page id')
        return True

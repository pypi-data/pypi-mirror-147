    ...
    if 'linear' in model_regression:
        if return_metric:
            result, model, metrics_values = linear(df, X_columns, y_column, model_regression, metric=metric,
                                                   ...
                                                   random_state=random_state, force_coeffs=force_coeffs, coeffs=coeffs, intercept=intercept)
        else:
            result, model = linear(df, X_columns, y_column, model_regression, metric=metric, return_metric=return_metric,
                                   size=size, positive=positive, random_state=random_state, force_coeffs=force_coeffs, coeffs=coeffs, intercept=intercept)
        ...
        if return_metric:
            ...
            result, model = logLinear(df, X_columns, y_column, model_regression, metric=metric, return_metric=return_metric,
                                      size=size, positive=positive, random_state=random_state, force_coeffs=force_coeffs, coeffs=coeffs, intercept=intercept)
        ...
        if return_metric:
            ...
            result, model = logLog(df, X_trasformations_columns, X_columns, y_column, model_regression,
                                   model_regression=model_regression_log_log, metric=metric,
                                   return_metric=return_metric, size=size, positive=positive, random_state=random_state, force_coeffs=force_coeffs, coeffs=coeffs, intercept=intercept)

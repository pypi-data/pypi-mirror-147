from cassandra.data.dataAnalysis.correlation import *
from cassandra.model.modelEvaluation.plot import show_coefficients
from cassandra.data.featureSelection.ols import ols
import nevergrad as ng
from cassandra.model.linear import *
from cassandra.model.logLinear import *
from cassandra.model.logLog import *
from cassandra.model.mlpRegressor import *
from cassandra.model.ridge import *
from cassandra.model.modelEvaluation.plot import *
from cassandra.model.nestedModel import *
from cassandra.data.dataProcessing.prophet import *
from cassandra.data.trasformations.trasformations import *
from cassandra.model.modelEvaluation.evaluation import *
from cassandra.data.dataAnalysis.plot import *
from cassandra.data.dataProcessing.clean_and_format import *
from cassandra.data.featureSelection.ols import *
from cassandra.budgetAllocator.budget_allocator import *
from cassandra.budgetAllocator.utils import *
from cassandra.utils.save import *
import pandas as pd


# Example how create a dict_columns_aggregate dict_columns_aggregate = { 'keywords_impressions' : [
# 'adopta_albero_impressions', 'regalar_arbol_impressions', 'regala_arbol_impressions', 'plantar_arbol_impressions',
# 'treedom_impressions'] }
def create_csv(csv_path, dict_columns_aggregate={}):
    csv = pd.read_csv(csv_path)

    for key, value in dict_columns_aggregate.items():
        # Aggregate columns into one for each key
        csv[key] = csv[value].sum(axis=1)

        # Drop un-used columns
        csv.drop(value, axis=1, inplace=True)
        csv.fillna(value=0)

    return csv


def define_features(df, media_split=True):
    media = [col for col in df.columns if 'spend' in col]

    organic = organic = [col for col in df.columns if 'mm' in col]

    seasonality = []
    if 'trend' in df.columns:
        seasonality.append('trend')
    if 'holidays' in df.columns:
        seasonality.append('holidays')
    if 'yearly' in df.columns:
        seasonality.append('yearly')
    if 'monthly' in df.columns:
        seasonality.append('monthly')
    if 'weekly' in df.columns:
        seasonality.append('weekly')
    if 'daily' in df.columns:
        seasonality.append('daily')

    price = [col for col in df.columns if 'avg' in col]

    media_google = [col for col in media if 'g_' in col]

    media_fb = [col for col in media if 'fb_' in col]

    media_others = [col for col in media if 'fb_' not in col if 'g_' not in col]

    everything = media + organic + price + seasonality
    no_price = media + organic + seasonality
    no_seasonality = media + organic + seasonality
    no_organic = media + price + seasonality
    no_media = organic + price + seasonality

    if media_split:
        return everything, media, organic, price, seasonality, no_media, no_organic, no_price, no_seasonality, media_google, media_fb, media_others
    else:
        return everything, media, organic, price, seasonality, no_media, no_organic, no_price, no_seasonality


def show_media(df, name_date_column, name_target_column, media, media_google=[], media_fb=[], media_others=[]):
    if media_google:
        #Google media graph
        fig_media_google = px.line(df, x=name_date_column, y=media_google, title='Media Google over time')
        fig_media_google.show()
        show_trendline_curves(media_google, df, name_target_column)

    if media_fb:
        #Facebook media graph
        fig_media_fb = px.line(df, x=name_date_column, y=media_fb, title='Media Facebook over time')
        fig_media_fb.show()
        show_trendline_curves(media_fb, df, name_target_column)

    if media_others:
        #Others media graph
        fig_media_others = px.line(df, x=name_date_column, y=media_others, title='Media Others over time')
        fig_media_others.show()
        show_trendline_curves(media_others, df, name_target_column)

    #Target vs media
    show_target_vs_media_spent_graph(df, name_date_column, name_target_column, media)


def show_no_media(df, name_date_column, name_target_column, organic=[], price=[]):
    if organic:
        # Organic graph
        fig_organic = px.line(df, x=name_date_column, y=organic, title='Organic over time')
        fig_organic.show()

    if price:
        # Organic graph
        fig_price = px.line(df, x=name_date_column, y=price, title='Price over time')
        fig_price.show()



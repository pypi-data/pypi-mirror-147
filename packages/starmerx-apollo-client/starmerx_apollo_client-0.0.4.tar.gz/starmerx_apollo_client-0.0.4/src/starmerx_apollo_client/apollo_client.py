...
class ApolloClient(object):
    ...
    def get_json_from_net(self, namespace='application'):
        url = '{}/configs/{}/{}/{}?releaseKey={}&ip={}'.format(self.config_url, self.app_id, self.cluster, namespace,
                                                               "", self.ip)
        try:
            code, body = http_request(self.http_client, url, timeout=3, headers=self._signHeaders(url))
            if code == 200:
    ...
    def _long_poll(self):
        ...
        try:
            ...
            url = url + '?' + param_str
            code, body = http_request(self.http_client, url, self._pull_timeout, headers=self._signHeaders(url))
            http_code = code
            if http_code == 304:

...
class AWSEventStep(abc.ABC):
    ...
    @abstractmethod
    def execute(self, event: dict, contex: dict) -> Tuple[Dict, Dict]:
        ...
...
def lambda_event_pipe(event: dict, context: dict,
    ...
    for mw in steps:
        try:
            event, context = mw.execute(event, context)
        except Exception as e:
            logger.exception(f"An error occurred when execute middleware: {mw.name}. ", exc_info=e)
            if mw.raise_fails:
                return mw.response
    try:
        return processor(event, context)
    except Exception as e:
        logger.exception(f"An error occurred when execute final processor... ", exc_info=e)

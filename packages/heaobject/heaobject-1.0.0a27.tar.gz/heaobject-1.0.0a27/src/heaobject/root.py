...
class AbstractDesktopObject(AbstractHEAObject, DesktopObject, abc.ABC):
    ...
    def get_permissions(self, sub: str) -> List[Permission]:
        ...
        if self.owner == sub:
            return list(p for p in Permission if p != Permission.CHECK_DYNAMIC)

        result = set(perm for share in self.shares for perm in share.permissions if share.user == sub)
    ...
    def has_permissions(self, sub: str, perms: List[Permission]) -> bool:
        ...
        def share_has_perms(share):
            return share.user == sub and any(perm in perms for perm in share.permissions)

        return self.owner == sub or \
               any(share_has_perms(share) for share in self.shares) or \

...
class UnencryptedModuleOutput(BioLibBinaryFormatBasePackage):
    ...
    def convert_to_serialized_module_output(self) -> bytes:
        ...
        files_data_start = files_info_start + metadata['files_info_length']
        files_data_end = files_data_start + metadata['files_data_length'] - 1

        # Get all file data at once
        files_data_buffer = InMemoryIndexableBuffer(data=self._buffer.get_data(files_data_start, files_data_end))
        ...
        while files_info_buffer.pointer < len(files_info_buffer):
            ...
            files_dict[path] = files_data_buffer.get_data(data_start, data_end)

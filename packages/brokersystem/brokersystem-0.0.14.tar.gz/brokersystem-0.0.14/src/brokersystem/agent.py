...
class Agent:
    ...
    def __init__(self, broker_url):
        ...
        self.interface = AgentInterface(self._agent_funcs)
    ...
    @classmethod
    def register_func(cls, func_name, func):
        logger.debug(f"AM REGISTER FUNC: {func_name}, {cls._agent_funcs}")
        cls._agent_funcs[func_name] = func
...
def config(func):
    ...
    Agent.register_func("make_config", wrapper)
    return wrapper

def negotiation(func):
    def wrapper(*args, **kwargs):
        return func(*args, **kwargs)
    Agent.register_func("negotiation", wrapper)
    return wrapper

def job_func(func):
    def wrapper(*args, **kwargs):
        return func(*args, **kwargs)
    Agent.register_func("job_func", wrapper)
    return wrapper
...
class AgentManager:
    ...
    def start_watching_loop(self):
        logger.info("Start automatic agent runner...")
        while True:
            try:
                self.dog_watching()
    ...
    def dog_watching(self):
        exisiting_files = set(glob.glob(os.path.join(self.dir_path, "sample_agent.py")))
    ...
    def run_module(self, module):
        LocalAgent = None
        if hasattr(module, "agent") and hasattr(getattr(module, "agent"), "Agent"):
            LocalAgent = getattr(getattr(module, "agent"), "Agent")
        if LocalAgent is None:
            return
        agent = LocalAgent(self.broker_url)
        return agent
...
def _check_if_batch_mode_is_on():
    return len(sys.argv) >= 2 and ("http://" in sys.argv[1] and "https://" in sys.argv[1])

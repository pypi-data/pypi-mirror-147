...
class TemplateContainer:
    ...
    def __setattr__(self, key: str, value):
        ...
        if not isinstance(value, ValueTemplate):
            logger.error(f"You can only set ValueTemplate object(such as Number, Stirng, or Range) for input, condition and output: {key}")
            return
...
def run(broker_url):
    ...
    interface.prepare()
    Agent(broker_url, interface.secret_token, interface)

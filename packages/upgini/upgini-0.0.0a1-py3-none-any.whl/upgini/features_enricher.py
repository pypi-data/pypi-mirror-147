...
class FeaturesEnricher(TransformerMixin):  # type: ignore
    ...
    def __inner_fit(
        ...
        **fit_params,
        ...
        self.enriched_X = self.__enrich(df, self._search_task.get_all_initial_raw_features(), X.index, self.keep_input)
    ...
    def calculate_metrics(self, X: pd.DataFrame, y, scoring: Union[str, Callable, None] = None):
        ...
        fitting_enriched_X = self.enriched_X.drop(columns=self.search_keys.keys())
        ...
        if self.etalon_metric is not None:
            self.uplift, metric = self.enriched_metric - self.etalon_metric

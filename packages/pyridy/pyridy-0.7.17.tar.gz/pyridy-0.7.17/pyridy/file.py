    ...
    def do_map_matching(self, v_thres: float = config.options["MAP_MATCHING_V_THRES"],
        ...
        if self.osm:
            ...
            if not s_n or not e_n:
                ...
                m_ratios = [len(set(self.matched_nodes).intersection(line.nodes)) / len(self.matched_nodes) for line in
                            self.osm.railway_lines]
                ...
                if max(m_ratios) > config.options["MAP_MATCHING_MIN_LINE_MATCH_RATIO"]:
                    ...
                    logger.debug("(%s) Matched Railway Line: %s" % (self.filename, self.matched_line.name))
                else:
                    logger.debug("(%s) Could not match map matching results to a specific railway line" % self.filename)
        else:

...
class Session:
    ...
    def authorization(self, user, pw):
        ...
        if resp.status_code == 200 and resp.text:
            ...
            # cookie = dict_from_cookiejar(cookies_data)
    ...
    def __request(self, method, api, params={}, data={}, kwargs={}):
        ...
        if 'session' not in self.cookies.items():
            extract_cookies_to_jar(self.cookies, resp.request, resp.raw)
        else:
            for cookie in self.cookies:
                if cookie.name == 'session':
                    if cookie.expires <= datetime.datetime.now():
                        self.cookies.pop('session')

        # 處理 responds 的 session,拿到新 session 就加入，沒日到新的代表有舊的，就看看過期了沒

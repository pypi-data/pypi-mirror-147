...
class MultipleFactor(object):
    ...
    def _transformer(self, signal):
        ...
        short['signal'] = short['signal'].apply(lambda x: x if x < 0 else  0-x)
        long['signal'] = long['signal'].apply(lambda x: x if x > 0 else 0-x)
        return pd.concat([long,short,equal],axis=0).sort_values(
            by=['signal']).reset_index(drop=True)
    ...
    def _weighted_normalised(self, target_pos):
        long_target = target_pos[target_pos['weight'] > 0]
        short_target = target_pos[target_pos['weight'] < 0]
        equal_target = target_pos[target_pos['weight'] == 0]
        long_target = long_target.reset_index().set_index(['trade_date']).groupby(
            level=['trade_date']).apply(lambda x: self._weighted(data=x, equal=0, name='weight')
            ).reset_index()[['trade_date','weight','code']]
        short_target = short_target.reset_index().set_index(['trade_date']).groupby(
            level=['trade_date']).apply(lambda x: self._weighted(data=x, equal=0, name='weight')
            ).reset_index()[['trade_date','weight','code']]
        short_target['weight'] = 0 - short_target['weight']
        new_target_pos = long_target.append(short_target).append(equal_target.reset_index())
        return new_target_pos.sort_values(by=['trade_date','code']).set_index(['trade_date','code'])

    ### 权重计算
    def weighted_run(self, columns=None, default_volatility=0.1, volatility_name='STD20D', period=20, 
                    is_symmetry=True, is_normalised=True):
        ...
        return target_pos if not is_normalised else self._weighted_normalised(target_pos)
    ...
    def equal_run(self, columns=None, equal=1,is_symmetry=True):
        ...
        ## weight
        long_weighted = long_signal.set_index(['trade_date']).groupby(
            level=['trade_date']).apply(
                lambda x: self._weighted(data=x, equal=equal, name='signal')
            ).reset_index()[['trade_date','weight','code']]

        short_weighted = short_signal.set_index(['trade_date']).groupby(
            level=['trade_date']).apply(
                lambda x: self._weighted(data=x, equal=equal, name='signal')
            ).reset_index()[['trade_date','weight','code']]

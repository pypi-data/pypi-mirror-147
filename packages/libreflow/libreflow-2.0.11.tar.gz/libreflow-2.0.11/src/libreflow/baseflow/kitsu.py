...
class KitsuAPIWrapper(flow.Object):
    ...
    def get_shots(self, task_status_filters=None):
        if task_status_filters is not None:
            shots = []
            shot_is_valid = defaultdict(lambda: True)
            target_tasks = {}
            
            for type_name, status_name in task_status_filters.items():
                type_id = gazu.task.get_task_type_by_name(type_name)['id']
                status_id = gazu.task.get_task_status_by_name(status_name)['id']
                target_tasks[type_id] = status_id
            
            for t in gazu.task.all_tasks_for_project(self.get_project_id()):
                if t['task_type_id'] not in target_tasks:
                    continue
                # print(t)
                shot_is_valid[t['entity_id']] = (
                    shot_is_valid[t['entity_id']]
                    and target_tasks[t['task_type_id']] == t['task_status_id']
                )
            
            shots = [
                gazu.shot.get_shot(shot_id)
                for shot_id, valid in shot_is_valid.items() if valid
            ]
        else:

...
class TradeClass():
    ...
    def QryOptCombPosition(self,accountMask, qryIndex=""):
        if  qryIndex=="":
            ...
            self.last=""
        self.lock.acquire()
        obj = {"Request":"QUERYOPTCOMBPOSITION","SessionKey":self.tsessionKey,"AccountMask":accountMask,"QryIndex":qryIndex}
        self.tsocket.send_string(json.dumps(obj))
        message = self.tsocket.recv()[:-1]
        data = json.loads(message)
        self.lock.release()
        if data["Reply"] == "QUERYOPTCOMBPOSITION":
            ...
            for ord in data["Positions"]:
                self.datas.append(ord)
                self.last = ord["QryIndex"]
            return self.QryOptCombPosition(accountMask,self.last)
    ...
    def SubHistory(self,symbol, type, startTime, endTime):
        ...
        def GetHistory(symbol, type, startTime, endTime, qryIndex=""):
            ...
            if len(message["HisData"]) == 0:
                ...
                historydata.clear()
                if len(temp)!=0:
                    if self.realdate and symbol in self.realdate.keys() and self.realdate[symbol]<=endTime:

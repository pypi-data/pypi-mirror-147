...
class IAEEngineClient(Client):
    ...
    def __init__(self, server_url, service_instance_name, service_instance_id, volume, token):
        ...
        self.iae_instance_client = IAEInstanceClient(
            self.server_url, self.service_instance_name, self.service_instance_id, self.volume, self.token)
    ...
    def add_delegation_token_to_payload(self, conf: dict, job_args: dict):
        ...
        """
        if all(conf.get(param) for param in constants.DELEGATION_TOKEN_PARAMS):
            # All token details are already present in conf, so do nothing and return
            logger.info(
                "The delegation token details are already set in the job payload.")
            return

        delegation_token_details = None
        delegation_token_found = False
        monitoring_run_params = get(job_args, "monitoring_run_params")
        ...
        if monitoring_run_params:
            logger.info(
                "Fetching the delegation token details from monitoring run parameters.")
            delegation_token_details = dict()
            for param in constants.DELEGATION_TOKEN_PARAMS:
                if param in monitoring_run_params:
                    delegation_token_details[param] = monitoring_run_params.get(
                        param)
            # Checking if all parameters related to delegation token are found
            delegation_token_found = JoblibUtils.does_dict_contain_req_details(
                delegation_token_details, constants.DELEGATION_TOKEN_PARAMS)

        # If token details not found in run parameters, checking if vault secret_urn is specified in job_params
        if delegation_token_found is False and delegation_token_urn:
            logger.info(
            ...
            delegation_token_details = get(secret, "data.secret.generic.value")
            delegation_token_found = JoblibUtils.does_dict_contain_req_details(
                delegation_token_details, constants.DELEGATION_TOKEN_PARAMS)
        ...
        if delegation_token_found is False and delegation_token_endpoint:
            logger.info(
                "Fetching the delegation token details from the API endpoint hosted in user's cluster.")
            delegation_token_details = self.iae_instance_client.invoke_delegation_token_provider(
                delegation_token_endpoint)
            delegation_token_found = JoblibUtils.does_dict_contain_req_details(
                delegation_token_details, constants.DELEGATION_TOKEN_PARAMS)

        # Set the token details in job payload under conf section
        if delegation_token_details:
            conf.update(delegation_token_details)
        # If not all token details are specified, raise an error mentioning list of missing token parameters
        if delegation_token_found is False:
            missing_params = constants.DELEGATION_TOKEN_PARAMS if conf is None else list(set(
                constants.DELEGATION_TOKEN_PARAMS) - set(conf.keys()))

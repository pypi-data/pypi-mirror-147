...
from mcli.config_objects.mcli_secret import (MCLIDockerRegistrySecret, MCLIGenericEnvironmentSecret,
                                             MCLIGenericMountedSecret, MCLIS3Secret, MCLISecret, MCLISSHSecret,
                                             SecretType)
...
SECRETS: Dict[SecretType, MCLISecret] = {
    ...
    SecretType.generic_environment:
...
def check_volume_and_mount(secret: MCLIGenericMountedSecret, mcli_job: MCLIK8sJob):
...
def test_mounted_secret_add_to_job(mcli_job):
    ...
    secret = SECRETS[SecretType.generic_mounted]
    assert isinstance(secret, MCLIGenericMountedSecret)
    secret.add_to_job(mcli_job)
...
def test_env_secret_add_to_job(mcli_job):
    ...
    secret = SECRETS[SecretType.generic_environment]
    assert isinstance(secret, MCLIGenericEnvironmentSecret)
    secret.add_to_job(mcli_job)

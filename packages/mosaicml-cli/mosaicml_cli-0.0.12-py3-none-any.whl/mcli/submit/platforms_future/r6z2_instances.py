""" Available Instances for the R6Z2 Platform """
import re
...
def add_experimental_flags_to_instance(instance: InstanceType) -> None:
...
for r6z2_instance in ALLOWED_INSTANCES:
    add_node_selector_to_instance(r6z2_instance)
    add_experimental_flags_to_instance(r6z2_instance)

valid_r6z2_regex_configs = [
    'r6z2-g[1-8]-a100$',
    r'r6z2-g[1-8]-c[1-9]\d?\d?-a100',  # type: ignore
]


class R6Z2InstanceList(InstanceList):
    """ Available Instances for the R6Z2 Platform """
    ...
    def get_instance_by_name(self, instance_name: str) -> Optional[InstanceType]:
        ...
        if any((re.match(x, instance_name) for x in valid_r6z2_regex_configs)):
...
R6Z2_INSTANCE_LIST = R6Z2InstanceList(instances=ALLOWED_INSTANCES)

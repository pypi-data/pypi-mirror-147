...
def get_smallworld_cnn(seq_len, seq_depth, kernel_size, dropout, n_of_kernels=256, n_of_layers=1):
    ...
    for i_layer in range(n_of_layers - 1):
        ...
        for i_prev_layer in range(1, len(conv_layers) + 1):
            permut = np.random.permutation(n_of_kernels)
...
if __name__ == '__main__':
    ...
    kernel_size = 11
    dropout = 0.1
    smallworld_cnn = get_smallworld_cnn(seq_len, seq_depth, kernel_size, dropout, n_of_kernels=256, n_of_layers=10)

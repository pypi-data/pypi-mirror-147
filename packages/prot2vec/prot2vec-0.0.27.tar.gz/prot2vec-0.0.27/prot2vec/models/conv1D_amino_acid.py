...
def get_primitive_cnn(seq_len, seq_depth, kernel_size, dropout, n_of_kernels):
    input_layer = Input(shape=(seq_len, seq_depth), name='input_seq')

    x = Conv1D(n_of_kernels, kernel_size, activation='relu', strides=1, padding='same', dilation_rate=1)(input_layer)
    x = Dropout(dropout)(x)

...
def horner(Xin: np.array, M: np.array, N=int(0), D=int(0), Scale=True) -> np.array:
    # TODO, right now, the polynomial order is required to be a vector with the same dimension has the number of
    #  variables.
    ...
    if (N == 0) or (D == 0):
        if isinstance(Xin, np.ndarray):
            ...
            HornerDict = {'error': True,
                          'errormsg': 'Make sure you send a numpy array for your matrix of independent variables.'}
            return HornerDict
    ...
    if isinstance(M, int):
        ...
        if np.size(M, 0) == D:
            ...
            HornerDict = {'error': True,
                          'errormsg': 'Provide a polynomial order for each independent variable, or a single integer.'}
            return HornerDict
    ...
    if D == np.size(M, 0):
        ...
        HornerDict = {'error': True,
                      'errormsg': 'Wrong input format to PolyHorner. The polynomial order must be a column vector the '
                                  'same size has the number of columns in Xin.'}
        return HornerDict
    ...
    HornerDict = {'X': X, 'exponents': expo, 'scalingfactors': scalingfactors, 'Nterms': Nterms, 'Dim': D,
                  'error': False}

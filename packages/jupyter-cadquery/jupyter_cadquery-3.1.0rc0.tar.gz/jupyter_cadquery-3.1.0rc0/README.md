...
1. For using _Jupyter-CadQuery_ **in Jupyterlab**
   ...
   - Install _Jupyter-CadQuery_ (note, matplotlib is only used for the examples)
     ```bash
     pip install jupyter-cadquery==3.1.0rc0 matplotlib
     ```
...
1. For using _Jupyter-CadQuery_ **in Jupyterlab**
   ...
   - Start _Jupyter-Cadquery_
     ...
     docker run -it --rm -v $WORKDIR:/home/cq -p 8888:8888 bwalter42/jupyter_cadquery:3.1.0rc0
     ```
   ...
   - To start with examples, you can
     ...
     - omit the volume mapping and just run `docker run -it --rm -p 8888:8888 bwalter42/jupyter_cadquery:3.1.0rc0` or
...
2. For running _Jupyter-CadQuery_ as **standalone viewer**
   ...
   - Start the browser based viewer (eventually add cli options, see notes below):
     ...
     ```bash
     docker run -it --rm -p 8888:8888 --name jcq -p 5555:5555 bwalter42/jupyter_cadquery:3.1.0rc0 -v
     ```

...
def payment(request, uuid):
    ...
    if pay_obj.is_success():
        return redirect(reverse('aparnik:shops:payments:pay-success', kwargs={'uuid': uuid}))

    return redirect(reverse('aparnik:shops:payments:pay-failed', kwargs={'uuid': uuid}))


def pay_success(request, uuid):
    payment_obj = get_object_or_404(Payment.objects.all(), uuid=uuid)
    html_head = Setting.objects.get(key='PROJECT_HTML_HEAD').get_value()
    context = {
        'obj': payment_obj,
        'app_url': request.build_absolute_uri(payment_obj.get_api_uri()),
        'html_head': html_head,
    }
    return render(request=request, template_name='suit/paysuc.html', status=200, context=context)


def pay_failed(request, uuid):
    payment_obj = get_object_or_404(Payment.objects.all(), uuid=uuid)
    html_head = Setting.objects.get(key='PROJECT_HTML_HEAD').get_value()
    context = {
        'obj': payment_obj,

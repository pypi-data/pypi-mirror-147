...
class CouponManager(models.Manager):
    ...
    def status(self, code, user, order):
        ...
        # check coupon include special products
        if coupon.products.exists():
            product_ids = order.items.all().values_list('product_obj_id', flat=True)
            if coupon.products.constrained_target.filter(product_obj_id__in=product_ids).count() != product_ids.count():
                raise ValidationError({'coupon_code': [_('The coupon code entered is not valid for this course.')]})

        # add coupon
        if coupon.for_users == Coupon.COUPON_FOR_ALL_USER and \
                (not coupon.disposable or coupon.membership_set.filter(user=user, is_redeem=True).count() == 0):
...
class Coupon(models.Model):
    ...
    type = models.CharField(max_length=2, choices=COUPON_TYPE_CHOICES, default=COUPON_TYPE_PERCENT,
                            verbose_name=_('type'))
    products = models.ManyToManyField(Product, through='ProductCoupon', blank=True, verbose_name=_('Products'))
    ...
    disposable = models.BooleanField(default=True, verbose_name=_('Is Disposable'))
...
class ProductCoupon(models.Model):
    ...
    class Meta:
        ...
        verbose_name_plural = _('Product Coupon relations')

...
class F5669():
    ...
    def goTo5669(self):
        return [
            ...
            "label": "Start/Edit for 5669 form",
    ...
    def sectionABlockApplicant(self,person:PrModel):
        return [
            ...
            },
            {
                "action_type": "Input",
                "label": "Full name",
                "value": person.personal.native_first_name+" "+person.personal.native_last_name,
    ...
    def sectionABlock(self,relationship,family_member:Family):
        return [
            ...
            },
            {
                "action_type": "Input",
                "label": "Date of death",
                "value": family_member.date_of_death.strftime("%Y/%m/%d") if family_member.date_of_death else None,
    ...
    def makeSectionB(self,person:PrModel):
        return [
            {
                "action_type":Action.Radio.value,
                "id": "#isConvictedInCanada_yes" if person.prbackground.q1 else "#isConvictedInCanada_no" ,
                "label":"Yes" if person.prbackground.q1 else "No"
            },
            {
                "action_type":Action.Radio.value,
                "id": "#isConvictedOutsideCanada_yes" if person.prbackground.q2 else "#isConvictedOutsideCanada_no",
                "label":"Yes" if person.prbackground.q2 else "No"
            },
            {
                "action_type":Action.Radio.value,
                "id": "#isClaimedRefugeeProtection_yes" if person.prbackground.q3 else "#isClaimedRefugeeProtection_no" ,
                "label":"Yes" if person.prbackground.q3 else "No"
            },
            {
                "action_type":Action.Radio.value,
                "id": "#isConvictedInCanada_yes" if person.prbackground.q3 else "#isConvictedInCanada_no" ,
                "label":"Yes" if person.prbackground.q3 else "No"
            },
            {
                "action_type":Action.Radio.value,
                "id": "#isRefusedRefugeeOrVisa_yes" if person.prbackground.q4 else "#isRefusedRefugeeOrVisa_no" ,
                "label":"Yes" if person.prbackground.q4 else "No"
            },
            {
                "action_type":Action.Radio.value,
                "id": "#isOrderedToLeaveCountry_yes" if person.prbackground.q5 else "#isOrderedToLeaveCountry_no" ,
                "label":"Yes" if person.prbackground.q5 else "No"
            },
            {
                "action_type":Action.Radio.value,
                "id": "#isWarCriminal_yes" if person.prbackground.q6 else "#isWarCriminal_no" ,
                "label":"Yes" if person.prbackground.q6 else "No"
            },
            {
                "action_type":Action.Radio.value,
                "id": "#isCommittedActOfViolence_yes" if person.prbackground.q7 else "#isCommittedActOfViolence_no" ,
                "label":"Yes" if person.prbackground.q7 else "No"
            },
            {
                "action_type":Action.Radio.value,
                "id": "#isAssociatedWithViolentGroup_yes" if person.prbackground.q8 else "#isAssociatedWithViolentGroup_no" ,
                "label":"Yes" if person.prbackground.q8 else "No"
            },
            {
                "action_type":Action.Radio.value,
                "id": "#isMemberOfCriminalOrg_yes" if person.prbackground.q9 else "#isMemberOfCriminalOrg_no" ,
                "label":"Yes" if person.prbackground.q9 else "No"
            },
            {
                "action_type":Action.Radio.value,
                "id": "#isDetainedOrJailed_yes" if person.prbackground.q10 else "#isDetainedOrJailed_no" ,
                "label":"Yes" if person.prbackground.q10 else "No"
            },
            {
                "action_type":Action.Radio.value,
                "id": "#isPhysicalOrMentalDisorder_yes" if person.prbackground.q11 else "#isPhysicalOrMentalDisorder_no" ,
        ...
        for index, edu in enumerate(person.education):
            block=[
                ...
                },
                {
                    ...
                    "value":edu.city+"/"+edu.country if edu.city and edu.country else None,
                    "length": 21,
                    "required": True,
                },
                {
        ...
        for index, history in enumerate(person.history):
            block=[
                ...
                },
                {
                    ...
                    "label": "To",
                    "id": "#to"+str(index),
                    "value":history.end_date.strftime("%Y/%m") if history.end_date else datetime.today().strftime("%Y/%m"),
                    "length": 7,
                    "required": True,
                },
                {
        ...
        for index, member in enumerate(person.member):
            block=[
                ...
                },
                {
                    ...
                    "label": "To",
                    "id": "#to"+str(index),
                    "value":member.end_date.strftime("%Y/%m") if member.end_date else datetime.today().strftime("%Y/%m"),
                    "length": 7,
                    "required": True,
                },
                {
    ...
    def makeSectionF(self,person:PrModel):
        ...
        for index, government in enumerate(person.government):
            block=[
                ...
                },
                {
                    ...
                    "label": "To",
                    "id": "#to"+str(index),
                    "value":government.end_date.strftime("%Y/%m") if government.end_date else datetime.today().strftime("%Y/%m"),
                    "length": 7,
                    "required": True,
                },
                {
        ...
        ] if len(person.government)>0 else [
            {
                "action_type": Action.Input.value,
                "label": "From",
                "id": "#dateFrom0",
        ...
        for index, military in enumerate(person.military):
            block=[
                ...
                },
                {
                    ...
                    "label": "To",
                    "id": "#to"+str(index),
                    "value":military.end_date.strftime("%Y/%m") if military.end_date else datetime.today().strftime("%Y/%m"),
                    "length": 7,
                    "required": True,
                },
                {
        ...
        for index, address in enumerate(person.addresshistory):
            block=[
                ...
                },
                {
                    ...
                    "label": "To",
                    "id": "#to"+str(index),
                    "value":address.end_date.strftime("%Y/%m") if address.end_date else datetime.today().strftime("%Y/%m"),
                    "length": 7,
                    "required": True,
                },
                {
    ...
    def sectionA(self,person:PrModel,role):
        ...
        return applicant+father+mother+self.pdf+self.saveContinue()
    
    def sectionB(self,person:PrModel):
        return self.makeSectionB(person)+self.pdf+self.saveContinue()
    
    def sectionC(self,person:PrModel):
        return self.makeSectionC(person)+self.pdf+self.saveContinue()
    
    def sectionD(self,person:PrModel):
        return self.makeSectionD(person)+self.pdf+self.saveContinue()
    
    def sectionE(self,person:PrModel):
        return self.makeSectionE(person)+self.pdf+self.saveContinue()
    
    def sectionF(self,person:PrModel):
        return self.makeSectionF(person)+self.pdf+self.saveContinue()
    
    def sectionG(self,person:PrModel):
        return self.makeSectionG(person)+self.pdf+self.saveContinue()
    
    def sectionH(self,person:PrModel):
        return self.makeSectionH(person)+self.pdf+self.saveContinue()
    
    def fill(self):
        pa_form=self.add_family_member()+self.pick_whom(role=Role.PA)+self.sectionA(self.pa,role=Role.PA)+self.sectionB(self.pa)+self.sectionC(self.pa)+self.sectionD(self.pa)+self.sectionE(self.pa)+self.sectionF(self.pa)+self.sectionG(self.pa)+self.sectionH(self.pa)
        sp_form=self.add_family_member()+self.pick_whom(role=Role.SP)+self.sectionA(self.sp,role=Role.SP)+self.sectionB(self.sp)+self.sectionC(self.sp)+self.sectionD(self.sp)+self.sectionE(self.sp)+self.sectionF(self.sp)+self.sectionG(self.sp)+self.sectionH(self.sp) if self.sp else []
        dp_forms=[]
        for dp in self.dps:
            if dp.personal.age>=18:
                dp_forms+=self.add_family_member()+self.pick_whom(role=Role.DP)+self.sectionA(dp,role=Role.DP)+self.sectionB(dp)+self.sectionC(dp)+self.sectionD(dp)+self.sectionE(dp)+self.sectionF(dp)+self.sectionG(dp)+self.sectionH(dp)

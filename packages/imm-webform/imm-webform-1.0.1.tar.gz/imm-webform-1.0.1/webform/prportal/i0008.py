...
class F0008():
    ...
    def goTo0008(self):
        return [
            ...
            "label": "Start/Edit for 0008 form",
        ...
        return  [
            {
                ...
                "id": f"body > pra-root > pra-localized-app > main > div > pra-imm0008-page{page} > div > button"
            }
        ] 
        ...
        return  [
            {
                "action_type": Action.Turnpage.value,
                "label": "Save and continue",
                "id": "body > pra-root > pra-localized-app > main > div > pra-imm0008-dependant > div.buttons-container > button"
            }
        ]
        ...
        return  [
            {
                "action_type": Action.Turnpage.value,
                "label": "Save and continue",
                "id": "body > pra-root > pra-localized-app > main > div > pra-imm0008-dependant > div.buttons-container > button.btn.btn-primary.ng-star-inserted"
            }
        ]
    ...
    def applicationDetails(self):
        ...
        },
        {
            ...
            "value": interpreter_map["Yes"] if self.pa.prcase.need_translator else interpreter_map["No"],
        ...
        },
        {
            "action_type": "Select",
            "label": "City",
            "value": city_map[self.pa.prcase.intended_province][self.pa.prcase.intended_city],
            "id": "#city",
        }
        ]+self.pdf+self.saveContinue(2)
        ...
        for index,c in enumerate(cor):
            ...
            },
            {
            ...
            },
                {
                    ...
                    "label": "Start date",
                    "id": "#personalDetailsForm-prevStartDateOfImmigrationStatus"+str(index),
                ...
                },
                {
                    "action_type": Action.Input.value,
                    "label": "End date",
                    "id": "#personalDetailsForm-prevEndDateOfImmigrationStatus"+str(index),
            ...
            other=[
                {
                    ...
                    "id": "#personalDetailsForm-prevOtherImmigrationStatus"+str(index), 
                    "value": c.explanation,
                    "length": 30,
                    "required": True,
                }
    ...
    def personalDetails(self,person:PrModel,role:Role):
        relationship2pa={
                "action_type": "Select",
                "label": "Relationship",
                "value": relationship_to_pa[person.personal.relationship_to_pa] if person.personal.relationship_to_pa else None,
        ...
        sp_dp=[
            {
                ...
                "id":"#dependantDetailsForm-accompanyingPA-yes" if  person.personal.accompany_to_canada else "#dependantDetailsForm-accompanyingPA-no",
            },
            relationship2pa,
            # 由于这个点击经常出错，所以这里点击2次。
            {
        ...
        dependant_type_set=[
            {
                ...
                "value": dependant_type[person.personal.dependant_type] if person.personal.dependant_type else None,
                "id": "#dependantDetailsForm-dependantType",
            }
        ] if role!=Role.PA and person.personal.relationship_to_pa in ["Adopted Child","Child", "Grandchild","Step-Child","Step-Grandchild","Parent", "Adoptive Parent"] else []
        
        dependant_type_set=[{
            "action_type":Action.DependantSelect.value,
            "select1":relationship2pa,
            "select2":dependant_type_set[0] 
        }] if dependant_type_set else [relationship2pa]   #如果是spouse，common-law则没有后续选择
        ...
        used_name=[
            {
                ...
                "value": has_used_name,
                "id":"#personalDetailsForm-usedOtherName-yes" if has_used_name else "#personalDetailsForm-usedOtherName-no",
            }
        ]
        used_name+=[
            {
        ...
        citizens=[
            ...
            },
            {
                "action_type": "Select",
                "label": "Citizen 2",
                "value": citizen[person.personal.citizen2] if person.personal.citizen2 else None,
        ...
        last_entry=[
            {
                ...
                "value": person.personal.last_entry_date.strftime("%Y/%m/%d") if person.personal.last_entry_date else None,
        ...
        match ccor.status:
            ...
            case "Worker" | "Student" |"Visitor":
                current_cor+=ccor_period+last_entry if ccor.country=="Canada" else ccor_period
            case "Refugess":
                current_cor+=last_entry if ccor.country=='Canada' else current_cor
            case "Other":
                current_cor+=ccor_period+other+last_entry if ccor.country=='Canada' else ccor_period+other
        ...
        previous_cor=[
            {
                ...
                "id":"#personalDetailsForm-hasPreviousCountries-yes" if has_previous_cor else "#personalDetailsForm-hasPreviousCountries-no" ,
            }
        ]
        previous_cor=previous_cor+self.makePreviousCOR() if has_previous_cor else previous_cor
        # marriage
        marital_status=[
            {
                "action_type": "Select",
        ...
        married_date=[
            {
                ...
                "value": person.marriage.married_date.strftime("%Y/%m/%d") if person.marriage.married_date else None,
                "length": 10,
                "required": True,
            }
        ]
        ...
        has_pre_marriage=[
            {
                ...
                "id":"#personalDetailsForm-previouslyMarriedOrCommonLaw-yes" if person.marriage.previous_married else "#personalDetailsForm-previouslyMarriedOrCommonLaw-no"
            }
        ]
        pre_marriage=[
            {
            ...
            },
            {
                ...
                "value": person.marriage.pre_sp_dob.strftime("%Y/%m/%d") if person.marriage.pre_sp_dob else None,
                "length": 10,
                "required": True,
            },
            {
            ...
            },
            {
                ...
                "value": person.marriage.pre_start_date.strftime("%Y/%m/%d") if person.marriage.pre_start_date else None,
                "length": 10,
                "required": True,
            },
            {
                ...
                "value": person.marriage.pre_end_date.strftime("%Y/%m/%d") if person.marriage.pre_end_date else None,
                "length": 10,
                "required": True,
            }
        ...
        # summary
        save_continue=self.saveContinue(3) if role==Role.PA else self.saveContinueDep()
        return sp_dp+dependant_type_set+name+used_name+uci+physical_characteristics+birth_info+citizens+current_cor+previous_cor+marital_status+has_pre_marriage+self.pdf+save_continue
    ...
    def makeAddress(self,address,type):
        ...
        the_address=[
            ...
            },
            {
                "action_type": "Select",
                "label": "Country",
                "value": country_residence[address.country] if address.country else None,
        ...
        the_address+= province+post_code if address.country=="Canada" else post_code+district
        return the_address
    ...
    def contactInfo(self):
        ...
        residential_address=self.makeAddress(residential,'residential') if mailing!=residential else []
        ...
        ] if not phone.isCanadaUs else []
        number_ext=[
            {
            "action_type": Action.Input.value,
            "label": "Country code",
            "id": "#PrimaryNumber",
            "value": "-".join(phone.NA_format_list) if phone.isCanadaUs else phone.number,
            "length": 20,
            "required": False,
            },
            {
        ...
        return mailing_address+is_same_address+residential_address+primary_phone+contact_using_the_email+self.pdf+self.saveContinue(4)
    ...
    def passport_id(self,person:PrModel,role:Role):
        ...
        has_passport= [
            {
                ...
                "id":"#validPassportYes" if ids.passport.number else "#validPassportNo"
            }
        ]
        has_id= [
            {
        ...
        passport_content=[
            ...
            },
            {
                ...
                "value": country_residence[ids.passport.country] if ids.passport.country else None,
                "id":"#countryOfIssue"
            },
            {
                "action_type": Action.Input.value,
        ...
        ] if ids.national_id.number else []
        id_content=[
            {
                "action_type": Action.Input.value,
                "label": "ID number",
            ...
            },
            {
                "action_type": "Select",
                "label": "Country of issue",
                "value": country_residence[ids.national_id.country] if ids.national_id.country else None,
                "id":"#countryOfIssue"
            },
            {
                "action_type": Action.Input.value,
                "label": "Issue date",
                "id": "#issueDate",
                "value": ids.national_id.issue_date.strftime("%Y/%m/%d") if ids.national_id.issue_date else None,
                "length": 10,
                "required": True,
            },
            {
                ...
                "label": "Expiry date",
                "id": "#expiryDate",
                "value": ids.national_id.expiry_date.strftime("%Y/%m/%d") if ids.national_id.expiry_date else None,
                "length": 10,
                "required": True,
            }
        ] if ids.national_id.number else []
        ...
        passport_continue=self.saveContinue(5) if role==Role.PA else self.saveContinueDep1()
        national_id_continue=self.saveContinue(6) if role==Role.PA else self.saveContinueDep1()
        return passport+self.pdf+passport_continue+national_id+self.pdf+national_id_continue
    
    def education(self,person:PrModel,role:Role):
        edu=EducationHistory(person.education)
        highest_eud=edu.highest
        years=person.personal.primary_school_years+person.personal.secondary_school_years+person.personal.post_secondary_school_years+person.personal.other_school_years
        education_set= [
            {
                "action_type": "Select",
        ...
        save_continue=self.saveContinue(7) if role==Role.PA else self.saveContinueDep1()
        return education_set+self.pdf+save_continue
        ...
        which_one_better=[
            {
                ...
                "value": english_french_map[person.personal.which_one_better] if person.personal.which_one_better else None,
                "id":"#preferredLanguage"
            }
        ] if person.personal.english_french=='Both' else []
        
        taken_test=[
            {
                "action_type": Action.Radio.value,
                "label": "Taken language test",
                "id": "#testing-yes" if person.personal.language_test else "#testing-no"
            }
        ]
        save_continue=self.saveContinue(8) if role==Role.PA else self.saveContinueDep1()
        return language+which_one_better+taken_test+self.pdf+save_continue
    
    def gotoPage(self,page_num:int):
        pages=[]
        for page in range(2,page_num):
            pages+=self.saveContinue(page)
        return pages
        ...
        return  [
            {
                ...
                "id": "#dependantNo" if not self.sp and len(self.dps)==0 else "#dependantYes"  
            },
            {
            "action_type": Action.Turnpage.value,
    ...
    def fill(self):
        pa_form=self.applicationDetails()+self.personalDetails(self.pa,Role.PA) +self.contactInfo()+self.passport_id(self.pa,Role.PA)+self.education(self.pa,Role.PA)+self.language(self.pa,Role.PA)
        
        sp_form=self.addDependants()+self.personalDetails(self.sp,Role.SP)+self.education(self.sp,Role.SP)+self.language(self.sp,Role.SP)+self.passport_id(self.sp,Role.SP) if self.sp else []
        dp_forms=[]
        for dp in self.dps:
            if dp.personal.age>=18:
                dp_forms+=self.addDependants()+self.personalDetails(dp,Role.DP)+self.education(dp,Role.DP)+self.language(dp,Role.DP)+self.passport_id(dp,Role.DP)
        
        return self.goTo0008()+pa_form+sp_form+dp_forms
    
    def fill_pa(self):
        return self.goTo0008()+self.applicationDetails()+self.personalDetails(self.pa,Role.PA) +self.contactInfo()+self.passport_id(self.pa,Role.PA)+self.education(self.pa,Role.PA)+self.language(self.pa,Role.PA)

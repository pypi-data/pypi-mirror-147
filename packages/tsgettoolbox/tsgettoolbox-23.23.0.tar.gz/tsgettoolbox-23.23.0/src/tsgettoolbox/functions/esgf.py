...
def search(
    project, model, experiment, time_frequency, realm, variables, ensemble="r1i1p1"
):
...
def esgf_gsod_cli(stationid, datatypeid=None, start_date=None, end_date=None):
    ...
    National Centers for Environmental Information (esgf) Global Summary of the
    MONTH (GSOM)
    ...
    National Centers for Environmental Information (esgf) Global Summary of the
    YEAR (GSOY)
    https://gis.ncdc.noaa.gov/all-records/catalog/search/resource/details.page?id=gov.noaa.esgf:C00947
    Cite this dataset when used as a source: Lawrimore, Jay (2016). Global
    ...
    esgf cannot assume liability for any damages caused by any errors or
    omissions in the data, nor as a result of the failure of the data to
    function on a particular system. esgf makes no warranty, expressed or
    implied, nor does the fact of distribution constitute such a warranty. esgf
    can only certify that the data it distributes are an authentic copy of the
    records that were accepted for inclusion in the esgf archives.
    ...
    "esgf Summary of the Month". There are unique elements that are produced
    ...
    """
    tsutils._printiso(
        esgf_gsod(stationid, datatypeid=datatypeid, start_date=None, end_date=None),
    )


# https://www.esgf.noaa.gov/data/global-summary-of-the-day/access/
# GSOD
def esgf_gsod(stationid, datatypeid=None, start_date=None, end_date=None):
    r"""Access esgf Global Summary of the Day."""
    stationid = stationid.replace("-", "")
    df = esgf_cdo_json_to_df(
        "GSOD",
        stationid,
...
def esgf_gsom_cli(stationid, datatypeid=None, start_date=None, end_date=None):
    ...
    National Centers for Environmental Information (esgf) Global Summary of the MONTH (GSOM)
    https://gis.ncdc.noaa.gov/all-records/catalog/search/resource/details.page
    Cite this dataset when used as a source: Lawrimore, Jay (2016). Global
    Summary of the Month, Version 1.0. [indicate subset used]. NOAA National
    Centers for Environmental Information. DOI:10.7289/V5QV3JJ5

    National Centers for Environmental Information (esgf) Global Summary of the YEAR (GSOY)
    https://gis.ncdc.noaa.gov/all-records/catalog/search/resource/details.page?id=gov.noaa.esgf:C00947
    Cite this dataset when used as a source: Lawrimore, Jay (2016). Global
    Summary of the Year, Version 1.0. [indicate subset used]. NOAA National
    Centers for Environmental Information
...
def esgf_gsoy_cli(stationid, datatypeid=None, start_date=None, end_date=None):
    ...
    National Centers for Environmental Information (esgf) Global Summary of the YEAR
    (GSOY)
    https://gis.ncdc.noaa.gov/all-records/catalog/search/resource/details.page?id=gov.noaa.esgf:C00947
...
def esgf_nexrad2_cli(stationid, start_date=None, end_date=None):
    ...
    observation/element is missing."""
    tsutils._printiso(
        esgf_nexrad2(stationid, start_date=start_date, end_date=end_date),
    )
...
def esgf_nexrad3_cli(stationid, start_date=None, end_date=None):
    ...
    return tsutils._printiso(
        esgf_nexrad3(stationid, start_date=start_date, end_date=end_date),
    )
...
def esgf_normal_ann_cli(stationid, datatypeid=None):
    ...
    end_date
        Many different formats can be used here for the date
        string, however the closest to ISO8601, the better."""
    tsutils._printiso(esgf_normal_ann(stationid, datatypeid))
...
def esgf_normal_dly_cli(stationid, datatypeid=None):
    ...
    tsutils._printiso(esgf_normal_dly(stationid, datatypeid=datatypeid))
...
def esgf_normal_hly_cli(stationid, datatypeid=None):
    ...
    end_date
        End date in ISO8601 format."""
    tsutils._printiso(esgf_normal_hly(stationid, datatypeid=datatypeid))
...
def esgf_normal_mly_cli(stationid, datatypeid=None):
    ...
    end_date
        End date in ISO8601
        format."""
    tsutils._printiso(esgf_normal_mly(stationid, datatypeid=None))
...
def esgf_ish(station, datatypeid=None, start_date=None, end_date=None):
    r"""Download from the Global Historical Climatology Network - Daily."""
    station = station.replace("-", "")
    final = utils.file_downloader(
        "https://www.esgf.noaa.gov/data/global-hourly/access/{year}/{station}.csv",
        station,
...
def esgf_cirs_cli(elements=None, by_state=False, location_names="abbr"):
    ...
    location_names:
        ...
        option)."""
    tsutils._printiso(
        esgf_cirs(elements=elements, by_state=by_state, location_names=location_names)
    )
...
esgf_ghcnd.__doc__ = esgf_ghcnd_cli.__doc__
esgf_gsod.__doc__ = esgf_gsod_cli.__doc__
esgf_gsom.__doc__ = esgf_gsom_cli.__doc__
esgf_gsoy.__doc__ = esgf_gsoy_cli.__doc__
esgf_ish.__doc__ = esgf_ish_cli.__doc__
esgf_nexrad2.__doc__ = esgf_nexrad2_cli.__doc__
esgf_nexrad3.__doc__ = esgf_nexrad3_cli.__doc__
esgf_normal_ann.__doc__ = esgf_normal_ann_cli.__doc__
esgf_normal_dly.__doc__ = esgf_normal_dly_cli.__doc__
esgf_normal_hly.__doc__ = esgf_normal_hly_cli.__doc__
esgf_normal_mly.__doc__ = esgf_normal_mly_cli.__doc__
esgf_precip_15.__doc__ = esgf_precip_15_cli.__doc__
esgf_precip_hly.__doc__ = esgf_precip_hly_cli.__doc__
esgf_annual.__doc__ = esgf_annual_cli.__doc__
esgf_ghcndms.__doc__ = esgf_ghcndms_cli.__doc__
esgf_cirs.__doc__ = esgf_cirs_cli.__doc__

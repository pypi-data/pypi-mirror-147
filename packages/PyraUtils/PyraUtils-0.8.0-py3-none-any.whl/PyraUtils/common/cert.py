...
class CertUtils:
    ...
    def load_x509_cert_use_openssl(self, cert_data: bytes, cert_format="pem"):
        ...
        cert_x509_data = OpenSSL.crypto.dump_certificate(
            OpenSSL.crypto.FILETYPE_TEXT, cert_X509)
    ...
    def load_x509_cert_use_cryptography(self, cert_data: bytes, cert_format="pem"):
        ...
        assert isinstance(cert_data, bytes)
        if cert_format == "der":
            cert_x509_data = x509.load_der_x509_certificate(
                cert_data, default_backend())
        else:
            cert_x509_data = x509.load_pem_x509_certificate(
                cert_data, default_backend())
        return cert_x509_data

...
class hubspot:
  ...
  def authenticate(self, client_id, client_secret, refresh_token):
    ...
    result = post(post_url, self.auth_headers, auth_body)
  ...
  def revoke_token(self):
    post_url = f"{self.base_url}oauth/v1/refresh-tokens/{self.refresh_token}"
    return delete(post_url, self.auth_headers)

  def custom_request(self, method=None, endpoint=None, data=None):
    HTTP.Requests.getattr(method)(endpoint, self.headers, data=data)
  ...
  def update_property(self, object_type, property_name, data):
    ...
    result = patch(post_url, self.headers, payload=json.dumps(data))
    if not Utils.is_success(result['status_code']):

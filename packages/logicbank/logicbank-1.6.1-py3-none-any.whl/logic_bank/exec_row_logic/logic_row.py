    ...
    def save_altered_parents(self, do_not_adjust_list: List = None):
        ...
        if self.parent_logic_row is None:  # save *only altered* parents (often does nothing)
            ...
            if parent_logic_row.row_sets.is_client_insert(parent_logic_row.row):
                do_defer_adjustment = do_defer_adjustment  # FIXME - temp debug
                self.child_logic_row.log("Adjustment deferral does not apply for *inserted* parent" + self.parent_role_name)

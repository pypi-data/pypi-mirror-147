...
def get_args_from_file():
    ...
    for i in range(len(csv)):
        if i==0:
            ...
            stat['mean_r_per_c'] = df[1][2]
            stat['mean_UMI_per_c'] = df[1][3]
            stat['median_UMI_per_c'] = df[1][4]
            stat['total_gene'] = df[1][5]
            stat['mean_genes_per_c'] = df[1][6]
            stat['median_genes_per_c'] = df[1][7] 
            stat['saturation'] = str(df[1][8])
        if i==1:
            df = pd.read_csv(open(csv[i]),encoding="utf_8",dtype=str,header=None,sep=",")
...
def write_param_to_template():
    ...
    cols = ["samplename","species","estm_Num_cell","mean_r_per_c","mean_UMI_per_c","median_UMI_per_c","total_gene",\
        "mean_genes_per_c","median_genes_per_c","saturation","Fraction","cluster_cell","cDNA_num_frag","cDNA_frag_pass_QC","cDNA_Q30_r",\
        "index_num_frag","index_frag_pass_QC","index_Q30_r","mito_ratio","r_m_geno","r_m_ex","r_m_intro",\
    ...
    metrics_summary_df.columns =["SampleName","species","Estimated number of cell","Mean reads per cell","Mean UMI count per cell",\
        ...
        "Fraction Reads in cell","Number of cells used for clustering",\
        "cDNA Number of reads","cDNA Reads pass QC","cDNA Q30 bases in reads","index Number of reads","index Reads pass QC",\
        "index Q30 bases in reads","Mitochondria ratio","Reads mapped to genome (Map Quality >= 0)","Reads mapped to exonic regions",\

...
from shatail.admin.models import get_object_usage
from shatail.coreutils import string_to_ascii
from shatail.images.exceptions import InvalidFilterSpecError
from shatail.images.image_operations import (
    FilterOperation,
    ImageTransform,
    TransformOperation,
)
from shatail.images.rect import Rect
from shatail.models import CollectionMember
from shatail.search import index
from shatail.search.queryset import SearchableQuerySetMixin

logger = logging.getLogger("shatail.images")
...
def get_rendition_storage():
    ...
    """
    storage = getattr(settings, "SHATAILIMAGES_RENDITION_STORAGE", default_storage)
...
class AbstractImage(ImageFileMixin, CollectionMember, index.Indexed, models.Model):
    ...
    @property
    def usage_url(self):
        return reverse("shatailimages:image_usage", args=(self.id,))
    ...
    def run(self, image, output):
        with image.get_willow_image() as willow:
            ...
            if "output-format" in env:
                ...
                conversion = getattr(settings, "SHATAILIMAGES_FORMAT_CONVERSIONS", {})
            ...
            if output_format == "jpeg":
                ...
                if "jpeg-quality" in env:
                    ...
                    quality = getattr(settings, "SHATAILIMAGES_JPEG_QUALITY", 85)
                ...
                if (
                    ...
                    quality = getattr(settings, "SHATAILIMAGES_WEBP_QUALITY", 85)
...
class AbstractRendition(ImageFileMixin, models.Model):
    ...
    def full_url(self):
        url = self.url
        if hasattr(settings, "SHATAILADMIN_BASE_URL") and url.startswith("/"):
            url = settings.SHATAILADMIN_BASE_URL + url
        return url

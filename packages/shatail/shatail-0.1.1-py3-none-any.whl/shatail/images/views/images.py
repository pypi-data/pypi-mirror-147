...
from shatail.admin.models import popular_tags_for_model
from shatail.admin.views.pages.utils import get_valid_next_url_from_request
from shatail.images import get_image_model
from shatail.images.exceptions import InvalidFilterSpecError
from shatail.images.forms import URLGeneratorForm, get_image_form
from shatail.images.models import Filter, SourceImageIOError
from shatail.images.permissions import permission_policy
from shatail.images.utils import generate_signature
from shatail.models import Collection, Site
from shatail.search import index as search_index

permission_checker = PermissionPolicyChecker(permission_policy)

INDEX_PAGE_SIZE = getattr(settings, "SHATAILIMAGES_INDEX_PAGE_SIZE", 20)
USAGE_PAGE_SIZE = getattr(settings, "SHATAILIMAGES_USAGE_PAGE_SIZE", 20)
...
class BaseListingView(TemplateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        next_url = reverse("shatailimages:index")
...
class IndexView(BaseListingView):
    template_name = "shatailimages/images/index.html"
...
class ListingResultsView(BaseListingView):
    template_name = "shatailimages/images/results.html"
...
def edit(request, image_id):
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            redirect_url = "shatailimages:index"
            if next_url:
    ...
    try:
        reverse("shatailimages_serve", args=("foo", "1", "bar"))
    ...
    if image.is_stored_locally():
        ...
        if not os.path.isfile(image.file.path):
            messages.error(
                ...
                buttons=[
                    messages.button(
                        reverse("shatailimages:delete", args=(image.id,)), _("Delete")
                    )
                ],
            )
    ...
    return TemplateResponse(
        request,
        "shatailimages/images/edit.html",
        {
            "image": image,
            "form": form,
...
def url_generator(request, image_id):
    ...
    return TemplateResponse(
        request,
        "shatailimages/images/url_generator.html",
        {
            "image": image,
            "form": form,
        },
...
def generate_url(request, image_id, filter_spec):
    ...
    url = reverse("shatailimages_serve", args=(signature, image_id, filter_spec))
    ...
    preview_url = reverse("shatailimages:preview", args=(image_id, filter_spec))
...
def delete(request, image_id):
    ...
    if request.method == "POST":
        ...
        return redirect(next_url) if next_url else redirect("shatailimages:index")

    return TemplateResponse(
        request,
        "shatailimages/images/confirm_delete.html",
        {
            "image": image,
            "next": next_url,
        },
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            messages.success(
                ...
                buttons=[
                    messages.button(
                        reverse("shatailimages:edit", args=(image.id,)), _("Edit")
                    )
                ],
            )
            return redirect("shatailimages:index")
        else:
...
def usage(request, image_id):
    ...
    return TemplateResponse(
        request, "shatailimages/images/usage.html", {"image": image, "used_by": used_by}
    )

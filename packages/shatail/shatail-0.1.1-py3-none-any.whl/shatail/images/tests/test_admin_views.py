...
from shatail.admin.admin_url_finder import AdminURLFinder
from shatail.images import get_image_model
from shatail.images.models import UploadedImage
from shatail.images.utils import generate_signature
from shatail.models import Collection, GroupCollectionPermission, get_root_collection_id
from shatail.test.testapp.models import CustomImage, CustomImageWithAuthor
...
class TestImageIndexView(TestCase, ShatailTestUtils):
    def setUp(self):
        self.login()

    def get(self, params={}):
        return self.client.get(reverse("shatailimages:index"), params)

    def test_simple(self):
        response = self.get()
        self.assertEqual(response.status_code, 200)
        self.assertTemplateUsed(response, "shatailimages/images/index.html")
    ...
    def test_edit_image_link_contains_next_url(self):
        ...
        edit_url = reverse("shatailimages:edit", args=(image.id,))
...
class TestImageListingResultsView(TestCase, ShatailTestUtils):
    def setUp(self):
        self.login()

    def get(self, params={}):
        return self.client.get(reverse("shatailimages:listing_results"), params)
...
class TestImageAddView(TestCase, ShatailTestUtils):
    def setUp(self):
        self.login()

    def get(self, params={}):
        return self.client.get(reverse("shatailimages:add"), params)

    def post(self, post_data={}):
        return self.client.post(reverse("shatailimages:add"), post_data)

    def test_get(self):
        response = self.get()
        self.assertEqual(response.status_code, 200)
        self.assertTemplateUsed(response, "shatailimages/images/add.html")
        ...
        self.assertNotContains(response, "shatailadmin/js/draftail.js")
    ...
    @override_settings(SHATAILIMAGES_IMAGE_MODEL="tests.CustomImage")
    def test_get_with_custom_image_model(self):
        ...
        self.assertContains(response, "shatailadmin/js/draftail.js")
    ...
    def test_add(self):
        ...
        self.assertRedirects(response, reverse("shatailimages:index"))
    ...
    @override_settings(
        DEFAULT_FILE_STORAGE="shatail.test.dummy_external_storage.DummyExternalStorage"
    )
    ...
    @override_settings(SHATAILIMAGES_MAX_UPLOAD_SIZE=1)
    ...
    @override_settings(SHATAILIMAGES_MAX_IMAGE_PIXELS=1)
...
class TestImageAddViewWithLimitedCollectionPermissions(TestCase, ShatailTestUtils):
    def setUp(self):
        add_image_permission = Permission.objects.get(
            content_type__app_label="shatailimages", codename="add_image"
        )
...
class TestImageEditView(TestCase, ShatailTestUtils):
    def setUp(self):
    ...
    def get(self, params={}):
        return self.client.get(
            reverse("shatailimages:edit", args=(self.image.id,)), params
        )

    def post(self, post_data={}):
        return self.client.post(
            reverse("shatailimages:edit", args=(self.image.id,)), post_data
        )

    def test_simple(self):
        response = self.get()
        self.assertEqual(response.status_code, 200)
        self.assertTemplateUsed(response, "shatailimages/images/edit.html")
    ...
    def test_next_url_is_present_in_edit_form(self):
        ...
        expected_next_url = (
            reverse("shatailimages:index")
            + "?"
            + urlencode({"collection_id": evil_plans_collection.id})
        )

        response = self.client.get(
            reverse("shatailimages:edit", args=(image.id,)), {"next": expected_next_url}
        )
        self.assertEqual(response.status_code, 200)
        self.assertContains(
    ...
    def test_edit_with_next_url(self):
        ...
        response = self.client.post(
            reverse("shatailimages:edit", args=(image.id,)),
            {
                "title": "Edited",
                "collection": evil_plans_collection.id,
    ...
    def check_get_missing_file_displays_warning(self):
        ...
        response = self.client.get(reverse("shatailimages:edit", args=(image.pk,)))
        self.assertEqual(response.status_code, 200)
        self.assertTemplateUsed(response, "shatailimages/images/edit.html")
    ...
    def test_no_thousand_separators_in_focal_point_editor(self):
        ...
        response = self.client.get(
            reverse("shatailimages:edit", args=(large_image.id,))
        )
...
@override_settings(SHATAILIMAGES_IMAGE_MODEL="tests.CustomImage")
class TestImageEditViewWithCustomImageModel(TestCase, ShatailTestUtils):
    def setUp(self):
        self.login()
...
class TestImageDeleteView(TestCase, ShatailTestUtils):
    def setUp(self):
        self.user = self.login()
    ...
    def get(self, params={}):
        return self.client.get(
            reverse("shatailimages:delete", args=(self.image.id,)), params
        )

    def post(self, post_data={}):
        return self.client.post(
            reverse("shatailimages:delete", args=(self.image.id,)), post_data
        )

    @override_settings(SHATAIL_USAGE_COUNT_ENABLED=False)
    def test_simple(self):
        response = self.get()
        self.assertEqual(response.status_code, 200)
        self.assertTemplateUsed(response, "shatailimages/images/confirm_delete.html")
...
class TestImageChooserView(TestCase, ShatailTestUtils):
    def setUp(self):
        self.user = self.login()

    def get(self, params={}):
        return self.client.get(reverse("shatailimages:chooser"), params)
    ...
    def test_simple(self):
        ...
        self.assertTemplateUsed(response, "shatailimages/chooser/chooser.html")

        # draftail should NOT be a standard JS include on this page
        self.assertNotIn("shatailadmin/js/draftail.js", response_json["html"])
    ...
    def test_choose_permissions(self):
        ...
        GroupCollectionPermission.objects.create(
            ...
            permission=Permission.objects.get(
                content_type__app_label="shatailimages", codename="choose_image"
            ),
        )
    ...
    def test_with_custom_image_model(self):
        ...
        self.assertIn("shatailadmin/js/draftail.js", response_json["html"])
...
class TestImageChooserChosenView(TestCase, ShatailTestUtils):
    def setUp(self):
        self.login()
    ...
    def get(self, params={}):
        return self.client.get(
            reverse("shatailimages:image_chosen", args=(self.image.id,)), params
        )
...
class TestImageChooserSelectFormatView(TestCase, ShatailTestUtils):
    def setUp(self):
        self.login()
    ...
    def get(self, params={}):
        return self.client.get(
            reverse("shatailimages:chooser_select_format", args=(self.image.id,)),
            params,
        )
    ...
    def test_simple(self):
        ...
        self.assertTemplateUsed(response, "shatailimages/chooser/select_format.html")
...
class TestImageChooserUploadView(TestCase, ShatailTestUtils):
    def setUp(self):
        self.login()

    def get(self, params={}):
        return self.client.get(reverse("shatailimages:chooser_upload"), params)

    def test_simple(self):
        response = self.get()
        self.assertEqual(response.status_code, 200)
        self.assertTemplateUsed(response, "shatailimages/chooser/upload_form.html")
        response_json = json.loads(response.content.decode())
        self.assertEqual(response_json["step"], "reshow_upload_form")

    def test_upload(self):
        response = self.client.post(
            reverse("shatailimages:chooser_upload"),
            {
    ...
    def test_upload_duplicate(self):
        ...
        self.assertTemplateUsed(
            response, "shatailimages/chooser/confirm_duplicate_upload.html"
        )
        ...
        choose_new_image_action = reverse(
            "shatailimages:image_chosen", args=(new_image.id,)
        )
        self.assertEqual(
        ...
        choose_existing_image_action = (
            reverse("shatailimages:delete", args=(new_image.id,))
            + "?"
            + urlencode(
                {
                    "next": reverse(
                        "shatailimages:image_chosen", args=(existing_image.id,)
                    )
                }
            )
        )
    ...
    def test_upload_duplicate_select_format(self):
        def post_image(title="Test image"):
            return self.client.post(
                reverse("shatailimages:chooser_upload") + "?select_format=true",
                {
                    "image-chooser-upload-title": title,
                    "image-chooser-upload-file": SimpleUploadedFile(
                        "test.png", get_test_image_file().file.getvalue()
        ...
        choose_new_image_action = reverse(
            "shatailimages:chooser_select_format", args=(new_image.id,)
        )
        self.assertEqual(
            response.context["confirm_duplicate_upload_action"], choose_new_image_action
        )
        ...
        choose_existing_image_action = (
            ...
            + urlencode(
                {
                    "next": reverse(
                        "shatailimages:chooser_select_format", args=(existing_image.id,)
                    )
                }
            )
        )
    ...
    def test_select_format_flag_after_upload_form_error(self):
        submit_url = reverse("shatailimages:chooser_upload") + "?select_format=true"
        response = self.client.post(
            submit_url,
            {
                "image-chooser-upload-title": "Test image",
...
class TestImageChooserUploadViewWithLimitedPermissions(TestCase, ShatailTestUtils):
    def setUp(self):
        add_image_permission = Permission.objects.get(
            content_type__app_label="shatailimages", codename="add_image"
        )
    ...
    def test_get(self):
        response = self.client.get(reverse("shatailimages:chooser_upload"))
        self.assertEqual(response.status_code, 200)
        self.assertTemplateUsed(response, "shatailimages/chooser/upload_form.html")
    ...
    def test_get_chooser(self):
        response = self.client.get(reverse("shatailimages:chooser"))
        self.assertEqual(response.status_code, 200)
        self.assertTemplateUsed(response, "shatailimages/chooser/chooser.html")
...
class TestMultipleImageUploader(TestCase, ShatailTestUtils):
    """
    This tests the multiple image upload views located in shatailimages/views/multiple.py
    """
    ...
    def test_add(self):
        ...
        """
        # Send request
        response = self.client.get(reverse("shatailimages:add_multiple"))

        # Check response
        self.assertEqual(response.status_code, 200)
        self.assertTemplateUsed(response, "shatailimages/multiple/add.html")
    ...
    @override_settings(SHATAILIMAGES_MAX_UPLOAD_SIZE=1000)
    ...
    def test_add_error_max_file_size_escaped(self):
        url = reverse("shatailimages:add_multiple")
        template_name = "shatailimages/multiple/add.html"
    ...
    def test_add_post(self):
        ...
        """
        response = self.client.post(
            reverse("shatailimages:add_multiple"),
            {
                "title": "test title",
        ...
        self.assertEqual(response["Content-Type"], "application/json")
        self.assertTemplateUsed(
            response, "shatailadmin/generic/multiple_upload/edit_form.html"
        )
    ...
    def test_add_post_nofile(self):
        ...
        """
        response = self.client.post(reverse("shatailimages:add_multiple"), {})
    ...
    def test_add_post_duplicate(self):
        ...
        # Check template used
        self.assertTemplateUsed(
            response, "shatailimages/images/confirm_duplicate_upload.html"
        )
    ...
    def test_edit_get(self):
        ...
        # Send request
        response = self.client.get(
            reverse("shatailimages:edit_multiple", args=(self.image.id,))
        )
    ...
    def test_delete_get(self):
        ...
        # Send request
        response = self.client.get(
            reverse("shatailimages:delete_multiple", args=(self.image.id,))
        )
...
@override_settings(SHATAILIMAGES_IMAGE_MODEL="tests.CustomImage")
class TestMultipleImageUploaderWithCustomImageModel(TestCase, ShatailTestUtils):
    """
    This tests the multiple image upload views located in shatailimages/views/multiple.py
...
@override_settings(SHATAILIMAGES_IMAGE_MODEL="tests.CustomImageWithAuthor")
class TestMultipleImageUploaderWithCustomRequiredFields(TestCase, ShatailTestUtils):
    """
    This tests the multiple image upload views located in shatailimages/views/multiple.py
    with a custom image model
    """
    ...
    def test_create_from_upload_invalid_post(self):
        ...
        response = self.client.post(
            reverse(
                "shatailimages:create_multiple_from_uploaded_image",
    ...
    def test_delete_uploaded_image(self):
        ...
        response = self.client.post(
            reverse(
                "shatailimages:delete_upload_multiple", args=(self.uploaded_image.id,)
            )
        )
...
class TestURLGeneratorView(TestCase, ShatailTestUtils):
    def setUp(self):
        # Create an image for running tests on
        self.image = Image.objects.create(
            title="Test image",
    ...
    def test_get(self):
        ...
        # Get
        response = self.client.get(
            reverse("shatailimages:url_generator", args=(self.image.id,))
        )

        # Check response
        self.assertEqual(response.status_code, 200)
        self.assertTemplateUsed(response, "shatailimages/images/url_generator.html")
    ...
    def test_get_bad_permissions(self):
        ...
        # Check response
        self.assertRedirects(response, reverse("shatailadmin_home"))


class TestGenerateURLView(TestCase, ShatailTestUtils):
    def setUp(self):
        # Create an image for running tests on
        self.image = Image.objects.create(
            title="Test image",
    ...
    def test_get(self):
        ...
        # Get
        response = self.client.get(
            reverse("shatailimages:generate_url", args=(self.image.id, "fill-800x600"))
        )
        ...
        expected_preview_url = reverse(
            "shatailimages:preview", args=(self.image.id, "fill-800x600")
        )
    ...
    def test_get_bad_image(self):
        ...
        response = self.client.get(
            reverse(
                "shatailimages:generate_url", args=(self.image.id + 1, "fill-800x600")
            )
        )
    ...
    def test_get_bad_filter_spec(self):
        ...
        response = self.client.get(
            reverse(
                "shatailimages:generate_url", args=(self.image.id, "bad-filter-spec")
            )
        )
...
class TestPreviewView(TestCase, ShatailTestUtils):
    def setUp(self):
        # Create an image for running tests on
        self.image = Image.objects.create(
            title="Test image",
    ...
    def test_get(self):
        ...
        # Get the image
        response = self.client.get(
            reverse("shatailimages:preview", args=(self.image.id, "fill-800x600"))
        )
    ...
    def test_get_invalid_filter_spec(self):
        ...
        done with Shatails built in URL generator. We should test it
        anyway though.
        """
        # Get the image
        response = self.client.get(
            reverse("shatailimages:preview", args=(self.image.id, "bad-filter-spec"))
        )
...
class TestEditOnlyPermissions(TestCase, ShatailTestUtils):
    def setUp(self):
        # Create an image to edit
        self.image = Image.objects.create(
            title="Test image",
        ...
        change_permission = Permission.objects.get(
            content_type__app_label="shatailimages", codename="change_image"
        )
        admin_permission = Permission.objects.get(
            content_type__app_label="shatailadmin", codename="access_admin"
        )
    ...
    def test_get_index(self):
        response = self.client.get(reverse("shatailimages:index"))
        self.assertEqual(response.status_code, 200)
        self.assertTemplateUsed(response, "shatailimages/images/index.html")
    ...
    def test_search(self):
        response = self.client.get(reverse("shatailimages:index"), {"q": "Hello"})
        self.assertEqual(response.status_code, 200)
        self.assertEqual(response.context["query_string"], "Hello")

    def test_get_add(self):
        response = self.client.get(reverse("shatailimages:add"))
        # permission should be denied
        self.assertRedirects(response, reverse("shatailadmin_home"))

    def test_get_edit(self):
        response = self.client.get(reverse("shatailimages:edit", args=(self.image.id,)))
        self.assertEqual(response.status_code, 200)
        self.assertTemplateUsed(response, "shatailimages/images/edit.html")

    def test_get_delete(self):
        response = self.client.get(
            reverse("shatailimages:delete", args=(self.image.id,))
        )
        self.assertEqual(response.status_code, 200)
        self.assertTemplateUsed(response, "shatailimages/images/confirm_delete.html")
...
class TestImageAddMultipleView(TestCase, ShatailTestUtils):

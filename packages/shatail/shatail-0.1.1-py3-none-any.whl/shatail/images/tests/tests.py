...
from shatail.images import get_image_model, get_image_model_string
from shatail.images.fields import ShatailImageField
from shatail.images.formats import Format, get_image_format, register_image_format
from shatail.images.forms import get_image_form
from shatail.images.models import Image as ShatailImage
from shatail.images.permissions import update_permission_policy
from shatail.images.rect import Rect, Vector
from shatail.images.utils import generate_signature, verify_signature
from shatail.images.views.serve import ServeView
from shatail.test.testapp.models import CustomImage, CustomImageFilePath
from shatail.test.utils import ShatailTestUtils, disconnect_signal_receiver
...
class TestImageTag(TestCase):
    ...
    def render_image_tag(self, image, filter_spec):
        temp = template.Template(
            "{% load shatailimages_tags %}{% image image_obj " + filter_spec + "%}"
        )
    ...
    def render_image_tag_as(self, image, filter_spec):
        temp = template.Template(
            "{% load shatailimages_tags %}{% image image_obj "
            + filter_spec
    ...
    def render_image_tag_with_extra_attributes(self, image, title):
        temp = template.Template(
            '{% load shatailimages_tags %}{% image image_obj width-400 \
    ...
    def render_image_tag_with_filters(self, image):
        temp = template.Template(
            "{% load shatailimages_tags %}{% image image_primary|default:image_alternate width-400 %}"
        )
    ...
    def test_context_may_only_contain_one_argument(self):
        with self.assertRaises(template.TemplateSyntaxError):
            temp = template.Template(
                "{% load shatailimages_tags %}{% image image_obj fill-200x200"
    ...
    def test_no_image_filter_provided_when_using_as(self):
        ...
        with self.assertRaises(template.TemplateSyntaxError):
            temp = template.Template(
                "{% load shatailimages_tags %}{% image image_obj as foo %}"
            )
            context = template.Context({"image_obj": self.image})
            temp.render(context)
    ...
    def test_no_image_filter_provided_but_attributes_provided(self):
        ...
        with self.assertRaises(template.TemplateSyntaxError):
            temp = template.Template(
                '{% load shatailimages_tags %}{% image image_obj class="cover-image"%}'
            )
            context = template.Context({"image_obj": self.image})
            temp.render(context)
    ...
    def render_image_url_tag(self, image, view_name):
        temp = template.Template(
            '{% load shatailimages_tags %}{% image_url image_obj "width-400" "'
            + view_name
            + '" %}'
        )
        context = template.Context({"image_obj": image})
    ...
    def test_image_url(self):
        result = self.render_image_url_tag(self.image, "shatailimages_serve")
        self.assertRegex(
            result,
    ...
    def test_image_url_custom_view(self):
        result = self.render_image_url_tag(
            self.image, "shatailimages_serve_custom_view"
        )
    ...
    def test_image_url_no_imageserve_view_added(self):
        ...
        with self.assertRaises(ImproperlyConfigured):
            temp = template.Template(
                '{% load shatailimages_tags %}{% image_url image_obj "width-400" "mynonexistingimageserve_view" %}'
            )
            context = template.Context({"image_obj": self.image})
            temp.render(context)
...
class TestFormat(TestCase, ShatailTestUtils):
    def setUp(self):
        # test format
...
class TestFrontendServeView(TestCase):
    ...
    def test_get(self):
        ...
        response = self.client.get(
            reverse(
                "shatailimages_serve", args=(signature, self.image.id, "fill-800x600")
            )
        )
    ...
    def test_get_with_serve_action(self):
        ...
        response = self.client.get(
            reverse(
                "shatailimages_serve_action_serve",
    ...
    def test_get_with_redirect_action(self):
        ...
        response = self.client.get(
            reverse(
                "shatailimages_serve_action_redirect",
                args=(signature, self.image.id, "fill-800x600"),
            )
        )
    ...
    def test_get_with_custom_key(self):
        ...
        response = self.client.get(
            reverse(
                "shatailimages_serve_custom_key",
                args=(signature, self.image.id, "fill-800x600"),
            )
            + "test.png"
        )
...
class TestGetImageForm(TestCase, ShatailTestUtils):
    ...
    def test_file_field(self):
        form = get_image_form(ShatailImage)

        self.assertIsInstance(form.base_fields["file"], ShatailImageField)
...
class TestGetImageModel(ShatailTestUtils, TestCase):
    ...
    def test_standard_get_image_model(self):
        """Test get_image_model with no SHATAILIMAGES_IMAGE_MODEL"""
        del settings.SHATAILIMAGES_IMAGE_MODEL
    ...
    def test_standard_get_image_model_string(self):
        """Test get_image_model_STRING with no SHATAILIMAGES_IMAGE_MODEL"""
        del settings.SHATAILIMAGES_IMAGE_MODEL
        self.assertEqual(get_image_model_string(), "shatailimages.Image")

    @disconnect_signal_receiver(
        signal=setting_changed, receiver=update_permission_policy
    )
    @override_settings(SHATAILIMAGES_IMAGE_MODEL="tests.UnknownModel")
    ...
    @override_settings(SHATAILIMAGES_IMAGE_MODEL="invalid-string")

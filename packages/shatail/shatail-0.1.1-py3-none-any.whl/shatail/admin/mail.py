...
from shatail.admin.auth import users_with_page_permission
from shatail.coreutils import camelcase_to_underscore
from shatail.models import GroupApprovalTask, TaskState, WorkflowState
from shatail.users.models import UserProfile

logger = logging.getLogger("shatail.admin")
...
def send_mail(subject, message, recipient_list, from_email=None, **kwargs):
    ...
    """
    if not from_email:
        if hasattr(settings, "SHATAILADMIN_NOTIFICATION_FROM_EMAIL"):
            from_email = settings.SHATAILADMIN_NOTIFICATION_FROM_EMAIL
...
def send_moderation_notification(revision, notification, excluded_user=None):
    ...
    if notification == "submitted":
        ...
        include_superusers = getattr(
            settings, "SHATAILADMIN_NOTIFICATION_INCLUDE_SUPERUSERS", True
        )
...
def send_notification(recipient_users, notification, extra_context):
    ...
    # Get template
    template_subject = "shatailadmin/notifications/" + notification + "_subject.txt"
    template_text = "shatailadmin/notifications/" + notification + ".txt"
    template_html = "shatailadmin/notifications/" + notification + ".html"
    ...
    with OpenedConnection(connection) as open_connection:
        ...
        for recipient in email_recipients:
            try:
                ...
                with override(recipient.shatail_userprofile.get_preferred_language()):
                ...
                kwargs = {}
                if getattr(settings, "SHATAILADMIN_NOTIFICATION_USE_HTML", False):
...
class Notifier:
    ...
    notification = ""
    template_directory = "shatailadmin/notifications/"
            ...
            with OpenedConnection(connection) as open_connection:
                ...
                for recipient in recipients:
                    try:
                        ...
                        # Translate text to the recipient language settings
                        with override(
                            recipient.shatail_userprofile.get_preferred_language()
                        ):
                            # Get email subject and content
                        ...
                        kwargs = {}
                        if getattr(
                            settings, "SHATAILADMIN_NOTIFICATION_USE_HTML", False
                        ):

...
import shatail.admin.rich_text.editors.draftail.features as draftail_features
from shatail import __version__, hooks
from shatail.admin.admin_url_finder import (
    ModelAdminURLFinder,
    register_admin_url_finder,
)
from shatail.admin.auth import user_has_any_page_permission
from shatail.admin.forms.collections import GroupCollectionManagementPermissionFormSet
from shatail.admin.menu import MenuItem, SubmenuMenuItem, reports_menu, settings_menu
from shatail.admin.navigation import get_explorable_root_page
from shatail.admin.rich_text.converters.contentstate import link_entity
from shatail.admin.rich_text.converters.editor_html import (
    LinkTypeRule,
    PageLinkHandler,
    WhitelistRule,
)
from shatail.admin.rich_text.converters.html_to_contentstate import (
    BlockElementHandler,
...
from shatail.admin.site_summary import PagesSummaryItem
from shatail.admin.ui.sidebar import (
    PageExplorerMenuItem as PageExplorerMenuItemComponent,
)
from shatail.admin.ui.sidebar import SubMenuItem as SubMenuItemComponent
from shatail.admin.views.pages.bulk_actions import (
    DeleteBulkAction,
    MoveBulkAction,
    PublishBulkAction,
    UnpublishBulkAction,
)
from shatail.admin.viewsets import viewsets
from shatail.admin.widgets import Button, ButtonWithDropdownFromHook, PageListingButton
from shatail.models import Collection, Page, Task, UserPagePermissionsProxy, Workflow
from shatail.permissions import (
    collection_permission_policy,
    task_permission_policy,
    workflow_permission_policy,
)
from shatail.whitelist import allow_without_attributes, attribute_rule, check_url
...
def register_explorer_menu_item():
    return ExplorerMenuItem(
        _("Pages"),
        reverse("shatailadmin_explore_root"),
        name="explorer",
...
class SettingsMenuItem(SubmenuMenuItem):
    def render_component(self, request):
        return SubMenuItemComponent(
            ...
            icon_name=self.icon_name,
            classnames=self.classnames,
            footer_text="Shatail v." + __version__,
        )
...
class PageSearchArea(SearchArea):
    def __init__(self):
        super().__init__(
            _("Pages"),
            reverse("shatailadmin_pages:search"),
            name="pages",
            icon_name="folder-open-inverse",
            order=100,
        )
...
def register_collections_menu_item():
    return CollectionsMenuItem(
        _("Collections"),
        reverse("shatailadmin_collections:index"),
...
def register_workflows_menu_item():
    return WorkflowsMenuItem(
        _("Workflows"),
        reverse("shatailadmin_workflows:index"),
        icon_name="tasks",
        order=100,
    )
...
def register_workflow_tasks_menu_item():
    return WorkflowTasksMenuItem(
        _("Workflow tasks"),
        reverse("shatailadmin_workflows:task_index"),
        icon_name="thumbtack",
        order=150,
    )
...
def page_listing_buttons(page, page_perms, is_parent=False, next_url=None):
    if page_perms.can_edit():
        yield PageListingButton(
            _("Edit"),
            reverse("shatailadmin_pages:edit", args=[page.id]),
            attrs={
    ...
    if page.has_unpublished_changes and page.is_previewable():
        yield PageListingButton(
            _("View draft"),
            reverse("shatailadmin_pages:view_draft", args=[page.id]),
            attrs={
    ...
    if page_perms.can_add_subpage():
        if is_parent:
            yield Button(
                _("Add child page"),
                reverse("shatailadmin_pages:add_subpage", args=[page.id]),
                attrs={
...
def page_listing_more_buttons(page, page_perms, is_parent=False, next_url=None):
    if page_perms.can_move():
        yield Button(
            _("Move"),
            reverse("shatailadmin_pages:move", args=[page.id]),
            attrs={
    ...
    if page_perms.can_copy():
        url = reverse("shatailadmin_pages:copy", args=[page.id])
        if next_url:
    ...
    if page_perms.can_delete():
        url = reverse("shatailadmin_pages:delete", args=[page.id])

        # After deleting the page, it is impossible to redirect to it.
        if next_url == reverse("shatailadmin_explore", args=[page.id]):
            next_url = None
    ...
    if page_perms.can_unpublish():
        url = reverse("shatailadmin_pages:unpublish", args=[page.id])
        if next_url:
            url += "?" + urlencode({"next": next_url})
    ...
    if page_perms.can_view_revisions():
        yield Button(
            _("History"),
            reverse("shatailadmin_pages:history", args=[page.id]),
            attrs={
        ...
        draftail_features.EntityFeature(
            {
                ...
                # Keep only the attributes Shatail needs.
            ...
            js=[
                "shatailadmin/js/page-chooser-modal.js",
            ],
        ),
    )
    features.register_converter_rule(
...
class WorkflowReportMenuItem(MenuItem):
    def is_shown(self, request):
        return getattr(settings, "SHATAIL_WORKFLOW_ENABLED", True)
...
class AgingPagesReportMenuItem(MenuItem):
    def is_shown(self, request):
        return getattr(settings, "SHATAIL_AGING_PAGES_ENABLED", True)
...
def register_locked_pages_menu_item():
    return LockedPagesMenuItem(
        _("Locked Pages"),
        reverse("shatailadmin_reports:locked_pages"),
        icon_name="lock",
        order=700,
    )
...
def register_workflow_report_menu_item():
    return WorkflowReportMenuItem(
        _("Workflows"),
        reverse("shatailadmin_reports:workflow"),
        icon_name="tasks",
        order=800,
    )
...
def register_workflow_tasks_report_menu_item():
    return WorkflowReportMenuItem(
        _("Workflow tasks"),
        reverse("shatailadmin_reports:workflow_tasks"),
        icon_name="thumbtack",
        order=900,
    )
...
def register_site_history_report_menu_item():
    return SiteHistoryReportMenuItem(
        _("Site history"),
        reverse("shatailadmin_reports:site_history"),
        icon_name="history",
        order=1000,
    )
...
def register_aging_pages_report_menu_item():
    return AgingPagesReportMenuItem(
        _("Aging pages"),
        reverse("shatailadmin_reports:aging_pages"),
        icon_name="time",
        order=1100,
    )
...
def register_icons(icons):
    for icon in [
        ...
        icons.append("shatailadmin/icons/{}".format(icon))
    return icons
...
class PageAdminURLFinder:
    ...
    def get_edit_url(self, instance):
        if self.page_perms and not self.page_perms.for_page(instance).can_edit():
            ...
            return reverse("shatailadmin_pages:edit", args=(instance.pk,))
...
class CollectionAdminURLFinder(ModelAdminURLFinder):
    ...
    edit_url_name = "shatailadmin_collections:edit"
...
class WorkflowAdminURLFinder(ModelAdminURLFinder):
    ...
    edit_url_name = "shatailadmin_workflows:edit"
...
class WorkflowTaskAdminURLFinder(ModelAdminURLFinder):
    ...
    edit_url_name = "shatailadmin_workflows:edit_task"

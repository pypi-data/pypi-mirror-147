...
from shatail.admin.views.mixins import ExcelDateFormatter
from shatail.models import Page, PageLogEntry
from shatail.test.utils import ShatailTestUtils


class TestLockedPagesView(TestCase, ShatailTestUtils):
    def setUp(self):
        self.user = self.login()

    def get(self, params={}):
        return self.client.get(reverse("shatailadmin_reports:locked_pages"), params)

    def test_simple(self):
        response = self.get()
        self.assertEqual(response.status_code, 200)
        self.assertTemplateUsed(response, "shatailadmin/reports/locked_pages.html")
...
class TestFilteredLockedPagesView(TestCase, ShatailTestUtils):
    fixtures = ["test.json"]
...
class TestFilteredLogEntriesView(TestCase, ShatailTestUtils):
    fixtures = ["test.json"]
    ...
    def setUp(self):
        ...
        self.create_log = PageLogEntry.objects.log_action(
            self.home_page, "shatail.create"
        )
        self.edit_log_1 = PageLogEntry.objects.log_action(
            self.home_page, "shatail.edit"
        )
    ...
    def get(self, params={}):
        return self.client.get(reverse("shatailadmin_reports:site_history"), params)
    ...
    def test_filter_by_action(self):
        response = self.get(params={"action": "shatail.edit"})
        self.assertEqual(response.status_code, 200)
        self.assert_log_entries(
            response,
            [
...
class TestAgingPagesView(TestCase, ShatailTestUtils):
    def setUp(self):
        self.user = self.login()
    ...
    def get(self, params={}):
        return self.client.get(reverse("shatailadmin_reports:aging_pages"), params)
    ...
    def test_simple(self):
        response = self.get()
        self.assertEqual(response.status_code, 200)
        self.assertTemplateUsed(response, "shatailadmin/reports/aging_pages.html")
    ...
    def test_csv_export(self):
        ...
        if settings.USE_TZ:
            self.assertEqual(
                data_lines[1],
                "Welcome to your new Shatail site!,live + draft,2013-01-01 12:00:00+00:00,test@email.com,Page\r",
            )
        else:
            self.assertEqual(
                data_lines[1],
                "Welcome to your new Shatail site!,live + draft,2013-01-01 12:00:00,test@email.com,Page\r",
            )
    ...
    def test_xlsx_export(self):
        ...
        self.assertEqual(
            cell_array[1],
            [
                "Welcome to your new Shatail site!",
                "live + draft",
                datetime.datetime(2013, 1, 1, 12, 0),
                "test@email.com",
                "Page",
...
class TestFilteredAgingPagesView(TestCase, ShatailTestUtils):
    fixtures = ["test.json"]

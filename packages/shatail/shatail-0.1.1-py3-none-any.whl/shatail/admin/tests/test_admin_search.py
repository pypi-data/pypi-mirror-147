...
class BaseSearchAreaTestCase(ShatailTestUtils, TestCase):
    rf = RequestFactory()

    def search_other(self, current_url="/admin/", data=None):
        request = self.rf.get(current_url, data=data)
        request.user = self.user
        template = Template("{% load shatailadmin_tags %}{% search_other %}")
...
class TestSearchAreas(BaseSearchAreaTestCase):
    ...
    def test_other_searches(self):
        search_url = reverse("shatailadmin_pages:search")
        query = "Hello"
        ...
        self.assertTemplateUsed(response, "shatailadmin/pages/search.html")
        self.assertTemplateUsed(response, "shatailadmin/shared/search_area.html")
        self.assertTemplateUsed(response, "shatailadmin/shared/search_other.html")
    ...
    def test_search_other(self):
        ...
        self.assertIn(reverse("shatailadmin_pages:search"), rendered)
...
class TestSearchAreaNoPagePermissions(BaseSearchAreaTestCase):
    ...
    def test_dashboard(self):
        ...
        self.assertNotContains(response, reverse("shatailadmin_pages:search"))
        self.assertContains(
            response,
            '{"_type": "shatail.sidebar.SearchModule", "_args": ["/customsearch/"]}',
        )

    def test_search_other(self):
        """The pages search link should be hidden, custom search should be visible."""
        rendered = self.search_other()
        self.assertNotIn(reverse("shatailadmin_pages:search"), rendered)
        self.assertIn("/customsearch/", rendered)

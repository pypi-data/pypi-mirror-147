...
from shatail.admin import widgets
from shatail.admin.forms.tags import TagField
from shatail.models import Page
from shatail.test.testapp.forms import AdminStarDateInput
from shatail.test.testapp.models import EventPage, RestaurantTag, SimplePage
...
class TestAdminPageChooserWidget(TestCase):
    ...
    def test_render_js_init(self):
        ...
        self.assertIn(
            'createPageChooser("test-id", null, {"model_names": ["shatailcore.page"], "can_choose_root": false, "user_perms": null});',
            html,
        )
    ...
    def test_render_js_init_with_user_perm(self):
        ...
        html = widget.render("test", None, {"id": "test-id"})
        self.assertIn(
            'createPageChooser("test-id", null, {"model_names": ["shatailcore.page"], "can_choose_root": false, "user_perms": "copy_to"});',
            html,
        )
    ...
    def test_render_with_value(self):
        ...
        self.assertIn(
            'createPageChooser("test-id", %d, {"model_names": ["shatailcore.page"], "can_choose_root": false, "user_perms": null});'
            % self.root_page.id,
            html,
        )
    ...
    def test_render_js_init_with_can_choose_root(self):
        ...
        html = widget.render("test", self.child_page, {"id": "test-id"})
        self.assertIn(
            'createPageChooser("test-id", %d, {"model_names": ["shatailcore.page"], "can_choose_root": true, "user_perms": null});'
            % self.root_page.id,
            html,
        )
...
class TestAdminDateInput(TestCase):
    ...
    @override_settings(SHATAIL_DATE_FORMAT="%d.%m.%Y.")
    ...
    def test_media_inheritance(self):
        ...
        self.assertIn("shatailadmin/js/date-time-chooser.js", media_html)
...
class TestAdminTimeInput(TestCase):
    ...
    @override_settings(SHATAIL_TIME_FORMAT="%H:%M:%S")
    def test_render_js_init_with_format_from_settings(self):
        widget = widgets.AdminTimeInput()
...
class TestAdminDateTimeInput(TestCase):
    ...
    @override_settings(
        SHATAIL_DATETIME_FORMAT="%d.%m.%Y. %H:%M", SHATAIL_TIME_FORMAT="%H:%M %p"
    )
    def test_render_js_init_with_format_from_settings(self):
        widget = widgets.AdminDateTimeInput()

...
from shatail.admin.views.chooser import can_choose_page
from shatail.models import Locale, Page, UserPagePermissionsProxy
...
class TestChooserBrowse(TestCase, ShatailTestUtils):
    def setUp(self):
    ...
    def get(self, params={}):
        return self.client.get(reverse("shatailadmin_choose_page"), params)

    def test_simple(self):
        response = self.get()
        self.assertEqual(response.status_code, 200)
        self.assertTemplateUsed(response, "shatailadmin/chooser/browse.html")
...
class TestCanChooseRootFlag(TestCase, ShatailTestUtils):
    def setUp(self):
        self.login()
...
class TestChooserBrowseChild(TestCase, ShatailTestUtils):
    def setUp(self):
        self.root_page = Page.objects.get(id=2)
    ...
    def get(self, params={}):
        return self.client.get(
            reverse("shatailadmin_choose_page_child", args=(self.root_page.id,)), params
        )

    def get_invalid(self, params={}):
        return self.client.get(
            reverse("shatailadmin_choose_page_child", args=(9999999,)), params
        )
    ...
    def test_with_blank_page_type(self):
        ...
        # (or an explicit page_type of shatailcore.page)
    ...
    def test_with_bad_page_type(self):
        response = self.get({"page_type": "shatailcore.site"})
        self.assertEqual(response.status_code, 404)
    ...
    def test_with_admin_display_title(self):
        ...
        response = self.get({"page_type": "shatailcore.Page"})
        self.assertEqual(response.status_code, 200)
        self.assertTemplateUsed(response, "shatailadmin/chooser/browse.html")
    ...
    def test_parent_with_admin_display_title(self):
        ...
        response = self.client.get(
            reverse("shatailadmin_choose_page_child", args=(self.child_page.id,)),
            params={"page_type": "shatailcore.Page"},
        )
        self.assertEqual(response.status_code, 200)
        self.assertTemplateUsed(response, "shatailadmin/chooser/browse.html")
    ...
    def test_admin_display_title_breadcrumb(self):
        ...
        response = self.client.get(
            reverse("shatailadmin_choose_page_child", args=(leaf_page.id,))
        )
        self.assertEqual(response.status_code, 200)
        self.assertTemplateUsed(response, "shatailadmin/chooser/browse.html")
...
class TestChooserSearch(TestCase, ShatailTestUtils):
    def setUp(self):
        self.root_page = Page.objects.get(id=2)
    ...
    def get(self, params=None):
        return self.client.get(reverse("shatailadmin_choose_page_search"), params or {})

    def test_simple(self):
        response = self.get({"q": "foobarbaz"})
        self.assertEqual(response.status_code, 200)
        self.assertTemplateUsed(response, "shatailadmin/chooser/_search_results.html")
...
class TestAutomaticRootPageDetection(TestCase, ShatailTestUtils):
    def setUp(self):
    ...
    def get_best_root(self, params={}):
        response = self.client.get(reverse("shatailadmin_choose_page"), params)
    ...
    def test_type_page(self):
        self.assertEqual(
            self.get_best_root({"page_type": "shatailcore.Page"}), self.tree_root
        )
...
class TestChooserExternalLink(TestCase, ShatailTestUtils):
    def setUp(self):
        self.login()
    ...
    def get(self, params={}):
        return self.client.get(
            reverse("shatailadmin_choose_page_external_link"), params
        )

    def post(self, post_data={}, url_params={}):
        url = reverse("shatailadmin_choose_page_external_link")
        if url_params:
    ...
    def test_simple(self):
        response = self.get()
        self.assertEqual(response.status_code, 200)
        self.assertTemplateUsed(response, "shatailadmin/chooser/external_link.html")
    ...
    @override_settings(SHATAILADMIN_EXTERNAL_LINK_CONVERSION="")
    ...
    @override_settings(SHATAILADMIN_EXTERNAL_LINK_CONVERSION="exact")
    ...
    @override_settings(SHATAILADMIN_EXTERNAL_LINK_CONVERSION="confirm")
...
class TestChooserAnchorLink(TestCase, ShatailTestUtils):
    def setUp(self):
        self.login()

    def get(self, params={}):
        return self.client.get(reverse("shatailadmin_choose_page_anchor_link"), params)

    def post(self, post_data={}, url_params={}):
        url = reverse("shatailadmin_choose_page_anchor_link")
        if url_params:
            url += "?" + urlencode(url_params)
        return self.client.post(url, post_data)
    ...
    def test_simple(self):
        response = self.get()
        self.assertEqual(response.status_code, 200)
        self.assertTemplateUsed(response, "shatailadmin/chooser/anchor_link.html")
...
class TestChooserEmailLink(TestCase, ShatailTestUtils):
    def setUp(self):
        self.login()

    def get(self, params={}):
        return self.client.get(reverse("shatailadmin_choose_page_email_link"), params)

    def post(self, post_data={}, url_params={}):
        url = reverse("shatailadmin_choose_page_email_link")
        if url_params:
            url += "?" + urlencode(url_params)
        return self.client.post(url, post_data)
    ...
    def test_simple(self):
        response = self.get()
        self.assertEqual(response.status_code, 200)
        self.assertTemplateUsed(response, "shatailadmin/chooser/email_link.html")
...
class TestChooserPhoneLink(TestCase, ShatailTestUtils):
    def setUp(self):
        self.login()

    def get(self, params={}):
        return self.client.get(reverse("shatailadmin_choose_page_phone_link"), params)

    def post(self, post_data={}, url_params={}):
        url = reverse("shatailadmin_choose_page_phone_link")
        if url_params:
            url += "?" + urlencode(url_params)
        return self.client.post(url, post_data)
    ...
    def test_simple(self):
        response = self.get()
        self.assertEqual(response.status_code, 200)
        self.assertTemplateUsed(response, "shatailadmin/chooser/phone_link.html")
...
class TestCanChoosePage(TestCase, ShatailTestUtils):
    fixtures = ["test.json"]
...
class TestPageChooserLocaleSelector(TestCase, ShatailTestUtils):
    fixtures = ["test.json"]
    ...
    def get(self, parent_page_id):
        return self.client.get(
            reverse("shatailadmin_choose_page_child", args=[parent_page_id])
        )

    def get_choose_page_url(self, locale=None, parent_page_id=None, html=True):
        if parent_page_id is not None:
            url = reverse("shatailadmin_choose_page_child", args=[parent_page_id])
        else:
            url = reverse("shatailadmin_choose_page")
        ...
        return f"{url}?page_type=shatailcore.page{suffix}"

    def test_locale_selector_present_in_root_view(self):
        response = self.client.get(reverse("shatailadmin_choose_page"))
        html = response.json().get("html")

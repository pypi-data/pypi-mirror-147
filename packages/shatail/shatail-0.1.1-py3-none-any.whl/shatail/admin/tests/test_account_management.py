...
from shatail.admin.localization import (
    SHATAILADMIN_PROVIDED_LANGUAGES,
    get_available_admin_languages,
    get_available_admin_time_zones,
)
from shatail.admin.views.account import account, profile_tab
...
class TestAuthentication(TestCase, ShatailTestUtils):
    """
    ...
    def test_login_view(self):
        ...
        """
        # Get login page
        response = self.client.get(reverse("shatailadmin_login"))

        # Check that the user received a login page
        self.assertEqual(response.status_code, 200)
        self.assertTemplateUsed(response, "shatailadmin/login.html")
    ...
    def test_login_view_post(self):
        ...
        response = self.client.post(
            reverse("shatailadmin_login"),
            {
                ...
                "next": reverse("shatailadmin_home"),
            },
        )
    ...
    def test_logout(self):
        ...
        # Get logout page
        response = self.client.get(reverse("shatailadmin_logout"))

        # Check that the user was redirected to the login page
        self.assertRedirects(response, reverse("shatailadmin_login"))
    ...
    def test_not_logged_in_redirect(self):
        ...
        self.assertRedirects(
            response,
            reverse("shatailadmin_login") + "?next=" + reverse("shatailadmin_home"),
        )
    ...
    def test_not_logged_in_gives_403_to_ajax_requests(self):
        ...
        response = self.client.get(
            reverse("shatailadmin_home"), HTTP_X_REQUESTED_WITH="XMLHttpRequest"
        )
    ...
    def test_not_logged_in_redirect_default_settings(self):
        ...
        # this must be the same URL as 'shatailadmin_login'
    ...
    def test_logged_in_no_permission_redirect(self):
        ...
        # Get dashboard
        response = self.client.get(reverse("shatailadmin_home"), follow=True)
...
class TestAccountSectionUtilsMixin:
    ...
    def post_form(self, extra_post_data):
        ...
        return self.client.post(reverse("shatailadmin_account"), post_data)


class TestAccountSection(TestCase, ShatailTestUtils, TestAccountSectionUtilsMixin):
    """
    ...
    def test_account_view(self):
        ...
        """
        # Get account page
        response = self.client.get(reverse("shatailadmin_account"))

        # Check that the user received an account page
        self.assertEqual(response.status_code, 200)
        self.assertTemplateUsed(response, "shatailadmin/account/account.html")
    ...
    def test_change_name_post(self):
        ...
        self.assertRedirects(response, reverse("shatailadmin_account"))
    ...
    @override_settings(SHATAIL_EMAIL_MANAGEMENT_ENABLED=False)
    ...
    @override_settings(SHATAIL_PASSWORD_MANAGEMENT_ENABLED=False)
    ...
    @override_settings(
        SHATAILADMIN_PERMITTED_LANGUAGES=[("en", "English"), ("es", "Spanish")]
    )
    ...
    def test_available_admin_languages_by_default(self):
        self.assertListEqual(
            get_available_admin_languages(), SHATAILADMIN_PROVIDED_LANGUAGES
        )

    @override_settings(SHATAILADMIN_PERMITTED_LANGUAGES=[("en", "English")])
    ...
    @unittest.skipUnless(settings.USE_TZ, "Timezone support is disabled")
    @override_settings(
        SHATAIL_USER_TIME_ZONES=["Africa/Addis_Ababa", "America/Argentina/Buenos_Aires"]
    )
    ...
    @unittest.skipUnless(settings.USE_TZ, "Timezone support is disabled")
    @override_settings(SHATAIL_USER_TIME_ZONES=["Europe/London"])
    ...
    @unittest.skipUnless(settings.USE_TZ, "Timezone support is disabled")
    @override_settings(
        SHATAIL_USER_TIME_ZONES=["Europe/London"],
        SHATAILADMIN_PERMITTED_LANGUAGES=[("en", "English")],
    )
    ...
    def test_sensitive_post_parameters(self):
        request = RequestFactory().post("shatailadmin_account", data={})
...
class TestAccountUploadAvatar(TestCase, ShatailTestUtils, TestAccountSectionUtilsMixin):
    def setUp(self):
        self.user = self.login()
...
class TestAccountManagementForNonModerator(TestCase, ShatailTestUtils):
    """
...
class TestAccountManagementForAdminOnlyUser(
    TestCase, ShatailTestUtils, TestAccountSectionUtilsMixin
):
    """
...
class TestPasswordReset(TestCase, ShatailTestUtils):
    """
    ...
    def test_password_reset_view(self):
        ...
        response = self.client.get(reverse("shatailadmin_password_reset"))

        # Check that the user received a password reset page
        self.assertEqual(response.status_code, 200)
        self.assertTemplateUsed(
            response, "shatailadmin/account/password_reset/form.html"
        )
    ...
    def test_password_reset_view_post(self):
        ...
        response = self.client.post(reverse("shatailadmin_password_reset"), post_data)

        # Check that the user was redirected to the done page
        self.assertRedirects(response, reverse("shatailadmin_password_reset_done"))
    ...
    def test_password_reset_confirm_view_invalid_link(self):
        ...
        response = self.client.get(
            reverse("shatailadmin_password_reset_confirm", kwargs=self.url_kwargs)
        )

        # Check that the user received a password confirm done page
        self.assertEqual(response.status_code, 200)
        self.assertTemplateUsed(
            response, "shatailadmin/account/password_reset/confirm.html"
        )
    ...
    def test_password_reset_confirm_view_post(self):
        ...
        self.assertRedirects(response, reverse("shatailadmin_password_reset_complete"))
    ...
    def test_password_reset_done_view(self):
        ...
        response = self.client.get(reverse("shatailadmin_password_reset_done"))

        # Check that the user received a password reset done page
        self.assertEqual(response.status_code, 200)
        self.assertTemplateUsed(
            response, "shatailadmin/account/password_reset/done.html"
        )
    ...
    def test_password_reset_complete_view(self):
        ...
        response = self.client.get(reverse("shatailadmin_password_reset_complete"))

        # Check that the user received a password reset complete page
        self.assertEqual(response.status_code, 200)
        self.assertTemplateUsed(
            response, "shatailadmin/account/password_reset/complete.html"
        )

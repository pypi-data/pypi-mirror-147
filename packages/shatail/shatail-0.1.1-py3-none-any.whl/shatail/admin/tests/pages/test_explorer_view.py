...
from shatail.admin.tests.pages.timestamps import local_datetime
from shatail.models import GroupPagePermission, Locale, Page
from shatail.test.testapp.models import SimplePage, SingleEventPage, StandardIndex
from shatail.test.utils import ShatailTestUtils


class TestPageExplorer(TestCase, ShatailTestUtils):
    def setUp(self):
        # Find root page
    ...
    def test_explore(self):
        response = self.client.get(
            reverse("shatailadmin_explore", args=(self.root_page.id,))
        )
        self.assertEqual(response.status_code, 200)
        self.assertTemplateUsed(response, "shatailadmin/pages/index.html")
    ...
    def test_explore_root(self):
        response = self.client.get(reverse("shatailadmin_explore_root"))
        self.assertEqual(response.status_code, 200)
        self.assertTemplateUsed(response, "shatailadmin/pages/index.html")
    ...
    def test_pagination(self):
        ...
        response = self.client.get(
            reverse("shatailadmin_explore", args=(self.root_page.id,)), {"p": 2}
        )
    ...
    def test_pagination_out_of_range(self):
        ...
        response = self.client.get(
            reverse("shatailadmin_explore", args=(self.root_page.id,)), {"p": 99999}
        )
    ...
    def test_exploring_uses_specific_page_with_custom_display_title(self):
        ...
        response = self.client.get(
            reverse("shatailadmin_explore", args=(new_event.id,))
        )
        self.assertContains(response, "New event 0 (single event)")

    def test_parent_page_is_specific(self):
        response = self.client.get(
            reverse("shatailadmin_explore", args=(self.child_page.id,))
        )
        self.assertEqual(response.status_code, 200)
    ...
    def test_explorer_no_perms(self):
        ...
        admin = reverse("shatailadmin_home")
        self.assertRedirects(
            self.client.get(reverse("shatailadmin_explore", args=(self.root_page.id,))),
            admin,
        )
        self.assertRedirects(
            self.client.get(reverse("shatailadmin_explore_root")), admin
        )
    ...
    def test_explore_with_missing_page_model(self):
        ...
        response = self.client.get(
            reverse("shatailadmin_explore", args=(self.old_page.id,))
        )
        self.assertEqual(response.status_code, 200)
        self.assertTemplateUsed(response, "shatailadmin/pages/index.html")


class TestBreadcrumb(TestCase, ShatailTestUtils):
    fixtures = ["test.json"]
...
class TestPageExplorerSignposting(TestCase, ShatailTestUtils):
    fixtures = ["test.json"]
    ...
    def test_admin_at_root(self):
        ...
        self.assertContains(
            response,
            (
                "The root level is where you can add new sites to your Shatail installation. "
    ...
    def test_admin_at_non_site_page(self):
        self.login(username="superuser", password="password")
        response = self.client.get(
            reverse("shatailadmin_explore", args=(self.no_site_page.id,))
        )
        self.assertEqual(response.status_code, 200)
    ...
    def test_admin_at_site_page(self):
        self.login(username="superuser", password="password")
        response = self.client.get(
            reverse("shatailadmin_explore", args=(self.site_page.id,))
        )
        self.assertEqual(response.status_code, 200)
    ...
    def test_bad_permissions_at_root(self):
        ...
        self.assertEqual(
            ...
            (302, reverse("shatailadmin_explore", args=(self.site_page.pk,))),
        )
    ...
    def test_bad_permissions_at_site_page(self):
        ...
        self.assertEqual(
            (response.status_code, response["Location"]),
            (302, reverse("shatailadmin_explore", args=(self.no_site_page.pk,))),
        )


class TestExplorablePageVisibility(TestCase, ShatailTestUtils):
    """
    ...
    User "mary" is is no Groups, but she has the "access shatail admin" permission.
    ...
    def test_admin_can_explore_every_page(self):
        ...
        for page in Page.objects.all():
            response = self.client.get(reverse("shatailadmin_explore", args=[page.pk]))
            self.assertEqual(response.status_code, 200)
    ...
    def test_admin_sees_breadcrumbs_up_to_root_page(self):
        self.login(username="superman", password="password")
        response = self.client.get(reverse("shatailadmin_explore", args=[6]))
        self.assertEqual(response.status_code, 200)
        expected = """
    ...
    def test_admin_home_page_changes_with_permissions(self):
        ...
        self.assertContains(response, "Welcome to the example.com Shatail CMS")
...
class TestLocaleSelector(TestCase, ShatailTestUtils):
    ...
    def test_locale_selector(self):
        response = self.client.get(
            reverse("shatailadmin_explore", args=[self.events_page.id])
        )

        self.assertContains(response, '<li class="header-meta--locale">')

        add_translation_url = reverse(
            "shatailadmin_explore", args=[self.translated_events_page.id]
        )
        self.assertContains(
            response,

...
from shatail.search.index import SearchField
from shatail.test.testapp.models import SimplePage, SingleEventPage
from shatail.test.utils import ShatailTestUtils


class TestPageSearch(TestCase, ShatailTestUtils):
    def setUp(self):
        self.user = self.login()

    def get(self, params=None, **extra):
        return self.client.get(
            reverse("shatailadmin_pages:search"), params or {}, **extra
        )
    ...
    def test_ajax(self):
        ...
        self.assertEqual(response.status_code, 200)
        self.assertTemplateNotUsed(response, "shatailadmin/pages/search.html")
        self.assertTemplateUsed(response, "shatailadmin/pages/search_results.html")
        self.assertEqual(response.context["query_string"], "Hello")

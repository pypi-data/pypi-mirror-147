...
from shatail.test.testapp.models import BusinessChild, SimplePage
from shatail.test.utils import ShatailTestUtils


class TestBulkMove(TestCase, ShatailTestUtils):
    fixtures = ["test.json"]
        ...
        self.assertInHTML(
            '<li><a href="{edit_page_url}" target="_blank" rel="noreferrer">Hello world! (simple page)</a></li>'.format(
                edit_page_url=reverse(
                    "shatailadmin_pages:edit", args=[self.test_page_b.id]
                ),
            ),
            html,
        )
        ...
        for child_page in self.pages_to_be_moved:
            self.assertInHTML(
                '<li><a href="{edit_page_url}" target="_blank" rel="noreferrer">{page_title}</a></li>'.format(
                    edit_page_url=reverse(
                        "shatailadmin_pages:edit", args=[child_page.id]
                    ),

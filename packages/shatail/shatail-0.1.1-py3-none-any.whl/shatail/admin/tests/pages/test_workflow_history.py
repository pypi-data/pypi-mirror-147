...
class TestWorkflowHistoryDetail(TestCase, ShatailTestUtils):
    fixtures = ["test.json"]
    ...
    def test_get_index(self):
        response = self.client.get(
            reverse(
                "shatailadmin_pages:workflow_history", args=[self.christmas_event.id]
            )
        )
        self.assertEqual(response.status_code, 200)

        self.assertContains(
            response, reverse("shatailadmin_pages:edit", args=[self.christmas_event.id])
        )
        self.assertContains(
            response,
            reverse(
                "shatailadmin_pages:workflow_history_detail",

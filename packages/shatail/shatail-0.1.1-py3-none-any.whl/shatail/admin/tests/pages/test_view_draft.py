...
from shatail.test.testapp.models import SimplePage, StreamPage
from shatail.test.utils import ShatailTestUtils


class TestDraftAccess(TestCase, ShatailTestUtils):
    ...
    def test_draft_access_admin(self):
        ...
        response = self.client.get(
            reverse("shatailadmin_pages:view_draft", args=(self.child_page.id,))
        )
    ...
    def test_page_without_preview_modes_is_unauthorized(self):
        ...
        # Try getting page draft
        response = self.client.get(
            reverse("shatailadmin_pages:view_draft", args=(self.stream_page.id,))
        )

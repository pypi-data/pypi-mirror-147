...
class TestRevisions(TestCase, ShatailTestUtils):
    fixtures = ["test.json"]
    ...
    def test_get_revisions_index(self):
        response = self.client.get(
            reverse(
                "shatailadmin_pages:revisions_index", args=(self.christmas_event.id,)
            )
        )
        history_url = reverse(
            "shatailadmin_pages:history", args=(self.christmas_event.id,)
        )
        self.assertRedirects(response, history_url)

    def request_preview_revision(self):
        last_christmas_preview_url = reverse(
            "shatailadmin_pages:revisions_view",
    ...
    def test_preview_revision_with_no_page_permissions_redirects_to_admin(self):
        ...
        admin_only_user.user_permissions.add(
            Permission.objects.get_by_natural_key(
                codename="access_admin", app_label="shatailadmin", model="admin"
            )
        )
        ...
        self.assertEqual(response["Location"], reverse("shatailadmin_home"))
    ...
    def test_revert_revision(self):
        last_christmas_preview_url = reverse(
            "shatailadmin_pages:revisions_revert",
            args=(self.christmas_event.id, self.last_christmas_revision.id),
        )
    ...
    def test_scheduled_revision(self):
        ...
        this_christmas_unschedule_url = reverse(
            "shatailadmin_pages:revisions_unschedule",
            args=(self.christmas_event.id, this_christmas_revision.id),
        )
        response = self.client.get(
            reverse("shatailadmin_pages:history", args=(self.christmas_event.id,))
        )
        self.assertEqual(response.status_code, 200)
...
class TestCompareRevisions(TestCase, ShatailTestUtils):
    ...
    def test_compare_revisions(self):
        compare_url = reverse(
            "shatailadmin_pages:revisions_compare",
            args=(
...
class TestCompareRevisionsWithPerUserEditHandlers(TestCase, ShatailTestUtils):
    fixtures = ["test.json"]
...
class TestCompareRevisionsWithNonModelField(TestCase, ShatailTestUtils):
    """
...
class TestRevisionsUnschedule(TestCase, ShatailTestUtils):
    fixtures = ["test.json"]
    ...
    def test_unschedule_view(self):
        ...
        self.assertTemplateUsed(
            response, "shatailadmin/pages/revisions/confirm_unschedule.html"
        )
    ...
    def test_unschedule_view_invalid_page_id(self):
        ...
        # Get unschedule page
        response = self.client.get(
            reverse("shatailadmin_pages:revisions_unschedule", args=(12345, 67894))
        )
...
class TestRevisionsUnscheduleForUnpublishedPages(TestCase, ShatailTestUtils):
    fixtures = ["test.json"]
    ...
    def test_unschedule_view_post(self):
        ...
        self.assertRedirects(
            response,
            reverse("shatailadmin_pages:history", args=(self.unpublished_event.id,)),
        )

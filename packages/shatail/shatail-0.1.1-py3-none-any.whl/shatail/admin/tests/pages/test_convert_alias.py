...
class TestConvertAlias(TestCase, ShatailTestUtils):
    def setUp(self):
        # Find root page
    ...
    def test_convert_alias(self):
        response = self.client.get(
            reverse("shatailadmin_pages:convert_alias", args=[self.alias_page.id])
        )
        self.assertEqual(response.status_code, 200)

    def test_convert_alias_not_alias(self):
        response = self.client.get(
            reverse("shatailadmin_pages:convert_alias", args=[self.child_page.id])
        )
    ...
    def test_post_convert_alias(self):
        ...
        self.assertRedirects(
            response, reverse("shatailadmin_pages:edit", args=[self.alias_page.id])
        )

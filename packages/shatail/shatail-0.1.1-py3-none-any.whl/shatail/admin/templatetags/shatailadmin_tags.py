...
from shatail.admin.localization import get_js_translation_strings
from shatail.admin.menu import admin_menu
from shatail.admin.navigation import get_explorable_root_page
from shatail.admin.search import admin_search_areas
from shatail.admin.staticfiles import versioned_static as versioned_static_func
from shatail.admin.ui import sidebar
from shatail.admin.utils import get_admin_base_url
from shatail.admin.views.bulk_action.registry import bulk_action_registry
from shatail.admin.views.pages.utils import get_valid_next_url_from_request
from shatail.admin.widgets import ButtonWithDropdown, PageListingButton
from shatail.coreutils import camelcase_to_underscore
from shatail.coreutils import cautious_slugify as _cautious_slugify
...
from shatail.users.utils import get_gravatar_url
...
@register.inclusion_tag("shatailadmin/shared/breadcrumb.html", takes_context=True)
...
@register.inclusion_tag("shatailadmin/shared/move_breadcrumb.html", takes_context=True)
...
@register.inclusion_tag("shatailadmin/shared/search_other.html", takes_context=True)
...
def usage_count_enabled():
    return getattr(settings, "SHATAIL_USAGE_COUNT_ENABLED", False)
...
def allow_unicode_slugs():
    return getattr(settings, "SHATAIL_ALLOW_UNICODE_SLUGS", True)


@register.simple_tag
def auto_update_preview():
    return getattr(settings, "SHATAIL_AUTO_UPDATE_PREVIEW", False)
...
@register.inclusion_tag(
    "shatailadmin/pages/listing/_pagination.html", takes_context=True
)
...
@register.inclusion_tag("shatailadmin/pages/listing/_buttons.html", takes_context=True)
...
@register.inclusion_tag(
    "shatailadmin/pages/listing/_modern_dropdown.html", takes_context=True
)
...
def bulk_action_choices(context, app_label, model_name):
    ...
    bulk_action_buttons = [
        PageListingButton(
            action.display_name,
            reverse(
                "shatail_bulk_action", args=[app_label, model_name, action.action_type]
            )
            + "?"
...
def avatar_url(user, size=50, gravatar_only=False):
    ...
    if (
        not gravatar_only
        and hasattr(user, "shatail_userprofile")
        and user.shatail_userprofile.avatar
    ):
        return user.shatail_userprofile.avatar.url
    ...
    return versioned_static_func("shatailadmin/images/default-user-avatar.png")
...
def notification_static(path):
    ...
    a full URL using SHATAILADMIN_BASE_URL if the static URL isn't already a full URL.
    """
...
def versioned_static(path):
    ...
    that updates on each Shatail version
    """
    return versioned_static_func(path)


@register.inclusion_tag("shatailadmin/shared/icon.html", takes_context=False)
...
@register.simple_tag
def i18n_enabled():
    return getattr(settings, "SHATAIL_I18N_ENABLED", False)
...
def sidebar_collapsed(context):
    ...
    collapsed = request.COOKIES.get("shatail_sidebar_collapsed", "0")
    if collapsed == "0":
        return False
    return True
...
def sidebar_props(context):
    ...
    account_menu = [
        sidebar.LinkMenuItem(
            "account", _("Account"), reverse("shatailadmin_account"), icon_name="user"
        ),
        sidebar.LinkMenuItem(
            "logout", _("Log out"), reverse("shatailadmin_logout"), icon_name="logout"
        ),
    ]

    modules = [
        sidebar.ShatailBrandingModule(),
    ...
    return json_script(
        ...
        element_id="shatail-sidebar-props",
    )


@register.simple_tag
def get_comments_enabled():
    return getattr(settings, "SHATAILADMIN_COMMENTS_ENABLED", True)


@register.simple_tag
def resolve_url(url):
    # Used by shatailadmin/shared/pagination_nav.html - given an input that may be a URL route

...
def css_install_check(app_configs, **kwargs):
    ...
    css_path = os.path.join(
        os.path.dirname(__file__), "static", "shatailadmin", "css", "normalize.css"
    )
    ...
    if not os.path.isfile(css_path):
        error_hint = (
            ...
            Shatail and have not built the static assets -
            see https://docs.shatail.org/en/latest/contributing/developing.html
        ...
        errors.append(
            Warning(
                "CSS for the Shatail admin is missing",
                hint=error_hint,
                id="shatailadmin.W001",
            )
        )
    return errors
...
def base_form_class_check(app_configs, **kwargs):
    ...
    from shatail.models import get_page_models

    errors = []

    for cls in get_page_models():
        if not issubclass(cls.base_form_class, ShatailAdminPageForm):
            errors.append(
                Error(
                    "{}.base_form_class does not extend ShatailAdminPageForm".format(
                        cls.__name__
                    ),
                    hint="Ensure that {}.{} extends ShatailAdminPageForm".format(
...
def get_form_class_check(app_configs, **kwargs):
    ...
    for cls in get_page_models():
        ...
        if not issubclass(edit_handler.get_form_class(), ShatailAdminPageForm):
            errors.append(
                Error(
                    "{cls}.get_edit_handler().get_form_class() does not extend ShatailAdminPageForm".format(
                        cls=cls.__name__
                    ),
                    hint="Ensure that the panel definition for {cls} creates a subclass of ShatailAdminPageForm".format(
                        cls=cls.__name__
                    ),
                    obj=cls,
...
def check_panels_in_model(cls, context="model"):
    ...
    from shatail.admin.panels import InlinePanel, PanelGroup
...
{} tab for the {} to render in.""".format(
        ...
        error = Warning(error_title, hint=error_hint, obj=cls, id="shatailadmin.W002")

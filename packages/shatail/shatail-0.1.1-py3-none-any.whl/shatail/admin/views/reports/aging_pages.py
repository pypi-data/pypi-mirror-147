...
from shatail.admin.filters import ContentTypeFilter, ShatailFilterSet
from shatail.admin.widgets import AdminDateInput
from shatail.coreutils import get_content_type_label
from shatail.models import Page, PageLogEntry, UserPagePermissionsProxy, get_page_models
...
class AgingPagesReportFilterSet(ShatailFilterSet):
...
class AgingPagesView(PageReportView):
    template_name = "shatailadmin/reports/aging_pages.html"
    title = _("Aging pages")
    ...
    def get_queryset(self):
        latest_publishing_log = PageLogEntry.objects.filter(
            page=OuterRef("pk"), action__exact="shatail.publish"
        )
        self.queryset = (

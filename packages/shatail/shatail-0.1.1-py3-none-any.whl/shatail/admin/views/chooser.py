...
from shatail.admin.forms.choosers import (
...
from shatail.models import Locale, Page, Site, UserPagePermissionsProxy
...
def browse(request, parent_page_id=None):
    ...
    # (i.e. descendants of shatailcore.page)
    page_type_string = request.GET.get("page_type") or "shatailcore.page"
    ...
    locale_options = []
    show_locale_labels = getattr(settings, "SHATAIL_I18N_ENABLED", False)
    if show_locale_labels:
        ...
        if parent_page.is_root():
            ...
            choose_url = reverse("shatailadmin_choose_page")
            locale_options = [
                {
                    "locale": locale,
                    "url": choose_url
            ...
            for locale in Locale.objects.filter(
                ...
                choose_child_url = reverse(
                    "shatailadmin_choose_page_child",
    ...
    context = shared_context(
        request,
        {
            ...
            "page_types_restricted": (page_type_string != "shatailcore.page"),
    ...
    return render_modal_workflow(
        request,
        "shatailadmin/chooser/browse.html",
        None,
        context,
...
def search(request, parent_page_id=None):
    # A missing or empty page_type parameter indicates 'all page types' (i.e. descendants of shatailcore.page)
    page_type_string = request.GET.get("page_type") or "shatailcore.page"
    ...
    return TemplateResponse(
        request,
        "shatailadmin/chooser/_search_results.html",
        shared_context(
            request,
            {
...
def external_link(request):
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            link_conversion = getattr(
                settings, "SHATAILADMIN_EXTERNAL_LINK_CONVERSION", LINK_CONVERSION_ALL
            ).lower()
            ...
            sites = getattr(request, "_shatail_cached_site_root_paths", None)
            if sites is None:
            ...
            for pk, url in possible_sites:
                try:
                    ...
                    internal_data = {
                        ...
                        "editUrl": reverse(
                            "shatailadmin_pages:edit", args=(matched_page.pk,)
                        ),
                    ...
                    # Let's confirm the conversion with them explicitly
                        ...
                        return render_modal_workflow(
                            request,
                            "shatailadmin/chooser/confirm_external_to_internal.html",
                            None,
                            {
    ...
    return render_modal_workflow(
        request,
        "shatailadmin/chooser/external_link.html",
        None,
        shared_context(
            request,
            {
...
def anchor_link(request):
    ...
    return render_modal_workflow(
        request,
        "shatailadmin/chooser/anchor_link.html",
        None,
        shared_context(
            request,
            {
...
def email_link(request):
    ...
    return render_modal_workflow(
        request,
        "shatailadmin/chooser/email_link.html",
        None,
        shared_context(
            request,
            {
...
def phone_link(request):
    ...
    return render_modal_workflow(
        request,
        "shatailadmin/chooser/phone_link.html",
        None,
        shared_context(
            request,
            {

...
from shatail.admin.site_summary import SiteSummaryPanel
...
class UpgradeNotificationPanel(Component):
    ...
    template_name = "shatailadmin/home/upgrade_notification.html"
    order = 100

    def render_html(self, parent_context):
        if parent_context["request"].user.is_superuser and getattr(
            settings, "SHATAIL_ENABLE_UPDATE_CHECK", True
        ):
...
class PagesForModerationPanel(Component):
    ...
    template_name = "shatailadmin/home/pages_for_moderation.html"
    order = 200
...
class UserPagesInWorkflowModerationPanel(Component):
    ...
    template_name = "shatailadmin/home/user_pages_in_workflow_moderation.html"
    order = 210
...
class WorkflowPagesToModeratePanel(Component):
    ...
    template_name = "shatailadmin/home/workflow_pages_to_moderate.html"
    order = 220
...
class LockedPagesPanel(Component):
    name = "locked_pages"
    template_name = "shatailadmin/home/locked_pages.html"
    order = 300
...
class RecentEditsPanel(Component):
    name = "recent_edits"
    template_name = "shatailadmin/home/recent_edits.html"
    order = 250
    ...
    def get_context_data(self, parent_context):
        ...
        # Last n edited pages
        edit_count = getattr(settings, "SHATAILADMIN_RECENT_EDITS_LIMIT", 5)
        if connection.vendor == "mysql":
            ...
            last_edits = PageRevision.objects.raw(
                """
                SELECT wp.* FROM
                    shatailcore_pagerevision wp JOIN (
                        SELECT max(created_at) AS max_created_at, page_id FROM
                            shatailcore_pagerevision WHERE user_id = %s GROUP BY page_id ORDER BY max_created_at DESC LIMIT %s
...
@permission_required("shatailadmin.access_admin", login_url="shatailadmin_login")
def default(request):
    """
    Called whenever a request comes in with the correct prefix (eg /admin/) but
    doesn't actually correspond to a Shatail view.
...
def icons():
    ...
    if _icons_html is None:
        ...
        _full_sprite_html = render_to_string(
            "shatailadmin/shared/icons.html", {"icons": combined_icon_markup}
        )

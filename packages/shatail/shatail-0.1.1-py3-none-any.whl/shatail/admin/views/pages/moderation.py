...
from shatail.admin.mail import send_moderation_notification
from shatail.models import PageRevision
...
def approve_moderation(request, revision_id):
    ...
    if request.method == "POST":
        ...
        buttons.append(
            messages.button(
                reverse("shatailadmin_pages:edit", args=(revision.page.id,)), _("Edit")
            )
        )
...
def reject_moderation(request, revision_id):
    ...
    if request.method == "POST":
        ...
        messages.success(
            ...
            buttons=[
                messages.button(
                    reverse("shatailadmin_pages:edit", args=(revision.page.id,)),
                    _("Edit"),
                )
            ],
        )

...
from shatail.models import Page, UserPagePermissionsProxy


def revisions_index(request, page_id):
    return redirect("shatailadmin_pages:history", page_id)
...
def revisions_revert(request, page_id, revision_id):
    ...
    user_avatar = render_to_string(
        "shatailadmin/shared/user_avatar.html", {"user": revision.user}
    )
...
def revisions_compare(request, page_id, revision_id_a, revision_id_b):
    ...
    return TemplateResponse(
        request,
        "shatailadmin/pages/revisions/compare.html",
        {
            "page": page,
...
def revisions_unschedule(request, page_id, revision_id):
    ...
    if request.method == "POST":
        ...
        messages.success(
            ...
            buttons=[
                messages.button(
                    reverse("shatailadmin_pages:edit", args=(page.id,)), _("Edit")
                )
            ],
        )
        ...
        return redirect("shatailadmin_pages:history", page.id)

    return TemplateResponse(
        request,
        "shatailadmin/pages/revisions/confirm_unschedule.html",

...
from shatail.compat import AUTH_USER_APP_LABEL, AUTH_USER_MODEL_NAME
...
from shatail.users.forms import UserCreationForm, UserEditForm
from shatail.users.models import UserProfile
from shatail.users.views.groups import GroupViewSet
from shatail.users.views.users import get_user_creation_form, get_user_edit_form
from shatail.users.shatail_hooks import get_group_viewset_cls
...
class TestGroupUsersView(TestCase, ShatailTestUtils):
    ...
    def get(self, params={}, group_id=None):
        return self.client.get(
            reverse(
                "shatailusers_groups:users", args=(group_id or self.test_group.pk,)
            ),
            params,
        )
    ...
    def test_simple(self):
        ...
        self.assertTemplateUsed(response, "shatailusers/users/index.html")
...
class TestUserIndexView(TestCase, ShatailTestUtils):
    def setUp(self):
        # create a user that should be visible in the listing
        self.test_user = self.create_user(
            username="testuser",
    ...
    def get(self, params={}):
        return self.client.get(reverse("shatailusers_users:index"), params)
    ...
    def test_allows_negative_ids(self):
        # see https://github.com/shatail/shatail/issues/565
...
class TestUserCreateView(TestCase, ShatailTestUtils):
    def setUp(self):
        self.login()

    def get(self, params={}):
        return self.client.get(reverse("shatailusers_users:add"), params)

    def post(self, post_data={}):
        return self.client.post(reverse("shatailusers_users:add"), post_data)

    def test_simple(self):
        response = self.get()
        self.assertEqual(response.status_code, 200)
        self.assertTemplateUsed(response, "shatailusers/users/create.html")
    ...
    def test_create(self):
        ...
        self.assertRedirects(response, reverse("shatailusers_users:index"))
    ...
    @override_settings(
        SHATAIL_USER_CREATION_FORM="shatail.users.tests.CustomUserCreationForm",
        SHATAIL_USER_CUSTOM_FIELDS=["country", "document"],
    )
    ...
    @override_settings(SHATAILUSERS_PASSWORD_REQUIRED=False)
    def test_password_fields_exist_when_not_required(self):
        """Password fields should still be shown if SHATAILUSERS_PASSWORD_REQUIRED is False"""
        response = self.get()
        self.assertEqual(response.status_code, 200)
        self.assertTemplateUsed(response, "shatailusers/users/create.html")
        self.assertContains(response, "Password:")
    ...
    def test_create_with_password_not_required(self):
        """Password should not be required if SHATAILUSERS_PASSWORD_REQUIRED is False"""
        response = self.post(
            {
                "username": "testuser",
                "email": "test@user.com",
    ...
    def test_optional_password_is_still_validated(self):
        """When SHATAILUSERS_PASSWORD_REQUIRED is False, password validation should still apply if a password _is_ supplied"""
        response = self.post(
            {
                "username": "testuser",
                "email": "test@user.com",
    ...
    def test_password_still_accepted_when_optional(self):
        """When SHATAILUSERS_PASSWORD_REQUIRED is False, we should still allow a password to be set"""
        response = self.post(
            {
                "username": "testuser",
                "email": "test@user.com",
    ...
    @override_settings(SHATAILUSERS_PASSWORD_ENABLED=False)
    def test_password_fields_not_shown_when_disabled(self):
        """SHATAILUSERS_PASSWORD_ENABLED=False should cause password fields to be removed"""
        response = self.get()
        self.assertEqual(response.status_code, 200)
        self.assertTemplateUsed(response, "shatailusers/users/create.html")
    ...
    def test_password_fields_ignored_when_disabled(self):
        """When SHATAILUSERS_PASSWORD_ENABLED is False, users should always be created without a usable password"""
        response = self.post(
            {
                "username": "testuser",
                "email": "test@user.com",
    ...
    def test_before_create_user_hook(self):
        ...
        with self.register_hook("before_create_user", hook_func):
            response = self.client.get(reverse("shatailusers_users:add"))
        self.assertEqual(response.status_code, 200)
    ...
    def test_before_create_user_hook_post(self):
        ...
        with self.register_hook("before_create_user", hook_func):
            ...
            response = self.client.post(reverse("shatailusers_users:add"), post_data)
        self.assertEqual(response.status_code, 200)
        self.assertEqual(response.content, b"Overridden!")
...
class TestUserDeleteView(TestCase, ShatailTestUtils):
    def setUp(self):
        # create a user that should be visible in the listing
        self.test_user = self.create_user(
    ...
    def get(self, params={}):
        return self.client.get(
            reverse("shatailusers_users:delete", args=(self.test_user.pk,)), params
        )

    def post(self, post_data={}):
        return self.client.post(
            reverse("shatailusers_users:delete", args=(self.test_user.pk,)), post_data
        )

    def test_simple(self):
        response = self.get()
        self.assertEqual(response.status_code, 200)
        self.assertTemplateUsed(response, "shatailusers/users/confirm_delete.html")
    ...
    def test_user_cannot_delete_self(self):
        response = self.client.get(
            reverse("shatailusers_users:delete", args=(self.current_user.pk,))
        )
    ...
    def test_user_can_delete_other_superuser(self):
        response = self.client.get(
            reverse("shatailusers_users:delete", args=(self.superuser.pk,))
        )
        self.assertEqual(response.status_code, 200)
        self.assertTemplateUsed(response, "shatailusers/users/confirm_delete.html")
    ...
    def test_before_delete_user_hook(self):
        ...
        with self.register_hook("before_delete_user", hook_func):
            response = self.client.get(
                reverse("shatailusers_users:delete", args=(self.test_user.pk,))
            )
...
class TestUserDeleteViewForNonSuperuser(TestCase, ShatailTestUtils):
    def setUp(self):
        # create a user that should be visible in the listing
        self.test_user = self.create_user(
            username="testuser", email="testuser@email.com", password="password"
    ...
    def test_user_cannot_delete_self(self):
        response = self.client.post(
            reverse("shatailusers_users:delete", args=(self.deleter_user.pk,))
        )
...
class TestUserEditView(TestCase, ShatailTestUtils):
    def setUp(self):
    ...
    def get(self, params={}, user_id=None):
        return self.client.get(
            reverse("shatailusers_users:edit", args=(user_id or self.test_user.pk,)),
            params,
        )
    ...
    def test_simple(self):
        response = self.get()
        self.assertEqual(response.status_code, 200)
        self.assertTemplateUsed(response, "shatailusers/users/edit.html")
        self.assertContains(response, "Password:")
        self.assertContains(response, "Password confirmation:")
    ...
    def test_editing_own_password_does_not_log_out(self):
        ...
        response = self.client.get(reverse("shatailusers_users:index"))
        self.assertEqual(response.status_code, 200)
    ...
    @override_settings(
        SHATAIL_USER_EDIT_FORM="shatail.users.tests.CustomUserEditForm",
    )
    ...
    @override_settings(SHATAILUSERS_PASSWORD_ENABLED=False)
    def test_password_fields_ignored_when_disabled(self):
        """When SHATAILUSERS_PASSWORD_REQUIRED is False, existing password should be left unchanged"""
        response = self.post(
            {
                "username": "testuser",
                "email": "test@user.com",
    ...
    def test_before_edit_user_hook(self):
        ...
        with self.register_hook("before_edit_user", hook_func):
            response = self.client.get(
                reverse("shatailusers_users:edit", args=(self.test_user.pk,))
            )
    ...
    def test_before_edit_user_hook_post(self):
        ...
        with self.register_hook("before_edit_user", hook_func):
            ...
            response = self.client.post(
                reverse("shatailusers_users:edit", args=(self.test_user.pk,)), post_data
            )
...
class TestUserProfileCreation(TestCase, ShatailTestUtils):
    def setUp(self):
        # Create a user
        self.test_user = self.create_user(
            username="testuser",
    ...
    def test_user_created_without_profile(self):
        ...
        with self.assertRaises(UserProfile.DoesNotExist):
            self.test_user.shatail_userprofile
...
class TestUserEditViewForNonSuperuser(TestCase, ShatailTestUtils):
    def setUp(self):
    ...
    def test_user_cannot_escalate_privileges(self):
        ...
        response = self.client.post(
            reverse("shatailusers_users:edit", args=(self.editor_user.pk,)), post_data
        )
        # Should redirect back to index
        self.assertRedirects(response, reverse("shatailusers_users:index"))
...
class TestGroupIndexView(TestCase, ShatailTestUtils):
    def setUp(self):
        self.login()

    def get(self, params={}):
        return self.client.get(reverse("shatailusers_groups:index"), params)

    def test_simple(self):
        response = self.get()
        self.assertEqual(response.status_code, 200)
        self.assertTemplateUsed(response, "shatailusers/groups/index.html")
...
class TestGroupCreateView(TestCase, ShatailTestUtils):
    def setUp(self):
        self.login()
        self.add_doc_permission = Permission.objects.get(
            content_type__app_label="shataildocs", codename="add_document"
        )
        self.change_doc_permission = Permission.objects.get(
            content_type__app_label="shataildocs", codename="change_document"
        )

    def get(self, params={}):
        return self.client.get(reverse("shatailusers_groups:add"), params)
    ...
    def post(self, post_data={}):
        ...
        return self.client.post(reverse("shatailusers_groups:add"), post_data)

    def test_simple(self):
        response = self.get()
        self.assertEqual(response.status_code, 200)
        self.assertTemplateUsed(response, "shatailusers/groups/create.html")

    def test_create_group(self):
        response = self.post({"name": "test group"})

        # Should redirect back to index
        self.assertRedirects(response, reverse("shatailusers_groups:index"))
...
class TestGroupEditView(TestCase, ShatailTestUtils):
    def setUp(self):
    ...
    def get(self, params={}, group_id=None):
        return self.client.get(
            reverse("shatailusers_groups:edit", args=(group_id or self.test_group.pk,)),
            params,
        )
    ...
    def add_non_registered_perm(self):
        ...
        # hook-registered as part of the shatail interface. We need to ensure
    ...
    def test_simple(self):
        response = self.get()
        self.assertEqual(response.status_code, 200)
        self.assertTemplateUsed(response, "shatailusers/groups/edit.html")
    ...
    def test_group_edit_adding_document_permissions_same_collection(self):
        ...
        self.assertEqual(
            self.test_group.collection_permissions.filter(
                permission__content_type__app_label="shataildocs"
            ).count(),
            1,
        )
        response = self.post(
...
class TestGroupViewSet(TestCase):
    def setUp(self):
        self.app_config = apps.get_app_config("shatailusers")
    ...
    def test_get_group_viewset_cls_with_custom_form(self):
        with unittest.mock.patch.object(
            self.app_config,
            "group_viewset",
            new="shatail.users.tests.CustomGroupViewSet",
        ):
    ...
    def test_get_group_viewset_cls_custom_form_does_not_exist(self):
        with unittest.mock.patch.object(
            self.app_config,
            "group_viewset",
            new="shatail.users.tests.CustomClassDoesNotExist",
        ):
            with self.assertRaises(ImproperlyConfigured) as exc_info:
                get_group_viewset_cls(self.app_config)
            self.assertIn(
                'Module "shatail.users.tests" does not define a "CustomClassDoesNotExist" attribute/class',

...
from shatail.users.urls import users
from shatail.users.utils import user_can_delete_user
from shatail.users.views.bulk_actions import (
    AssignRoleBulkAction,
    DeleteBulkAction,
    SetActiveStateBulkAction,
)
from shatail.users.widgets import UserListingButton


@hooks.register("register_admin_urls")
def register_admin_urls():
    return [
        path("users/", include(users, namespace="shatailusers_users")),
    ]
...
def register_viewset():
    app_config = apps.get_app_config("shatailusers")
    group_viewset_cls = get_group_viewset_cls(app_config)
    return group_viewset_cls("shatailusers_groups", url_prefix="groups")
...
def register_users_menu_item():
    return UsersMenuItem(
        _("Users"), reverse("shatailusers_users:index"), icon_name="user", order=600
    )
...
def register_groups_menu_item():
    return GroupsMenuItem(
        _("Groups"), reverse("shatailusers_groups:index"), icon_name="group", order=601
    )
...
def register_users_search_area():
    return UsersSearchArea(
        _("Users"),
        reverse("shatailusers_users:index"),
        name="users",
        icon_name="user",
        order=600,
    )
...
def user_listing_buttons(context, user):
    yield UserListingButton(
        _("Edit"),
        reverse("shatailusers_users:edit", args=[user.pk]),
    ...
    if user_can_delete_user(context.request.user, user):
        yield UserListingButton(
            _("Delete"),
            reverse("shatailusers_users:delete", args=[user.pk]),
            classes={"no"},
...
class UserAdminURLFinder(ModelAdminURLFinder):
    edit_url_name = "shatailusers_users:edit"

...
def get_site_for_hostname(hostname, port):
    """Return the shatailcore.Site object for the given hostname and port."""
...
class Site(models.Model):
    ...
    def find_for_request(request):
        ...
        * if there is no matching hostname at all, or no matching
        ...
        The site will be cached via request._shatail_site
        """

        if request is None:
            return None

        if not hasattr(request, "_shatail_site"):
            site = Site._find_for_request(request)
            setattr(request, "_shatail_site", site)
        return request._shatail_site
    ...
    def get_site_root_paths():
        ...
        """
        result = cache.get("shatail_site_root_paths")

        # Shatail 2.11 changed the way site root paths were stored. This can cause an upgraded 2.11
        # site to break when loading cached site root paths that were cached with 2.10.2 or older
        # versions of Shatail. The line below checks if the any of the cached site urls is consistent
        # with an older version of Shatail and invalidates the cache.
        if result is None or any(len(site_record) == 3 for site_record in result):
            ...
            cache.set("shatail_site_root_paths", result, 3600)

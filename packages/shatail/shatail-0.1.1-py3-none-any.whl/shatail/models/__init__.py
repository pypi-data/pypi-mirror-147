"""
shatail.models is split into submodules for maintainability. All definitions intended as
public should be imported here (with 'noqa' comments as required) and outside code should continue
to import them from shatail.models (e.g. `from shatail.models import Site`, not
`from shatail.models.sites import Site`.)
...
from shatail.actions.copy_for_translation import CopyPageForTranslationAction
from shatail.actions.copy_page import CopyPageAction
from shatail.actions.create_alias import CreatePageAliasAction
from shatail.actions.delete_page import DeletePageAction
from shatail.actions.move_page import MovePageAction
from shatail.actions.publish_page_revision import PublishPageRevisionAction
from shatail.actions.unpublish_page import UnpublishPageAction
from shatail.coreutils import (
    SHATAIL_APPEND_SLASH,
    camelcase_to_underscore,
    get_supported_content_language_variant,
    resolve_model_string,
)
from shatail.fields import StreamField
from shatail.forms import TaskStateCommentForm
from shatail.log_actions import log
from shatail.query import PageQuerySet
from shatail.search import index
from shatail.signals import (
    page_published,
    page_slug_changed,
    pre_validate_delete,
    task_approved,
...
from shatail.treebeard import TreebeardPathFixMixin
from shatail.url_routing import RouteResult
...
logger = logging.getLogger("shatail")

PAGE_TEMPLATE_VAR = "page"
COMMENTS_RELATION_NAME = getattr(
    settings, "SHATAIL_COMMENTS_RELATION_NAME", "shatail_admin_comments"
)
...
class Page(AbstractPage, index.Indexed, ClusterableModel, metaclass=PageBase):
    ...
    # Do not allow plain Page instances to be created through the Shatail admin
    is_creatable = False
    ...
    # The canonical definition is in shatailadmin.panels.
    content_panels = []
    promote_panels = []
    settings_panels = []
    ...
    def full_clean(self, *args, **kwargs):
        ...
        if not self.slug:
            ...
            allow_unicode = getattr(settings, "SHATAIL_ALLOW_UNICODE_SLUGS", True)
    ...
    def save(self, clean=True, user=None, log_action=False, **kwargs):
        ...
        # Check if this is a root page of any sites and clear the 'shatail_site_root_paths' key if so
        ...
        if self.is_site_root():
            cache.delete("shatail_site_root_paths")
                ...
                if field.remote_field.on_delete == models.CASCADE:
                    errors.append(
                        checks.Warning(
                            ...
                            hint="Set on_delete=models.SET_NULL and make sure the field is nullable or set on_delete=models.PROTECT. Shatail does not allow simple database CASCADE because it will corrupt its tree storage.",
                            obj=field,
                            id="shatailcore.W001",
                        )
                    )
        ...
        if not isinstance(cls.objects, PageManager):
            errors.append(
                checks.Error(
                    ...
                    hint="Ensure that custom Page managers inherit from shatail.models.PageManager",
                    obj=cls,
                    id="shatailcore.E002",
                )
            )
    ...
    def get_specific(self, deferred=False, copy_attrs=None, copy_attrs_exclude=None):
        ...
        * Attributes set elsewhere in Python code.
        ...
        best course of action, but there is currently no safe way for Shatail
    ...
    def save_revision(
        ...
        :param log_action: flag for logging the action. Pass False to skip logging. Can be passed an action string.
            Defaults to 'shatail.edit' when no 'previous_revision' param is passed, otherwise 'shatail.revert'
    ...
    def update_aliases(
        ...
        This is called by Shatail whenever a page with aliases is published.
        ...
        # created an alias loop (which is impossible to do with the UI Shatail provides)
        ...
        for alias in self.specific_class.objects.filter(alias_of=self).exclude(
            ...
            # For example, In Shatail Localize, we use aliases to create mirrored trees, but those
    ...
    def _get_site_root_paths(self, request=None):
        ...
        try:
            return cache_object._shatail_cached_site_root_paths
        except AttributeError:
            cache_object._shatail_cached_site_root_paths = Site.get_site_root_paths()
            return cache_object._shatail_cached_site_root_paths
    ...
    def get_url_parts(self, request=None):
        ...
        use_shatail_i18n = getattr(settings, "SHATAIL_I18N_ENABLED", False)

        if use_shatail_i18n:
            # If the active language code is a variant of the page's language, then
            # use that instead
            # This is used when LANGUAGES contain more languages than SHATAIL_CONTENT_LANGUAGES
            try:
                if (
        ...
        # The page may not be routable because shatail_serve is not registered
        # This may be the case if Shatail is used headless
        try:
            if use_shatail_i18n:
                with translation.override(language_code):
                    page_path = reverse(
                        "shatail_serve", args=(self.url_path[len(root_path) :],)
                    )
            else:
                page_path = reverse(
                    "shatail_serve", args=(self.url_path[len(root_path) :],)
        ...
        # SHATAIL_APPEND_SLASH is False and we're not trying to serve
        # the root path
        if not SHATAIL_APPEND_SLASH and page_path != "/":
    ...
    def get_url(self, request=None, current_site=None):
        ...
        within the Shatail backend and actual website templates;
    ...
    def get_indexed_instance(self):
        # This is accessed on save by the shatailsearch signal handler, and in edge
    ...
    def create_alias(
        ...
        *,
        ...
        update_locale=None,
        user=None,
        log_action="shatail.create_alias",
        reset_translation_key=True,
        _mpnode_attrs=None,
    ):
    ...
    def serve_preview(self, request, mode_name):
        ...
        Shatail will allow the user to specify one of those modes when previewing, and
        ...
        the shatail user bar to be displayed. This request will always be a GET.
        """
    ...
    password_required_template = getattr(
        settings, "PASSWORD_REQUIRED_TEMPLATE", "shatailcore/password_required.html"
    )
    ...
    def with_content_json(self, content):
        ...
        * ``first_published_at``
        * ``alias_of``
        * ``shatail_admin_comments`` (COMMENTS_RELATION_NAME)
        """

        # Old revisions (pre Shatail 2.15) may have saved comment data under the name 'comments'
    ...
    def has_workflow(self):
        ...
        if not getattr(settings, "SHATAIL_WORKFLOW_ENABLED", True):
            return False
        return (
...
class PageRevision(models.Model):
    ...
    def save(self, user=None, *args, **kwargs):
        ...
        if (
            ...
            log(
                instance=page,
                action="shatail.schedule.cancel",
                data={
                    "revision": {
                        "id": self.id,
    ...
    def approve_moderation(self, user=None):
        if self.submitted_for_moderation:
            ...
            log(
                ...
                action="shatail.moderation.approve",
                user=user,
                revision=self,
            )
            self.publish()
    ...
    def reject_moderation(self, user=None):
        if self.submitted_for_moderation:
            ...
            log(
                instance=self.as_page_object(),
                action="shatail.moderation.reject",
                user=user,
                revision=self,
            )
...
class PagePermissionTester:
    ...
    def page_locked(self):
        ...
        if getattr(settings, "SHATAILADMIN_GLOBAL_PAGE_EDIT_LOCK", False):
...
class PageViewRestriction(BaseViewRestriction):
    ...
    def save(self, user=None, **kwargs):
        ...
        if specific_instance:
            log(
                ...
                action="shatail.view_restriction.create"
                if is_new
                else "shatail.view_restriction.edit",
                user=user,
                data={
                    "restriction": {
    ...
    def delete(self, user=None, **kwargs):
        ...
        if specific_instance:
            log(
                instance=specific_instance,
                action="shatail.view_restriction.delete",
                user=user,
                data={
                    "restriction": {
                        "type": self.restriction_type,
...
class GroupApprovalTask(Task):
    ...
    @classmethod
    def get_description(cls):
        return _("Members of the chosen Shatail Groups can approve this task")
...
class WorkflowState(models.Model):
    ...
    on_finish = import_string(
        getattr(
            settings,
            "SHATAIL_FINISH_WORKFLOW_ACTION",
            "shatail.workflows.publish_workflow_state",
        )
    )
    ...
    def resume(self, user=None):
        ...
        log(
            ...
            action="shatail.workflow.resume",
            data={
                "workflow": {
                    "id": self.workflow_id,
    ...
    def successful_task_states(self):
        ...
        if getattr(settings, "SHATAIL_WORKFLOW_REQUIRE_REAPPROVAL_ON_EDIT", False):
    ...
    def cancel(self, user=None):
        ...
        log(
            instance=self.page.specific,
            action="shatail.workflow.cancel",
            data={
                "workflow": {
                    "id": self.workflow_id,
                    "title": self.workflow.name,
    ...
    def _get_applicable_task_states(self):
        ...
        # If SHATAIL_WORKFLOW_REQUIRE_REAPPROVAL_ON_EDIT=True, this is only task states created on the current revision
        if getattr(settings, "SHATAIL_WORKFLOW_REQUIRE_REAPPROVAL_ON_EDIT", False):
            latest_revision_id = (
                self.revisions()
                .order_by("-created_at", "-id")
...
class TaskState(models.Model):
    ...
    content_type = models.ForeignKey(
        ContentType,
        verbose_name=_("content type"),
        related_name="shatail_task_states",
        on_delete=models.CASCADE,
    )
    exclude_fields_in_copy = []
    ...
    def log_state_change_action(self, user, action):
        ...
        log(
            instance=page,
            action="shatail.workflow.{}".format(action),
            user=user,
            data={
                "workflow": {
...
class Comment(ClusterableModel):
    ...
    def log_create(self, **kwargs):
        self._log("shatail.comments.create", **kwargs)

    def log_edit(self, **kwargs):
        self._log("shatail.comments.edit", **kwargs)

    def log_resolve(self, **kwargs):
        self._log("shatail.comments.resolve", **kwargs)

    def log_delete(self, **kwargs):
        self._log("shatail.comments.delete", **kwargs)
...
class CommentReply(models.Model):
    ...
    def log_create(self, **kwargs):
        self._log("shatail.comments.create_reply", **kwargs)

    def log_edit(self, **kwargs):
        self._log("shatail.comments.edit_reply", **kwargs)

    def log_delete(self, **kwargs):
        self._log("shatail.comments.delete_reply", **kwargs)

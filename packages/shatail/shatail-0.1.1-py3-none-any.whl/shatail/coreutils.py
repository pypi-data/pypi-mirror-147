...
SHATAIL_APPEND_SLASH = getattr(settings, "SHATAIL_APPEND_SLASH", True)
...
def get_content_type_label(content_type):
    ...
    in place of the default 'shatailcore | page' representation
    """
...
def get_content_languages():
    """
    Cache of settings.SHATAIL_CONTENT_LANGUAGES in a dictionary for easy lookups by key.
    """
    content_languages = getattr(settings, "SHATAIL_CONTENT_LANGUAGES", None)
    ...
    for language_code, name in content_languages:
        if language_code not in languages:
            raise ImproperlyConfigured(
                "The language {} is specified in SHATAIL_CONTENT_LANGUAGES but not LANGUAGES. "
                "SHATAIL_CONTENT_LANGUAGES must be a subset of LANGUAGES.".format(
                    language_code
                )
            )
...
def get_supported_content_language_variant(lang_code, strict=False):
    ...
    but reads the `SHATAIL_CONTENT_LANGUAGES` setting instead.
    """
    if lang_code:
...
def get_locales_display_names() -> dict:
    ...
    """
    from shatail.models import Locale  # inlined to avoid circular imports
...
def reset_cache(**kwargs):
    """
    Clear cache when global SHATAIL_CONTENT_LANGUAGES/LANGUAGES/LANGUAGE_CODE settings are changed
    """
    if kwargs["setting"] in ("SHATAIL_CONTENT_LANGUAGES", "LANGUAGES", "LANGUAGE_CODE"):

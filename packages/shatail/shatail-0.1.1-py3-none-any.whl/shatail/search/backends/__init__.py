...
def get_search_backend_config():
    search_backends = getattr(settings, "SHATAILSEARCH_BACKENDS", {})
...
def import_backend(dotted_path):
    ...
    eg:
      old: shatail.search.backends.elasticsearch.ElasticsearchSearchBackend
      new: shatail.search.backends.elasticsearch
...
def get_search_backend(backend="default", **kwargs):
    ...
    try:
        # Try to get the SHATAILSEARCH_BACKENDS entry for the given backend name first
...
def _backend_requires_auto_update(backend_name, params):
    ...
    # _SHATAILSEARCH_FORCE_AUTO_UPDATE is only used by Shatail tests. It allows
    # us to test AUTO_UPDATE behaviour against Elasticsearch without having to
    # have AUTO_UPDATE enabed for every test.
    force_auto_update = getattr(settings, "_SHATAILSEARCH_FORCE_AUTO_UPDATE", [])

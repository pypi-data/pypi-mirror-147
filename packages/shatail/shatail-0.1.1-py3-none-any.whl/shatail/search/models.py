...
from shatail.search.utils import MAX_QUERY_STRING_LENGTH, normalise_query_string
...
class QueryDailyHits(models.Model):
    ...
    def garbage_collect(cls, days=None):
        ...
        """
        days = (
            getattr(settings, "SHATAILSEARCH_HITS_MAX_AGE", 7) if days is None else days
        )
    ...
    if fts5_available():
        ...
        class SQLiteFTSIndexEntry(models.Model):
            ...
            index_entry = OneToOneField(
                primary_key=True,
                to="shatailsearch.indexentry",
                on_delete=models.CASCADE,
                db_column="rowid",
            )

            class Meta:
                db_table = "shatailsearch_indexentry_fts"

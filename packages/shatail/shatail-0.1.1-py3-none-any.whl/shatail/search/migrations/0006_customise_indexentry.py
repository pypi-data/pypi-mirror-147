...
class Migration(migrations.Migration):
    ...
    dependencies = [
        ("shatailsearch", "0005_create_indexentry"),
    ]
    ...
    if connection.vendor == "postgresql":
        ...
        operations = [
            ...
            migrations.AddIndex(
                ...
                index=django.contrib.postgres.indexes.GinIndex(
                    fields=["autocomplete"], name="shatailsear_autocom_476c89_gin"
                ),
            ),
            migrations.AddIndex(
                model_name="indexentry",
                index=django.contrib.postgres.indexes.GinIndex(
                    fields=["title"], name="shatailsear_title_9caae0_gin"
                ),
            ),
            migrations.AddIndex(
                model_name="indexentry",
                index=django.contrib.postgres.indexes.GinIndex(
                    fields=["body"], name="shatailsear_body_90c85d_gin"
                ),
            ),
        ]
            ...
            operations.append(
                migrations.SeparateDatabaseAndState(
                    ...
                    database_operations=[
                        ...
                        migrations.RunSQL(
                            sql=(
                                "CREATE TRIGGER insert_shatailsearch_indexentry_fts AFTER INSERT ON %s BEGIN INSERT INTO %s_fts(title, body, autocomplete, rowid) VALUES (NEW.title, NEW.body, NEW.autocomplete, NEW.id); END"
                                % (IndexEntry._meta.db_table, IndexEntry._meta.db_table)
                            ),
                            reverse_sql=(
                                "DROP TRIGGER IF EXISTS insert_shatailsearch_indexentry_fts"
                            ),
                        ),
                        migrations.RunSQL(
                            sql=(
                                "CREATE TRIGGER update_shatailsearch_indexentry_fts AFTER UPDATE ON %s BEGIN UPDATE %s_fts SET title=NEW.title, body=NEW.body, autocomplete=NEW.autocomplete WHERE rowid=NEW.id; END"
                                % (IndexEntry._meta.db_table, IndexEntry._meta.db_table)
                            ),
                            reverse_sql=(
                                "DROP TRIGGER IF EXISTS update_shatailsearch_indexentry_fts"
                            ),
                        ),
                        migrations.RunSQL(
                            sql=(
                                "CREATE TRIGGER delete_shatailsearch_indexentry_fts AFTER DELETE ON %s BEGIN DELETE FROM %s_fts WHERE rowid=OLD.id; END"
                                % (IndexEntry._meta.db_table, IndexEntry._meta.db_table)
                            ),
                            reverse_sql=(
                                "DROP TRIGGER IF EXISTS delete_shatailsearch_indexentry_fts"
                            ),
                        ),
                    ],
                )
        ...
        operations.append(
            migrations.RunSQL(
                sql="""
                ALTER TABLE shatailsearch_indexentry

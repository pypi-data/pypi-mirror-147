...
from shatail.search.index import get_indexed_models
...
def group_models_by_index(backend, models):
    ...
    ... ])
    {
        <Index shatailcore_page>: [shatailcore.Page, myapp.HomePage, myapp.StandardPage],
        <Index shatailimages_image>: [shatailimages.Image],
    }
    """
    indices = {}
...
class Command(BaseCommand):
    ...
    def handle(self, **options):
        ...
        elif hasattr(settings, "SHATAILSEARCH_BACKENDS"):
            # index all backends listed in settings
            backend_names = settings.SHATAILSEARCH_BACKENDS.keys()
        else:

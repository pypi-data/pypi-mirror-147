...
from shatail.contrib.forms.forms import FormBuilder
from shatail.contrib.forms.models import (
    FORM_FIELD_CHOICES,
    AbstractEmailForm,
    AbstractFormField,
...
from shatail.contrib.settings.models import BaseSetting, register_setting
from shatail.contrib.sitemaps import Sitemap
from shatail.contrib.table_block.blocks import TableBlock
from shatail.documents import get_document_model
from shatail.documents.models import AbstractDocument, Document
from shatail.fields import RichTextField, StreamField
from shatail.images import get_image_model
from shatail.images.blocks import ImageChooserBlock
from shatail.images.models import AbstractImage, AbstractRendition, Image
...
from shatail.snippets.models import register_snippet
...
class PageWithOldStyleRouteMethod(Page):
    """
    Prior to Shatail 0.4, the route() method on Page returned an HttpResponse
...
# Override the standard ShatailAdminPageForm to add validation on start/end dates
# that appears as a non-field error


class EventPageForm(ShatailAdminPageForm):
    def clean(self):
...
# Override the standard ShatailAdminPageForm to add field that is not in model
...
class FormPage(AbstractEmailForm):
    ...
    # https://docs.shatail.org/en/stable/reference/contrib/forms/customisation.html#customise-form-submissions-listing-in-shatail-admin
    # works without triggering circular dependency issues -
    # see https://github.com/shatail/shatail/issues/6265
...
class AdvertPlacement(models.Model):
    page = ParentalKey(
        "shatailcore.Page", related_name="advert_placements", on_delete=models.CASCADE
    )
...
StandardChild.edit_handler = TabbedInterface(
    ...
    base_form_class=ShatailAdminPageForm,
)
...
class PageChooserModel(models.Model):
    page = models.ForeignKey(
        "shatailcore.Page", help_text="help text", on_delete=models.CASCADE
    )
...
class ImportantPages(BaseSetting):
    sign_up_page = models.ForeignKey(
        "shatailcore.Page", related_name="+", null=True, on_delete=models.SET_NULL
    )
...
class UserProfile(models.Model):
    # Shatail's schema must be able to coexist alongside a custom UserProfile model

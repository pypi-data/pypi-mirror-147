...
from shatail.admin.forms.search import SearchForm
from shatail.admin.panels import ObjectList, extract_panel_definitions_from_model_class
from shatail.admin.ui.tables import Column, DateColumn, UserColumn
from shatail.admin.views.generic import CreateView, DeleteView, EditView, IndexView
from shatail.log_actions import log
from shatail.log_actions import registry as log_registry
from shatail.models import Locale, TranslatableMixin
from shatail.search.backends import get_search_backend
from shatail.search.index import class_is_indexed
from shatail.snippets.action_menu import SnippetActionMenu
from shatail.snippets.models import get_snippet_models
from shatail.snippets.permissions import get_permission_name, user_can_edit_snippet_type
...
def index(request):
    ...
    if snippet_model_opts:
        return TemplateResponse(
            request,
            "shatailsnippets/snippets/index.html",
            {
...
class ListView(TemplateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        enable_locale_filter = getattr(
            settings, "SHATAIL_I18N_ENABLED", False
        ...
        if enable_locale_filter:
            if "locale" in self.request.GET:
                try:
                    ...
                    return redirect(
                        "shatailsnippets:list", self.app_label, self.model_name
                    )
            else:
    ...
    def get_template_names(self):
        if self.results_only:
            return ["shatailsnippets/snippets/results.html"]
        else:
            return ["shatailsnippets/snippets/type_index.html"]


class Create(CreateView):
    template_name = "shatailsnippets/snippets/create.html"
    ...
    def _get_locale(self):
        if getattr(settings, "SHATAIL_I18N_ENABLED", False) and issubclass(
    ...
    def get_add_url(self):
        url = reverse("shatailsnippets:add", args=[self.app_label, self.model_name])
        if self.locale:
    ...
    def get_success_url(self):
        ...
        return (
            reverse("shatailsnippets:list", args=[self.app_label, self.model_name])
            + urlquery
        )
...
class Edit(EditView):
    template_name = "shatailsnippets/snippets/edit.html"
    ...
    def get_success_url(self):
        return reverse("shatailsnippets:list", args=[self.app_label, self.model_name])
...
class Delete(DeleteView):
    template_name = "shatailsnippets/snippets/confirm_delete.html"
    ...
    def get_delete_url(self):
        return (
            reverse(
                "shatailsnippets:delete-multiple",
                args=(self.app_label, self.model_name),
            )
            + "?"
    ...
    def delete_action(self):
        with transaction.atomic():
            for instance in self.objects:
                log(instance=instance, action="shatail.delete")
                instance.delete()
...
class Usage(IndexView):
    template_name = "shatailsnippets/snippets/usage.html"
    paginate_by = 20
    page_kwarg = "p"
...
def redirect_to_edit(request, app_label, model_name, pk):
    return redirect("shatailsnippets:edit", app_label, model_name, pk, permanent=True)


def redirect_to_delete(request, app_label, model_name, pk):
    return redirect("shatailsnippets:delete", app_label, model_name, pk, permanent=True)


def redirect_to_usage(request, app_label, model_name, pk):
    return redirect("shatailsnippets:usage", app_label, model_name, pk, permanent=True)


class HistoryView(IndexView):
    template_name = "shatailadmin/generic/index.html"

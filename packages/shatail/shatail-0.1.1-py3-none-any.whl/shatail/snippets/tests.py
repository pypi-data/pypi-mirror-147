...
from shatail.admin.forms import ShatailAdminModelForm
from shatail.admin.panels import FieldPanel, ObjectList
from shatail.blocks.field_block import FieldBlockAdapter
from shatail.models import Locale, ModelLogEntry, Page
from shatail.snippets.action_menu import (
    ActionMenuItem,
    get_base_snippet_action_menu_items,
)
from shatail.snippets.blocks import SnippetChooserBlock
from shatail.snippets.models import SNIPPET_MODELS, register_snippet
from shatail.snippets.views.snippets import get_snippet_edit_handler
from shatail.snippets.widgets import (
    AdminSnippetChooser,
    SnippetChooserAdapter,
    SnippetListingButton,
)
from shatail.test.snippets.forms import FancySnippetForm
from shatail.test.snippets.models import (
    AlphaSnippet,
    FancySnippet,
    FileUploadSnippet,
    RegisterDecorator,
...
from shatail.test.testapp.models import (
    Advert,
...
class TestSnippetIndexView(TestCase, ShatailTestUtils):
    def setUp(self):
        self.user = self.login()

    def get(self, params={}):
        return self.client.get(reverse("shatailsnippets:index"), params)
    ...
    def test_simple(self):
        ...
        self.assertTemplateUsed(response, "shatailsnippets/snippets/index.html")

    def test_displays_snippet(self):
        self.assertContains(self.get(), "Adverts")


class TestSnippetListView(TestCase, ShatailTestUtils):
    def setUp(self):
        self.login()
    ...
    def get(self, params={}):
        return self.client.get(
            reverse("shatailsnippets:list", args=("tests", "advert")), params
        )

    def test_simple(self):
        response = self.get()
        self.assertEqual(response.status_code, 200)
        self.assertTemplateUsed(response, "shatailsnippets/snippets/type_index.html")
    ...
    def test_simple_pagination(self):
        ...
        for page in pages:
            ...
            self.assertTemplateUsed(
                response, "shatailsnippets/snippets/type_index.html"
            )
    ...
    def test_register_snippet_listing_buttons_hook(self):
        ...
        def page_listing_buttons(snippet, user, next_url=None):
            ...
            self.assertEqual(
                next_url, reverse("shatailsnippets:list", args=("tests", "advert"))
            )
        ...
        self.assertEqual(response.status_code, 200)
        self.assertTemplateUsed(
            response, "shatailsnippets/snippets/listing_buttons.html"
        )
...
@override_settings(SHATAIL_I18N_ENABLED=True)
class TestLocaleSelectorOnList(TestCase, ShatailTestUtils):
    def setUp(self):
    ...
    def test_locale_selector(self):
        response = self.client.get(
            reverse(
                "shatailsnippets:list", args=["snippetstests", "translatablesnippet"]
            )
        )
        ...
        add_url = (
            reverse(
                "shatailsnippets:add", args=["snippetstests", "translatablesnippet"]
            )
            + "?locale=en"
        )
        self.assertContains(
    ...
    @override_settings(SHATAIL_I18N_ENABLED=False)
    ...
    def test_locale_selector_not_present_on_non_translatable_snippet(self):
        response = self.client.get(
            reverse("shatailsnippets:list", args=["tests", "advert"])
        )

        self.assertNotContains(response, 'aria-label="French" class="u-link is-live">')

        # Check that the add URLs don't include the locale
        add_url = reverse("shatailsnippets:add", args=["tests", "advert"])
        self.assertContains(
            response, f'<a href="{add_url}" class="button bicolor button--icon">'
        )
        self.assertContains(
...
class TestModelOrdering(TestCase, ShatailTestUtils):
    def setUp(self):
        for i in range(1, 10):
    ...
    def test_listing_respects_model_ordering(self):
        response = self.client.get(
            reverse("shatailsnippets:list", args=("tests", "advertwithtabbedinterface"))
        )
        self.assertEqual(response.status_code, 200)
    ...
    def test_chooser_respects_model_ordering(self):
        response = self.client.get(
            reverse(
                "shatailsnippets:choose", args=("tests", "advertwithtabbedinterface")
            )
        )
        self.assertEqual(response.status_code, 200)
        self.assertEqual(response.context["items"][0].text, "aaaadvert")
...
class TestSnippetListViewWithSearchableSnippet(TestCase, ShatailTestUtils):
    def setUp(self):
        self.login()
    ...
    def get(self, params={}):
        return self.client.get(
            reverse(
                "shatailsnippets:list", args=("snippetstests", "searchablesnippet")
            ),
            params,
        )
...
class TestSnippetCreateView(TestCase, ShatailTestUtils):
    def setUp(self):
        self.user = self.login()

    def get(self, params={}, model=Advert):
        args = (model._meta.app_label, model._meta.model_name)
        return self.client.get(reverse("shatailsnippets:add", args=args), params)

    def post(self, post_data={}, model=Advert):
        args = (model._meta.app_label, model._meta.model_name)
        return self.client.post(reverse("shatailsnippets:add", args=args), post_data)
    ...
    def test_simple(self):
        response = self.get()
        self.assertEqual(response.status_code, 200)
        self.assertTemplateUsed(response, "shatailsnippets/snippets/create.html")
        self.assertNotContains(response, 'role="tablist"', html=True)

    def test_snippet_with_tabbed_interface(self):
        response = self.client.get(
            reverse("shatailsnippets:add", args=("tests", "advertwithtabbedinterface"))
        )
    ...
    def test_create(self):
        ...
        self.assertRedirects(
            response, reverse("shatailsnippets:list", args=("tests", "advert"))
        )
    ...
    def test_create_file_upload_multipart(self):
        ...
        self.assertRedirects(
            response,
            reverse(
                "shatailsnippets:list", args=("snippetstests", "fileuploadsnippet")
            ),
        )
...
@override_settings(SHATAIL_I18N_ENABLED=True)
class TestLocaleSelectorOnCreate(TestCase, ShatailTestUtils):
    fixtures = ["test.json"]
    ...
    def test_locale_selector_not_present_on_non_translatable_snippet(self):
        response = self.client.get(
            reverse("shatailsnippets:add", args=["tests", "advert"])
        )

        switch_to_french_url = (
            reverse("shatailsnippets:add", args=["tests", "advert"]) + "?locale=fr"
        )
        self.assertNotContains(
            response,
            f'<a href="{switch_to_french_url}" aria-label="French" class="u-link is-live">',
...
class BaseTestSnippetEditView(TestCase, ShatailTestUtils):
    def get(self, params={}):
        snippet = self.test_snippet
        args = (snippet._meta.app_label, snippet._meta.model_name, quote(snippet.pk))
        return self.client.get(reverse("shatailsnippets:edit", args=args), params)

    def post(self, post_data={}):
        snippet = self.test_snippet
        args = (snippet._meta.app_label, snippet._meta.model_name, quote(snippet.pk))
        return self.client.post(reverse("shatailsnippets:edit", args=args), post_data)
...
class TestSnippetEditView(BaseTestSnippetEditView):
    ...
    def test_simple(self):
        response = self.get()
        self.assertEqual(response.status_code, 200)
        self.assertTemplateUsed(response, "shatailsnippets/snippets/edit.html")
        self.assertNotContains(response, 'role="tablist"')

        # "Last updated" timestamp should be present
        self.assertContains(
            response, 'data-shatail-tooltip="Sept. 30, 2021, 10:01 a.m."'
        )
    ...
    def test_nonexistant_id(self):
        response = self.client.get(
            reverse("shatailsnippets:edit", args=("tests", "advert", 999999))
        )
        self.assertEqual(response.status_code, 404)
...
class TestSnippetDelete(TestCase, ShatailTestUtils):
    fixtures = ["test.json"]
    ...
    def test_usage_link(self):
        ...
        self.assertEqual(response.status_code, 200)
        self.assertTemplateUsed(
            response, "shatailsnippets/snippets/confirm_delete.html"
        )
    ...
    def test_before_delete_snippet_hook_get(self):
        ...
        with self.register_hook("before_delete_snippet", hook_func):
            response = self.client.get(
                reverse(
                    "shatailsnippets:delete", args=["tests", "advert", quote(advert.pk)]
                )
            )
...
class TestSnippetDeleteMultipleWithOne(TestCase, ShatailTestUtils):
    ...
    def test_delete_get(self):
        url = reverse("shatailsnippets:delete-multiple", args=("tests", "advert"))
...
class TestSnippetDeleteMultipleWithThree(TestCase, ShatailTestUtils):
...
class TestSnippetChooserPanel(TestCase, ShatailTestUtils):
    fixtures = ["test.json"]
...
class TestSnippetHistory(TestCase, ShatailTestUtils):
    fixtures = ["test.json"]

    def get(self, params={}):
        snippet = self.test_snippet
        args = (snippet._meta.app_label, snippet._meta.model_name, quote(snippet.pk))
        return self.client.get(reverse("shatailsnippets:history", args=args), params)
...
class TestSnippetChoose(TestCase, ShatailTestUtils):
    fixtures = ["test.json"]
    ...
    def get(self, params=None):
        return self.client.get(
            reverse("shatailsnippets:choose", args=self.url_args), params or {}
        )

    def test_simple(self):
        response = self.get()
        self.assertTemplateUsed(response, "shatailsnippets/chooser/choose.html")
...
class TestSnippetChooseWithSearchableSnippet(TestCase, ShatailTestUtils):
    def setUp(self):
        self.login()
    ...
    def get(self, params=None):
        return self.client.get(
            reverse(
                "shatailsnippets:choose", args=("snippetstests", "searchablesnippet")
            ),
            params or {},
        )
...
class TestSnippetChosen(TestCase, ShatailTestUtils):
    fixtures = ["test.json"]
    ...
    def get(self, pk, params=None):
        return self.client.get(
            reverse("shatailsnippets:chosen", args=("tests", "advert", pk)),
            params or {},
        )
...
class TestAddOnlyPermissions(TestCase, ShatailTestUtils):
    fixtures = ["test.json"]
...
class TestDeleteOnlyPermissions(TestCase, ShatailTestUtils):
    fixtures = ["test.json"]
...
class TestSnippetEditHandlers(TestCase, ShatailTestUtils):
    def test_standard_edit_handler(self):
        edit_handler = get_snippet_edit_handler(StandardSnippet)
        form_class = edit_handler.get_form_class()
        self.assertTrue(issubclass(form_class, ShatailAdminModelForm))
...
class TestInlinePanelMedia(TestCase, ShatailTestUtils):
    """
...
class TestAdminSnippetChooserWidget(TestCase, ShatailTestUtils):
    def test_adapt(self):
...
class TestSnippetListViewWithCustomPrimaryKey(TestCase, ShatailTestUtils):
    def setUp(self):
        self.login()
...
class TestSnippetViewWithCustomPrimaryKey(TestCase, ShatailTestUtils):
    fixtures = ["test.json"]
...
class TestSnippetChooserPanelWithCustomPrimaryKey(TestCase, ShatailTestUtils):
    fixtures = ["test.json"]
...
class TestSnippetChooseWithCustomPrimaryKey(TestCase, ShatailTestUtils):
    fixtures = ["test.json"]
    ...
    def get(self, params=None):
        return self.client.get(
            reverse(
                "shatailsnippets:choose", args=("tests", "advertwithcustomprimarykey")
            ),
            params or {},
        )
...
class TestSnippetChosenWithCustomPrimaryKey(TestCase, ShatailTestUtils):
    fixtures = ["test.json"]
...
class TestSnippetChosenWithCustomUUIDPrimaryKey(TestCase, ShatailTestUtils):
    fixtures = ["test.json"]
...
class TestPanelConfigurationChecks(TestCase, ShatailTestUtils):
    def setUp(self):
        self.warning_id = "shatailadmin.W002"

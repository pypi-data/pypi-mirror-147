...
from shatail.log_actions import LogActionRegistry
...
from shatail.test.testapp.models import SimplePage
from shatail.test.utils import ShatailTestUtils


class TestAuditLogManager(TestCase, ShatailTestUtils):
    def setUp(self):
    ...
    def test_log_action(self):
        ...
        with freeze_time(now):
            entry = PageLogEntry.objects.log_action(
                self.page, "shatail.edit", user=self.user
            )
    ...
    def test_get_for_model(self):
        PageLogEntry.objects.log_action(self.page, "shatail.edit")
        PageLogEntry.objects.log_action(self.simple_page, "shatail.edit")
...
class TestAuditLog(TestCase):
    ...
    def test_page_create(self):
        ...
        self.assertEqual(log_entry.action, "shatail.create")
    ...
    def test_page_edit(self):
        ...
        self.assertEqual(PageLogEntry.objects.count(), 1)
        self.assertEqual(PageLogEntry.objects.filter(action="shatail.edit").count(), 1)

        # passing a string for the action should log this.
        self.home_page.save_revision(log_action="shatail.revert")
        self.assertEqual(
            PageLogEntry.objects.filter(action="shatail.revert").count(), 1
        )
    ...
    def test_page_publish(self):
        ...
        self.assertEqual(PageLogEntry.objects.count(), 1)
        self.assertEqual(
            PageLogEntry.objects.filter(action="shatail.publish").count(), 1
        )
    ...
    def test_page_rename(self):
        ...
        self.assertEqual(
            PageLogEntry.objects.filter(action="shatail.rename").count(), 0
        )
        ...
        self.assertEqual(
            PageLogEntry.objects.filter(action="shatail.publish").count(), 2
        )
        self.assertEqual(
            PageLogEntry.objects.filter(action="shatail.rename").count(), 1
        )
    ...
    def test_page_unpublish(self):
        ...
        self.assertEqual(PageLogEntry.objects.count(), 1)
        self.assertEqual(
            PageLogEntry.objects.filter(action="shatail.unpublish").count(), 1
        )
    ...
    def test_revision_schedule_publish(self):
        ...
        log_entries = PageLogEntry.objects.filter(action="shatail.publish.schedule")
    ...
    def test_revision_cancel_schedule(self):
        ...
        self.assertEqual(
            PageLogEntry.objects.filter(action="shatail.schedule.cancel").count(), 1
        )

    def test_page_lock_unlock(self):
        self.home_page.save(log_action="shatail.lock")
        self.home_page.save(log_action="shatail.unlock")

        self.assertEqual(
            PageLogEntry.objects.filter(
                action__in=["shatail.lock", "shatail.unlock"]
            ).count(),
            2,
        )
    ...
    def test_page_copy(self):
        ...
        self.assertListEqual(
            list(PageLogEntry.objects.values_list("action", flat=True)),
            ["shatail.publish", "shatail.copy", "shatail.create"],
        )
    ...
    def test_page_reorder(self):
        ...
        # This should log as `shatail.reorder` because the page was moved under the same parent page
        section_1.move(self.root_page, user=user, pos="last-child")

        self.assertEqual(
            PageLogEntry.objects.filter(action="shatail.reorder", user=user).count(), 1
        )
        self.assertEqual(
            PageLogEntry.objects.filter(action="shatail.move", user=user).count(), 0
        )
    ...
    def test_page_move(self):
        ...
        self.assertEqual(
            PageLogEntry.objects.filter(action="shatail.move", user=user).count(), 1
        )
        self.assertEqual(
            PageLogEntry.objects.filter(action="shatail.reorder", user=user).count(), 0
        )
    ...
    def test_page_delete(self):
        ...
        self.assertEqual(
            PageLogEntry.objects.filter(action="shatail.delete").count(), 4
        )
        self.assertEqual(
            set(
                PageLogEntry.objects.filter(action="shatail.delete").values_list(
                    "label", flat=True
                )
            ),
            {
    ...
    def test_workflow_actions(self):
        ...
        workflow_entry = PageLogEntry.objects.filter(action="shatail.workflow.start")
    ...
    def test_workflow_completions_logs_publishing_user(self):
        ...
        self.assertFalse(PageLogEntry.objects.filter(action="shatail.publish").exists())
        ...
        self.assertEqual(
            PageLogEntry.objects.get(action="shatail.publish").user, publisher
        )
    ...
    def test_page_privacy(self):
        ...
        restriction.save()
        self.assertEqual(
            PageLogEntry.objects.filter(action="shatail.view_restriction.edit").count(),
            1,
        )
...
class TestAuditLogHooks(TestCase, ShatailTestUtils):
    def setUp(self):
        self.root_page = Page.objects.get(id=2)

    def test_register_log_actions_hook(self):
        log_actions = LogActionRegistry()
        self.assertTrue(log_actions.action_exists("shatail.create"))
    ...
    def test_action_format_message(self):
        ...
        log_entry = PageLogEntry.objects.log_action(
            self.root_page, action="shatail.create"
        )

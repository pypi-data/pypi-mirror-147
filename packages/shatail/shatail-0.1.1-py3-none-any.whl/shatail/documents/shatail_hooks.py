...
from shatail.admin.navigation import get_site_for_user
from shatail.admin.search import SearchArea
from shatail.admin.site_summary import SummaryItem
from shatail.documents import admin_urls, get_document_model
from shatail.documents.api.admin.views import DocumentsAdminAPIViewSet
from shatail.documents.forms import GroupDocumentPermissionFormSet
from shatail.documents.permissions import permission_policy
from shatail.documents.rich_text import DocumentLinkHandler
from shatail.documents.rich_text.contentstate import (
    ContentstateDocumentLinkConversionRule,
)
from shatail.documents.rich_text.editor_html import EditorHTMLDocumentLinkConversionRule
from shatail.documents.views.bulk_actions import (
    AddTagsBulkAction,
    AddToCollectionBulkAction,
    DeleteBulkAction,
)
from shatail.models import BaseViewRestriction
from shatail.shatail_hooks import require_shatail_login


@hooks.register("register_admin_urls")
def register_admin_urls():
    return [
        path("documents/", include(admin_urls, namespace="shataildocs")),
    ]
...
def register_document_feature(features):
    ...
    features.register_editor_plugin(
        ...
        draftail_features.EntityFeature(
            ...
            js=["shataildocs/js/document-chooser-modal.js"],
        ),
    )
...
class DocumentsSummaryItem(SummaryItem):
    order = 300
    template_name = "shataildocs/homepage/site_summary_documents.html"
...
def describe_collection_docs(collection):
    ...
    if docs_count:
        url = reverse("shataildocs:index") + ("?collection_id=%d" % collection.id)
        return {
            "count": docs_count,
            "count_text": ngettext(
    ...
    for restriction in document.collection.get_view_restrictions():
        if not restriction.accept_request(request):
            if restriction.restriction_type == BaseViewRestriction.PASSWORD:
                ...
                action_url = reverse(
                    "shataildocs_authenticate_with_password", args=[restriction.id]
                )

                password_required_template = getattr(
                    settings,
                    "DOCUMENT_PASSWORD_REQUIRED_TEMPLATE",
                    "shataildocs/password_required.html",
                )
...
class DocumentAdminURLFinder(ModelAdminURLFinder):
    edit_url_name = "shataildocs:edit"

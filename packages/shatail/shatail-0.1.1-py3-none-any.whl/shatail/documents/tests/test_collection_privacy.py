...
class TestCollectionPrivacyDocument(TestCase, ShatailTestUtils):
    fixtures = ["test.json"]
    ...
    def get_document(self, collection):
        ...
        url = reverse(
            "shataildocs_serve", args=(secret_document.id, secret_document.filename)
        )
    ...
    def test_anonymous_user_must_authenticate(self):
        ...
        self.assertEqual(
            response.templates[0].name, "shataildocs/password_required.html"
        )

        submit_url = reverse(
            "shataildocs_authenticate_with_password", args=[self.view_restriction.id]
        )

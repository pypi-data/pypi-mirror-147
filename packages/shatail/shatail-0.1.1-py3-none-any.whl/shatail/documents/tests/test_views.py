...
from shatail.documents import models


@override_settings(SHATAILDOCS_SERVE_METHOD=None)
class TestServeView(TestCase):
    ...
    def get(self, document=None):
        ...
        self.response = self.client.get(
            reverse("shataildocs_serve", args=(document.id, document.filename))
        )
    ...
    @mock.patch("shatail.documents.views.serve.hooks")
    @mock.patch("shatail.documents.views.serve.get_object_or_404")
    ...
    def test_with_incorrect_filename(self):
        response = self.client.get(
            reverse("shataildocs_serve", args=(self.document.id, "incorrectfilename"))
        )
        self.assertEqual(response.status_code, 404)
    ...
    def clear_sendfile_cache(self):
        from shatail.utils.sendfile import _get_sendfile

        _get_sendfile.clear()


@override_settings(SHATAILDOCS_SERVE_METHOD="redirect")
class TestServeViewWithRedirect(TestCase):
    def setUp(self):
        self.document = models.Document(title="Test document")
        self.document.file.save("example.doc", ContentFile("A boring example document"))
        self.serve_view_url = reverse(
            "shataildocs_serve", args=(self.document.id, self.document.filename)
        )
...
@override_settings(SHATAILDOCS_SERVE_METHOD="direct")
...
class TestServeWithUnicodeFilename(TestCase):
    ...
    def test_response_code(self):
        response = self.client.get(
            reverse("shataildocs_serve", args=(self.document.id, self.filename))
        )
        self.assertEqual(response.status_code, 200)
    ...
    def test_non_local_filesystem_unicode_content_disposition_header(
        ...
        response = self.client.get(
            reverse("shataildocs_serve", args=(self.document.id, mock_doc.filename))
        )

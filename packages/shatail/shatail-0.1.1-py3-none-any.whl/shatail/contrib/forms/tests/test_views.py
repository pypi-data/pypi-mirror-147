...
from shatail.admin.forms import ShatailAdminPageForm
from shatail.admin.panels import get_form_for_model
from shatail.contrib.forms.models import FormSubmission
from shatail.contrib.forms.panels import FormSubmissionsPanel
from shatail.contrib.forms.tests.utils import (
    make_form_page,
    make_form_page_with_custom_submission,
)
from shatail.models import Locale, Page
...
class TestFormResponsesPanel(TestCase):
    def setUp(self):
        ...
        self.FormPageForm = get_form_for_model(
            FormPage,
            form_class=ShatailAdminPageForm,
    ...
    def test_render_with_submissions(self):
        ...
        url = reverse("shatailforms:list_submissions", args=(self.form_page.id,))
...
class TestFormResponsesPanelWithCustomSubmissionClass(TestCase, ShatailTestUtils):
    def setUp(self):
        self.request = RequestFactory().get("/")
        user = AnonymousUser()  # technically, Anonymous users cannot access the admin
        self.request.user = user
...
class TestFormsIndex(TestCase, ShatailTestUtils):
    fixtures = ["test.json"]
    ...
    def test_forms_index(self):
        response = self.client.get(reverse("shatailforms:index"))

        # Check response
        self.assertEqual(response.status_code, 200)
        self.assertTemplateUsed(response, "shatailforms/index.html")
    ...
    def test_forms_index_pagination(self):
        ...
        # Get page two
        response = self.client.get(reverse("shatailforms:index"), {"p": 2})
    ...
    def test_forms_index_pagination_invalid(self):
        ...
        # Get page two
        response = self.client.get(reverse("shatailforms:index"), {"p": "Hello world!"})
    ...
    def test_forms_index_pagination_out_of_range(self):
        ...
        # Get page two
        response = self.client.get(reverse("shatailforms:index"), {"p": 99999})
...
class TestFormsIndexWithLocalisationEnabled(TestCase, ShatailTestUtils):
    fixtures = ["test.json"]
    ...
    def setUp(self):
        ...
        self.forms_index_url = reverse("shatailforms:index")
...
class TestFormsSubmissionsList(TestCase, ShatailTestUtils):
    def setUp(self):
        # Create a form page
        self.form_page = make_form_page()
    ...
    def test_list_submissions(self):
        response = self.client.get(
            reverse("shatailforms:list_submissions", args=(self.form_page.id,))
        )

        # Check response
        self.assertEqual(response.status_code, 200)
        self.assertTemplateUsed(response, "shatailforms/index_submissions.html")
...
class TestFormsSubmissionsExport(TestCase, ShatailTestUtils):
    def setUp(self):
        # Create a form page
        self.form_page = make_form_page()
...
class TestCustomFormsSubmissionsExport(TestCase, ShatailTestUtils):
...
class TestCustomFormsSubmissionsList(TestCase, ShatailTestUtils):
    def create_test_user_without_admin(self, username):
        return self.create_user(username=username, password="123")
...
class TestDeleteFormSubmission(TestCase, ShatailTestUtils):
    fixtures = ["test.json"]
    ...
    def test_delete_submission_show_confirmation(self):
        response = self.client.get(
            reverse("shatailforms:delete_submissions", args=(self.form_page.id,))
            + "?selected-submissions={}".format(FormSubmission.objects.first().id)
        )
        # Check show confirm page when HTTP method is GET
        self.assertTemplateUsed(response, "shatailforms/confirm_delete.html")
...
class TestDeleteCustomFormSubmission(TestCase, ShatailTestUtils):
    fixtures = ["test.json"]
...
class TestFormsWithCustomSubmissionsList(TestCase, ShatailTestUtils):
    def create_test_user_without_admin(self, username):
        return self.create_user(username=username, password="123")
...
class TestFormsWithCustomFormBuilderSubmissionsList(TestCase, ShatailTestUtils):
    def setUp(self):
        home_page = Page.objects.get(url_path="/home/")
...
class TestDuplicateFormFieldLabels(TestCase, ShatailTestUtils):
    """
    If a user creates two fields with the same label, data cannot be saved correctly.
    See: https://github.com/shatail/shatail/issues/585
    """
    ...
    def test_adding_duplicate_form_labels(self):
        ...
        response = self.client.post(
            reverse(
                "shatailadmin_pages:add", args=("tests", "formpage", self.root_page.id)
            ),
            post_data,
        )

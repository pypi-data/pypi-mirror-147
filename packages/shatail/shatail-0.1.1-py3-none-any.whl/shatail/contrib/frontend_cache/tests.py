...
from shatail.contrib.frontend_cache.backends import (
    AzureCdnBackend,
...
from shatail.contrib.frontend_cache.utils import get_backends
from shatail.models import Page
from shatail.test.testapp.models import EventIndex
...
class TestBackendConfiguration(TestCase):
    ...
    def test_varnish(self):
        backends = get_backends(
            backend_settings={
                "varnish": {
                    "BACKEND": "shatail.contrib.frontend_cache.backends.HTTPBackend",
    ...
    def test_cloudflare(self):
        backends = get_backends(
            backend_settings={
                "cloudflare": {
                    "BACKEND": "shatail.contrib.frontend_cache.backends.CloudflareBackend",
                    "EMAIL": "test@test.com",
    ...
    def test_cloudfront(self):
        backends = get_backends(
            backend_settings={
                "cloudfront": {
                    "BACKEND": "shatail.contrib.frontend_cache.backends.CloudfrontBackend",
    ...
    def test_azure_cdn(self):
        backends = get_backends(
            backend_settings={
                "azure_cdn": {
                    "BACKEND": "shatail.contrib.frontend_cache.backends.AzureCdnBackend",
                    "RESOURCE_GROUP_NAME": "test-resource-group",
                    "CDN_PROFILE_NAME": "shatail-io-profile",
                    "CDN_ENDPOINT_NAME": "shatail-io-endpoint",
                },
            }
        )
        ...
        self.assertEqual(backends["azure_cdn"]._cdn_profile_name, "shatail-io-profile")
        self.assertEqual(
            backends["azure_cdn"]._cdn_endpoint_name, "shatail-io-endpoint"
        )
    ...
    def test_azure_front_door(self):
        backends = get_backends(
            backend_settings={
                "azure_front_door": {
                    "BACKEND": "shatail.contrib.frontend_cache.backends.AzureFrontDoorBackend",
                    "RESOURCE_GROUP_NAME": "test-resource-group",
                    "FRONT_DOOR_NAME": "shatail-io-front-door",
                },
            }
        )
        ...
        self.assertEqual(
            backends["azure_front_door"]._front_door_name, "shatail-io-front-door"
        )
        ...
        backends = get_backends(
            backend_settings={
                "azure_front_door": {
                    "BACKEND": "shatail.contrib.frontend_cache.backends.AzureFrontDoorBackend",
                    "RESOURCE_GROUP_NAME": "test-resource-group",
                    "FRONT_DOOR_NAME": "shatail-io-fake-front-door-name",
                    "SUBSCRIPTION_ID": "fake-subscription-id",
                    "CREDENTIALS": mock_credentials,
                },
            }
    ...
    @mock.patch(
        "shatail.contrib.frontend_cache.backends.AzureCdnBackend._make_purge_call"
    )
    def test_azure_cdn_purge(self, make_purge_call_mock):
        ...
        backends["azure_cdn"].purge(
            "http://www.shatail.org/home/events/christmas/?test=1"
        )
    ...
    @mock.patch(
        "shatail.contrib.frontend_cache.backends.AzureFrontDoorBackend._make_purge_call"
    )
    ...
    def test_http_httperror(self):
        ...
        self.assertIn(
            "Couldn't purge 'http://www.shatail.org/home/events/christmas/' from HTTP cache. HTTPError: 500 Internal Server Error",
            log_output.output[0],
        )
    ...
    def test_http_urlerror(self):
        ...
        self.assertIn(
            "Couldn't purge 'http://www.shatail.org/home/events/christmas/' from HTTP cache. URLError: just for tests",
            log_output.output[0],
        )

    @mock.patch("shatail.contrib.frontend_cache.backends.urlopen")
    def _test_http_with_side_effect(self, urlopen_mock, urlopen_side_effect):
        ...
        backends.get("varnish").purge("http://www.shatail.org/home/events/christmas/")
    ...
    @mock.patch(
        "shatail.contrib.frontend_cache.backends.CloudfrontBackend._create_invalidation"
    )
        ...
        backends = get_backends(
            backend_settings={
                "cloudfront": {
                    "BACKEND": "shatail.contrib.frontend_cache.backends.CloudfrontBackend",
                    "DISTRIBUTION_ID": {
                        "www.shatail.org": "frontend",
                    },
                },
            }
        )
        backends.get("cloudfront").purge(
            "http://www.shatail.org/home/events/christmas/"
        )
    ...
    @override_settings(SHATAILFRONTENDCACHE_LOCATION="http://localhost:8000")
...
@override_settings(
    SHATAILFRONTENDCACHE={
        "varnish": {
            "BACKEND": "shatail.contrib.frontend_cache.tests.MockBackend",
        },
    }
)
...
@override_settings(
    SHATAILFRONTENDCACHE={
        "cloudflare": {
            "BACKEND": "shatail.contrib.frontend_cache.tests.MockCloudflareBackend",
        },
    }
)
...
class TestCachePurgingSignals(TestCase):
    ...
    @override_settings(
        ROOT_URLCONF="shatail.test.urls_multilang",
        LANGUAGE_CODE="en",
        SHATAILFRONTENDCACHE_LANGUAGES=["en", "fr", "pt-br"],
    )
    ...
    @override_settings(
        ROOT_URLCONF="shatail.test.urls_multilang",
        LANGUAGE_CODE="en",
        SHATAIL_I18N_ENABLED=True,
        SHATAIL_CONTENT_LANGUAGES=[("en", "English"), ("fr", "French")],
    )
    ...
    def test_purge_on_publish_without_i18n_enabled(self):
        # It should ignore SHATAIL_CONTENT_LANGUAGES as SHATAIL_I18N_ENABLED isn't set
        PURGED_URLS[:] = []  # reset PURGED_URLS to the empty list
        page = EventIndex.objects.get(url_path="/home/events/")
        page.save_revision().publish()
        self.assertEqual(
...
class TestPurgeBatchClass(TestCase):
    ...
    @mock.patch("shatail.contrib.frontend_cache.backends.requests.delete")

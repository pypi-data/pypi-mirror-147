...
from shatail.coreutils import get_dummy_request
...
@override_settings(SHATAILREDIRECTS_AUTO_CREATE=True)
class TestAutocreateRedirects(TestCase, ShatailTestUtils):
    fixtures = ["test.json"]
    ...
    def trigger_page_slug_changed_signal(self, page):
        page.slug += "-extra"
        page.save(log_action="shatail.publish", user=self.user, clean=False)
    ...
    @override_settings(SHATAILREDIRECTS_AUTO_CREATE=False)

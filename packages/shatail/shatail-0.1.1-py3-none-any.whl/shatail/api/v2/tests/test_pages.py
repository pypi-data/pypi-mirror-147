...
from shatail.api.v2 import signal_handlers
from shatail.models import Locale, Page, Site
from shatail.models.view_restrictions import BaseViewRestriction
from shatail.test.demosite import models
from shatail.test.testapp.models import StreamPage
...
class TestPageListing(TestCase, ShatailTestUtils):
    fixtures = ["demosite.json"]

    def get_response(self, **params):
        return self.client.get(reverse("shatailapi_v2:pages:listing"), params)
    ...
    def test_private_pages_with_user_login(self):
        ...
        response = client.get(reverse("shatailapi_v2:pages:listing"))
    ...
    def test_fields_foreign_key(self):
        ...
        for page in content["items"]:
            ...
            if feed_image is not None:
                ...
                self.assertEqual(feed_image["meta"]["type"], "shatailimages.Image")
                self.assertEqual(
    ...
    def test_filtering_tags(self):
        response = self.get_response(type="demosite.BlogEntryPage", tags="shatail")
        content = json.loads(response.content.decode("UTF-8"))
    ...
    def test_filtering_multiple_tags(self):
        response = self.get_response(type="demosite.BlogEntryPage", tags="shatail,bird")
        content = json.loads(response.content.decode("UTF-8"))
    ...
    @override_settings(SHATAILAPI_LIMIT_MAX=None)
    ...
    @override_settings(SHATAILAPI_LIMIT_MAX=10)
    ...
    @override_settings(SHATAILAPI_LIMIT_MAX=2)
    def test_limit_default_changes_with_max(self):
        # The default limit is 20. If SHATAILAPI_LIMIT_MAX is less than that,
    ...
    @override_settings(SHATAILAPI_SEARCH_ENABLED=False)
    ...
    def test_search_when_filtering_by_tag_gives_error(self):
        response = self.get_response(
            type="demosite.BlogEntryPage", search="blog", tags="shatail"
        )
        content = json.loads(response.content.decode("UTF-8"))
...
class TestPageDetail(TestCase):
    ...
    def get_response(self, page_id, **params):
        return self.client.get(
            reverse("shatailapi_v2:pages:detail", args=(page_id,)), params
        )
    ...
    def test_basic(self):
        ...
        self.assertEqual(content["tags"], ["bird", "shatail"])
        ...
        self.assertEqual(content["feed_image"]["meta"]["type"], "shatailimages.Image")
        self.assertEqual(
...
class TestPageFind(TestCase):
    ...
    def get_response(self, **params):
        return self.client.get(reverse("shatailapi_v2:pages:find"), params)
    ...
    def test_find_by_id(self):
        ...
        self.assertRedirects(
            response,
            "http://localhost" + reverse("shatailapi_v2:pages:detail", args=[5]),
    ...
    def test_find_by_html_path(self):
        ...
        self.assertRedirects(
            response,
            "http://localhost" + reverse("shatailapi_v2:pages:detail", args=[8]),
            fetch_redirect_response=False,
        )
...
class TestPageDetailWithStreamField(TestCase):
    ...
    def test_can_fetch_streamfield_content(self):
        ...
        response_url = reverse("shatailapi_v2:pages:detail", args=(stream_page.id,))
    ...
    def test_image_block_with_custom_get_api_representation(self):
        ...
        response_url = "{}?extended=1".format(
            reverse("shatailapi_v2:pages:detail", args=(stream_page.id,))
        )
        response = self.client.get(response_url)
        content = json.loads(response.content.decode("utf-8"))
...
@override_settings(
    ...
    SHATAILAPI_BASE_URL="http://api.example.com",
)
@mock.patch("shatail.contrib.frontend_cache.backends.HTTPBackend.purge")

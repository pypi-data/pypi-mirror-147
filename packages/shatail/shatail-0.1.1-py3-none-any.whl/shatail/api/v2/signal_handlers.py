...
from shatail.contrib.frontend_cache.utils import purge_url_from_cache
...
from shatail.signals import page_published, page_unpublished
...
def purge_page_from_cache(instance, **kwargs):
    ...
    purge_url_from_cache(
        base_url + reverse("shatailapi_v2:pages:detail", args=(instance.id,))
    )
...
def purge_image_from_cache(instance, **kwargs):
    if not kwargs.get("created", False):
        base_url = get_base_url()
        purge_url_from_cache(
            base_url + reverse("shatailapi_v2:images:detail", args=(instance.id,))
        )
...
def purge_document_from_cache(instance, **kwargs):
    if not kwargs.get("created", False):
        base_url = get_base_url()
        purge_url_from_cache(
            base_url + reverse("shatailapi_v2:documents:detail", args=(instance.id,))
        )

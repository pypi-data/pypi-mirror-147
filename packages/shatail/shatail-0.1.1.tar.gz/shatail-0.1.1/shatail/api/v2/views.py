...
from shatail.api import APIField
from shatail.models import Page, PageViewRestriction, Site
...
from .pagination import ShatailPagination
...
class BaseAPIViewSet(GenericViewSet):
    ...
    pagination_class = ShatailPagination
    ...
    def find_view(self, request):
        ...
        url = get_object_detail_url(
            self.request.shatailapi_router, request, self.model, obj.pk
        )
    ...
    def get_serializer_class(self):
        ...
        return self._get_serializer_class(
            self.request.shatailapi_router,
            model,
            fields_config,
    ...
    def get_serializer_context(self):
        ...
        return {
            "request": self.request,
            "view": self,
            "router": self.request.shatailapi_router,
        }
...
class PagesAPIViewSet(BaseAPIViewSet):
    ...
    def get_detail_default_fields(cls, model):
        ...
        if not getattr(settings, "SHATAIL_I18N_ENABLED", False):
    ...
    def get_queryset(self):
        ...
        try:
            models = page_models_from_string(
                request.GET.get("type", "shatailcore.Page")
            )

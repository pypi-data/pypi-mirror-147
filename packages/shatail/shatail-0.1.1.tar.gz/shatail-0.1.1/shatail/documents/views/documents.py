...
from shatail.documents.forms import get_document_form
from shatail.documents.permissions import permission_policy
from shatail.models import Collection
...
class BaseListingView(TemplateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        next_url = reverse("shataildocs:index")
...
class IndexView(BaseListingView):
    template_name = "shataildocs/documents/index.html"
...
class ListingResultsView(BaseListingView):
    template_name = "shataildocs/documents/results.html"
...
def add(request):
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            messages.success(
                ...
                buttons=[
                    messages.button(
                        reverse("shataildocs:edit", args=(doc.id,)), _("Edit")
                    )
                ],
            )
            return redirect("shataildocs:index")
        else:
    ...
    return TemplateResponse(
        request,
        "shataildocs/documents/add.html",
        {
            "form": form,
        },
    )
...
def edit(request, document_id):
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            redirect_url = "shataildocs:index"
            if next_url:
    ...
    if local_path:
        ...
        if not os.path.isfile(local_path):
            messages.error(
                ...
                buttons=[
                    messages.button(
                        reverse("shataildocs:delete", args=(doc.id,)), _("Delete")
                    )
                ],
            )
    ...
    return TemplateResponse(
        request,
        "shataildocs/documents/edit.html",
        {
            "document": doc,
...
def delete(request, document_id):
    ...
    if request.method == "POST":
        ...
        return redirect(next_url) if next_url else redirect("shataildocs:index")

    return TemplateResponse(
        request,
        "shataildocs/documents/confirm_delete.html",
        {
            "document": doc,
            "next": next_url,
        },
...
def usage(request, document_id):
    ...
    return TemplateResponse(
        request,
        "shataildocs/documents/usage.html",

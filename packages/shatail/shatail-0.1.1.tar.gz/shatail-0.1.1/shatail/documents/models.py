...
class AbstractDocument(CollectionMember, index.Indexed, models.Model):
    ...
    def clean(self):
        """
        Checks for SHATAILDOCS_EXTENSIONS and validates the uploaded file
        ...
        """
        allowed_extensions = getattr(settings, "SHATAILDOCS_EXTENSIONS", None)
    ...
    @property
    def url(self):
        if getattr(settings, "SHATAILDOCS_SERVE_METHOD", None) == "direct":
            try:
        ...
        return reverse("shataildocs_serve", args=[self.id, self.filename])
    ...
    @property
    def usage_url(self):
        return reverse("shataildocs:document_usage", args=(self.id,))
    ...
    def content_type(self):
        content_types_lookup = getattr(settings, "SHATAILDOCS_CONTENT_TYPES", {})
        return (
    ...
    def content_disposition(self):
        inline_content_types = getattr(
            settings, "SHATAILDOCS_INLINE_CONTENT_TYPES", ["application/pdf"]
        )

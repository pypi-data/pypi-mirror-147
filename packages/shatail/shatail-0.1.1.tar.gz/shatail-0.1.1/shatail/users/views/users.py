...
from shatail.users.utils import user_can_delete_user
...
def get_user_creation_form():
    form_setting = "SHATAIL_USER_CREATION_FORM"
...
def get_user_edit_form():
    form_setting = "SHATAIL_USER_EDIT_FORM"
    if hasattr(settings, form_setting):
        return get_custom_form(form_setting)
    else:
        return UserEditForm
...
def index(request, *args):
    ...
    paginator = Paginator(users.select_related("shatail_userprofile"), per_page=20)
    users = paginator.get_page(request.GET.get("p"))

    if request.headers.get("x-requested-with") == "XMLHttpRequest":
        return TemplateResponse(
            request,
            "shatailusers/users/results.html",
            {
                "users": users,
...
def create(request):
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            with transaction.atomic():
                user = form.save()
                log(user, "shatail.create")
            messages.success(
                request,
                _("User '{0}' created.").format(user),
                buttons=[
                    messages.button(
                        reverse("shatailusers_users:edit", args=(user.pk,)), _("Edit")
                    )
                ],
            )
            for fn in hooks.get_hooks("after_create_user"):
                ...
                if hasattr(result, "status_code"):
                    return result
            return redirect("shatailusers_users:index")
        else:
    ...
    return TemplateResponse(
        request,
        "shatailusers/users/create.html",
        {
            "form": form,
        },
    )
...
def delete(request, user_id):
    ...
    return TemplateResponse(
        request,
        "shatailusers/users/confirm_delete.html",
        {
            "user": user,
        },
    )

...
# Custom fields
if hasattr(settings, "SHATAIL_USER_CUSTOM_FIELDS"):
    custom_fields = set(settings.SHATAIL_USER_CUSTOM_FIELDS)
else:
    custom_fields = set()
...
class UserForm(UsernameForm):
    ...
    def password_required(self):
        return getattr(settings, "SHATAILUSERS_PASSWORD_REQUIRED", True)

    @property
    def password_enabled(self):
        return getattr(settings, "SHATAILUSERS_PASSWORD_ENABLED", True)
...
class GroupForm(forms.ModelForm):
    ...
    def save(self):
        ...
        # accessible in the shatail admin interface, as otherwise these would
...
class BaseGroupPagePermissionFormSet(forms.BaseFormSet):
    ...
    def as_admin_panel(self):
        return render_to_string(
            "shatailusers/groups/includes/page_permissions_formset.html",
            {"formset": self},
        )

...
def create_admin_access_permissions(apps, schema_editor):
    ...
    # Add a content type to hang the 'can access Shatail admin' permission off
    shatailadmin_content_type, created = ContentType.objects.get_or_create(
        app_label="shatailadmin", model="admin"
    )

    # Create admin permission
    admin_permission, created = Permission.objects.get_or_create(
        content_type=shatailadmin_content_type,
        codename="access_admin",
        name="Can access Shatail admin",
    )
...
def remove_admin_access_permissions(apps, schema_editor):
    ...
    ContentType = apps.get_model("contenttypes.ContentType")
    Permission = apps.get_model("auth.Permission")
    shatailadmin_content_type = ContentType.objects.get(
        app_label="shatailadmin",
        model="admin",
    )

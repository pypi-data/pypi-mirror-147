...
from shatail.admin.filters import (
    ContentTypeFilter,
    DateRangePickerWidget,
    ShatailFilterSet,
)
from shatail.coreutils import get_content_type_label
from shatail.log_actions import registry as log_action_registry
from shatail.models import PageLogEntry
...
class SiteHistoryReportFilterSet(ShatailFilterSet):
...
class LogEntriesView(ReportView):
    template_name = "shatailadmin/reports/site_history.html"
    ...
    def decorate_paginated_queryset(self, queryset):
        ...
        for log_model_index, pks in pks_by_log_model_index.items():
            log_entries = (
                ...
                .objects.prefetch_related("user__shatail_userprofile", "content_type")
                .filter(pk__in=pks)
                .with_instances()
            )

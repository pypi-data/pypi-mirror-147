...
from shatail.admin.forms.account import (
    AvatarPreferencesForm,
    LocalePreferencesForm,
    NameEmailForm,
    NotificationPreferencesForm,
)
from shatail.admin.forms.auth import LoginForm, PasswordChangeForm, PasswordResetForm
...
from shatail.models import UserPagePermissionsProxy
from shatail.users.models import UserProfile
from shatail.utils.loading import get_custom_form


def get_user_login_form():
    form_setting = "SHATAILADMIN_USER_LOGIN_FORM"
...
def password_management_enabled():
    return getattr(settings, "SHATAIL_PASSWORD_MANAGEMENT_ENABLED", True)


def email_management_enabled():
    return getattr(settings, "SHATAIL_EMAIL_MANAGEMENT_ENABLED", True)


def password_reset_enabled():
    return getattr(
        settings, "SHATAIL_PASSWORD_RESET_ENABLED", password_management_enabled()
    )
...
class BaseSettingsPanel:
    ...
    tab = profile_tab
    help_text = None
    template_name = "shatailadmin/account/settings_panels/base.html"
    form_class = None
    form_object = "user"
...
class AvatarSettingsPanel(BaseSettingsPanel):
    ...
    order = 300
    template_name = "shatailadmin/account/settings_panels/avatar.html"
...
def account(request):
    ...
    if request.method == "POST":
        ...
        if all(form.is_valid() or not form.is_bound for form in panel_forms):
            ...
            return redirect("shatailadmin_account")
    ...
    return TemplateResponse(
        request,
        "shatailadmin/account/account.html",
        {
...
class PasswordResetEnabledViewMixin:
    ...
    - SHATAIL_PASSWORD_RESET_ENABLED
    - SHATAIL_PASSWORD_MANAGEMENT_ENABLED
    """
...
class PasswordResetView(PasswordResetEnabledViewMixin, auth_views.PasswordResetView):
    template_name = "shatailadmin/account/password_reset/form.html"
    email_template_name = "shatailadmin/account/password_reset/email.txt"
    subject_template_name = "shatailadmin/account/password_reset/email_subject.txt"
    form_class = PasswordResetForm
    success_url = reverse_lazy("shatailadmin_password_reset_done")


class PasswordResetDoneView(
    PasswordResetEnabledViewMixin, auth_views.PasswordResetDoneView
):
    template_name = "shatailadmin/account/password_reset/done.html"


class PasswordResetConfirmView(
    PasswordResetEnabledViewMixin, auth_views.PasswordResetConfirmView
):
    template_name = "shatailadmin/account/password_reset/confirm.html"
    success_url = reverse_lazy("shatailadmin_password_reset_complete")


class PasswordResetCompleteView(
    PasswordResetEnabledViewMixin, auth_views.PasswordResetCompleteView
):
    template_name = "shatailadmin/account/password_reset/complete.html"


class LoginView(auth_views.LoginView):
    template_name = "shatailadmin/login.html"

    def get_success_url(self):
        return self.get_redirect_url() or reverse("shatailadmin_home")

    def get(self, *args, **kwargs):
        # If user is already logged in, redirect them to the dashboard
        if self.request.user.is_authenticated and self.request.user.has_perm(
            "shatailadmin.access_admin"
        ):

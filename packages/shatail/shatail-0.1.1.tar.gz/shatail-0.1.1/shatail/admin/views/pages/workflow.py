...
from shatail.models import Page, Task, TaskState, WorkflowState
...
class BaseWorkflowFormView(View):
    ...
    def dispatch(self, request, page_id, action_name, task_state_id):
        ...
        if not self.redirect_to or not url_has_allowed_host_and_scheme(
            ...
            self.redirect_to = reverse("shatailadmin_pages:edit", args=[page_id])
    ...
    def render_modal_form(self, request, form):
        return render_modal_workflow(
            request,
            "shatailadmin/pages/workflow_action_modal.html",
            None,
            {
                "page": self.page,
                "form": form,
...
def confirm_workflow_cancellation(request, page_id):
    ...
    if (not workflow_state) or not getattr(
        settings, "SHATAIL_WORKFLOW_CANCEL_ON_PUBLISH", True
    ):
        return render_modal_workflow(
    ...
    return render_modal_workflow(
        request,
        "shatailadmin/pages/confirm_workflow_cancellation.html",
        None,
        {
...
def workflow_status(request, page_id):
    ...
    return render_modal_workflow(
        request,
        "shatailadmin/workflows/workflow_status.html",
        None,
        {
            "page": page,

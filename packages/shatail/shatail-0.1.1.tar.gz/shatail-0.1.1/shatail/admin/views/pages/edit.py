...
from shatail.admin.action_menu import PageActionMenu
from shatail.admin.mail import send_notification
from shatail.admin.side_panels import PageSidePanels
from shatail.admin.views.generic import HookResponseMixin
...
class EditView(TemplateResponseMixin, ContextMixin, HookResponseMixin, View):
    def get_template_names(self):
        if self.page.alias_of_id:
            return ["shatailadmin/pages/edit_alias.html"]

        else:
            return ["shatailadmin/pages/edit.html"]
    ...
    def get_edit_message_button(self):
        return messages.button(
            reverse("shatailadmin_pages:edit", args=(self.page.id,)), _("Edit")
        )

    def get_view_draft_message_button(self):
        return messages.button(
            reverse("shatailadmin_pages:view_draft", args=(self.page.id,)),
            _("View draft"),
            new_window=False,
        )
    ...
    def get(self, request):
        if self.page_perms.user_has_lock():
            ...
            lock_message += format_html(
                ...
                reverse("shatailadmin_pages:unlock", args=(self.page.id,)),
                _("Unlock"),
            )
            ...
            if self.page.locked_by and self.page.locked_at:
                ...
                # Page was probably locked with an old version of Shatail, or a script
                lock_message = format_html(
    ...
    def redirect_away(self):
        if self.next_url:
            ...
            return redirect("shatailadmin_explore", self.page.get_parent().id)

    def redirect_and_remain(self):
        target_url = reverse("shatailadmin_pages:edit", args=[self.page.id])
        if self.next_url:
    ...
    def get_context_data(self, **kwargs):
        ...
        context.update(
            {
                ...
                and getattr(settings, "SHATAIL_WORKFLOW_CANCEL_ON_PUBLISH", True),
                "locale": None,
                "translations": [],
        ...
        if getattr(settings, "SHATAIL_I18N_ENABLED", False):
                    ...
                    "translations": [
                        {
                            ...
                            "url": reverse(
                                "shatailadmin_pages:edit", args=[translation.id]
                            ),
                        }

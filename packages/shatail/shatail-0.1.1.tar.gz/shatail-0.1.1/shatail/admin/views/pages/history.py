...
from shatail.admin.auth import user_has_any_page_permission, user_passes_test
from shatail.admin.filters import DateRangePickerWidget, ShatailFilterSet
from shatail.admin.views.reports import ReportView
from shatail.log_actions import registry as log_action_registry
...
class PageHistoryReportFilterSet(ShatailFilterSet):
    ...
    def filter_hide_commenting_actions(self, queryset, name, value):
        if value:
            queryset = queryset.exclude(action__startswith="shatail.comments")
        return queryset
...
def workflow_history(request, page_id):
    ...
    return TemplateResponse(
        request,
        "shatailadmin/pages/workflow_history/index.html",
        {
            "page": page,
...
def workflow_history_detail(request, page_id, workflow_state_id):
    ...
    return TemplateResponse(
        request,
        "shatailadmin/pages/workflow_history/detail.html",
        {
            "page": page,
            "workflow_state": workflow_state,
            "tasks": tasks,
...
class PageHistoryView(ReportView):
    template_name = "shatailadmin/pages/history.html"
    ...
    def get_queryset(self):
        return PageLogEntry.objects.filter(page=self.page).select_related(
            "revision", "user", "user__shatail_userprofile"
        )

...
from shatail.admin.auth import PermissionPolicyChecker
from shatail.admin.forms.workflows import (
    TaskChooserSearchForm,
    WorkflowPagesFormSet,
    get_task_form_class,
    get_workflow_edit_handler,
)
from shatail.admin.modal_workflow import render_modal_workflow
from shatail.admin.views.generic import CreateView, DeleteView, EditView, IndexView
from shatail.coreutils import resolve_model_string
...
from shatail.permissions import task_permission_policy, workflow_permission_policy
from shatail.workflows import get_task_types
...
class Index(IndexView):
    ...
    template_name = "shatailadmin/workflows/index.html"
    add_url_name = "shatailadmin_workflows:add"
    edit_url_name = "shatailadmin_workflows:edit"
    index_url_name = "shatailadmin_workflows:index"
...
class Create(CreateView):
    ...
    template_name = "shatailadmin/workflows/create.html"
...
class Edit(EditView):
    ...
    template_name = "shatailadmin/workflows/edit.html"
    success_message = _("Workflow '{0}' updated.")
    add_url_name = "shatailadmin_workflows:add"
    edit_url_name = "shatailadmin_workflows:edit"
    delete_url_name = "shatailadmin_workflows:disable"
    delete_item_label = _("Disable")
    index_url_name = "shatailadmin_workflows:index"
    enable_item_label = _("Enable")
    enable_url_name = "shatailadmin_workflows:enable"
    header_icon = "tasks"
    edit_handler = None
    MAX_PAGES = 5
...
class Disable(DeleteView):
    ...
    template_name = "shatailadmin/workflows/confirm_disable.html"
...
def usage(request, pk):
    ...
    return render(
        request,
        "shatailadmin/workflows/usage.html",
        {
            "workflow": workflow,
            "used_by": pages,
        },
...
def enable_workflow(request, pk):
    ...
    if redirect_to and url_has_allowed_host_and_scheme(
        ...
        return redirect("shatailadmin_workflows:edit", workflow.id)
...
def remove_workflow(request, page_pk, workflow_pk=None):
    ...
    if redirect_to and url_has_allowed_host_and_scheme(
        ...
        return redirect("shatailadmin_explore", page.id)
...
class TaskIndex(IndexView):
    ...
    template_name = "shatailadmin/workflows/task_index.html"
    add_url_name = "shatailadmin_workflows:select_task_type"
    edit_url_name = "shatailadmin_workflows:edit_task"
    index_url_name = "shatailadmin_workflows:task_index"
...
def select_task_type(request):
    ...
    if len(task_types) == 1:
        ...
        return redirect("shatailadmin_workflows:add_task", app_label, model_name)

    return render(
        request,
        "shatailadmin/workflows/select_task_type.html",
        {
...
class CreateTask(CreateView):
    ...
    template_name = "shatailadmin/workflows/create_task.html"
    success_message = _("Task '{0}' created.")
    add_url_name = "shatailadmin_workflows:add_task"
    edit_url_name = "shatailadmin_workflows:edit_task"
    index_url_name = "shatailadmin_workflows:task_index"
    header_icon = "thumbtack"
...
class EditTask(EditView):
    ...
    template_name = "shatailadmin/workflows/edit_task.html"
    success_message = _("Task '{0}' updated.")
    add_url_name = "shatailadmin_workflows:select_task_type"
    edit_url_name = "shatailadmin_workflows:edit_task"
    delete_url_name = "shatailadmin_workflows:disable_task"
    index_url_name = "shatailadmin_workflows:task_index"
    delete_item_label = _("Disable")
    enable_item_label = _("Enable")
    enable_url_name = "shatailadmin_workflows:enable_task"
    header_icon = "thumbtack"
...
class DisableTask(DeleteView):
    ...
    template_name = "shatailadmin/workflows/confirm_disable_task.html"
...
def enable_task(request, pk):
    ...
    if redirect_to and url_has_allowed_host_and_scheme(
        ...
        return redirect("shatailadmin_workflows:edit_task", task.id)
...
def get_task_chosen_response(request, task):
    ...
    result_data = {
        "id": task.id,
        "name": task.name,
        "edit_url": reverse("shatailadmin_workflows:edit_task", args=[task.id]),
    }
...
class BaseTaskChooserView(TemplateView):
    ...
    def get_task_listing_context_data(self):
        ...
        if search_form.is_searching():
            # Note: I decided not to use shatailsearch here. This is because
            # shatailsearch creates a new index for each model you make
    ...
    def get_create_tab_context_data(self):
        return {
            ...
            "add_url": reverse("shatailadmin_workflows:task_chooser_create")
            + "?"
...
class TaskChooserView(BaseTaskChooserView):
    ...
    def render_to_response(self, context):
        ...
        return render_modal_workflow(
            self.request,
            "shatailadmin/workflows/task_chooser/chooser.html",
            None,
            context,
...
class TaskChooserCreateView(BaseTaskChooserView):
    ...
    def render_to_response(self, context):
        tab_html = render_to_string(
            "shatailadmin/workflows/task_chooser/includes/create_tab.html",
            context,
            self.request,
        )
...
class TaskChooserResultsView(BaseTaskChooserView):
    template_name = "shatailadmin/workflows/task_chooser/includes/results.html"

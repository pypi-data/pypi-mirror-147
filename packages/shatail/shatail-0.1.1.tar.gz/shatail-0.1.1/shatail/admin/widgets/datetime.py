...
from shatail.admin.datetimepicker import to_datetimepicker_format
...
class AdminDateInput(widgets.DateInput):
    template_name = "shatailadmin/widgets/date_input.html"
    ...
    def __init__(self, attrs=None, format=None):
        ...
        if fmt is None:
            fmt = getattr(settings, "SHATAIL_DATE_FORMAT", DEFAULT_DATE_FORMAT)
    ...
    def media(self):
        return forms.Media(
            js=[
                versioned_static("shatailadmin/js/date-time-chooser.js"),
            ]
        )


class AdminDateInputAdapter(WidgetAdapter):
    js_constructor = "shatail.widgets.AdminDateInput"
...
class AdminTimeInput(widgets.TimeInput):
    template_name = "shatailadmin/widgets/time_input.html"
    ...
    def __init__(self, attrs=None, format=None):
        ...
        fmt = format
        if fmt is None:
            fmt = getattr(settings, "SHATAIL_TIME_FORMAT", DEFAULT_TIME_FORMAT)
        self.js_format = to_datetimepicker_format(fmt)
        super().__init__(attrs=default_attrs, format=fmt)
...
class AdminTimeInputAdapter(WidgetAdapter):
    js_constructor = "shatail.widgets.AdminTimeInput"
...
class AdminDateTimeInput(widgets.DateTimeInput):
    template_name = "shatailadmin/widgets/datetime_input.html"
    ...
    def __init__(self, attrs=None, format=None, time_format=None):
        ...
        if attrs:
            default_attrs.update(attrs)
        if fmt is None:
            fmt = getattr(settings, "SHATAIL_DATETIME_FORMAT", DEFAULT_DATETIME_FORMAT)
        time_fmt = time_format
        if time_fmt is None:
            time_fmt = getattr(settings, "SHATAIL_TIME_FORMAT", DEFAULT_TIME_FORMAT)
        self.js_format = to_datetimepicker_format(fmt)
...
class AdminDateTimeInputAdapter(WidgetAdapter):
    js_constructor = "shatail.widgets.AdminDateTimeInput"

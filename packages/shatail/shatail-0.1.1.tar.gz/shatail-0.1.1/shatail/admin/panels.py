...
from shatail.admin import compare
from shatail.admin.forms.comments import CommentForm
from shatail.admin.staticfiles import versioned_static
from shatail.admin.templatetags.shatailadmin_tags import avatar_url, user_display_name
from shatail.admin.ui.components import Component
from shatail.admin.widgets import AdminPageChooser
from shatail.blocks import BlockField
from shatail.coreutils import camelcase_to_underscore
from shatail.models import COMMENTS_RELATION_NAME, Page
from shatail.utils.decorators import cached_classmethod
...
from .forms.models import (  # NOQA
    ...
    ShatailAdminModelForm,
    formfield_for_dbfield,
)
from .forms.pages import ShatailAdminPageForm


def get_form_for_model(
    model,
    form_class=ShatailAdminModelForm,
    **kwargs,
):
    """
...
class Panel:
    """
    Defines part (or all) of the edit form interface for pages and other models within the Shatail
    ...
    # RemovedInShatail50Warning - edit handlers should override get_form_options instead
    ...
    def get_form_class(self):
        ...
        # If that is not defined, use ShatailAdminModelForm.
        model_form_class = getattr(self.model, "base_form_class", ShatailAdminModelForm)
...
class EditHandler(Panel):
    def __init__(self, *args, **kwargs):
        warn(
            "shatail.admin.edit_handlers.EditHandler has been renamed to shatail.admin.panels.Panel",
            category=RemovedInShatail50Warning,
            stacklevel=2,
        )
...
class BaseCompositeEditHandler(PanelGroup):
    def __init__(self, *args, **kwargs):
        warn(
            "shatail.admin.edit_handlers.BaseCompositeEditHandler has been renamed to shatail.admin.panels.PanelGroup",
            category=RemovedInShatail50Warning,
            stacklevel=2,
        )
        super().__init__(*args, **kwargs)
...
class TabbedInterface(PanelGroup):
    class BoundPanel(PanelGroup.BoundPanel):
        template_name = "shatailadmin/panels/tabbed_interface.html"


class ObjectList(PanelGroup):
    class BoundPanel(PanelGroup.BoundPanel):
        template_name = "shatailadmin/panels/object_list.html"


class FieldRowPanel(PanelGroup):
    class BoundPanel(PanelGroup.BoundPanel):
        template_name = "shatailadmin/panels/field_row_panel.html"
...
class MultiFieldPanel(PanelGroup):
    ...
    class BoundPanel(PanelGroup.BoundPanel):
        template_name = "shatailadmin/panels/multi_field_panel.html"
...
class HelpPanel(Panel):
    def __init__(
        self,
        content="",
        template="shatailadmin/panels/help_panel.html",
        heading="",
        classname="",
    ):
...
class FieldPanel(Panel):
    ...
    class BoundPanel(Panel.BoundPanel):
        object_template_name = "shatailadmin/panels/single_field_panel.html"
        field_template_name = "shatailadmin/panels/field_panel_field.html"
...
class RichTextFieldPanel(FieldPanel):
    def __init__(self, *args, **kwargs):
        warn(
            "shatail.admin.edit_handlers.RichTextFieldPanel is obsolete and should be replaced by shatail.admin.panels.FieldPanel",
            category=RemovedInShatail50Warning,
            stacklevel=2,
        )
        super().__init__(*args, **kwargs)
...
class BaseChooserPanel(FieldPanel):
    def __init__(self, *args, **kwargs):
        warn(
            "shatail.admin.edit_handlers.BaseChooserPanel is obsolete and should be replaced by shatail.admin.panels.FieldPanel",
            category=RemovedInShatail50Warning,
            stacklevel=2,
        )
        super().__init__(*args, **kwargs)
...
class InlinePanel(Panel):
    ...
    class BoundPanel(Panel.BoundPanel):
        template_name = "shatailadmin/panels/inline_panel.html"
...
class PrivacyModalPanel(Panel):
    ...
    class BoundPanel(Panel.BoundPanel):
        template_name = "shatailadmin/pages/privacy_switch_panel.html"
        ...
        @cached_property
        def media(self):
            return Media(js=[versioned_static("shatailadmin/js/privacy-switch.js")])
...
class CommentPanel(Panel):
    def get_form_options(self):
        ...
        return {
            ...
            # Note, this field is defined directly on ShatailAdminPageForm.
    ...
    class BoundPanel(Panel.BoundPanel):
        template_name = "shatailadmin/panels/comments/comment_panel.html"
        ...
        def get_context_data(self, parent_context=None):
            ...
            authors = {
                ...
                .select_related("shatail_userprofile")
            }
...
# Now that we've defined panels, we can set up shatailcore.Page to have some.
def set_default_page_edit_handlers(cls):
    ...
    if getattr(settings, "SHATAILADMIN_COMMENTS_ENABLED", True):
        cls.settings_panels.append(CommentPanel())

    cls.base_form_class = ShatailAdminPageForm
...
def get_edit_handler(cls):
    """
    Get the panel to use in the Shatail admin when editing this page type.
    """
...
def reset_page_edit_handler_cache(**kwargs):
    """
    Clear page edit handler cache when global SHATAILADMIN_COMMENTS_ENABLED settings are changed
    """
    if kwargs["setting"] == "SHATAILADMIN_COMMENTS_ENABLED":
        set_default_page_edit_handlers(Page)
...
class StreamFieldPanel(FieldPanel):
    def __init__(self, *args, **kwargs):
        warn(
            "shatail.admin.edit_handlers.StreamFieldPanel is obsolete and should be replaced by shatail.admin.panels.FieldPanel",
            category=RemovedInShatail50Warning,
            stacklevel=2,
        )
        super().__init__(*args, **kwargs)

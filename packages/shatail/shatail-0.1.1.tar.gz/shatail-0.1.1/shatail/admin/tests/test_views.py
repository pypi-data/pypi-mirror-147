...
class TestLoginView(TestCase, ShatailTestUtils):
    fixtures = ["test.json"]
    ...
    def test_success_redirect_honour_redirect_get_parameter(self):
        homepage_admin_url = reverse("shatailadmin_pages:edit", args=[self.homepage.pk])
        login_url = reverse("shatailadmin_login") + "?next={}".format(
            homepage_admin_url
        )
        response = self.client.post(
            login_url,
    ...
    @override_settings(
        SHATAILADMIN_USER_LOGIN_FORM="shatail.admin.tests.test_forms.CustomLoginForm"
    )

...
from shatail.models import PAGE_TEMPLATE_VAR, Page
from shatail.test.testapp.models import BusinessChild, BusinessIndex
from shatail.test.utils import ShatailTestUtils


class TestUserbarTag(TestCase, ShatailTestUtils):
    def setUp(self):
    ...
    def test_userbar_tag(self):
        template = Template("{% load shatailuserbar %}{% shatailuserbar %}")
        ...
        self.assertIn("<!-- Shatail user bar embed code -->", content)

    def test_userbar_does_not_break_without_request(self):
        template = Template("{% load shatailuserbar %}{% shatailuserbar %}boom")
...
class TestUserbarFrontend(TestCase, ShatailTestUtils):
    def setUp(self):
        self.login()
        self.homepage = Page.objects.get(id=2)
    ...
    def test_userbar_frontend(self):
        response = self.client.get(
            reverse("shatailadmin_userbar_frontend", args=(self.homepage.id,))
        )

        self.assertEqual(response.status_code, 200)
        self.assertTemplateUsed(response, "shatailadmin/userbar/base.html")
...
class TestUserbarAddLink(TestCase, ShatailTestUtils):
    fixtures = ["test.json"]
    ...
    def test_page_allowing_subpages(self):
        response = self.client.get(
            reverse("shatailadmin_userbar_frontend", args=(self.event_index.id,))
        )

        # page allows subpages, so the 'add page' button should show
        expected_url = reverse(
            "shatailadmin_pages:add_subpage", args=(self.event_index.id,)
        )
        needle = f"""
            <a href="{expected_url}" target="_parent" role="menuitem">
                <svg class="icon icon-plus shatail-action-icon" aria-hidden="true">
    ...
    def test_page_disallowing_subpages(self):
        response = self.client.get(
            reverse("shatailadmin_userbar_frontend", args=(self.business_child.id,))
        )
...
class TestUserbarModeration(TestCase, ShatailTestUtils):
    def setUp(self):
        self.login()
        self.homepage = Page.objects.get(id=2)
    ...
    def test_userbar_moderation(self):
        response = self.client.get(
            reverse("shatailadmin_userbar_moderation", args=(self.revision.id,))
        )

...
from shatail.admin.forms import ShatailAdminModelForm, ShatailAdminPageForm
from shatail.admin.panels import (
    CommentPanel,
    FieldPanel,
    FieldRowPanel,
    InlinePanel,
...
from shatail.admin.widgets import (
    AdminAutoHeightTextInput,
    AdminDateInput,
    AdminPageChooser,
)
from shatail.models import Comment, CommentReply, Page, Site
from shatail.test.testapp.forms import ValidatedPageForm
...
class TestGetFormForModel(TestCase):
    ...
    def test_get_form_for_model_without_explicit_fields(self):
        ...
        self.assertTrue(issubclass(EventPageForm, ShatailAdminPageForm))
        form = EventPageForm()
    ...
    def test_get_form_for_model_without_formsets(self):
        ...
        # form should be a subclass of ShatailAdminModelForm
        self.assertTrue(issubclass(EventPageForm, ShatailAdminModelForm))
    ...
    def test_direct_form_field_overrides(self):
        ...
        self.assertTrue(issubclass(SimplePageForm, ShatailAdminPageForm))
        ...
        EventPageForm = get_form_for_model(
            EventPage, form_class=ShatailAdminPageForm, fields=["title", "slug", "body"]
        )
...
class TestPageEditHandlers(TestCase):
    ...
    def test_get_edit_handler(self):
        """
        Forms for pages should have a base class of ShatailAdminPageForm.
        """
        edit_handler = EventPage.get_edit_handler()
        EventPageForm = edit_handler.get_form_class()

        # The generated form should inherit from ShatailAdminPageForm
        self.assertTrue(issubclass(EventPageForm, ShatailAdminPageForm))
    ...
    def test_check_invalid_base_form_class(self):
        ...
        invalid_base_form = checks.Error(
            "ValidatedPage.base_form_class does not extend ShatailAdminPageForm",
            hint="Ensure that shatail.admin.tests.test_edit_handlers.BadFormClass extends ShatailAdminPageForm",
            obj=ValidatedPage,
            id="shatailadmin.E001",
        )

        invalid_edit_handler = checks.Error(
            "ValidatedPage.get_edit_handler().get_form_class() does not extend ShatailAdminPageForm",
            hint="Ensure that the panel definition for ValidatedPage creates a subclass of ShatailAdminPageForm",
            obj=ValidatedPage,
            id="shatailadmin.E002",
        )
    ...
    def test_custom_edit_handler_form_class(self):
        ...
        with mock.patch.object(
            ...
            self.assertTrue(issubclass(form_class, ShatailAdminPageForm))
    ...
    def test_repr(self):
        ...
        self.assertIn(
            "model=<class 'shatail.test.testapp.models.ValidatedPage'>",
            handler_repr,
        )
...
class TestFieldPanel(TestCase):
    ...
    def test_repr(self):
        ...
        self.assertIn(
            "model=<class 'shatail.test.testapp.models.EventPage'>", field_panel_repr
        )
...
class TestPageChooserPanel(TestCase):
    ...
    def test_render_js_init(self):
        ...
        expected_js = 'createPageChooser("{id}", {parent}, {{"model_names": ["{model}"], "can_choose_root": false, "user_perms": null}});'.format(
            id="id_page", model="shatailcore.page", parent=self.events_index_page.id
        )
    ...
    def test_target_models(self):
        panel = PageChooserPanel("page", "shatailcore.site").bind_to_model(
            PageChooserModel
        )
...
class TestInlinePanel(TestCase, ShatailTestUtils):
    fixtures = ["test.json"]
    ...
    def test_no_thousand_separators_in_js(self):
        ...
        https://github.com/shatail/shatail/pull/2699
        https://github.com/shatail/shatail/issues/3227
        """
        speaker_object_list = ObjectList(
            [
                InlinePanel(
...
class TestCommentPanel(TestCase, ShatailTestUtils):
    fixtures = ["test.json"]
    ...
    def test_comments_disabled_setting(self):
        """
        Test that the comment panel is missing if SHATAILADMIN_COMMENTS_ENABLED=False
        """
        self.assertFalse(

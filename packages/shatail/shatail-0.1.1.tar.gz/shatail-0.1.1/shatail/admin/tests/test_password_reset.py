...
class TestUserPasswordReset(TestCase, ShatailTestUtils):
    fixtures = ["test.json"]
    ...
    @override_settings(SHATAIL_PASSWORD_RESET_ENABLED=False)
    ...
    def test_password_reset_view_disabled(self):
        ...
        when setting SHATAIL_PASSWORD_RESET_ENABLED is False
        """
    ...
    @override_settings(ROOT_URLCONF="shatail.admin.urls")
    def test_email_found_default_url(self):
        response = self.client.post(
            reverse("shatailadmin_password_reset"), {"email": "siteeditor@example.com"}
        )
    ...
    @override_settings(
        ROOT_URLCONF="shatail.admin.urls", SHATAILADMIN_BASE_URL="http://mysite.com"
    )

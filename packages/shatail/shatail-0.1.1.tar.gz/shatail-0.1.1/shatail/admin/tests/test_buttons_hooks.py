...
from shatail.admin import widgets as shatailadmin_widgets
from shatail.admin.shatail_hooks import page_listing_more_buttons
...
class TestButtonsHooks(TestCase, ShatailTestUtils):
    def setUp(self):
    ...
    def test_register_page_listing_buttons(self):
        def page_listing_buttons(page, page_perms, is_parent=False, next_url=None):
            yield shatailadmin_widgets.PageListingButton(
        ...
        self.assertTemplateUsed(
            response, "shatailadmin/pages/listing/_button_with_dropdown.html"
        )
        self.assertTemplateUsed(response, "shatailadmin/pages/listing/_buttons.html")
    ...
    def test_register_page_listing_more_buttons(self):
        def page_listing_more_buttons(page, page_perms, is_parent=False, next_url=None):
            yield shatailadmin_widgets.Button(
    ...
    def test_custom_button_with_dropdown(self):
        def page_custom_listing_buttons(
            ...
            yield shatailadmin_widgets.ButtonWithDropdownFromHook(
    ...
    def test_register_page_header_buttons(self):
        ...
        with hooks.register_temporarily(
            ...
            response = self.client.get(
                reverse("shatailadmin_pages:edit", args=(self.root_page.id,))
            )

        self.assertEqual(response.status_code, 200)
        self.assertTemplateUsed(
            response, "shatailadmin/pages/listing/_modern_dropdown.html"
        )
    ...
    def test_delete_button_next_url(self):
        ...
        page = self.root_page
        base_url = reverse("shatailadmin_pages:delete", args=[page.id])
        ...
        next_url = reverse("shatailadmin_explore", args=[page.id])
        delete_button = next(
            page_listing_more_buttons(
                page, page_perms, is_parent=False, next_url=next_url
            )

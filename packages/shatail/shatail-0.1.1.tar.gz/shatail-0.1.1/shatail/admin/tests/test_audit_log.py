...
from shatail.models import GroupPagePermission, Page, PageLogEntry, PageViewRestriction
from shatail.test.testapp.models import SimplePage
from shatail.test.utils import ShatailTestUtils


class TestAuditLogAdmin(TestCase, ShatailTestUtils):
    def setUp(self):
    ...
    def _update_page(self, page):
        ...
        with freeze_time(timezone.now() + timedelta(seconds=2)):
            ...
            # lock/unlock
            page.save(user=self.editor, log_action="shatail.lock")
            page.save(user=self.editor, log_action="shatail.unlock")
    ...
    def test_page_history(self):
        ...
        history_url = reverse(
            "shatailadmin_pages:history", kwargs={"page_id": self.hello_page.id}
        )
    ...
    def test_page_history_filters(self):
        ...
        response = self.client.get(history_url + "?action=shatail.edit")
        self.assertEqual(response.status_code, 200)
    ...
    def test_site_history(self):
        ...
        site_history_url = reverse("shatailadmin_reports:site_history")
    ...
    def test_history_with_deleted_user(self):
        ...
        # check site history
        response = self.client.get(reverse("shatailadmin_reports:site_history"))
        self.assertContains(response, expected_deleted_string)
    ...
    def test_edit_form_has_history_link(self):
        ...
        self.login(user=self.editor)
        response = self.client.get(
            reverse("shatailadmin_pages:edit", args=[self.hello_page.id])
        )
        self.assertEqual(response.status_code, 200)
        history_url = reverse("shatailadmin_pages:history", args=[self.hello_page.id])
    ...
    def test_create_and_publish_does_not_log_revision_save(self):
        ...
        self.assertListEqual(
            ...
            ["shatail.publish", "shatail.create"],
        )
    ...
    def test_revert_and_publish_logs_reversion_and_publish(self):
        ...
        self.assertListEqual(
            list(entries),
            ["shatail.publish", "shatail.rename", "shatail.revert", "shatail.create"],
        )

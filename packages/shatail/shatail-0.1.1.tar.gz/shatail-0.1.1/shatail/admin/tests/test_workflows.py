...
from shatail.signals import page_published
from shatail.test.testapp.models import SimplePage, SimpleTask
from shatail.test.utils import ShatailTestUtils
from shatail.users.models import UserProfile
...
class TestWorkflowMenus(TestCase, ShatailTestUtils):
    def setUp(self):
        self.login()
    ...
    @override_settings(SHATAIL_WORKFLOW_ENABLED=False)
...
class TestWorkflowsIndexView(TestCase, ShatailTestUtils):
    def setUp(self):
    ...
    def get(self, params={}):
        return self.client.get(reverse("shatailadmin_workflows:index"), params)

    def test_simple(self):
        response = self.get()
        self.assertEqual(response.status_code, 200)
        self.assertTemplateUsed(response, "shatailadmin/workflows/index.html")
...
class TestWorkflowsCreateView(TestCase, ShatailTestUtils):
    def setUp(self):
        delete_existing_workflows()
        self.login()
    ...
    def get(self, params={}):
        return self.client.get(reverse("shatailadmin_workflows:add"), params)

    def post(self, post_data={}):
        return self.client.post(reverse("shatailadmin_workflows:add"), post_data)

    def test_get(self):
        response = self.get()
        self.assertEqual(response.status_code, 200)
        self.assertTemplateUsed(response, "shatailadmin/workflows/create.html")
    ...
    def test_post(self):
        ...
        self.assertRedirects(response, reverse("shatailadmin_workflows:index"))
...
class TestWorkflowsEditView(TestCase, ShatailTestUtils):
    def setUp(self):
        delete_existing_workflows()
        self.login()
    ...
    def get(self, params={}):
        return self.client.get(
            reverse("shatailadmin_workflows:edit", args=[self.workflow.id]), params
        )

    def post(self, post_data={}):
        return self.client.post(
            reverse("shatailadmin_workflows:edit", args=[self.workflow.id]), post_data
        )

    def test_get(self):
        response = self.get()
        self.assertEqual(response.status_code, 200)
        self.assertTemplateUsed(response, "shatailadmin/workflows/edit.html")
...
class TestRemoveWorkflow(TestCase, ShatailTestUtils):
    fixtures = ["test.json"]
    ...
    def post(self, post_data={}):
        return self.client.post(
            reverse(
                "shatailadmin_workflows:remove", args=[self.page.id, self.workflow.id]
            ),
            post_data,
        )
...
class TestTaskIndexView(TestCase, ShatailTestUtils):
    def setUp(self):
        delete_existing_workflows()
        self.login()
    ...
    def get(self, params={}):
        return self.client.get(reverse("shatailadmin_workflows:task_index"), params)

    def test_simple(self):
        response = self.get()
        self.assertEqual(response.status_code, 200)
        self.assertTemplateUsed(response, "shatailadmin/workflows/task_index.html")
...
class TestCreateTaskView(TestCase, ShatailTestUtils):
    def setUp(self):
        delete_existing_workflows()
        self.login()
    ...
    def get(self, url_kwargs=None, params={}):
        ...
        return self.client.get(
            reverse("shatailadmin_workflows:add_task", kwargs=url_kwargs), params
        )

    def post(self, post_data={}):
        return self.client.post(
            reverse(
                "shatailadmin_workflows:add_task",
                kwargs={
    ...
    def test_get(self):
        response = self.get()
        self.assertEqual(response.status_code, 200)
        self.assertTemplateUsed(response, "shatailadmin/workflows/create_task.html")

    def test_get_with_non_task_model(self):
        response = self.get(
            url_kwargs={"app_label": "shatailcore", "model_name": "Site"}
        )
        self.assertEqual(response.status_code, 404)

    def test_get_with_base_task_model(self):
        response = self.get(
            url_kwargs={"app_label": "shatailcore", "model_name": "Task"}
        )
        self.assertEqual(response.status_code, 404)
    ...
    def test_post(self):
        ...
        # Should redirect back to index
        self.assertRedirects(response, reverse("shatailadmin_workflows:task_index"))
...
class TestSelectTaskTypeView(TestCase, ShatailTestUtils):
    def setUp(self):
        delete_existing_workflows()
        self.login()

    def get(self):
        return self.client.get(reverse("shatailadmin_workflows:select_task_type"))

    def test_get(self):
        response = self.get()
        self.assertEqual(response.status_code, 200)
        self.assertTemplateUsed(
            response, "shatailadmin/workflows/select_task_type.html"
        )
...
class TestEditTaskView(TestCase, ShatailTestUtils):
    def setUp(self):
        delete_existing_workflows()
        self.login()
    ...
    def get(self, params={}):
        return self.client.get(
            reverse("shatailadmin_workflows:edit_task", args=[self.task.id]), params
        )

    def post(self, post_data={}):
        return self.client.post(
            reverse("shatailadmin_workflows:edit_task", args=[self.task.id]), post_data
        )

    def test_get(self):
        response = self.get()
        self.assertEqual(response.status_code, 200)
        self.assertTemplateUsed(response, "shatailadmin/workflows/edit_task.html")
...
class TestSubmitToWorkflow(TestCase, ShatailTestUtils):
    def setUp(self):
        delete_existing_workflows()
    ...
    def submit(self):
        ...
        return self.client.post(
            reverse("shatailadmin_pages:edit", args=(self.page.id,)), post_data
        )
    ...
    def test_submit_for_approval_changes_status_in_header_meta(self):
        edit_url = reverse("shatailadmin_pages:edit", args=(self.page.id,))
        ...
        workflow_status_url = reverse(
            "shatailadmin_pages:workflow_status", args=(self.page.id,)
        )
    ...
    def test_email_send_error(self, mock_fn):
        ...
        self.assertEqual(response.status_code, 302)
        response = self.client.get(reverse("shatailadmin_home"))
...
class TestApproveRejectWorkflow(TestCase, ShatailTestUtils):
    def setUp(self):
        delete_existing_workflows()
        self.submitter = self.create_user(
            username="submitter",
    ...
    @override_settings(SHATAIL_FINISH_WORKFLOW_ACTION="")
    def test_approve_task_and_workflow(self):
        ...
        """
        # Unset SHATAIL_FINISH_WORKFLOW_ACTION - default action should be to publish
        del settings.SHATAIL_FINISH_WORKFLOW_ACTION
        ...
        self.client.post(
            reverse(
                "shatailadmin_pages:workflow_action",
                args=(
                    self.page.id,
                    "approve",
    ...
    def test_workflow_action_get(self):
        ...
        self.assertEqual(response.status_code, 200)
        self.assertTemplateUsed(
            response, "shatailadmin/pages/workflow_action_modal.html"
        )
    ...
    def test_edit_view_workflow_cancellation_not_in_group(self):
        ...
        # Post
        response = self.client.post(
            reverse("shatailadmin_pages:edit", args=(self.page.id,)),
            {"action-cancel-workflow": "True"},
        )
    ...
    def test_collect_workflow_action_data_get(self):
        ...
        response = self.client.get(
            reverse(
                "shatailadmin_pages:collect_workflow_action_data",
                args=(
                    self.page.id,
                    "approve",
                    self.page.current_workflow_task_state.id,
        ...
        self.assertTemplateUsed(response, "shatailadmin/shared/non_field_errors.html")
        html = json.loads(response.content)["html"]
        self.assertTagInHTML(
            '<form action="'
            + reverse(
    ...
    def test_workflow_report(self):
        response = self.client.get(reverse("shatailadmin_reports:workflow"))
        self.assertEqual(response.status_code, 200)
        ...
        response = self.client.get(reverse("shatailadmin_reports:workflow_tasks"))
        self.assertEqual(response.status_code, 200)
        self.assertContains(response, "Hello world!")
    ...
    def test_workflow_report_filtered(self):
        ...
        response = self.client.get(
            reverse("shatailadmin_reports:workflow"), {"reviewable": "true"}
        )
        self.assertEqual(response.status_code, 200)
        self.assertContains(response, "Hello world!")
        self.assertContains(response, "test_workflow")
        ...
        response = self.client.get(
            reverse("shatailadmin_reports:workflow_tasks"), {"reviewable": "true"}
        )
        self.assertEqual(response.status_code, 200)
        self.assertContains(response, "Hello world!")
...
class TestNotificationPreferences(TestCase, ShatailTestUtils):
    def setUp(self):
        delete_existing_workflows()
        self.submitter = self.create_user(
            username="submitter",
    ...
    @override_settings(SHATAILADMIN_NOTIFICATION_INCLUDE_SUPERUSERS=True)
    def test_submitted_email_notifications_sent(self):
        ...
        # with `SHATAILADMIN_NOTIFICATION_INCLUDE_SUPERUSERS`, the superuser should get a task email
        ...
        # with `SHATAILADMIN_NOTIFICATION_INCLUDE_SUPERUSERS`, the superuser should get a workflow email
    ...
    @override_settings(SHATAILADMIN_NOTIFICATION_INCLUDE_SUPERUSERS=False)
    def test_submitted_email_notifications_superuser_settings(self):
        ...
        `SHATAILADMIN_NOTIFICATION_INCLUDE_SUPERUSERS=False`"""
        self.login(self.submitter)
        self.submit()
        ...
        # with `SHATAILADMIN_NOTIFICATION_INCLUDE_SUPERUSERS` off, the superuser should not get a task email
        self.assertNotIn(self.superuser.email, task_submission_emailed_addresses)

        # with `SHATAILADMIN_NOTIFICATION_INCLUDE_SUPERUSERS` off, the superuser should not get a workflow email
...
class TestDisableViews(TestCase, ShatailTestUtils):
    def setUp(self):
        delete_existing_workflows()
        self.submitter = self.create_user(
            username="submitter",
    ...
    def test_disable_workflow(self):
        ...
        response = self.client.post(
            reverse("shatailadmin_workflows:disable", args=(self.workflow.pk,))
        )
        self.assertEqual(response.status_code, 302)
        self.workflow.refresh_from_db()
    ...
    def test_disable_task_view(self):
        ...
        response = self.client.get(
            reverse("shatailadmin_workflows:disable_task", args=(self.task_1.pk,))
        )

        self.assertTemplateUsed(
            response, "shatailadmin/workflows/confirm_disable_task.html"
        )
        self.assertEqual(
        ...
        response = self.client.get(
            reverse("shatailadmin_workflows:disable_task", args=(unused_task.pk,))
        )
    ...
    def test_enable_workflow(self):
        ...
        response = self.client.post(
            reverse("shatailadmin_workflows:enable", args=(self.workflow.pk,))
        )
        self.assertEqual(response.status_code, 302)
        self.workflow.refresh_from_db()
    ...
    def test_enable_task(self):
        ...
        response = self.client.post(
            reverse("shatailadmin_workflows:enable_task", args=(self.task_1.pk,))
        )
        self.assertEqual(response.status_code, 302)
        self.task_1.refresh_from_db()
...
class TestTaskChooserView(TestCase, ShatailTestUtils):
    def setUp(self):
        self.login()
    ...
    def test_get(self):
        response = self.client.get(reverse("shatailadmin_workflows:task_chooser"))

        self.assertEqual(response.status_code, 200)

        self.assertTemplateUsed(
            response, "shatailadmin/workflows/task_chooser/chooser.html"
        )

        # Check that the "select task type" view was shown in the "new" tab
        self.assertTemplateUsed(
            response,
            "shatailadmin/workflows/task_chooser/includes/select_task_type.html",
        )
        self.assertTemplateUsed(
            response, "shatailadmin/workflows/task_chooser/includes/results.html"
        )
        self.assertTemplateNotUsed(
            response, "shatailadmin/workflows/task_chooser/includes/create_form.html"
        )
    ...
    def test_search(self):
        response = self.client.get(
            reverse("shatailadmin_workflows:task_chooser_results") + "?q=foo"
        )
    ...
    def test_pagination(self):
        response = self.client.get(
            reverse("shatailadmin_workflows:task_chooser_results") + "?p=2"
        )
    ...
    def test_get_with_create_model_selected(self):
        response = self.client.get(
            reverse("shatailadmin_workflows:task_chooser_create")
            + "?create_model=shatailcore.GroupApprovalTask"
        )
    ...
    def test_get_with_base_task_create_model_selected(self):
        ...
        response = self.client.get(
            reverse("shatailadmin_workflows:task_chooser_create")
            + "?create_model=shatailcore.Task"
        )

        self.assertEqual(response.status_code, 404)

    @mock.patch("shatail.admin.views.workflows.get_task_types")
        ...
        self.assertEqual(
            response.json(),
            {
                ...
                "result": {
                    ...
                    "edit_url": reverse(
                        "shatailadmin_workflows:edit_task", args=[task.id]
                    ),
                },
            },
        )
    ...
    def test_post_with_single_task_model(self, get_task_types):
        ...
        response = self.client.post(
            reverse("shatailadmin_workflows:task_chooser_create"), self.get_post_data()
        )
...
class TestTaskChooserChosenView(TestCase, ShatailTestUtils):
    def setUp(self):
        delete_existing_workflows()
        self.login()
    ...
    def test_get(self):
        response = self.client.get(
            reverse("shatailadmin_workflows:task_chosen", args=[self.task.id])
        )
        ...
        self.assertEqual(
            response.json(),
            {
                "result": {
                    "edit_url": reverse(
                        "shatailadmin_workflows:edit_task", args=[self.task.id]
                    ),
                    "id": self.task.id,
                    "name": "test_task",
                },
...
class TestWorkflowUsageView(TestCase, ShatailTestUtils):
    def setUp(self):
        self.login()
    ...
    def test_get(self):
        response = self.client.get(
            reverse("shatailadmin_workflows:usage", args=[self.workflow.id])
        )
...
class TestWorkflowStatus(TestCase, ShatailTestUtils):
    def setUp(self):
        delete_existing_workflows()
        self.submitter = self.create_user(
            username="submitter",
        ...
        self.edit_url = reverse("shatailadmin_pages:edit", args=(self.page.id,))
    ...
    def test_workflow_status_modal(self):
        ...
        self.assertIn(reverse("shatailadmin_pages:history", args=(self.page.id,)), html)

        self.assertTemplateUsed(response, "shatailadmin/workflows/workflow_status.html")

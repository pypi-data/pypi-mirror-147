...
from shatail.signals import post_page_move, pre_page_move
from shatail.test.testapp.models import SimplePage
from shatail.test.utils import ShatailTestUtils


class TestPageMove(TestCase, ShatailTestUtils):
    fixtures = ["test.json"]
    ...
    def test_page_move(self):
        response = self.client.get(
            reverse("shatailadmin_pages:move", args=(self.test_page_a.id,))
        )
        self.assertEqual(response.status_code, 200)

    def test_page_move_default_destination(self):
        response = self.client.get(
            reverse("shatailadmin_pages:move", args=(self.test_page_b.id,))
        )
        self.assertEqual(response.status_code, 200)
        ...
        for destination_page in destinations:
            move_url = reverse(
                "shatailadmin_pages:move_choose_destination",
    ...
    def test_user_without_bulk_delete_permission_can_move(self):
        ...
        response = self.client.get(
            reverse("shatailadmin_pages:move", args=(self.unpublished_page.id,))
        )
    ...
    def test_page_move_confirm(self):
        response = self.client.get(
            reverse(
                "shatailadmin_pages:move_confirm",

...
from shatail.admin.views.pages.preview import PreviewOnEdit
from shatail.models import Page
from shatail.test.testapp.models import EventCategory, SimplePage, StreamPage
from shatail.test.utils import ShatailTestUtils


class TestIssue2599(TestCase, ShatailTestUtils):
    """
...
class TestPreview(TestCase, ShatailTestUtils):
    fixtures = ["test.json"]
    ...
    def test_preview_on_create_with_m2m_field(self):
        ...
        preview_session_key = "shatail-preview-tests-eventpage-{}".format(
            self.home_page.id
        )
    ...
    def test_preview_on_edit_with_m2m_field(self):
        preview_url = reverse(
            "shatailadmin_pages:preview_on_edit", args=(self.event_page.id,)
        )
        response = self.client.post(preview_url, self.post_data)
        ...
        # Check the user can refresh the preview
        preview_session_key = "shatail-preview-{}".format(self.event_page.id)
        self.assertIn(preview_session_key, self.client.session)
    ...
    def test_preview_on_edit_expiry(self):
        ...
        with freeze_time(initial_datetime) as frozen_datetime:
            ...
            preview_url = reverse(
                "shatailadmin_pages:preview_on_edit", args=(self.home_page.id,)
            )
            response = self.client.post(preview_url, self.post_data)
            self.assertEqual(response.status_code, 200)
            response = self.client.get(preview_url)
...
class TestDisablePreviewButton(TestCase, ShatailTestUtils):
    """
    ...
    def test_disable_preview_on_edit(self):
        ...
        # preview button is available by default
        response = self.client.get(
            reverse("shatailadmin_pages:edit", args=(simple_page.id,))
        )
        self.assertEqual(response.status_code, 200)

        preview_url = reverse(
            "shatailadmin_pages:preview_on_edit", args=(simple_page.id,)
        )
        self.assertContains(response, '<li class="preview">')
        self.assertContains(response, 'data-action="%s"' % preview_url)
        ...
        # StreamPage has preview_modes = []
        response = self.client.get(
            reverse("shatailadmin_pages:edit", args=(stream_page.id,))
        )
        self.assertEqual(response.status_code, 200)

        preview_url = reverse(
            "shatailadmin_pages:preview_on_edit", args=(stream_page.id,)
        )
        self.assertNotContains(response, '<li class="preview">')
        self.assertNotContains(response, 'data-action="%s"' % preview_url)
    ...
    def test_disable_preview_on_revisions_list(self):
        ...
        response = self.client.get(
            reverse("shatailadmin_pages:history", args=(simple_page.id,))
        )
        preview_url = reverse(
        ...
        # StreamPage has preview_modes = []
        response = self.client.get(
            reverse("shatailadmin_pages:history", args=(stream_page.id,))
        )
        preview_url = reverse(
            "shatailadmin_pages:revisions_view",
    ...
    def disable_preview_in_moderation_list(self):
        ...
        preview_url = reverse(
            "shatailadmin_pages:preview_for_moderation", args=(latest_revision.id,)
        )
        self.assertNotContains(response, '<li class="preview">')
        self.assertNotContains(response, 'data-action="%s"' % preview_url)

...
class TestPageUnpublish(TestCase, ShatailTestUtils):
    ...
    def test_unpublish_view(self):
        ...
        response = self.client.get(
            reverse("shatailadmin_pages:unpublish", args=(self.page.id,))
        )

        # Check that the user received an unpublish confirm page
        self.assertEqual(response.status_code, 200)
        self.assertTemplateUsed(response, "shatailadmin/pages/confirm_unpublish.html")
    ...
    def test_unpublish_view_invalid_page_id(self):
        ...
        # Get unpublish page
        response = self.client.get(
            reverse("shatailadmin_pages:unpublish", args=(12345,))
        )
    ...
    def test_after_unpublish_page(self):
        ...
        with self.register_hook("after_unpublish_page", hook_func):
            post_data = {}
            response = self.client.post(
                reverse("shatailadmin_pages:unpublish", args=(self.page.id,)), post_data
            )
...
class TestPageUnpublishIncludingDescendants(TestCase, ShatailTestUtils):
    def setUp(self):
        self.user = self.login()
        # Find root page
        self.root_page = Page.objects.get(id=2)
    ...
    def test_unpublish_descendants_view(self):
        ...
        # Get unpublish page
        response = self.client.get(
            reverse("shatailadmin_pages:unpublish", args=(self.test_page.id,))
        )

...
from shatail.core.models import Page
from shatail.test.testapp.models import BusinessChild, BusinessIndex, SimplePage
from shatail.tests.utils import ShatailTestUtils


class TestPageReorder(TestCase, ShatailTestUtils):
    fixtures = ["test.json"]
    ...
    def test_page_set_page_position_get_request_with_simple_page(self):
        ...
        response = self.client.get(
            reverse("shatailadmin_pages:set_page_position", args=(self.child_1.id,))
        )
    ...
    def test_page_move_page_position_up(self):
        ...
        response = self.client.post(
            reverse("shatailadmin_pages:set_page_position", args=(self.child_3.id,))
            + "?position=0"
        )
        self.assertEqual(response.status_code, 200)

...
from shatail.models import GroupPagePermission, Locale, Page, PageRevision
...
class TestPageCreation(TestCase, ShatailTestUtils):
    def setUp(self):
        # Find root page
    ...
    def test_add_subpage(self):
        response = self.client.get(
            reverse("shatailadmin_pages:add_subpage", args=(self.root_page.id,))
        )
        self.assertEqual(response.status_code, 200)

        self.assertContains(response, "Simple page")
        target_url = reverse(
            "shatailadmin_pages:add", args=("tests", "simplepage", self.root_page.id)
        )
    ...
    def test_add_subpage_with_subpage_types(self):
        ...
        response = self.client.get(
            reverse("shatailadmin_pages:add_subpage", args=(business_index.id,))
        )
        self.assertEqual(response.status_code, 200)
    ...
    def test_add_subpage_with_one_valid_subpage_type(self):
        ...
        response = self.client.get(
            reverse("shatailadmin_pages:add_subpage", args=(business_subindex.id,))
        )
    ...
    def test_add_subpage_nonexistantparent(self):
        response = self.client.get(
            reverse("shatailadmin_pages:add_subpage", args=(100000,))
        )
    ...
    def test_create_multipart(self):
        ...
        self.assertTemplateUsed(response, "shatailadmin/pages/create.html")

        # check for FilePage which has file field
        response = self.client.get(
            reverse(
                "shatailadmin_pages:add", args=("tests", "filepage", self.root_page.id)
            )
        )
        self.assertEqual(response.status_code, 200)
    ...
    def test_create_nonexistantparent(self):
        response = self.client.get(
            reverse("shatailadmin_pages:add", args=("tests", "simplepage", 100000))
        )
        self.assertEqual(response.status_code, 404)
    ...
    def test_create_nonpagetype(self):
        response = self.client.get(
            reverse(
                "shatailadmin_pages:add",
                args=("shatailimages", "image", self.root_page.id),
            )
        )
        self.assertEqual(response.status_code, 404)
    ...
    def test_preview_on_create(self):
        ...
        preview_url = reverse(
            "shatailadmin_pages:preview_on_add",
            args=("tests", "simplepage", self.root_page.id),
        )
    ...
    @override_settings(SHATAIL_MODERATION_ENABLED=False)
    def test_hide_moderation_button(self):
        """
        Tests that if SHATAIL_MODERATION_ENABLED is set to False, the "Submit for Moderation" button is not shown.
        """
        response = self.client.get(
            reverse(
                "shatailadmin_pages:add",
    ...
    def test_create_sets_locale_to_parent_locale(self):
        ...
        response = self.client.get(
            reverse(
                "shatailadmin_pages:add", args=("tests", "simplepage", fr_homepage.id)
            )
        )
...
class TestPermissionedFieldPanels(TestCase, ShatailTestUtils):
    fixtures = ["test.json"]
...
class TestSubpageBusinessRules(TestCase, ShatailTestUtils):
    def setUp(self):
        # Find root page
        self.root_page = Page.objects.get(id=2)
    ...
    def test_standard_subpage(self):
        add_subpage_url = reverse(
            "shatailadmin_pages:add_subpage", args=(self.standard_index.id,)
        )

        # explorer should contain a link to 'add child page'
        response = self.client.get(
            reverse("shatailadmin_explore", args=(self.standard_index.id,))
        )
        self.assertEqual(response.status_code, 200)
    ...
    def test_business_subpage(self):
        add_subpage_url = reverse(
            "shatailadmin_pages:add_subpage", args=(self.business_index.id,)
        )

        # explorer should contain a link to 'add child page'
        response = self.client.get(
            reverse("shatailadmin_explore", args=(self.business_index.id,))
        )
        self.assertEqual(response.status_code, 200)
        self.assertContains(response, add_subpage_url)
    ...
    def test_business_child_subpage(self):
        add_subpage_url = reverse(
            "shatailadmin_pages:add_subpage", args=(self.business_child.id,)
        )

        # explorer should not contain a link to 'add child page', as this page doesn't accept subpages
        response = self.client.get(
            reverse("shatailadmin_explore", args=(self.business_child.id,))
        )
        self.assertEqual(response.status_code, 200)
        ...
        response = self.client.get(
            reverse("shatailadmin_pages:add_subpage", args=(self.business_child.id,))
        )
        self.assertEqual(response.status_code, 302)
    ...
    def test_not_prompted_for_page_type_when_only_one_choice(self):
        response = self.client.get(
            reverse("shatailadmin_pages:add_subpage", args=(self.business_subindex.id,))
        )
...
class TestInlinePanelMedia(TestCase, ShatailTestUtils):
    ...
    def test_inline_panel_media(self):
        ...
        response = self.client.get(
            reverse("shatailadmin_pages:add", args=("tests", "simplepage", homepage.id))
        )
        self.assertEqual(response.status_code, 200)
...
class TestInlineStreamField(TestCase, ShatailTestUtils):
    """
...
class TestIssue2994(TestCase, ShatailTestUtils):
    """
...
class TestInlinePanelWithTags(TestCase, ShatailTestUtils):
    # https://github.com/shatail/shatail/issues/5414#issuecomment-567080707
...
class TestInlinePanelNonFieldErrors(TestCase, ShatailTestUtils):
    """
    Test that non field errors will render for InlinePanels
    https://github.com/shatail/shatail/issues/3890
    """
...
@override_settings(SHATAIL_I18N_ENABLED=True)
class TestLocaleSelectorOnRootPage(TestCase, ShatailTestUtils):
    fixtures = ["test.json"]

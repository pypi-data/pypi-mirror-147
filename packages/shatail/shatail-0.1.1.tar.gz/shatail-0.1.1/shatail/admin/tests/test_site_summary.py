...
from shatail.models import GroupPagePermission, Page, Site
from shatail.test.testapp.models import SimplePage
from shatail.test.utils import ShatailTestUtils


class TestPagesSummary(TestCase, ShatailTestUtils):
    @classmethod
    def setUpClass(cls):
        super().setUpClass()

        cls.test_page = SimplePage(title="test", slug="test", content="test")
        cls.shatail_root = Page.get_first_root_node()
        cls.shatail_root.add_child(instance=cls.test_page)
    ...
    def assertSummaryContainsLinkToPage(self, page_pk):
        self.assertSummaryContains(reverse("shatailadmin_explore", args=[page_pk]))
    ...
    def test_multiple_sites_summary_links_to_shatail_root(self):
        Site.objects.create(hostname="foo.com", root_page=self.shatail_root)
        self.assertSummaryContainsLinkToPage(self.shatail_root.pk)

    def test_no_sites_summary_links_to_shatail_root(self):
        Site.objects.all().delete()
        self.assertSummaryContainsLinkToPage(self.shatail_root.pk)

    def test_summary_includes_page_count_without_shatail_root(self):
        self.assertSummaryContains(
            "<span>{}</span> Pages".format(Page.objects.count() - 1)
        )

    def test_summary_shows_zero_pages_if_none_exist_except_shatail_root(self):
        Page.objects.exclude(pk=self.shatail_root.pk).delete()

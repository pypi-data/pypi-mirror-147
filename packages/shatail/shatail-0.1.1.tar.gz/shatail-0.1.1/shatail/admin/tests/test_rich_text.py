...
from shatail.admin.rich_text import DraftailRichTextArea, get_rich_text_editor_widget
from shatail.admin.rich_text.converters.editor_html import PageLinkHandler
from shatail.admin.rich_text.editors.draftail.features import Feature
from shatail.blocks import RichTextBlock
from shatail.models import Page, get_page_models
from shatail.rich_text import RichText
from shatail.test.testapp.models import SingleEventPage
from shatail.test.testapp.rich_text import CustomRichTextArea, LegacyRichTextArea
...
class BaseRichTextEditHandlerTestCase(TestCase):
    def _clear_edit_handler_cache(self):
        ...
        """
        from shatail.test.testapp.models import DefaultRichBlockFieldPage
...
class TestGetRichTextEditorWidget(TestCase):
    @override_settings()  # create temporary copy of settings so we can remove SHATAILADMIN_RICH_TEXT_EDITORS
    def test_default(self):
        # Simulate the absence of a setting
        if hasattr(settings, "SHATAILADMIN_RICH_TEXT_EDITORS"):
            del settings.SHATAILADMIN_RICH_TEXT_EDITORS

        self.assertIsInstance(get_rich_text_editor_widget(), DraftailRichTextArea)

    @override_settings(
        SHATAILADMIN_RICH_TEXT_EDITORS={
            "default": {"WIDGET": "shatail.test.testapp.rich_text.CustomRichTextArea"},
        }
    )
    ...
    @override_settings(
        SHATAILADMIN_RICH_TEXT_EDITORS={
            "default": {"WIDGET": "shatail.test.testapp.rich_text.LegacyRichTextArea"},
            "custom": {"WIDGET": "shatail.test.testapp.rich_text.CustomRichTextArea"},
        }
    )
...
class TestDefaultRichText(BaseRichTextEditHandlerTestCase, ShatailTestUtils):
    def setUp(self):
        super().setUp()
        # Find root page
...
@override_settings(
    SHATAILADMIN_RICH_TEXT_EDITORS={
        "default": {
            "WIDGET": "shatail.admin.rich_text.DraftailRichTextArea",
            "OPTIONS": {"features": ["h2", "quotation"]},
        },
    }
)
class TestDraftailFeatureMedia(BaseRichTextEditHandlerTestCase, ShatailTestUtils):
    """
...
class TestOverriddenDefaultRichText(BaseRichTextEditHandlerTestCase, ShatailTestUtils):
    def setUp(self):
        super().setUp()
...
@override_settings(
    SHATAILADMIN_RICH_TEXT_EDITORS={
        "default": {
            "WIDGET": "shatail.admin.tests.test_rich_text.TestCustomDefaultRichText"
        },
        "custom": {"WIDGET": "shatail.test.testapp.rich_text.CustomRichTextArea"},
    }
)
class TestCustomDefaultRichText(BaseRichTextEditHandlerTestCase, ShatailTestUtils):
    def setUp(self):
        super().setUp()
...
class TestDraftailWithFeatureOptions(BaseRichTextEditHandlerTestCase, ShatailTestUtils):
    def setUp(self):
        super().setUp()
...
class TestDraftailWithAdditionalFeatures(
    BaseRichTextEditHandlerTestCase, ShatailTestUtils
):
    def setUp(self):
        super().setUp()
...
class TestDraftailFeature(SimpleTestCase):
    def test_versioned_static_media(self):
        feature = Feature(
            js=["shatailadmin/js/example/feature.js"],
            css={
                "all": ["shatailadmin/css/example/feature.css"],
            },
        )

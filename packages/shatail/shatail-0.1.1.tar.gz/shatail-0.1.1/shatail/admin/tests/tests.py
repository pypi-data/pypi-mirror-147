...
from shatail.admin.mail import send_mail
from shatail.admin.menu import MenuItem
from shatail.models import Page
from shatail.test.testapp.models import RestaurantTag
from shatail.test.utils import ShatailTestUtils


class TestHome(TestCase, ShatailTestUtils):
    def setUp(self):
        # Login
        self.login()
    ...
    def test_simple(self):
        ...
        self.assertContains(response, "Welcome to the Test Site Shatail CMS")
    ...
    def test_admin_menu(self):
        ...
        response = self.client.get(reverse("shatailadmin_home") + "?hide-kittens=true")
    ...
    def test_never_cache_header(self):
        # This tests that shatailadmins global cache settings have been applied correctly
        response = self.client.get(reverse("shatailadmin_home"))
...
class TestEditorHooks(TestCase, ShatailTestUtils):
    def setUp(self):
    ...
    def test_editor_css_hooks_on_add(self):
        response = self.client.get(
            reverse(
                "shatailadmin_pages:add", args=("tests", "simplepage", self.homepage.id)
            )
        )
        self.assertEqual(response.status_code, 200)
        self.assertContains(
    ...
    def test_editor_css_hooks_on_edit(self):
        response = self.client.get(
            reverse("shatailadmin_pages:edit", args=(self.homepage.id,))
        )
        self.assertEqual(response.status_code, 200)
        self.assertContains(
            response, '<link rel="stylesheet" href="/path/to/my/custom.css">'
...
class TestSendMail(TestCase):
    ...
    @override_settings(SHATAILADMIN_NOTIFICATION_FROM_EMAIL="anothertest@email.com")
    def test_send_fallback_to_shatailadmin_notification_from_email_setting(self):
...
class TestTagsAutocomplete(TestCase, ShatailTestUtils):
    def setUp(self):
        self.login()
    ...
    def test_tags_autocomplete(self):
        response = self.client.get(
            reverse("shatailadmin_tag_autocomplete"), {"term": "test"}
        )
    ...
    def test_tags_autocomplete_partial_match(self):
        response = self.client.get(
            reverse("shatailadmin_tag_autocomplete"), {"term": "te"}
        )
    ...
    def test_tags_autocomplete_different_term(self):
        response = self.client.get(
            reverse("shatailadmin_tag_autocomplete"), {"term": "hello"}
        )
    ...
    def test_tags_autocomplete_no_term(self):
        response = self.client.get(reverse("shatailadmin_tag_autocomplete"))
    ...
    def test_tags_autocomplete_custom_model(self):
        response = self.client.get(
            reverse(
                "shatailadmin_tag_model_autocomplete", args=("tests", "restauranttag")
            ),
            {"term": "ital"},
        )
...
class TestMenuItem(TestCase, ShatailTestUtils):
    def setUp(self):
        self.login()
        response = self.client.get(reverse("shatailadmin_home"))
    ...
    def test_menuitem_reverse_lazy_url_pass(self):
        menuitem = MenuItem(_("Test"), reverse_lazy("shatailadmin_home"))
        self.assertIs(menuitem.is_active(self.request), True)


class TestUserPassesTestPermissionDecorator(TestCase, ShatailTestUtils):
    """
...
class TestUserHasAnyPagePermission(TestCase, ShatailTestUtils):
...
class Test404(TestCase, ShatailTestUtils):
    def test_admin_404_template_used_append_slash_true(self):
        ...
        with self.settings(APPEND_SLASH=True):
            ...
            self.assertTemplateUsed(response, "shatailadmin/404.html")
    ...
    def test_not_logged_in_redirect(self):
        ...
        self.assertRedirects(
            response, reverse("shatailadmin_login") + "?next=/admin/sdfgdsfgdsfgsdf/"
        )


class TestAdminURLAppendSlash(TestCase, ShatailTestUtils):
    def setUp(self):
        # Find root page
    ...
    def test_return_correct_view_for_correct_url_without_ending_slash(self):
        ...
        with self.settings(APPEND_SLASH=True):
            ...
            response = self.client.get(
                reverse("shatailadmin_explore_root")[:-1], follow=True
            )

...
from shatail.admin.templatetags.shatailadmin_tags import (
    avatar_url,
    i18n_enabled,
    locale_label_from_id,
)
from shatail.admin.templatetags.shatailadmin_tags import locales as locales_tag
from shatail.admin.templatetags.shatailadmin_tags import (
    notification_static,
...
class TestAvatarTemplateTag(TestCase, ShatailTestUtils):
    ...
    @override_settings(SHATAIL_GRAVATAR_PROVIDER_URL="https://robohash.org")
    def test_custom_gravatar_provider(self):
        url = avatar_url(self.test_user)
        self.assertIn("robohash.org", url)

    @override_settings(SHATAIL_GRAVATAR_PROVIDER_URL=None)
...
class TestNotificationStaticTemplateTag(TestCase):
    ...
    def test_local_notification_static(self):
        url = notification_static("shatailadmin/images/email-header.jpg")
        self.assertEqual(
            "{}/static/shatailadmin/images/email-header.jpg".format(
                settings.SHATAILADMIN_BASE_URL
            ),
            url,
        )

    @override_settings(
        STATIC_URL="/static/", SHATAILADMIN_BASE_URL="http://localhost:8000"
    )
    def test_local_notification_static_baseurl(self):
        url = notification_static("shatailadmin/images/email-header.jpg")
        self.assertEqual(
            "http://localhost:8000/static/shatailadmin/images/email-header.jpg", url
        )

    @override_settings(
        STATIC_URL="https://s3.amazonaws.com/somebucket/static/",
        SHATAILADMIN_BASE_URL="http://localhost:8000",
    )
    def test_remote_notification_static(self):
        url = notification_static("shatailadmin/images/email-header.jpg")
        self.assertEqual(
            "https://s3.amazonaws.com/somebucket/static/shatailadmin/images/email-header.jpg",
            url,
        )
...
class TestVersionedStatic(TestCase):
    def test_versioned_static(self):
        result = versioned_static("shatailadmin/js/core.js")
        self.assertRegex(result, r"^/static/shatailadmin/js/core.js\?v=(\w+)$")

    @mock.patch("shatail.admin.staticfiles.static")
    def test_versioned_static_version_string(self, mock_static):
        mock_static.return_value = "/static/shatailadmin/js/core.js?v=123"
        result = versioned_static("shatailadmin/js/core.js")
        self.assertEqual(result, "/static/shatailadmin/js/core.js?v=123")
        mock_static.assert_called_once_with("shatailadmin/js/core.js")

    def test_versioned_static_absolute_path(self):
        result = versioned_static("/static/shatailadmin/js/core.js")
        self.assertEqual(result, "/static/shatailadmin/js/core.js")

    def test_versioned_static_url(self):
        result = versioned_static("http://example.org/static/shatailadmin/js/core.js")
        self.assertEqual(result, "http://example.org/static/shatailadmin/js/core.js")
...
class TestComponentTag(TestCase):
    def test_passing_context_to_component(self):
        ...
        template = Template(
            "{% load shatailadmin_tags %}{% with first_name='Kilroy' %}{% component my_component %}{% endwith %}"
        )
    ...
    def test_component_escapes_unsafe_strings(self):
        ...
        template = Template(
            "{% load shatailadmin_tags %}<h1>{% component my_component %}</h1>"
        )
        html = template.render(Context({"my_component": MyComponent()}))
        self.assertEqual(
...
class TestInternationalisationTags(TestCase):
    ...
    def test_i18n_enabled(self):
        with override_settings(SHATAIL_I18N_ENABLED=False):
            self.assertFalse(i18n_enabled())

        with override_settings(SHATAIL_I18N_ENABLED=True):

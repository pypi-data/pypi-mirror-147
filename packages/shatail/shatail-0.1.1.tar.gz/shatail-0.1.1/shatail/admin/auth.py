...
from shatail.log_actions import LogContext
from shatail.models import GroupPagePermission
...
def permission_denied(request):
    ...
    return redirect("shatailadmin_home")
...
def permission_required(permission_name):
    ...
    (The latter doesn't work anyway because Shatail doesn't define LOGIN_URL...)
    """
...
class PermissionPolicyChecker:
    ...
    returning the shatailadmin 'permission denied' response if permission not granted
    """
...
def reject_request(request):
    ...
    # shatail.admin.auth, specifically where custom user models are involved
    from django.contrib.auth.views import redirect_to_login as auth_redirect_to_login

    return auth_redirect_to_login(
        request.get_full_path(), login_url=reverse("shatailadmin_login")
    )
...
def require_admin_access(view_func):
    def decorated_view(request, *args, **kwargs):
        ...
        if user.has_perms(["shatailadmin.access_admin"]):
            try:
                preferred_language = None
                if hasattr(user, "shatail_userprofile"):
                    preferred_language = (
                        user.shatail_userprofile.get_preferred_language()
                    )
                    l18n.set_language(preferred_language)
                    time_zone = user.shatail_userprofile.get_current_time_zone()

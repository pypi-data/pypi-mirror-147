...
from shatail.admin.userbar import (
    AddPageItem,
    AdminItem,
...
from shatail.models import PAGE_TEMPLATE_VAR, Page, PageRevision
...
def shatailuserbar(context, position="bottom-right"):
    # Find request object
    try:
    ...
    if not user.has_perm("shatailadmin.access_admin"):
        return ""
    ...
    with translation.override(userprofile.get_preferred_language()):
        ...
        for fn in hooks.get_hooks("construct_shatail_userbar"):
            fn(request, items)
        ...
        return render_to_string(
            "shatailadmin/userbar/base.html",
            {
                "request": request,

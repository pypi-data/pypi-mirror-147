...
from shatail.images.formats import get_image_format
from shatail.images.forms import ImageInsertionForm, get_image_form
from shatail.images.permissions import permission_policy
from shatail.images.utils import find_image_duplicates
from shatail.search import index as search_index

permission_checker = PermissionPolicyChecker(permission_policy)

CHOOSER_PAGE_SIZE = getattr(settings, "SHATAILIMAGES_CHOOSER_PAGE_SIZE", 12)
...
def get_image_result_data(image):
    ...
    return {
        "id": image.id,
        "edit_link": reverse("shatailimages:edit", args=(image.id,)),
        "title": image.title,
        "preview": {
...
class ChooseView(BaseChooseView):
    ...
    def render_to_response(self):
        return render_modal_workflow(
            self.request,
            "shatailimages/chooser/chooser.html",
            None,
            ...
            json_data={
                ...
                "tag_autocomplete_url": reverse("shatailadmin_tag_autocomplete"),
            },
        )
...
class ChooseResultsView(BaseChooseView):
    def render_to_response(self):
        return TemplateResponse(
            self.request, "shatailimages/chooser/results.html", self.get_context_data()
        )
...
def duplicate_found(request, new_image, existing_image):
    next_step_url = (
        "shatailimages:chooser_select_format"
        if request.GET.get("select_format")
        else "shatailimages:image_chosen"
    )
    ...
    cancel_duplicate_upload_action = (
        f"{reverse('shatailimages:delete', args=(new_image.id,))}?"
        f"{urlencode({'next': choose_existing_image_url})}"
    )

    duplicate_upload_html = render_to_string(
        "shatailimages/chooser/confirm_duplicate_upload.html",
        {
            "new_image": new_image,
...
def chooser_upload(request):
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            if request.GET.get("select_format"):
                ...
                return render_modal_workflow(
                    request,
                    "shatailimages/chooser/select_format.html",
                    None,
    ...
    upload_form_html = render_to_string(
        "shatailimages/chooser/upload_form.html",
        {
            "form": form,

...
from shatail.images.models import Rendition, SourceImageIOError, get_rendition_storage
from shatail.images.rect import Rect
...
class TestImagePermissions(TestCase, ShatailTestUtils):
    def setUp(self):
...
class TestRenditions(TestCase):
    ...
    def test_minification(self):
        ...
        # via the Rendition.filter_spec attribute (in active use as of Shatail 1.8)
    ...
    def test_custom_rendition_backend_setting(self):
        """
        Test the usage of SHATAILIMAGES_RENDITION_STORAGE setting.
        """
        ...
        self.assertFalse(hasattr(settings, "SHATAILIMAGES_RENDITION_STORAGE"))
        ...
        setattr(
            settings,
            "SHATAILIMAGES_RENDITION_STORAGE",
            "shatail.images.tests.test_models.CustomStorage",
        )
        ...
        setattr(settings, "SHATAILIMAGES_RENDITION_STORAGE", CustomStorage2())
        backend = get_rendition_storage()
...
@override_settings(_SHATAILSEARCH_FORCE_AUTO_UPDATE=["elasticsearch"])
class TestIssue613(TestCase, ShatailTestUtils):
    def get_elasticsearch_backend(self):
        from django.conf import settings

        from shatail.search.backends import get_search_backend

        if "elasticsearch" not in settings.SHATAILSEARCH_BACKENDS:
    ...
    def add_image(self, **params):
        ...
        response = self.client.post(reverse("shatailimages:add"), post_data)
...
class TestRenditionOrientation(TestCase):
    ...
    def test_jpeg_with_orientation_1(self):
        with open("shatail/images/tests/image_files/landscape_1.jpg", "rb") as f:
    ...
    def test_jpeg_with_orientation_2(self):
        with open("shatail/images/tests/image_files/landscape_2.jpg", "rb") as f:
            image = Image.objects.create(title="Test image", file=File(f))
    ...
    def test_jpeg_with_orientation_3(self):
        with open("shatail/images/tests/image_files/landscape_3.jpg", "rb") as f:
            image = Image.objects.create(title="Test image", file=File(f))
    ...
    def test_jpeg_with_orientation_4(self):
        with open("shatail/images/tests/image_files/landscape_4.jpg", "rb") as f:
            image = Image.objects.create(title="Test image", file=File(f))
    ...
    def test_jpeg_with_orientation_5(self):
        with open("shatail/images/tests/image_files/landscape_6.jpg", "rb") as f:
            image = Image.objects.create(title="Test image", file=File(f))
    ...
    def test_jpeg_with_orientation_7(self):
        with open("shatail/images/tests/image_files/landscape_7.jpg", "rb") as f:
            image = Image.objects.create(title="Test image", file=File(f))
    ...
    def test_jpeg_with_orientation_8(self):
        with open("shatail/images/tests/image_files/landscape_8.jpg", "rb") as f:
            image = Image.objects.create(title="Test image", file=File(f))

...
def has_jpeg_support():
    shatail_jpg = os.path.join(os.path.dirname(__file__), "check_files", "shatail.jpg")
    succeeded = True

    with open(shatail_jpg, "rb") as f:
        try:
            Image.open(f)
...
@lru_cache()
def has_png_support():
    shatail_png = os.path.join(os.path.dirname(__file__), "check_files", "shatail.png")
    succeeded = True

    with open(shatail_png, "rb") as f:
        try:
            Image.open(f)
        except (IOError, Image.LoaderError):
            succeeded = False

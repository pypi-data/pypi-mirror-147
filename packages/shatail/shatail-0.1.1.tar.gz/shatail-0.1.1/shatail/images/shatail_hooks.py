...
from shatail.images import admin_urls, get_image_model, image_operations
from shatail.images.api.admin.views import ImagesAdminAPIViewSet
from shatail.images.forms import GroupImagePermissionFormSet
from shatail.images.permissions import permission_policy
from shatail.images.rich_text import ImageEmbedHandler
from shatail.images.rich_text.contentstate import ContentstateImageConversionRule
from shatail.images.rich_text.editor_html import EditorHTMLImageConversionRule
from shatail.images.views.bulk_actions import (
    AddTagsBulkAction,
...
def register_admin_urls():
    return [
        path("images/", include(admin_urls, namespace="shatailimages")),
    ]
        ...
        draftail_features.EntityFeature(
            {
                ...
                # Keep only images which are from Shatail.
                "whitelist": {
                    "id": True,
                },
            },
            js=[
                "shatailimages/js/image-chooser-modal.js",
            ],
        ),
    )
...
class ImagesSummaryItem(SummaryItem):
    order = 200
    template_name = "shatailimages/homepage/site_summary_images.html"
...
def describe_collection_docs(collection):
    ...
    if images_count:
        url = reverse("shatailimages:index") + ("?collection_id=%d" % collection.id)
        return {
            "count": images_count,
...
class ImageAdminURLFinder(ModelAdminURLFinder):
    edit_url_name = "shatailimages:edit"

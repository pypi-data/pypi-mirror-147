...
class Migration(migrations.Migration):
    ...
    operations = [
        ...
        # New step introduced in Shatail 1.8.1:
        #
        # Reduce max_length of rendition.focal_point_key to 16, from the previous value of 255
        # which existed on Shatail <= 1.8. MySQL has a limit of 767 (on InnoDB) or 1000 (on MyISAM)
        ...
        #   failed on shatailimages/0001_initial can now run (see #2925 / #2953);
        # * Here, so that previously-working Shatail <=1.7 installations that failed on the
        #   AlterUniqueTogether below when upgrading to 1.8 can now succeed;
        # * In the newly-added migration shatailimages/0017, so that existing Shatail 1.8 installations

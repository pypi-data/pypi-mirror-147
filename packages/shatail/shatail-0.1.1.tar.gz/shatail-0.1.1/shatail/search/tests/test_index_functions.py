...
@mock.patch("shatail.search.tests.DummySearchBackend", create=True)
@override_settings(
    SHATAILSEARCH_BACKENDS={
        "default": {"BACKEND": "shatail.search.tests.DummySearchBackend"}
    }
)
class TestInsertOrUpdateObject(TestCase, ShatailTestUtils):
    ...
    def test_catches_index_error(self, backend):
        ...
        with self.assertLogs("shatail.search.index", level="ERROR") as cm:
            index.insert_or_update_object(obj)
...
class TestRemoveObject(TestCase, ShatailTestUtils):
...
class TestSignalHandlers(TestCase, ShatailTestUtils):

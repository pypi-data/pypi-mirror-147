...
from shatail.search.backends import (
    InvalidSearchBackendError,
    get_search_backend,
    get_search_backends,
)
from shatail.search.backends.base import BaseSearchBackend, FieldError, FilterFieldError
from shatail.search.backends.database.fallback import DatabaseSearchBackend
from shatail.search.backends.database.sqlite.utils import fts5_available
from shatail.search.models import IndexEntry
from shatail.search.query import (
    MATCH_ALL,
    MATCH_NONE,
    And,
    Boost,
...
from shatail.test.search import models
from shatail.test.utils import ShatailTestUtils


class BackendTests(ShatailTestUtils):
    # To test a specific backend, subclass BackendTests and define self.backend_path.

    fixtures = ["search"]

    def setUp(self):
        # Search SHATAILSEARCH_BACKENDS for an entry that uses the given backend path
        for backend_name, backend_conf in settings.SHATAILSEARCH_BACKENDS.items():
            ...
            raise unittest.SkipTest(
                "No SHATAILSEARCH_BACKENDS entry for the backend %s" % self.backend_path
            )
...
@override_settings(
    SHATAILSEARCH_BACKENDS={"default": {"BACKEND": "shatail.search.backends.database"}}
)
class TestBackendLoader(TestCase):
    @mock.patch("shatail.search.backends.database.connection")
    ...
    def test_import_by_path_unknown_db_vendor(self, connection):
        connection.vendor = "unknown"
        db = get_search_backend(backend="shatail.search.backends.database")
        self.assertIsInstance(db, DatabaseSearchBackend)
    ...
    def test_import_by_full_path_unknown_db_vendor(self, connection):
        ...
        db = get_search_backend(
            backend="shatail.search.backends.database.SearchBackend"
        )
        self.assertIsInstance(db, DatabaseSearchBackend)
    ...
    def test_import_by_name_postgres_db_vendor(self):
        from shatail.search.backends.database.postgres.postgres import (
    ...
    def test_import_by_name_mysql_db_vendor(self):
        from shatail.search.backends.database.mysql.mysql import MySQLSearchBackend
    ...
    def test_import_by_name_sqlite_db_vendor(self):
        ...
        if not fts5_available():
            ...
            from shatail.search.backends.database.sqlite.sqlite import (
                SQLiteSearchBackend,
            )
    ...
    def test_nonexistent_backend_import(self):
        self.assertRaises(
            InvalidSearchBackendError,
            get_search_backend,
            backend="shatail.search.backends.doesntexist",
        )
    ...
    @override_settings(SHATAILSEARCH_BACKENDS={})
    ...
    @override_settings(
        SHATAILSEARCH_BACKENDS={
            "default": {"BACKEND": "shatail.search.backends.database"},
            "another-backend": {"BACKEND": "shatail.search.backends.database"},
        }
    )

...
from shatail.snippets import urls
from shatail.snippets.models import get_snippet_models
from shatail.snippets.permissions import (
    get_permission_name,
    user_can_edit_snippet_type,
    user_can_edit_snippets,
)
from shatail.snippets.widgets import SnippetListingButton


@hooks.register("register_admin_urls")
def register_admin_urls():
    return [
        path("snippets/", include(urls, namespace="shatailsnippets")),
    ]
...
def register_snippets_menu_item():
    return SnippetsMenuItem(
        _("Snippets"), reverse("shatailsnippets:index"), icon_name="snippet", order=500
    )

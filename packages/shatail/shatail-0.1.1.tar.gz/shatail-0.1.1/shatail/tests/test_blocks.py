...
from shatail import blocks
from shatail.blocks.field_block import FieldBlockAdapter
from shatail.blocks.list_block import ListBlockAdapter
from shatail.blocks.static_block import StaticBlockAdapter
from shatail.blocks.stream_block import StreamBlockAdapter
from shatail.blocks.struct_block import StructBlockAdapter
from shatail.models import Page
from shatail.rich_text import RichText
from shatail.test.testapp.blocks import LinkBlock as CustomLinkBlock
from shatail.test.testapp.blocks import SectionBlock
from shatail.test.testapp.models import EventPage, SimplePage
...
class TestFieldBlock(ShatailTestUtils, SimpleTestCase):
...
class TestRichTextBlock(TestCase):
    ...
    def test_adapter(self):
        from shatail.test.testapp.rich_text import CustomRichTextArea
    ...
    def test_adapter_with_draftail(self):
        from shatail.admin.rich_text import DraftailRichTextArea
    ...
    def test_get_searchable_content(self):
        ...
        value = RichText(
            ...
            "<p>Our Santa pet <b>Shatail</b> has some cool stuff in store for you all!</p>"
        )
        ...
        self.assertEqual(
            ...
            [
                ...
                "Our Santa pet Shatail has some cool stuff in store for you all!"
            ],
        )
...
class TestChoiceBlock(ShatailTestUtils, SimpleTestCase):
    def setUp(self):
...
class TestMultipleChoiceBlock(ShatailTestUtils, SimpleTestCase):
    def setUp(self):
        from django.db.models.fields import BLANK_CHOICE_DASH
    ...
    def test_subclassing(self):
        ...
        self.assertEqual(
            block.deconstruct(),
            (
                "shatail.blocks.MultipleChoiceBlock",
                [],
                {
                    "choices": [("tea", "Tea"), ("coffee", "Coffee")],
                    "required": False,
...
class TestStructBlock(SimpleTestCase):
    ...
    def test_get_api_representation_calls_same_method_on_fields_with_context(self):
        ...
        api_representation = block.get_api_representation(
            ...
            context={"en": "English", "shatail": "Shatail!"},
        )

        self.assertDictEqual(
            api_representation, {"language": "English", "author": "Shatail!"}
        )
    ...
    def test_render_unknown_field(self):
        ...
        self.assertIn("<dd>Shatail site</dd>", html)
        self.assertIn("<dt>link</dt>", html)
        self.assertIn("<dd>http://www.shatail.org</dd>", html)
    ...
    def test_get_form_context(self):
        ...
        self.assertEqual(context["children"]["title"].value, "Shatail site")
        self.assertIsInstance(context["children"]["link"], blocks.BoundBlock)
        self.assertEqual(context["children"]["link"].value, "http://www.shatail.org")
    ...
    def test_searchable_content(self):
        ...
        self.assertEqual(content, ["Shatail site"])
    ...
    def test_str_structvalue(self):
        ...
        https://github.com/shatail/shatail/issues/2874
        ...
        # "StructValue([('title', u'Hello'), ('body', <shatail.rich_text.RichText object at 0xb12d5eed>)])"
...
class TestListBlock(ShatailTestUtils, SimpleTestCase):
    ...
    def test_searchable_content(self):
        ...
        self.assertEqual(content, ["Shatail", "Django"])
...
class TestStreamBlock(ShatailTestUtils, SimpleTestCase):
    def test_initialisation(self):
        ...
        api_representation = block.get_api_representation(
            block.to_python(
                [
                    ...
                    {"type": "author", "value": "shatail", "id": "111111"},
                ]
            ),
            context={"en": "English", "shatail": "Shatail!"},
        )
...
class TestPageChooserBlock(TestCase):
    ...
    def test_adapt(self):
        from shatail.admin.widgets.chooser import AdminPageChooser
    ...
    def test_deconstruct_target_model_default(self):
        block = blocks.PageChooserBlock()
        self.assertEqual(
            block.deconstruct(), ("shatail.blocks.PageChooserBlock", (), {})
        )
    ...
    def test_deconstruct_target_model_string(self):
        ...
        self.assertEqual(
            block.deconstruct(),
            (
                "shatail.blocks.PageChooserBlock",
                (),
...
class TestDateBlock(TestCase):
    def test_adapt(self):
        from shatail.admin.widgets.datetime import AdminDateInput
...
class TestTimeBlock(TestCase):
    def test_adapt(self):
        from shatail.admin.widgets.datetime import AdminTimeInput
...
class TestDateTimeBlock(TestCase):
    def test_adapt(self):
        from shatail.admin.widgets.datetime import AdminDateTimeInput
...
class TestSystemCheck(TestCase):
    def test_name_cannot_contain_non_alphanumeric(self):
        ...
        self.assertEqual(errors[0].id, "shatailcore.E001")
        self.assertEqual(
            errors[0].hint,
    ...
    def test_system_checks_recurse_into_structs(self):
        ...
        self.assertEqual(errors[1].id, "shatailcore.E001")

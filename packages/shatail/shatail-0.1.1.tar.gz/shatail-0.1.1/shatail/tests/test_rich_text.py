...
from shatail.rich_text import RichText, expand_db_html
from shatail.rich_text.feature_registry import FeatureRegistry
from shatail.rich_text.pages import PageLinkHandler
from shatail.rich_text.rewriters import LinkRewriter, extract_attrs
from shatail.test.testapp.models import EventPage
...
class TestExpandDbHtml(TestCase):
    ...
    @patch("shatail.embeds.embeds.get_embed")
...
class TestRichTextValue(TestCase):
    ...
    def test_evaluate_value(self):
        ...
        value = RichText("<p>shatail</p>")
        self.assertTrue(value)


class TestFeatureRegistry(TestCase):
    def test_register_rich_text_features_hook(self):
        # testapp/shatail_hooks.py defines a 'blockquote' rich text feature with a Draftail
...
class TestLinkRewriterTagReplacing(TestCase):
    def test_should_follow_default_behaviour(self):
        ...
        external_type_link = rewriter('<a href="https://shatail.org/">')
        self.assertEqual(external_type_link, '<a href="https://shatail.org/">')
        email_type_link = rewriter('<a href="mailto:test@shatail.org">')
        self.assertEqual(email_type_link, '<a href="mailto:test@shatail.org">')
        ...
        link_without_linktype = rewriter('<a data-link="https://shatail.org">')
        self.assertEqual(link_without_linktype, '<a data-link="https://shatail.org">')
        ...
        link_with_custom_linktype = rewriter(
            '<a linktype="custom" href="https://shatail.org">'
        )
        self.assertNotEqual(link_with_custom_linktype, '<a href="https://shatail.org">')
    ...
    def test_supported_type_should_follow_given_rules(self):
        ...
        external_type_link = rewriter('<a href="https://shatail.org/">')
        self.assertEqual(
            external_type_link, '<a rel="nofollow" href="https://shatail.org/">'
        )
        external_type_link_http = rewriter('<a href="http://shatail.org/">')
        self.assertEqual(
            external_type_link_http, '<a rel="nofollow" href="http://shatail.org/">'
        )
        email_type_link = rewriter('<a href="mailto:test@shatail.org">')
        self.assertEqual(
            email_type_link, '<a data-email="true" href="mailto:test@shatail.org">'
        )
        anchor_type_link = rewriter('<a href="#test">')
...
class TestRichTextField(TestCase):
    ...
    def test_get_searchable_content(self):
        ...
        christmas_page.body = '<p><b>Merry Christmas from <a href="https://shatail.org/">Shatail!</a></b> &amp; co.</p>'
        ...
        self.assertEqual(result, ["Merry Christmas from Shatail! & co."])

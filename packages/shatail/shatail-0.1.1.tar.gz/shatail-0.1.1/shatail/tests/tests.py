...
from shatail.models import Locale, Page, Site, SiteRootPath
from shatail.templatetags.shatailcore_tags import richtext, slugurl
...
class TestPageUrlTags(TestCase):
    ...
    def test_pageurl_with_named_url_fallback(self):
        tpl = template.Template(
            """{% load shatailcore_tags %}<a href="{% pageurl page fallback='fallback' %}">Fallback</a>"""
        )
    ...
    def test_pageurl_with_get_absolute_url_object_fallback(self):
        ...
        tpl = template.Template(
            """{% load shatailcore_tags %}<a href="{% pageurl page fallback=object_with_url_method %}">Fallback</a>"""
        )
        result = tpl.render(
            template.Context(
    ...
    def test_pageurl_with_invalid_url_string_fallback(self):
        ...
        """
        tpl = template.Template(
            """{% load shatailcore_tags %}<a href="{% pageurl page fallback='not-existing-endpoint' %}">Fallback</a>"""
        )
    ...
    def test_pageurl_without_request_in_context(self):
        ...
        tpl = template.Template(
            """{% load shatailcore_tags %}<a href="{% pageurl page %}">{{ page.title }}</a>"""
        )
...
class TestShatailSiteTag(TestCase):
    fixtures = ["test.json"]

    def test_shatail_site_tag(self):
        request = HttpRequest()
        request.META["HTTP_HOST"] = "localhost"
        request.META["SERVER_PORT"] = 80

        tpl = template.Template(
            """{% load shatailcore_tags %}{% shatail_site as current_site %}{{ current_site.hostname }}"""
        )
        result = tpl.render(template.Context({"request": request}))
        self.assertEqual("localhost", result)

    def test_shatail_site_tag_with_missing_request_context(self):
        tpl = template.Template(
            """{% load shatailcore_tags %}{% shatail_site as current_site %}{{ current_site.hostname }}"""
        )
...
class TestSiteRootPathsCache(TestCase):
    ...
    def test_cache(self):
        ...
        self.assertEqual(
            cache.get("shatail_site_root_paths"),
            [
                SiteRootPath(
                    site_id=1,
                    root_path="/home/",
    ...
    def test_cache_clears_when_site_saved(self):
        ...
        self.assertTrue(cache.get("shatail_site_root_paths"))

        # Save the site
        Site.objects.get(is_default_site=True).save()

        # Check that the cache has been cleared
        self.assertFalse(cache.get("shatail_site_root_paths"))
    ...
    def test_cache_clears_when_site_root_moves(self):
        ...
        The issue was caused by the 'shatail_site_root_paths' cache
        ...
        Discussion: https://github.com/shatail/shatail/issues/7
        """
    ...
    def test_cache_clears_when_site_root_slug_changes(self):
        ...
        Fix: d6cce69a397d08d5ee81a8cbc1977ab2c9db2682
        Discussion: https://github.com/shatail/shatail/issues/157
        """
        # Get homepage
        homepage = Page.objects.get(url_path="/home/")
...
class TestResolveModelString(TestCase):
    def test_resolve_from_string(self):
        model = resolve_model_string("shatailcore.Page")

        self.assertEqual(model, Page)

    def test_resolve_from_string_with_default_app(self):
        model = resolve_model_string("Page", default_app="shatailcore")

        self.assertEqual(model, Page)

    def test_resolve_from_string_with_different_default_app(self):
        model = resolve_model_string("shatailcore.Page", default_app="shatailadmin")
    ...
    def test_resolve_from_string_invalid(self):
        self.assertRaises(ValueError, resolve_model_string, "shatail.core.Page")

    def test_resolve_from_string_with_incorrect_default_app(self):
        self.assertRaises(
            LookupError, resolve_model_string, "Page", default_app="shatailadmin"
        )

    def test_resolve_from_string_with_unknown_model_string(self):
        self.assertRaises(LookupError, resolve_model_string, "shatailadmin.Page")

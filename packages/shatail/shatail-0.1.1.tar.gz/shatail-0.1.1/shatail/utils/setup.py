...
from shatail import __semver__
...
class assets_mixin:
    ...
    def bump_client_version(self):
        """
        Writes the current Shatail version number into package.json
        """
...
class check_bdist_egg(bdist_egg):
    ...
    sentinel_dir = "shatail/shatailadmin/static/"
        ...
        if not os.path.isdir(self.sentinel_dir):
            print(
                "\n".join(
                    [  # noqa
                        "************************************************************",
                        "The front end assets for Shatail are missing.",

...
from shatail.embeds import oembed_providers
from shatail.embeds.blocks import EmbedBlock, EmbedValue
from shatail.embeds.embeds import get_embed, get_embed_hash
from shatail.embeds.exceptions import (
    EmbedNotFoundException,
    EmbedUnsupportedProviderException,
)
from shatail.embeds.finders import get_finders
from shatail.embeds.finders.embedly import (
    AccessDeniedEmbedlyException,
    EmbedlyException,
)
from shatail.embeds.finders.embedly import EmbedlyFinder as EmbedlyFinder
from shatail.embeds.finders.facebook import AccessDeniedFacebookOEmbedException
from shatail.embeds.finders.facebook import FacebookOEmbedFinder as FacebookOEmbedFinder
from shatail.embeds.finders.instagram import AccessDeniedInstagramOEmbedException
from shatail.embeds.finders.instagram import (
    InstagramOEmbedFinder as InstagramOEmbedFinder,
)
from shatail.embeds.finders.oembed import OEmbedFinder as OEmbedFinder
from shatail.embeds.models import Embed
from shatail.embeds.templatetags.shatailembeds_tags import embed_tag
...
class TestGetFinders(TestCase):
    ...
    # New SHATAILEMBEDS_FINDERS setting

    @override_settings(
        SHATAILEMBEDS_FINDERS=[{"class": "shatail.embeds.finders.oembed"}]
    )
    ...
    @override_settings(
        SHATAILEMBEDS_FINDERS=[
            {
                "class": "shatail.embeds.finders.embedly",
                "key": "foo",
            }
        ]
    )
    ...
    @override_settings(
        SHATAILEMBEDS_FINDERS=[
            {"class": "shatail.embeds.finders.oembed", "options": {"foo": "bar"}}
        ]
    )
    ...
    @override_settings(
        SHATAILEMBEDS_FINDERS=[
            {
                "class": "shatail.embeds.finders.instagram",
                "app_id": "1234567890",
    ...
    @override_settings(
        SHATAILEMBEDS_FINDERS=[
            {
                "class": "shatail.embeds.finders.facebook",
                "app_id": "1234567890",
                "app_secret": "abcdefghijklmnop",
            },
        ]
...
class TestEmbeds(TestCase):
    ...
    @override_settings(SHATAILEMBEDS_RESPONSIVE_HTML=True)
    ...
    @override_settings(SHATAILEMBEDS_FINDERS=[])
...
class TestChooser(TestCase, ShatailTestUtils):
    def setUp(self):
        # login
        self.login()
    ...
    def test_submit_valid_embed(self, get_embed):
        ...
        response = self.client.post(
            reverse("shatailembeds:chooser_upload"),
...
class TestEmbedTag(TestCase):
    ...
    def test_call_from_template(self, get_embed):
        ...
        temp = template.Template(
            '{% load shatailembeds_tags %}{% embed "http://www.youtube.com/watch/" %}'
        )

...
from shatail.admin.panels import FieldPanel, ObjectList, TabbedInterface
from shatail.contrib.settings.registry import SettingMenuItem
from shatail.contrib.settings.views import get_setting_edit_handler
...
class TestSettingMenu(TestCase, ShatailTestUtils):
    ...
    def test_menu_item_in_admin(self):
        ...
        self.assertContains(
            response, reverse("shatailsettings:edit", args=("tests", "testsetting"))
        )
...
class BaseTestSettingView(TestCase, ShatailTestUtils):
    ...
    def edit_url(self, setting, site_pk=1):
        ...
        return reverse("shatailsettings:edit", args=args)
...
class TestSettingEditView(BaseTestSettingView):
    ...
    def test_non_existant_model(self):
        response = self.client.get(
            reverse("shatailsettings:edit", args=["test", "foo", 1])
        )
    ...
    def test_get_edit_current_site(self):
        url = reverse("shatailsettings:edit", args=("tests", "testsetting"))
        default_site = Site.objects.get(is_default_site=True)
...
class TestMultiSite(BaseTestSettingView):
    ...
    def test_redirect_to_default(self):
        ...
        """
        start_url = reverse("shatailsettings:edit", args=["tests", "testsetting"])
        dest_url = reverse(
            "shatailsettings:edit", args=["tests", "testsetting", self.default_site.pk]
        )
    ...
    def test_redirect_to_current(self):
        ...
        start_url = reverse("shatailsettings:edit", args=["tests", "testsetting"])
        dest_url = reverse(
            "shatailsettings:edit", args=["tests", "testsetting", self.other_site.pk]
        )
        response = self.client.get(
...
class TestAdminPermission(TestCase, ShatailTestUtils):

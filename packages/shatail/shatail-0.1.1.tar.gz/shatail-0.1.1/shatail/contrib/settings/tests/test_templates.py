...
class TemplateTestCase(SettingsTestMixin, TestCase, ShatailTestUtils):
...
class TestTemplateTag(TemplateTestCase):
    ...
    def test_get_settings_variable_assignment_request_context(self):
        ...
        {% get_settings as shatail_settings %} works.
        """
        request = self.get_request(site=self.other_site)
        context = Context({"request": request})
        template = Template(
            "{% load shatailsettings_tags %}"
            "{% get_settings as shatail_settings %}"
            "{{ shatail_settings.tests.testsetting.title}}"
        )
    ...
    def test_get_settings_variable_assigment_use_default(self):
        """
        Check that assigning the setting to a context variable with
        {% get_settings use_default_site=True as shatail_settings %} works.
        """
        context = Context()
        template = Template(
            "{% load shatailsettings_tags %}"
            "{% get_settings use_default_site=True as shatail_settings %}"
            "{{ shatail_settings.tests.testsetting.title }}"
        )

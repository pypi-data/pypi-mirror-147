...
from shatail.models import GroupPagePermission, Page
from shatail.test.testapp.models import BusinessIndex, EventCategory, EventPage
from shatail.test.utils import ShatailTestUtils


class TestIndexView(TestCase, ShatailTestUtils):
    ...
    def test_using_core_page(self):
        ...
        response = self.client.get("/admin/shatailcore/page/")
        self.assertEqual(response.status_code, 200)
...
class TestExcludeFromExplorer(TestCase, ShatailTestUtils):
...
class TestEditView(TestCase, ShatailTestUtils):
    ...
    def test_using_core_page(self):
        # The core page is slightly different to other pages, so exclude it
        root_page = Page.objects.get(depth=1)
        response = self.client.get("/admin/shatailcore/page/{}/".format(root_page.id))
        self.assertEqual(response.status_code, 404)


class TestDeleteView(TestCase, ShatailTestUtils):
    fixtures = ["test_specific.json"]
...
class TestChooseParentView(TestCase, ShatailTestUtils):
    fixtures = ["test_specific.json"]
...
class TestChooseParentViewForNonSuperuser(TestCase, ShatailTestUtils):
    fixtures = ["test_specific.json"]
...
class TestModeratorAccess(TestCase, ShatailTestUtils):
    fixtures = ["test_specific.json"]
...
class TestSearch(TestCase, ShatailTestUtils):
    fixtures = ["test_specific.json"]

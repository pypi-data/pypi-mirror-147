...
from shatail.admin.panels import FieldPanel, TabbedInterface
from shatail.contrib.modeladmin.helpers.search import DjangoORMSearchHandler
...
from shatail.test.modeladmintest.models import (
    Author,
    Book,
    Publisher,
    RelatedLink,
...
from shatail.test.modeladmintest.shatail_hooks import BookModelAdmin
from shatail.test.utils import ShatailTestUtils


class TestBookIndexView(TestCase, ShatailTestUtils):
...
class TestAuthorIndexView(TestCase, ShatailTestUtils):
    fixtures = ["modeladmintest_test.json"]
...
class TestTranslatableBookIndexView(TestCase, ShatailTestUtils):
    fixtures = ["modeladmintest_test.json"]
...
class TestCreateView(TestCase, ShatailTestUtils):
    fixtures = ["modeladmintest_test.json"]
    ...
    def test_exclude_passed_to_extract_panel_definitions(self):
        path_to_form_fields_exclude_property = (
            "shatail.contrib.modeladmin.options.ModelAdmin.form_fields_exclude"
        )
        with mock.patch(
            "shatail.contrib.modeladmin.options.extract_panel_definitions_from_model_class"
        ) as m:
            with mock.patch(
    ...
    def test_clean_form_once(self):
        with mock.patch(
            "shatail.test.modeladmintest.shatail_hooks.PublisherModelAdminForm.clean"
        ) as mock_form_clean:
    ...
    def test_create_view_with_multifieldpanel(self):
        # https://github.com/shatail/shatail/issues/6413
...
@override_settings(SHATAIL_I18N_ENABLED=True)
class TestTranslatableCreateView(TestCase, ShatailTestUtils):
    fixtures = ["modeladmintest_test.json"]
...
class TestInspectView(TestCase, ShatailTestUtils):
    fixtures = ["modeladmintest_test.json"]
...
class TestEditView(TestCase, ShatailTestUtils):
    fixtures = ["modeladmintest_test.json"]
...
@override_settings(SHATAIL_I18N_ENABLED=True)
class TestTranslatableBookEditView(TestCase, ShatailTestUtils):
    fixtures = ["modeladmintest_test.json"]
...
class TestPageSpecificViews(TestCase, ShatailTestUtils):
    fixtures = ["modeladmintest_test.json"]
...
class TestConfirmDeleteView(TestCase, ShatailTestUtils):
    fixtures = ["modeladmintest_test.json"]
...
class TestDeleteViewWithProtectedRelation(TestCase, ShatailTestUtils):
    fixtures = ["modeladmintest_test.json"]
...
class TestDeleteViewModelReprPrimary(TestCase, ShatailTestUtils):
    fixtures = ["modeladmintest_test.json"]
...
class TestEditorAccess(TestCase, ShatailTestUtils):
    fixtures = ["modeladmintest_test.json"]
...
class TestHistoryView(TestCase, ShatailTestUtils):
    fixtures = ["modeladmintest_test.json"]
...
class TestQuoting(TestCase, ShatailTestUtils):
    fixtures = ["modeladmintest_test.json"]
...
class TestHeaderBreadcrumbs(TestCase, ShatailTestUtils):
    """
    Test that the <ul class="breadcrumbs">... is inserted within the
    <header> tag for potential future regression.
    See https://github.com/shatail/shatail/issues/3889
    """
    ...
    def test_choose_inspect_model(self):
        ...
        self.assertTemplateUsed(response, "shatailadmin/shared/header.html")

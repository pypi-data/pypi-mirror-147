...
from shatail.contrib.routable_page.templatetags.shatailroutablepage_tags import (
    routablepageurl,
)
from shatail.models import Page, Site
from shatail.test.routablepage.models import (
    RoutablePageTest,
...
class TestRoutablePageTemplateTag(TestCase):
    ...
    def test_templatetag_reverse_external_view_without_append_slash(self):
        with mock.patch("shatail.models.SHATAIL_APPEND_SLASH", False):
            url = routablepageurl(
                self.context, self.routable_page, "external_view", "joe-bloggs"
            )

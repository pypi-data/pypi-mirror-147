...
from shatail.actions.copy_for_translation import ParentNotTranslatedError
from shatail.contrib.simple_translation.forms import SubmitTranslationForm
from shatail.contrib.simple_translation.models import after_create_page
from shatail.contrib.simple_translation.views import (
    SubmitPageTranslationView,
    SubmitSnippetTranslationView,
    SubmitTranslationView,
)
from shatail.models import Locale, Page
from shatail.test.i18n.models import TestPage
from shatail.test.snippets.models import TranslatableSnippet
from shatail.test.utils import TestCase, ShatailTestUtils
...
class TestSubmitTranslationView(ShatailTestUtils, TestCase):
    def setUp(self):
...
class TestSubmitPageTranslationView(ShatailTestUtils, TestCase):
    def setUp(self):
        self.en_locale = Locale.objects.first()
        self.fr_locale = Locale.objects.create(language_code="fr")
        self.de_locale = Locale.objects.create(language_code="de")
    ...
    def test_get_subtitle(self):
        ...
        view.object = self.en_homepage
        self.assertEqual(view.get_subtitle(), "Welcome to your new Shatail site!")
    ...
    def test_submit_page_translation_view_test_post_single_locale(self):
        ...
        self.assertRedirects(
            response, reverse("shatailadmin_pages:edit", args=[translated_page.pk])
        )
...
class TestSubmitSnippetTranslationView(ShatailTestUtils, TestCase):
    def setUp(self):
        self.en_locale = Locale.objects.first()
        self.fr_locale = Locale.objects.create(language_code="fr")
...
@override_settings(
    ...
    SHATAILSIMPLETRANSLATION_SYNC_PAGE_TREE=True,
)
class TestPageTreeSync(ShatailTestUtils, TestCase):
    def setUp(self):
        self.en_locale = Locale.objects.first()
        self.fr_locale = Locale.objects.create(language_code="fr")
        self.de_locale = Locale.objects.create(language_code="de")
    ...
    @override_settings(SHATAILSIMPLETRANSLATION_SYNC_PAGE_TREE=False)

...
logger = logging.getLogger("shatail.frontendcache")
...
def get_backends(backend_settings=None, backends=None):
    # Get backend settings from SHATAILFRONTENDCACHE setting
    if backend_settings is None:
        backend_settings = getattr(settings, "SHATAILFRONTENDCACHE", None)

    # Fallback to using SHATAILFRONTENDCACHE_LOCATION setting (backwards compatibility)
    if backend_settings is None:
        cache_location = getattr(settings, "SHATAILFRONTENDCACHE_LOCATION", None)
...
def purge_urls_from_cache(urls, backend_settings=None, backends=None):
    # Convert each url to urls one for each managed language (SHATAILFRONTENDCACHE_LANGUAGES setting).
    # The managed languages are common to all the defined backends.
    # This depends on settings.USE_I18N
    # If SHATAIL_I18N_ENABLED is True, this defaults to SHATAIL_CONTENT_LANGUAGES
    shatail_i18n_enabled = getattr(settings, "SHATAIL_I18N_ENABLED", False)
    content_languages = get_content_languages() if shatail_i18n_enabled else {}
    languages = getattr(
        settings, "SHATAILFRONTENDCACHE_LANGUAGES", list(content_languages.keys())
    )
...
class PurgeBatch:
    ...
    def purge(self, backend_settings=None, backends=None):
        ...
        - backend_settings can be used to override the SHATAILFRONTENDCACHE setting for
          just this call

...
from shatail.admin.views.reports import ReportView
from shatail.contrib.redirects import models
from shatail.contrib.redirects.base_formats import DEFAULT_FORMATS
from shatail.contrib.redirects.filters import RedirectsReportFilterSet
from shatail.contrib.redirects.forms import ConfirmImportForm, ImportForm, RedirectForm
from shatail.contrib.redirects.permissions import permission_policy
from shatail.contrib.redirects.utils import (
    get_file_storage,
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            with transaction.atomic():
                form.save()
                log(instance=theredirect, action="shatail.edit")
            messages.success(
                request,
                _("Redirect '{0}' updated.").format(theredirect.title),
                buttons=[
                    messages.button(
                        reverse("shatailredirects:edit", args=(theredirect.id,)),
                        _("Edit"),
                    )
                ],
            )
            return redirect("shatailredirects:index")
        else:
...
def delete(request, redirect_id):
    ...
    if request.method == "POST":
        with transaction.atomic():
            log(instance=theredirect, action="shatail.delete")
            theredirect.delete()
        messages.success(
    ...
    return TemplateResponse(
        request,
        "shatailredirects/confirm_delete.html",
        {
            "redirect": theredirect,
        },
    )
...
def start_import(request):
    ...
    if not request.FILES or not form.is_valid():
        return render(
            request,
            "shatailredirects/choose_import_file.html",
            {
                "search_form": SearchForm(
                    data={"q": query_string} if query_string else None,
                    placeholder=_("Search redirects"),
    ...
    if extension not in supported_extensions:
        ...
        return redirect("shatailredirects:start_import")
    ...
    return render(
        request,
        "shatailredirects/confirm_import.html",
        {
...
def process_import(request):
    ...
    if import_summary["errors_count"] > 0:
        return render(
            request,
            "shatailredirects/import_summary.html",
            {
                "form": ImportForm(supported_extensions),
...
def create_redirects_from_dataset(dataset, config):
    ...
    for row in dataset:
        ...
        with transaction.atomic():
            redirect = form.save()
            log(instance=redirect, action="shatail.create")
        successes += 1
...
class RedirectsReportView(ReportView):
    ...
    template_name = "shatailredirects/reports/redirects_report.html"

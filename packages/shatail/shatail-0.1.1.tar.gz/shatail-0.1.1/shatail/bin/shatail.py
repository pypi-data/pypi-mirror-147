...
if CURRENT_PYTHON < REQUIRED_PYTHON:
    sys.stderr.write(
        "This version of Shatail requires Python {}.{} or above - you are running {}.{}\n".format(
...
class CreateProject(Command):
    description = "Creates the directory structure for a new Shatail project."

    def add_arguments(self, parser):
        parser.add_argument("project_name", help="Name for your Shatail project")
        parser.add_argument(
            "dest_dir",
            nargs="?",
    ...
    def run(self, project_name=None, dest_dir=None):
        ...
        print(
            "Creating a Shatail project called %(project_name)s"
            % {"project_name": project_name}
        )  # noqa

        # Create the project from the Shatail template using startapp

        # First find the path to Shatail
        import shatail

        shatail_path = os.path.dirname(shatail.__file__)
        template_path = os.path.join(shatail_path, "project_template")
...
class UpdateModulePaths(Command):
    description = "Update a Shatail project tree to use Shatail 2.x module paths"

    REPLACEMENTS = [
        # Added in Shatail 2.0
        (re.compile(r"\bshatail\.shatailcore\b"), "shatail"),
        (re.compile(r"\bshatail\.shatailadmin\b"), "shatail.admin"),
        (re.compile(r"\bshatail\.shataildocs\b"), "shatail.documents"),
        (re.compile(r"\bshatail\.shatailembeds\b"), "shatail.embeds"),
        (re.compile(r"\bshatail\.shatailimages\b"), "shatail.images"),
        (re.compile(r"\bshatail\.shatailsearch\b"), "shatail.search"),
        (re.compile(r"\bshatail\.shatailsites\b"), "shatail.sites"),
        (re.compile(r"\bshatail\.shatailsnippets\b"), "shatail.snippets"),
        (re.compile(r"\bshatail\.shatailusers\b"), "shatail.users"),
        (re.compile(r"\bshatail\.shatailforms\b"), "shatail.contrib.forms"),
        (re.compile(r"\bshatail\.shatailredirects\b"), "shatail.contrib.redirects"),
        (
            re.compile(r"\bshatail\.contrib\.shatailfrontendcache\b"),
            "shatail.contrib.frontend_cache",
        ),
        (
            re.compile(r"\bshatail\.contrib\.shatailroutablepage\b"),
            "shatail.contrib.routable_page",
        ),
        (
            re.compile(r"\bshatail\.contrib\.shatailsearchpromotions\b"),
            "shatail.contrib.search_promotions",
        ),
        (
            re.compile(r"\bshatail\.contrib\.shatailsitemaps\b"),
            "shatail.contrib.sitemaps",
        ),
        (
            re.compile(r"\bshatail\.contrib\.shatailstyleguide\b"),
            "shatail.contrib.styleguide",
        ),
        # Added in Shatail 3.0
        (re.compile(r"\bshatail\.tests\b"), "shatail.test"),
        (re.compile(r"\bshatail\.core\b"), "shatail"),
        (re.compile(r"\bshatail\.admin\.edit_handlers\b"), "shatail.admin.panels"),
        (
            re.compile(r"\bshatail\.contrib\.forms\.edit_handlers\b"),
            "shatail.contrib.forms.panels",
        ),
    ]
...
class Version(Command):
    description = "List which version of Shatail you are using"

    def run(self):
        import shatail

        version = shatail.get_version(shatail.VERSION)

        print("You are using Shatail %(version)s" % {"version": version})

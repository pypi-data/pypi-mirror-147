...
def _find_duplicate_timestamps(df):
    ...
    for ts in duplicates.array:
        ...
        yield timeseer.EventFrame(_EVENT_FRAME_NAME, py_ts, py_ts)
...
def run(
    ...
    check = timeseer.CheckResult(_CHECK_NAME, float(len(duplicate_timestamps) > 0))
    return timeseer.AnalysisResult([check], duplicate_timestamps)

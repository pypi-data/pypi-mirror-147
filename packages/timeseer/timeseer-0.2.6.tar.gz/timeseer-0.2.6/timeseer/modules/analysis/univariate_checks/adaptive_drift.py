...
from timeseer import AnalysisInput, AnalysisResult, CheckResult, DataType, EventFrame
...
def _run_drift_check(
    ...
    pct_upwards = calculate_local_score(df, analysis_start, mpos, median_archival_step)
    pct_downwards = calculate_local_score(
        df, analysis_start, mneg, median_archival_step
    )
    ...
    return pct_upwards, pct_downwards, list(all_frames), last_analyzed_point
...
def _is_valid_input(analysis_input, f_and_p) -> tuple[str, bool]:
...
def run(
    ...
    pct_upwards, pct_downwards, frames, last_analyzed_point = _run_drift_check(
        analysis_input, frequency_and_period, median_archival_step[0], open_event_frames
    )

    # Returning results
    check_upwards = CheckResult("Adaptive drift (upwards)", float(pct_upwards))
    check_downwards = CheckResult("Adaptive drift (downwards)", float(pct_downwards))
    return AnalysisResult(
        [check_upwards, check_downwards],
        frames,

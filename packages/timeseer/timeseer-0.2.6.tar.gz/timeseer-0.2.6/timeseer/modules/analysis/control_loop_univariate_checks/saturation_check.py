...
def _remove_short_intervals(intervals, min_length_hours=6):
    if len(intervals) == 0:
        return intervals
    min_persistence = timedelta(hours=min_length_hours)
    start_datetimes = intervals["start_date"].values
    end_datetimes = intervals["end_date"].values

    ext_intervals = intervals.copy()
    ext_intervals["duration"] = end_datetimes - start_datetimes
    ext_intervals = ext_intervals[ext_intervals["duration"] >= min_persistence]
    return ext_intervals.drop(columns=["duration"])
...
def _run_saturation(analysis_input, median_archival_step, open_event_frames):
...
def run(analysis_input: timeseer.AnalysisInput) -> timeseer.AnalysisResult:
    ...
    pct_active, frames, last_analyzed_point = _run_saturation(

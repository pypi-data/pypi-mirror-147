...
from typing import (TypeVar, Optional, Tuple, Callable, Any, Generic)
...
__003F29763 = TypeVar("__003F29763")

__003F29762 = TypeVar("__003F29762")

__003F29832 = TypeVar("__003F29832")

__003F29830 = TypeVar("__003F29830")

__003F29705 = TypeVar("__003F29705")

__003F29704 = TypeVar("__003F29704")

__003F29733 = TypeVar("__003F29733")

__003F29731 = TypeVar("__003F29731")
...
class FSharpList(Record, Generic[_T]):
    ...
    def __eq__(self, other: Any=None) -> bool:
        ...
        if xs is other:
            ...
            def loop(xs_1_mut: FSharpList[__003F29763], ys_1_mut: FSharpList[__003F29763]) -> bool:
                while True:
                    (xs_1, ys_1) = (xs_1_mut, ys_1_mut)
                    match_value : Tuple[Optional[FSharpList[__003F29763]], Optional[FSharpList[__003F29763]]] = (xs_1.tail, ys_1.tail)
                    if match_value[0] is not None:
                        if match_value[1] is not None:
                            xt : FSharpList[__003F29763] = match_value[0]
                            yt : FSharpList[__003F29763] = match_value[1]
            ...
            loop : Callable[[FSharpList[__003F29762], FSharpList[__003F29762]], bool] = loop
    ...
    def CompareTo(self, other: Any=None) -> int:
        xs : FSharpList[_T] = self
        def loop(xs_1_mut: FSharpList[__003F29832], ys_1_mut: FSharpList[__003F29832]) -> int:
            while True:
                (xs_1, ys_1) = (xs_1_mut, ys_1_mut)
                match_value : Tuple[Optional[FSharpList[__003F29832]], Optional[FSharpList[__003F29832]]] = (xs_1.tail, ys_1.tail)
                if match_value[0] is not None:
                    if match_value[1] is not None:
                        xt : FSharpList[__003F29832] = match_value[0]
                        yt : FSharpList[__003F29832] = match_value[1]
...
def FSharpList__get_Length(xs: FSharpList[_T]) -> int:
    def loop(i_mut: int, xs_1_mut: FSharpList[__003F29705], xs: FSharpList[_T]=xs) -> int:
        while True:
            (i, xs_1) = (i_mut, xs_1_mut)
            match_value : Optional[FSharpList[__003F29705]] = xs_1.tail
            if match_value is not None:
                i_mut = i + 1
...
def FSharpList__get_Item_Z524259A4(xs: FSharpList[_T], index: int) -> _T:
    def loop(i_mut: int, xs_1_mut: FSharpList[__003F29733], xs: FSharpList[_T]=xs, index: int=index) -> __003F29733:
        while True:
            (i, xs_1) = (i_mut, xs_1_mut)
            match_value : Optional[FSharpList[__003F29733]] = xs_1.tail
            if match_value is not None:
                if i == index:
                    return xs_1.head
...
def singleton(x: Optional[_A]=None) -> FSharpList[_A]:
...
def iterate(action: Callable[[_A], None], xs: FSharpList[_A]) -> None:
    def arrow_75(unit_var0: None, x: _A, action: Callable[[_A], None]=action, xs: FSharpList[_A]=xs) -> None:
        action(x)

    fold(arrow_75, None, xs)


def iterate2(action: Any, xs: FSharpList[_A], ys: FSharpList[_B]) -> None:
    def arrow_76(unit_var0: None, x: _A, y: _B, action: Any=action, xs: FSharpList[_A]=xs, ys: FSharpList[_B]=ys) -> None:
        action(x, y)

    fold2(arrow_76, None, xs, ys)


def iterate_indexed(action: Any, xs: FSharpList[_A]) -> None:
    def arrow_77(i: int, x: _A, action: Any=action, xs: FSharpList[_A]=xs) -> int:
        action(i, x)
        return i + 1
...
def iterate_indexed2(action: Any, xs: FSharpList[_A], ys: FSharpList[_B]) -> None:
    def arrow_78(i: int, x: _A, y: _B, action: Any=action, xs: FSharpList[_A]=xs, ys: FSharpList[_B]=ys) -> int:
        action(i, x, y)
        return i + 1
...
def scan(folder: Any, state: _STATE, xs: FSharpList[_T]) -> FSharpList[_STATE]:
    ...
    def arrow_81(folder: Any=folder, state: _STATE=state, xs: FSharpList[_T]=xs) -> FSharpList[_T]:
    ...
    while not FSharpList__get_IsEmpty(xs_3):
        ...
        def arrow_83(folder: Any=folder, state: _STATE=state, xs: FSharpList[_T]=xs) -> FSharpList[_STATE]:
...
def indexed(xs: FSharpList[_A]) -> FSharpList[Tuple[int, _A]]:
    def arrow_89(i: int, x: _A, xs: FSharpList[_A]=xs) -> Tuple[int, _A]:
        return (i, x)
...
def map_fold_back(mapping: Any, xs: FSharpList[_T], state: _STATE) -> Tuple[FSharpList[_RESULT], _STATE]:
    def arrow_94(acc: _STATE, x: _T, mapping: Any=mapping, xs: FSharpList[_T]=xs, state: _STATE=state) -> Tuple[_RESULT, _STATE]:
        return mapping(x, acc)

    return map_fold(arrow_94, state, reverse(xs))


def try_pick(f: Callable[[_T], Optional[_A]], xs: FSharpList[_T]) -> Optional[_A]:
    def loop(xs_1_mut: FSharpList[_T], f: Callable[[_T], Optional[_A]]=f, xs: FSharpList[_T]=xs) -> Optional[_A]:
        while True:
            (xs_1,) = (xs_1_mut,)
            if FSharpList__get_IsEmpty(xs_1):
                return None
                ...
                match_value : Optional[_A] = f(FSharpList__get_Head(xs_1))
                if match_value is None:
                    xs_1_mut = FSharpList__get_Tail(xs_1)
                    continue
...
def pick(f: Callable[[_A], Optional[_B]], xs: FSharpList[_A]) -> _B:
    match_value : Optional[_B] = try_pick(f, xs)
    if match_value is None:
...
def try_find(f: Callable[[_A], bool], xs: FSharpList[_A]) -> Optional[_A]:
    def arrow_95(x: Optional[_A]=None, f: Callable[[_A], bool]=f, xs: FSharpList[_A]=xs) -> Optional[_A]:
        return some(x) if f(x) else None

    return try_pick(arrow_95, xs)


def find(f: Callable[[_A], bool], xs: FSharpList[_A]) -> _A:
    match_value : Optional[_A] = try_find(f, xs)
    if match_value is None:
        raise Exception(SR_keyNotFoundAlt)
...
def try_find_back(f: Callable[[_A], bool], xs: FSharpList[_A]) -> Optional[_A]:
    return try_find_back_1(f, to_array(xs))


def find_back(f: Callable[[_A], bool], xs: FSharpList[_A]) -> _A:
    match_value : Optional[_A] = try_find_back(f, xs)
    if match_value is None:
        raise Exception(SR_keyNotFoundAlt)
...
def find_index(f: Callable[[_A], bool], xs: FSharpList[_A]) -> int:
...
def try_find_index_back(f: Callable[[_A], bool], xs: FSharpList[_A]) -> Optional[int]:
    return try_find_index_back_1(f, to_array(xs))


def find_index_back(f: Callable[[_A], bool], xs: FSharpList[_A]) -> int:
...
def replicate(n: int, x: _A) -> FSharpList[_A]:
    def arrow_101(_arg1: int, n: int=n, x: _A=x) -> _A:
        return x
...
def for_all(f: Callable[[_A], bool], xs: FSharpList[_A]) -> bool:
    def arrow_102(acc: bool, x: _A, f: Callable[[_A], bool]=f, xs: FSharpList[_A]=xs) -> bool:
        return f(x) if acc else False

    return fold(arrow_102, True, xs)


def for_all2(f: Any, xs: FSharpList[_A], ys: FSharpList[_B]) -> bool:
    def arrow_103(acc: bool, x: _A, y: _B, f: Any=f, xs: FSharpList[_A]=xs, ys: FSharpList[_B]=ys) -> bool:
        return f(x, y) if acc else False

    return fold2(arrow_103, True, xs, ys)


def exists(f: Callable[[_A], bool], xs: FSharpList[_A]) -> bool:
...
def unzip(xs: FSharpList[Tuple[_A, _B]]) -> Tuple[FSharpList[_A], FSharpList[_B]]:
    def arrow_104(tupled_arg: Tuple[_A, _B], tupled_arg_1: Tuple[FSharpList[_A], FSharpList[_B]], xs: FSharpList[Tuple[_A, _B]]=xs) -> Tuple[FSharpList[_A], FSharpList[_B]]:
...
def unzip3(xs: FSharpList[Tuple[_A, _B, _C]]) -> Tuple[FSharpList[_A], FSharpList[_B], FSharpList[_C]]:
    def arrow_105(tupled_arg: Tuple[_A, _B, _C], tupled_arg_1: Tuple[FSharpList[_A], FSharpList[_B], FSharpList[_C]], xs: FSharpList[Tuple[_A, _B, _C]]=xs) -> Tuple[FSharpList[_A], FSharpList[_B], FSharpList[_C]]:
...
def zip(xs: FSharpList[_A], ys: FSharpList[_B]) -> FSharpList[Tuple[_A, _B]]:
    def arrow_106(x: _A, y: _B, xs: FSharpList[_A]=xs, ys: FSharpList[_B]=ys) -> Tuple[_A, _B]:
        return (x, y)

    return map2(arrow_106, xs, ys)


def zip3(xs: FSharpList[_A], ys: FSharpList[_B], zs: FSharpList[_C]) -> FSharpList[Tuple[_A, _B, _C]]:
    def arrow_107(x: _A, y: _B, z: _C, xs: FSharpList[_A]=xs, ys: FSharpList[_B]=ys, zs: FSharpList[_C]=zs) -> Tuple[_A, _B, _C]:
        return (x, y, z)
    ...
    def loop(i_mut: int, acc_mut: FSharpList[_T], xs_1_mut: FSharpList[_T], count: int=count, xs: FSharpList[_T]=xs) -> FSharpList[_T]:
        while True:
            ...
            if i <= 0:
                ...
                i_mut = i - 1
                def arrow_128(i: int=i, acc: FSharpList[_T]=acc, xs_1: FSharpList[_T]=xs_1) -> FSharpList[_T]:
    ...
    def loop(acc_mut: FSharpList[_T], xs_1_mut: FSharpList[_T], predicate: Callable[[_T], bool]=predicate, xs: FSharpList[_T]=xs) -> FSharpList[_T]:
        while True:
            ...
            if FSharpList__get_IsEmpty(xs_1):
                ...
                def arrow_143(acc: FSharpList[_T]=acc, xs_1: FSharpList[_T]=xs_1) -> FSharpList[_T]:
                    t : FSharpList[_T] = FSharpList(FSharpList__get_Head(xs_1), None)
                    acc.tail = t
                    return t
    ...
    def loop(i_mut: int, acc_mut: FSharpList[_T], xs_1_mut: FSharpList[_T], count: int=count, xs: FSharpList[_T]=xs) -> FSharpList[_T]:
        while True:
            ...
            if i <= 0:
                ...
                i_mut = i - 1
                def arrow_144(i: int=i, acc: FSharpList[_T]=acc, xs_1: FSharpList[_T]=xs_1) -> FSharpList[_T]:
                    t : FSharpList[_T] = FSharpList(FSharpList__get_Head(xs_1), None)
                    acc.tail = t
                    return t
...
def insert_at(index: int, y: _T, xs: FSharpList[_T]) -> FSharpList[_T]:
    ...
    def arrow_145(index: int=index, y: _T=y, xs: FSharpList[_T]=xs) -> FSharpList[_T]:
        raise Exception((SR_indexOutOfBounds + "\\nParameter name: ") + "index")
...
def insert_many_at(index: int, ys: IEnumerable[_T], xs: FSharpList[_T]) -> FSharpList[_T]:
    ...
    result : FSharpList[_T] = fold(folder, FSharpList_get_Empty(), xs)
    def arrow_146(index: int=index, ys: IEnumerable[_T]=ys, xs: FSharpList[_T]=xs) -> FSharpList[_T]:
        raise Exception((SR_indexOutOfBounds + "\\nParameter name: ") + "index")

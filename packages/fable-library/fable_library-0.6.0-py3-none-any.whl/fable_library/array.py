...
def where(predicate: Callable[[_A], bool], array: Array[_A]) -> Array[_A]:
    return list(builtins.filter(predicate, array))


def contains(value: _T, array: Array[_T], eq: IEqualityComparer[Any]) -> bool:
    def loop(i_mut: int, value: _T=value, array: Array[_T]=array, eq: IEqualityComparer[Any]=eq) -> bool:
        while True:
            (i,) = (i_mut,)
            if i >= len(array):
                return False
...
def empty(cons: Cons_1[_A]) -> Array[_A]:
    return Helpers_allocateArrayFromCons(cons, 0)
...
def remove_in_place(item_1: _T, array: Array[_T]) -> bool:
...
def index_of(array: Array[_T], item_1: _T, start: Optional[int]=None, count: Optional[int]=None) -> int:
...
def pick(chooser: Callable[[_A], Optional[_B]], array: Array[_A]) -> _B:
    def loop(i_mut: int, chooser: Callable[[_A], Optional[_B]]=chooser, array: Array[_A]=array) -> _B:
        while True:
            (i,) = (i_mut,)
            if i >= len(array):
                return index_not_found()
                ...
                match_value : Optional[_B] = chooser(array[i])
...
def try_pick(chooser: Callable[[_A], Optional[_B]], array: Array[_A]) -> Optional[_B]:
    def loop(i_mut: int, chooser: Callable[[_A], Optional[_B]]=chooser, array: Array[_A]=array) -> Optional[_B]:
        while True:
            (i,) = (i_mut,)
            if i >= len(array):
                return None
...
def find_back(predicate: Callable[[_A], bool], array: Array[_A]) -> _A:
    def loop(i_mut: int, predicate: Callable[[_A], bool]=predicate, array: Array[_A]=array) -> _A:
        while True:
            (i,) = (i_mut,)
            if i < 0:
                return index_not_found()
...
def try_find_back(predicate: Callable[[_A], bool], array: Array[_A]) -> Optional[_A]:
    def loop(i_mut: int, predicate: Callable[[_A], bool]=predicate, array: Array[_A]=array) -> Optional[_A]:
        while True:
            (i,) = (i_mut,)
            if i < 0:
                return None
...
def find_last_index(predicate: Callable[[_A], bool], array: Array[_A]) -> int:
    def loop(i_mut: int, predicate: Callable[[_A], bool]=predicate, array: Array[_A]=array) -> int:
        while True:
            (i,) = (i_mut,)
            if i < 0:
                return -1
...
def find_index_back(predicate: Callable[[_A], bool], array: Array[_A]) -> int:
    def loop(i_mut: int, predicate: Callable[[_A], bool]=predicate, array: Array[_A]=array) -> int:
        while True:
            (i,) = (i_mut,)
            if i < 0:
...
def try_find_index_back(predicate: Callable[[_A], bool], array: Array[_A]) -> Optional[int]:
    def loop(i_mut: int, predicate: Callable[[_A], bool]=predicate, array: Array[_A]=array) -> Optional[int]:
        while True:
            (i,) = (i_mut,)
            if i < 0:
                return None
...
def fold_indexed(folder: Any, state: _STATE, array: Array[_T]) -> _STATE:
    def arrow_50(delegate_arg0: _STATE, delegate_arg1: _T, delegate_arg2: int, folder: Any=folder, state: _STATE=state, array: Array[_T]=array) -> _STATE:
...
def unzip(array: Array[Tuple[_A, _B]]) -> Tuple[Array[_A], Array[_B]]:
    len_1 : int = len(array) or 0
    res1 : Array[_A] = [None]*len_1
    res2 : Array[_B] = [None]*len_1
    def arrow_57(i: int, tupled_arg: Tuple[_A, _B], array: Array[Tuple[_A, _B]]=array) -> None:
        res1[i] = tupled_arg[0]
        res2[i] = tupled_arg[1]
...
def unzip3(array: Array[Tuple[_A, _B, _C]]) -> Tuple[Array[_A], Array[_B], Array[_C]]:
    len_1 : int = len(array) or 0
    res1 : Array[_A] = [None]*len_1
    res2 : Array[_B] = [None]*len_1
    res3 : Array[_C] = [None]*len_1
    def arrow_58(i: int, tupled_arg: Tuple[_A, _B, _C], array: Array[Tuple[_A, _B, _C]]=array) -> None:
        res1[i] = tupled_arg[0]
        res2[i] = tupled_arg[1]
        res3[i] = tupled_arg[2]
...
def item(index: int, array: Array[_A]) -> _A:
    return array[index]
...
def fold_back_indexed(folder: Any, array: Array[_T], state: _STATE) -> _STATE:
    def arrow_59(delegate_arg0: _STATE, delegate_arg1: _T, delegate_arg2: int, folder: Any=folder, array: Array[_T]=array, state: _STATE=state) -> _STATE:
...
def fold_back(folder: Any, array: Array[_T], state: _STATE) -> _STATE:
    def arrow_60(acc: _STATE, x: _T, folder: Any=folder, array: Array[_T]=array, state: _STATE=state) -> _STATE:
        return folder(x, acc)

    return functools.reduce(arrow_60, array[::-1], state)


def fold_indexed2(folder: Any, state: _A, array1: Array[_B], array2: Array[_C]) -> _A:
    acc : _A = state
    if len(array1) != len(array2):
...
def fold2(folder: Any, state: _STATE, array1: Array[_T1], array2: Array[_T2]) -> _STATE:
    def arrow_61(_arg1: int, acc: _STATE, x: _T1, y: _T2, folder: Any=folder, state: _STATE=state, array1: Array[_T1]=array1, array2: Array[_T2]=array2) -> _STATE:
...
def fold_back2(f: Any, array1: Array[_T1], array2: Array[_T2], state: _STATE) -> _STATE:
    def arrow_62(_arg1: int, x: _T1, y: _T2, acc: _STATE, f: Any=f, array1: Array[_T1]=array1, array2: Array[_T2]=array2, state: _STATE=state) -> _STATE:
        return f(x, y, acc)
...
def for_all2(predicate: Any, array1: Array[_A], array2: Array[_B]) -> bool:
    def arrow_63(acc: bool, x: _A, y: _B, predicate: Any=predicate, array1: Array[_A]=array1, array2: Array[_B]=array2) -> bool:
...
def exists(predicate: Callable[[_A], bool], array: Array[_A]) -> bool:
    return exists_offset(predicate, array, 0)


def exists_offset2(predicate_mut: Any, array1_mut: Array[_A], array2_mut: Array[_B], index_mut: int) -> bool:
    while True:
...
def exists2(predicate: Any, array1: Array[_A], array2: Array[_B]) -> bool:
    if len(array1) != len(array2):
        different_lengths()
...
def min_by(projection: Callable[[_A], _B], xs: Array[_A], comparer: IComparer[_B]) -> _A:
    def arrow_66(x: _A, y: _A, projection: Callable[[_A], _B]=projection, xs: Array[_A]=xs, comparer: IComparer[_B]=comparer) -> _A:
...
def min(xs: Array[_A], comparer: IComparer[_A]) -> _A:
    def arrow_67(x: _A, y: _A, xs: Array[_A]=xs, comparer: IComparer[_A]=comparer) -> _A:
...
def windowed(window_size: int, source: Array[_T]) -> Array[Array[_T]]:
    ...
    res : Array[Array[_T]]
    def arrow_68(x: int, y: int, window_size: int=window_size, source: Array[_T]=source) -> int:
        return compare_primitives(x, y)

    len_1 : int = max_1(arrow_68, 0, (len(source) - window_size) + 1) or 0
    res = [None]*len_1
...
def split_into(chunks: int, array: Array[_T]) -> Array[Array[_T]]:
    ...
    if len(array) == 0:
        ...
        result : Array[Array[_T]] = []
        def arrow_69(x: int, y: int, chunks: int=chunks, array: Array[_T]=array) -> int:
            return compare_primitives(x, y)
        ...
        for i in range(0, (chunks_1 - 1) + 1, 1):
            ...
            def arrow_70(x_1: int, y_1: int, chunks: int=chunks, array: Array[_T]=array) -> int:
...
def insert_at(index: int, y: _T, xs: Array[_T]) -> Array[_T]:
    ...
    target : Array[_T] = [x for i, x in enumerate(xs) if i < (len_1 + 1)]
...
def insert_many_at(index: int, ys: IEnumerable[_T], xs: Array[_T]) -> Array[_T]:
    ...
    target : Array[_T] = [x for i, x in enumerate(xs) if i < (len_1 + len2)]
    for i in range(0, (index - 1) + 1, 1):
        target[i] = xs[i]
    for i_1 in range(0, (len2 - 1) + 1, 1):
...
def remove_at(index: int, xs: Array[_T]) -> Array[_T]:
    ...
    i : int = -1
    def predicate(_arg1: Optional[_T]=None, index: int=index, xs: Array[_T]=xs) -> bool:
        nonlocal i
        i = (i + 1) or 0
        return i != index
...
def remove_many_at(index: int, count: int, xs: Array[_T]) -> Array[_T]:
    i : int = -1
    status : int = -1
    def predicate(_arg1: Optional[_T]=None, index: int=index, count: int=count, xs: Array[_T]=xs) -> bool:
        nonlocal i, status
        i = (i + 1) or 0
        if i == index:
            status = 0
...
def update_at(index: int, y: _T, xs: Array[_T]) -> Array[_T]:
    ...
    target : Array[_T] = [x for i, x in enumerate(xs) if i < len_1]
    for i in range(0, (len_1 - 1) + 1, 1):

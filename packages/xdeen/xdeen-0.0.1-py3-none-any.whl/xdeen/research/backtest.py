...
def _isolated_backtest(
    ...
    from xdeen.modes.backtest_mode import simulator
    from xdeen.config import config as xdeen_config, reset_config
    from xdeen.routes import router
    from xdeen.store import store
    from xdeen.config import set_config
    from xdeen.services import required_candles
    import xdeen.helpers as jh

    xdeen_config['app']['trading_mode'] = 'backtest'
    ...
    max_timeframe = jh.max_timeframe(xdeen_config['app']['considering_timeframes'])
    warm_up_num = config['warm_up_candles'] * jh.timeframe_to_one_minutes(max_timeframe)
    trading_candles = candles
    if warm_up_num != 0:
        for c in xdeen_config['app']['considering_candles']:
            key = jh.key(c[0], c[1])
...
def _format_config(config):
    """
    XDeen's required format for user_config is different from what this function accepts (so it

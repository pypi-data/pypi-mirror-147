...
from xdeen.exceptions import CandleNotFoundInExchange
from xdeen.models import Candle
from xdeen.modes.import_candles_mode.drivers import drivers
from xdeen.modes.import_candles_mode.drivers.interface import CandleExchange
from xdeen.services.db import store_candles
        ...
        if not already_exists:
            ...
            if not len(candles) or arrow.get(candles[0]['timestamp'] / 1000) > start_date:
                ...
                if temp_start_timestamp > first_existing_timestamp:
                    ...
                    msg = f'No candle exists in the market for {temp_start_time}. So XDeen started importing since the first existing date which is {temp_existing_time}'
                    if running_via_dashboard:
                        sync_publish('alert', {
                            'message': msg,
                            'type': 'success'

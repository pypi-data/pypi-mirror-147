...
def get_candles(exchange: str, symbol: str, timeframe: str, start_date: str, finish_date: str) -> np.ndarray:
    ...
    from xdeen.exceptions import CandleNotFoundInDatabase
    from xdeen.services.candle import generate_candle_from_one_minutes

    # check if .env file exists
    if not jh.is_xdeen_project():
        raise FileNotFoundError(
            'Invalid directory: ".env" file not found. To use XDeen inside notebooks, create notebooks inside the root of a XDeen project.'
        )
...
def store_candles(candles: np.ndarray, exchange: str, symbol: str) -> None:
    ...
    """
    from xdeen.services.db import store_candles as store_candles_from_list
    import xdeen.helpers as jh

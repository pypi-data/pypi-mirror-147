...
from xdeen.services import auth as authenticator
from xdeen.services.redis import async_redis, async_publish, sync_publish
from xdeen.services.web import fastapi_app, BacktestRequestJson, ImportCandlesRequestJson, CancelRequestJson, \
    LoginRequestJson, ConfigRequestJson, LoginXDeenTradeRequestJson, NewStrategyRequestJson, FeedbackRequestJson, \
...
def validate_cwd() -> None:
    """
    make sure we're in a XDeen project
    """
    if not jh.is_xdeen_project():
        print(
            jh.color(
                'Current directory is not a XDeen project. You must run commands from the root of a XDeen project. Read this page for more info: https://docs.xdeen.net/docs/getting-started/#create-a-new-xdeen-project',
                'red'
            )
        )
        os._exit(1)
...
# print(os.path.dirname(xdeen))
XDEEN_DIR = os.path.dirname(os.path.realpath(__file__))


# load homepage
@fastapi_app.get("/")
async def index():
    return FileResponse(f"{XDEEN_DIR}/static/index.html")
...
async def terminate_all(authorization: Optional[str] = Header(None)):
    ...
    from xdeen.services.multiprocessing import process_manager
...
def make_strategy(json_request: NewStrategyRequestJson, authorization: Optional[str] = Header(None)) -> JSONResponse:
    ...
    from xdeen.services import strategy_maker
...
def feedback(json_request: FeedbackRequestJson, authorization: Optional[str] = Header(None)) -> JSONResponse:
    ...
    from xdeen.services import xdeen_trade
    return xdeen_trade.feedback(json_request.description, json_request.email)
...
def report_exception(json_request: ReportExceptionRequestJson, authorization: Optional[str] = Header(None)) -> JSONResponse:
    ...
    from xdeen.services import xdeen_trade
    return xdeen_trade.report_exception(
...
def get_config(json_request: ConfigRequestJson, authorization: Optional[str] = Header(None)):
    ...
    from xdeen.modes.data_provider import get_config as gc
...
def update_config(json_request: ConfigRequestJson, authorization: Optional[str] = Header(None)):
    ...
    from xdeen.modes.data_provider import update_config as uc
...
@click.group()
@click.version_option(pkg_resources.get_distribution("xdeen").version)
def cli() -> None:
    pass
...
def install_live(strict: bool) -> None:
    from xdeen.services.installer import install
...
def run() -> None:
    ...
    from xdeen.services.migrator import run as run_migrations
    import peewee
    try:
        run_migrations()
...
def general_info(authorization: Optional[str] = Header(None)) -> JSONResponse:
    ...
    from xdeen.modes import data_provider
...
def import_candles(request_json: ImportCandlesRequestJson, authorization: Optional[str] = Header(None)) -> JSONResponse:
    ...
    from xdeen.modes import import_candles_mode
...
def backtest(request_json: BacktestRequestJson, authorization: Optional[str] = Header(None)):
    ...
    from xdeen.modes.backtest_mode import run as run_backtest
...
async def optimization(request_json: OptimizationRequestJson, authorization: Optional[str] = Header(None)):
    ...
    from xdeen.modes.optimize_mode import run as run_optimization
...
def shutdown_event():
    from xdeen.services.db import database
    database.close_connection()


if HAS_LIVE_TRADE_PLUGIN:
    from xdeen.services.web import fastapi_app, LiveRequestJson, LiveCancelRequestJson, GetCandlesRequestJson, \
    ...
    def live(request_json: LiveRequestJson, authorization: Optional[str] = Header(None)) -> JSONResponse:
        ...
        from xdeen import validate_cwd
        ...
        from xdeen_live import live_mode
        from xdeen.services.multiprocessing import process_manager
    ...
    def get_candles(json_request: GetCandlesRequestJson, authorization: Optional[str] = Header(None)) -> JSONResponse:
        ...
        from xdeen.modes.data_provider import get_candles as gc
    ...
    def get_logs(json_request: GetLogsRequestJson, authorization: Optional[str] = Header(None)) -> JSONResponse:
        ...
        from xdeen_live.services.data_provider import get_logs as gl
    ...
    def get_orders(json_request: GetOrdersRequestJson, authorization: Optional[str] = Header(None)) -> JSONResponse:
        ...
        from xdeen_live.services.data_provider import get_orders as go
...
fastapi_app.mount("/", StaticFiles(directory=f"{XDEEN_DIR}/static"), name="static")

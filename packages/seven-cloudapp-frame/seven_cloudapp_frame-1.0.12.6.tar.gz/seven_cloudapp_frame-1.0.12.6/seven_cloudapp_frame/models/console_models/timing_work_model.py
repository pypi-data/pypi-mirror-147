    ...
    def __process_work(self,work_name):
        ...
        while True:
            ...
            if now_hour >= self.start_hours and now_hour <= self.end_hours:
                invoke_result_data = InvokeResultData()
                is_stop_work = False
                execete_result = self.execete()
                if isinstance(execete_result,tuple):
                    invoke_result_data = execete_result[0]
                    is_stop_work = execete_result[1]
                else:
                    invoke_result_data = execete_result
                if invoke_result_data.success == True and self.fail_count > 0 and is_stop_work == False:

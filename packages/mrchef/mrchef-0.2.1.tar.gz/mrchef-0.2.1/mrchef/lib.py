...
class Worker:
    ...
    def warmup(self, *meal_paths: Path, force: bool = False):
        ...
        for key, value in self.all_meals().items():
            ...
            try:
                _git("remote", "set-url", ORIGIN, cold.url)
            except ProcessExecutionError:
                _git("remote", "add", ORIGIN, cold.uri)
            _git("fetch", ORIGIN, cold.rev or cold.branch)
            _git("switch", "--force-create", cold.branch, cold.rev or "FETCH_HEAD")
...
def git_origin(folder: Path) -> str:
    ...
    return git(folder)("remote", "get-url", ORIGIN).strip()

...
class HttpTool:
    ...
    def send_request(self,
        ...
        return self._send_request(
            url=url, method=method, params=params, data=data, headers=headers, cookies=cookies, auth=auth,
            encoding=encoding, raise_error=raise_error, log_input=log_input,
            log_output=log_output, log_error=log_error, retry=retry,
            timeout=timeout)
        ...
        for i in range(0, retry):
            ...
            try:
                if method == 'GET':
                    ...
                    response = requests.post(
                        url=url, headers=headers, data=payload, params=params, timeout=timeout, auth=auth, verify=False)
                else:
                    raise NotImplementedError(
                        f"Method {method} is not supported yet.")

...
def _emit_executionerror(event):
    _emit_prefix(event)
    if details := event.get('details'):
        if 'error' in details:
            print('ERROR:', details['error'], flush=True)
        else:
            print('ERROR: An ExecutionError occurred:', flush=True)
            for k, v in details.items():
                print(f'{k}: {v}', flush=True)
    else:
        print(f'An ExecutionError occurred: {event}', flush=True)


def _emit_event(kind, event, step_depth, job_depth, output_format, first):
    if output_format == 'json':
        if first:
            ...
            print(',\n    ', end='')
        print(
            '    '.join(json.dumps(event, indent=2).splitlines(keepends=True)), end=''
        )
        return
    if output_format == 'yaml':
        print('- ', end='')
        print('  '.join(yaml.safe_dump(event).splitlines(keepends=True)), end='')
        return

    if kind == 'Workflow':
        print('Workflow', event['metadata']['name'], flush=True)
        return
    if kind not in WATCHED_EVENTS:
        return
    if kind == 'ExecutionError':
        _emit_executionerror(event)
        return
    ...
    if kind == 'ExecutionResult':
        ...
        _emit_prefix(event)
        print(' ' * (len(event['metadata'].get('step_origin', []))), end='')
        print('Running action', event['metadata']['name'], flush=True)

...
def configuration(config_files=['config_project.ini', 'config_local.ini']):
    import shutil
    import configparser
    # Copies templates if no such file exists
    if len(config_files) > 1 and config_files[0] == 'config_local.ini' and \
...
def init_env(env: Environment = None, patch_scons_sig_fns: bool = True,
    ...
    if GetOption("config_files") is not None:
        config = configuration(config_files=GetOption("config_files").split(':'))
    else:

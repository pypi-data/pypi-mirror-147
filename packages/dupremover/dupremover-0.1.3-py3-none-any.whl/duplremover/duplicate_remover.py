...
from BaseColor.base_colors import hred, hgreen, yellow, red

from duplremover.multi_workers import BigSlaves, Slaves, FakeSlaves
from duplremover.resourse_and_gears import note_printer, printer, hash_str, get_file_size, get_file_size_path
...
class DuplRm(object):
    ...
    def __init__(self,
        ...
        :param patch: 文件读取批次大小
        :param no_sub_dirs: 不进入子文件夹
        ...
        self.file_patch = patch if isinstance(patch, int) and patch > 0 else 1024 * 1024
        self.no_sub_dirs = no_sub_dirs
    ...
    def start(self):
        ...
        printer(f'directory: {self.dir}')
        if self.no_sub_dirs:
            files_lis = [os.path.join(root, n) for root, dirs, files in os.walk(self.dir, topdown=False) for n in files if root == self.dir]
        else:
        ...
        files_suffixes = [x.split('.')[-1].lower() for x in files_paths]
        ...
        if self.INTERACTIVE_MODE and self.TYPE_FILTER == 'all':
            ...
            if not files_count_lis:
                printer(f'no type {red(str(list(self.TYPE_FILTER)))} in this directory: {red(self.dir)}')
                top_note = 'all types:'
                note_printer(files_count, top_note=top_note, end_note='end', front_color="yellow")
                return {}
            fc_tn = 'all types'
        ...
        printer('start processor', interactive_mode=self.INTERACTIVE_MODE)
        selected_paths = [x for x in files_paths if x.split('.')[-1].lower() in suffixes_selected_set]
        ...
        duplicated_list = FakeSlaves().work(self._check_duplicate, params_list=selected_lis)
        dump = [return_dict.update({lis[0][0].split('.')[-1]: lis}) for lis in duplicated_list if lis]
        del dump
        ...
        if return_dict:
            for the_type, dp_lis in return_dict.items():
                ...
                printer(tn)
                for dpl in dp_lis:
                    note_printer(dpl, top_note=get_file_size(dpl[0]), fill_str='-', disordered_mode=True, front_color="red")
        return return_dict
    ...
    def _check_duplicate(self, files_path_list):
        ...
        printer(f'get [ {yellow(suffix_type)} ] file size ...')
        file_path_size_list = Slaves(with_tq=True).work(get_file_size_path, params_list=files_path_list)
        f_size_path_map = dict()
        for fp, fs in file_path_size_list:
            if fs in f_size_path_map:
                f_size_path_map[fs] = f_size_path_map.get(fs) + [fp]
                duplicated_found = True
            else:
                f_size_path_map[fs] = [fp]
        if not duplicated_found:
            printer(f'No duplicates found in type: [ {hgreen(suffix_type)} ]', interactive_mode=self.INTERACTIVE_MODE)
            return []
        else:
            duplicated_found = False
            printer(f'get [ {yellow(suffix_type)} ] file hash ...')
            ...
            for f_path, file_id in fp_fid_list:
                if file_id:
                    ...
                    if self.RM_ZERO:
                        os.remove(f_path)
                        self.RM_COUNT += 1
                        printer(f'type: [ {red(suffix_type)} ] found and deleted: Zero size file :\n {f_path}', interactive_mode=self.INTERACTIVE_MODE)
            id_path_lis_dic = {}
            ...
            for i, ps in id_path_lis_dic.items():
                ...
                if self.AUTO_DELETE:
                    ...
                    sl_str = "\n".join(sl)
                    printer(f'type: [ {red(suffix_type)} ] auto deleted: \n{hred(sl_str)}\n', interactive_mode=self.INTERACTIVE_MODE)
                elif not self.AUTO_DELETE and self.INTERACTIVE_MODE:
                    note_printer(ps, top_note='Duplicate files detected', end_note='end', front_color="red", item_type='file')
                    ...
                    if rm_lis:
                        ...
                        note_printer(rm_lis, top_note=tn, fill_str='~', disordered_mode=True, front_color="red", show_mode="blinking", item_type='file')
                        confirm = input(' >>> please confirm(yes?): ')
                        if confirm.strip().lower() == 'yes' or confirm.strip().lower() == 'y':
                            sc_str = self._files_remover(rm_lis)
                            printer(f'type: [ {red(suffix_type)} ]  {sc_str} files has been delete', interactive_mode=self.INTERACTIVE_MODE)
                        else:
            ...
            if not duplicated_found:
                printer(f'No duplicates found in type: [ {hgreen(suffix_type)} ]', interactive_mode=self.INTERACTIVE_MODE)
            return list(id_path_lis_dic.values())
...
def rmduplicate(args=None):
    ...
    parser.add_argument("-pc", "--patch", type=int, dest="patch", default=1024*1024, help=f'{da} 若是哈希模式，设置按批次读取的大小，默认1Mb，越大内存占用越高，速度可能越快')
    parser.add_argument("-ns", "--no_sub_dirs", type=bool, dest="no_sub_dirs", nargs='?', default=False, help=f'{da}y/n 是否不进入子文件夹，默认进入')
    ...
    no_sub_dirs = format_args(args.no_sub_dirs, default=False)

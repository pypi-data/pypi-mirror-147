...
def sync_picture_fixtures(snippet, soup):
    ...
    merged_assets = {}
    merged_transforms = {}
    merged_base_transforms = {}

    for asset_to_transform in assets_to_transform:
        asset, variations, base_transforms = asset_to_transform

        if asset.key not in merged_assets:
            merged_assets[asset.key] = asset
            merged_base_transforms[asset.key] = []

        if asset.key not in merged_transforms:
            merged_transforms[asset.key] = {}

        merged_transforms[asset.key].update(variations)
    ...
    for key, transforms in merged_transforms.items():
        asset = merged_assets[key]
        for version in transforms.keys():
            asset_version_key = f'{version}:{key}'
            assets[asset_version_key][0].variations[version] \
                    = asset.variations[version]
    ...
    for picture_fixture in picture_fixtures:
        ...
        for source in sources:
            ...
            if not assets.get(asset_version_key):
                # Asset not in in the assets map for the snippet, ignore it.
                continue

            asset, _ = assets[asset_version_key]

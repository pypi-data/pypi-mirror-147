...
class Neo4jHelper:
    ...
    async def delete_property(self, node: str, property_node: dict, node_property: str):
        sql = f"MATCH ({node}:{dict_str(property_node)}) REMOVE {node}.{node_property} RETURN {node}"
        result = await self.execute_fetchall(sql)
        return result

    async def update_node(self, node_name: str, label_name: str, node_property_dict: dict):
        node_property_str = ",".join(
            ['%s=%r' % (property_name, property_value) for property_name, property_value in
             node_property_dict.items()])
        sql = f"MATCH ({node_name}:{label_name}) SET {node_property_str} RETURN {node_name}"
        result = await self.execute_fetchall(sql)
        return result
    ...
    async def query_node_page(self, node_name: str = None, label_name: str = None, match_property_dict: dict = {},
        ...
        total = await self.query_count(node_name, label_name, match_property_dict, node_property_dict)
        if result_list:
    ...
    def sync_update_node(self, node_name: str, label_name: str, node_property_dict: dict):
        node_property_str = ",".join(
            ['%s=%r' % (property_name, property_value) for property_name, property_value in
             node_property_dict.items()])
        sql = f"MATCH ({node_name}:{label_name}) SET {node_property_str} RETURN {node_name}"
    ...
    def sync_query_node_page(self, node_name: str = None, label_name: str = None, match_property_dict: dict = {},
        ...
        page_num = page_num if page_num > 1 else 1
        num = (page_num - 1) * page_size
        total = self.sync_query_count(node_name, label_name, match_property_dict, node_property_dict)
        if result_list:
            sql += f' RETURN {",".join(result_list)} SKIP {num} LIMIT {page_size}'
        else:
            if node_name:

...
class Person:
    ...
    def get_balance(self, contract_address: str, fee_limit=800000):
        self.contract_address = contract_address if contract_address else self.contract_address
        result = subprocess.run(['erc20-balance', '--fee-limit', str(fee_limit), '-p', RPC_PROVIDER,
                                 '-i', CHAIN_SPEC, '-u', '-e', self.contract_address, self.user_address], capture_output=True, text=True)
        try:
    ...
    def send(self, contract_address, signer_keyfile, fee_limit=800000, force=False):
        self.contract_address = contract_address
        if(not force and self.has_corrent_amount()):
            log.info(
                f"Skipping {self.phone_number} as balance is within range")
            log.info(
            ...
            cmd = ["erc20-transfer", "-p", RPC_PROVIDER, "-i", CHAIN_SPEC, "-u", "-e", contract_address, "--fee-limit",
                   str(fee_limit), "-y", signer_keyfile, "-a", self.user_address, str(self.send_amount), "-s"]
            log.debug("> " + " ".join(cmd))
            try:

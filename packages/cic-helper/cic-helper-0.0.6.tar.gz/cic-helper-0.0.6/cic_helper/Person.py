...
class Person:
    ...
    def get_balance(self, contract_address: str):
        self.contract_address = contract_address
        result = subprocess.run(['erc20-balance', '--fee-limit', '1000000000', '-p', RPC_PROVIDER,
                                 '-i', CHAIN_SPEC, '-u', '-e', contract_address, self.user_address], capture_output=True, text=True)
        try:
    ...
    def send(self, contract_address, signer_keyfile, fee_limit=80000, check_balance=True):
        self.contract_address = contract_address
        if(check_balance and self.has_corrent_amount()):
            log.info(

...
class WKFSAdapter:
    ...
    def begin_docviewer_session(self, 
    ...
    def get_docviewer_session(self, session_id, 
            access_token: str = None,
            log_config: dict = None):
        headers = self._api_handler._headers
        headers["Authorization"] = f"Bearer {access_token}"
    ...
    def generate_package(
        ...
        if interface_config:
            interface_details = {}
            interface_details["transaction_xml"] = cleaned_transaction_xml_payload_bytes.decode("utf-8").replace("\\n", "").replace("\\t", "")

...
async def folder_push(
        ...
        raise_on_exists=False, verbose=True, tests_to_run: int = 10, filter_tests_by: Optional[List[str]] = None):
            ...
            if name in to_run:
                if not dry_run:
                    if name not in remote_resource_names or resource_versions.get(name.replace(".", "_"), '') != latest_datasource_versions.get(name, '') or force or run_tests:
                        ...
                        try:
                            await exec_file(to_run[name], tb_client, force, check, debug and verbose, populate, subset, wait, tag,
                                            skip_table_checks, ignore_sql_errors, skip_confirmation, only_response_times, timeout,
                                            run_tests, as_standard, tests_to_run, filter_tests_by)
                            if not run_tests:
                        ...
                        if raise_on_exists:
                            raise click.ClickException(FeedbackManager.warning_name_already_exists(
                                name=name if to_run[name]['version'] is None else f'{name}__v{to_run[name]["version"]}'))
                        else:

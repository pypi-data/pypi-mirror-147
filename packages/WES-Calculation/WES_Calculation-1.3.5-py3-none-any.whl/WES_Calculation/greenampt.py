...
LastEditTime: 2022-04-23 09:41:31
LastEditors: BDFD
Description: 
...
# Secondary Inputs (default)
    ...
    if max(i)>0:
        for jj in range(j+1,l+2):#20220313      
            if fpu[jj]!='Inf' and F[jj]>0:#20220313
                ...
                ax1.plot(np.linspace(t1[j+1],dti*(l+1),l-j+1),1000*np.array(fpu[j+1:l+2]),label="修正后的理论入渗率",color="red",linewidth=1,zorder=5)
                break
    if dtheta==0: # (20211227)  
    ...
    note2 = [ ]
    if iyesno == 1:
        note_1 = ['说明:','1. 在图中，降雨强度过程实为整个柱状图（原本为浅蓝色），但可能局部或全部被实际入渗率（深蓝色）或地表坑洼充填速率（带叉的浅蓝色）所覆盖。']
        # note_1 = "1. 为与降雨强度过程相比较，下渗率按柱状图表示，某时刻的下渗率会显示在接下来的整个时间步长内。"
        note_2 = ['2. 无论是对于实际降雨还是净雨，除非特殊说明，一个雨强值从对应的时刻开始，会持续整个时间步长 (=',dti,'minutes)。']
        ...
        if Td>0:
            ...
            note_3 = ['3. 地表坑洼在第',round(Td,1), 'minute 被蓄满, 之前没有净雨产生。']
            note.append(note_3)
        ...
        if dtheta==0: # (20211227)
            ...
            note_4 = ['4. 下渗有可能在第',round(Td,1), 'minute 后发生。']# (20220311)
            note.append(note_4)  
        else:# (20211227)
            ...
            note_4 = ['4. 下渗有可能在第',round(Td,1), 'minute 后发生。为便于比较，图中的理论入渗率和修正后的潜势入渗率曲线均从这一时刻起始。理论入渗率和修正后的潜势入渗率在这一时刻均为无限大，图中的曲线在这一时间点上不专门显示。之后，修正后的潜势入渗率仅仅在有实际下渗发生时会有所变化。']# (20211227)
            note.append(note_4)
        ...
        if dda>dd:
            ...
            note_6 = ['6. 如果存在，起始于第',round(Td,1), 'minute 的净雨强度值会在第', (j+1)*dti,'minute 结束。']
            note.append(note_6)
        ...
        if sum(f[j:l+2])==0:# (20220311)
            ...
            note_7 = ['7. 由于没有实际下渗发生，修正后的潜势入渗率在此不适用.']# (20220311)
            note.append(note_7)
        # if Td>0:
        ...
        if dtheta==0:
            ...
            note2_1 = '说明:','1. 理论入渗率在时间零点处为无限大，图中的理论入渗率曲线在这一时间点上不专门显示。'
            note2.append(note2_1)

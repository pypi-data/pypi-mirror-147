...
from .s3_bucket_info import (
    META_ERROR_MESSAGE,
    META_FILE_SIZE,
    META_SYNCED,
    META_VERSION_ID,
    download_s3_file,
    preprocess_remote_s3_assets,
)
...
def _download_s3_artifact_asset(
    s3_uri,  # type: str
    version_id,  # type: str
    asset_id,  # type: str
    artifact_repr,  # type: str
    ...
    try:
        _write_artifact_asset_data_to_disk(
            ...
            asset_path=asset_path,
            overwrite=overwrite,
            writer=AssetDataWriterFromS3(
                s3_uri=s3_uri, version_id=version_id, monitor=_monitor
            ),
        )
    except Exception:
        LOGGER.debug("Error writing S3 artifact asset file on path", exc_info=True)
        raise ArtifactDownloadException(
            ARTIFACT_ASSET_DOWNLOAD_FAILED_REPR % (asset_id, artifact_repr)
        )
    return None
...
class Artifact(object):
    ...
    def add_remote(
        ...
        Args:
            ...
            sync_mode: if False we just log the uri as a string, if True we are going to collect information about
                remote assets from S3 bucket.
            max_synced_objects: controls the numbers of objects synced to Comet from related S3 bucket.
        """
        if asset_type:
        ...
        try:
            for result in results:
                ...
                if result.asset_synced is False:
                    ...
                    # downloaded synced remote asset
                    new_artifact_assets[
                        result.asset_filename
                    ] = PreprocessedSyncedRemoteAsset(

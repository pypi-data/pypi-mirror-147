...
def preprocess_remote_s3_assets(
    ...
    bucket_name, object_prefix = _parse_s3_uri(remote_uri)
...
def _preprocess_s3_asset_object(
    ...
    logical_path = _create_s3_obj_logical_path(
        key=s3_object.key, folder=object_prefix, logical_path=logical_path
    )
...
def _create_s3_obj_logical_path(key, folder=None, logical_path=None):
    # type: (str, Optional[str], Optional[str]) -> str
    """Creates logical path for the assets associated with remote artifact which is stored in S3 bucket."""
...
def download_s3_file_uri(s3_client, s3_uri, file_object, callback, version_id=None):
    ...
    bucket_name, obj_key = _parse_s3_uri(s3_uri=s3_uri)
...
def _parse_s3_uri(s3_uri):
    # type: (str) -> (str, str)
    """Parses S3 URI and extracts bucket name and object prefix (parent folders path)."""
    o = urlparse(s3_uri)

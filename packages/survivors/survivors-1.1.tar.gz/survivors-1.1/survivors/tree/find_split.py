    ...
    **kwargs : None
    ...
    if type_attr == "woe":
        df_woe_iv = (pd.crosstab(arr_notnan[0],arr_notnan[1])
                     .assign(woe=lambda dfx: np.log((dfx[1] + 1e-5) / (dfx[0] + 1e-5))))
        descr_np = np.vstack([df_woe_iv.index.to_numpy(), df_woe_iv['woe']])
        a = pd.DataFrame(arr_notnan.T)
        a[0] = a[0].map(dict(df_woe_iv['woe']))
        arr_notnan = a.to_numpy().T
    ...
    if best_attr["sign_split"] > 0:
        if type_attr == "cont":
            ...
            best_attr["values"] = [' in %s' % (best_attr["values"][i]) for i in [0,1]]
        elif type_attr == "woe":

...
The :mod:`CAlearn.genetic` module implements Genetic Programming. These
...
#              to fit the Cellular Automaton symbolic regression.
...
def _parallel_evolve(n_programs, parents, X, sample_weight, seeds, params):
    """Private function used to build a batch of programs within a job."""
    #n_samples = time/steps in CA
    ...
    for i in range(len(function_set)):
        if isinstance(function_set[i], str):
            if function_set[i] not in _function_map:
                raise ValueError('_programTest: unknown function string in def __int__')
            if _function_map[function_set[i]].arity !=0:
                pure_function_set.append(_function_map[function_set[i]])
            else:
                CA_function_set.append(_function_map[function_set[i]])
        elif isinstance(function_set[i], _Function):
            if function_set[i].arity != 0:
                pure_function_set.append(function_set[i])
            else:
                CA_function_set.append(function_set[i])
        else:
            raise ValueError('invalid type %s found in `function_set`.')
    n_features = int(len(CA_function_set))'''
    n_features = 0
    ...
    for i in range(n_programs):
        ...
        program.raw_fitness_ = program.raw_fitness(X, curr_sample_weight)
        if max_samples < n_samples:
            # Calculate OOB fitness
            program.oob_fitness_ = program.raw_fitness(X, oob_sample_weight)
...
class BaseSymbolic(BaseEstimator, metaclass=ABCMeta):
    ...
    def fit(self, X, sample_weight=None):
        ...
        self._function_set = deepcopy(self.function_set)
        self._CA_function_set = []
        self._pure_function_set = []
        #Split the function with airty 0 and non-zero and calculate the number of features(U^t_k)
        if len(self.function_set) <= 0:
            raise Exception("genetic: In __init__, the length of function_set is 0.")
        for i in range(len(self.function_set)):
            if isinstance(self.function_set[i], str):
                ...
                if _function_map[self.function_set[i]].arity !=0:
                    self._pure_function_set.append(_function_map[self.function_set[i]])
                else:
                    self._CA_function_set.append(_function_map[self.function_set[i]])
            elif isinstance(self.function_set[i], _Function):
                if self.function_set[i].arity != 0:
                    self._pure_function_set.append(function_set[i])
                else:
                    self._CA_function_set.append(function_set[i])
            else:
                raise ValueError('invalid type %s found in `function_set`.')
        self._n_features = int(len(self._CA_function_set))
        ...
        self._arities = {}
        for function in self._pure_function_set:

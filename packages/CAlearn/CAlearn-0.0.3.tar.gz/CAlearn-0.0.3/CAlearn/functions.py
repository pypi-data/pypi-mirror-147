...
The :mod:`CAlearn.functions` module contains all of the functions used by
CAlearn programs. It also contains helper methods for a user to define their
own custom functions.
"""

# Author: Trevor Stephens <trevorstephens.com>
# Modified by: Chen Shu <shuchenjp@gmail.com>
#              to add U^{t-1}_{k-1}-like operation and protected remainder(modulo) calculation.
...
def _CellTm1Cm1(CoorNow, CAMatrix):
    """A function to get the value of U^{t-1}_{k-1}."""
    if CAMatrix.shape[0] - 1 < CoorNow[0]:
        raise Exception("_CellTm1Cm1 called out of the max of matrix time axis?")
    if CoorNow[0] <= 0:
        raise Exception("_CellTm1Cm1 called in initial condition?")
    else:
        if CoorNow[1] <=0:
            a = 0
        else:
            a = CAMatrix[CoorNow[0] - 1][CoorNow[1] - 1]
    return np.multiply(a,1)

def _CellTm1C(CoorNow, CAMatrix):
    """A function to get the value of U^{t-1}_{k}."""
    if CAMatrix.shape[0] - 1 < CoorNow[0]:
        raise Exception("_CellTm1C called out of the max of matrix time axis?")
    if CoorNow[0] <= 0:
        raise Exception("_CellTm1C called in initial condition?")
    else:
        a = CAMatrix[CoorNow[0] - 1][CoorNow[1]]
    return np.multiply(a,1)

def _CellTm1Cp1(CoorNow, CAMatrix):
    """A function to get the value of U^{t-1}_{k+1}."""
    if CAMatrix.shape[0] - 1 < CoorNow[0]:
        raise Exception("_CellTm1Cp1 called out of the max of matrix time axis?")
    if CoorNow[0] <= 0:
        raise Exception("_CellTm1Cp1 called in initial condition?")
    else:
        if CoorNow[1] >= CAMatrix.shape[1] - 1:
            a=0
        else:
            a = CAMatrix[CoorNow[0] - 1][CoorNow[1] + 1]
    return np.multiply(a,1)
...
def _Mod(x1, x2):
    """Closure of modulo/remainder for zero denominator."""
    if x2 == 0:
        raise Exception("_Mod: x1 is divided by 0 in modulo calculation")
    else:
...
CellTm1Cm1 = _Function(function=_CellTm1Cm1, name='CellTm1Cm1', arity=0)
CellTm1C = _Function(function=_CellTm1C, name='CellTm1C', arity=0)
CellTm1Cp1 = _Function(function=_CellTm1Cp1, name='CellTm1Cp1', arity=0)
mod2 = _Function(function=_Mod, name='mod', arity=2)
...
_function_map = {'add': add2,
                 ...
                 'cos': cos1,
                 'tan': tan1,
                 'CellTm1Cm1': CellTm1Cm1,
                 'CellTm1C': CellTm1C,
                 'CellTm1Cp1': CellTm1Cp1,
                 'mod': mod2}

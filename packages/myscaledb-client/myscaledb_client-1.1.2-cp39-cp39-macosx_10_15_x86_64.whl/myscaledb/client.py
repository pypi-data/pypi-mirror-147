...
class Client:
    ...
    def fetch(
        ...
        *args,
        ...
        async def get_object_loop() -> List[Record]:
            list = [
                row
                async for row in self._execute(
                    query,
            ...
            if len(list) == 0:
                return list
            print("total number of files to download:", len(list))
            row = list[0]
            get_object_name = query[
                query.find("getObject") : query.find(")", query.find("getObject")) + 1
            ...
            async with session.create_client(
                ...
                for i in range(len(list)):
                    if this_batch_count == batch_size:
                        await asyncio.gather(*all_task)
                        this_batch_count = 0
                    one_task = asyncio.create_task(
                        self.getObject(list[i], get_object_name, client)
                    )
                    all_task.append(one_task)
                    this_batch_count += 1
                await asyncio.gather(*all_task)
            return list

        @force_sync
        async def sync_get_object():
            return await get_object_loop()

        is_get_object = query.find("getObject") != -1
        if is_get_object:
            return sync_get_object()

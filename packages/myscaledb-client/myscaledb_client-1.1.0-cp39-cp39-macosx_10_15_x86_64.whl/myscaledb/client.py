...
class Client:
    ...
    def fetch(
        ...
        def simple_wrapper(one_row) -> Record:
            binary = self.getObject(one_row[get_object_name][1], session)
            one_row.decode_again(get_object_name, binary)
        ...
        @force_sync
        async def sync_get_bin() -> List[Record]:
        ...
        if is_get_object:
            ...
            if len(response)==0:
                return response
            print("total number of files to download:", len(response))
            row = response[0]
            get_object_name = query[
                              query.find("getObject"):query.find(")", query.find("getObject")) + 1].translate(
                str.maketrans({"'": r"\'"}))

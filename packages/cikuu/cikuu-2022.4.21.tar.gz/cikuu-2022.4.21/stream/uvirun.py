...
ftsnt		= Client("ftsnt",host='172.17.0.1',port=6379) #__init__(self, index_name, host='localhost', port=6379, conn=None, password=None, decode_responses=True)
ftessay		= Client("ftessay",host='172.17.0.1',port=6379)
md5text		= lambda text: hashlib.md5(text.strip().encode("utf-8")).hexdigest()
zsum		= lambda key='rid-230537:essay_wordnum',ibeg=0, iend=-1: sum([v for k,v in redis.r.zrevrange(key, ibeg, iend, True)])

@app.get("/sse/zrevrange")
...
async def essay_score(request: Request) -> StreamingResponse:
	async def generate_score(request: Request) -> Iterator[str]:
		rid = int(request.query_params.get('rid','230537'))
		span = int(request.query_params.get('sleep','2'))
		while True:
...
@app.post('/stream/xinfo')
def stream_xinfo(keys:list=["rid-230537:xwordidf","xessay"], name:str="last-entry"):
	''' r.xinfo_stream("xessay")  '''
	return { key: redis.r.xinfo_stream(key)[name]  for key in keys }
...
def redis_cmd(cmd:str='zrevrange rid-230537:snt_cola 0 10 withscores'):
	...
	FT.SEARCH ftsnt '@cola:[0.5,0.9]' limit 0 2
	'''
	return redis.r.execute_command(cmd)

@app.get('/stream/zsum')
def redis_zsum(key:str='rid-230537:essay_wordnum',ibeg:int=0, iend:int=-1):
	''' sum( scores ) '''
	return  sum([v for k,v in redis.r.zrevrange(key, ibeg, iend, True)])

@app.get('/stream/stats')
def stream_rid_stats(rid:str="230537"):
	''' '''
	return {"errorsum": redis_zsum(f"rid-{rid}:cate"), "wordsum":  redis_zsum(f"rid-{rid}:essay_wordnum")
	, "essaysum": redis.r.zcard(f"rid-{rid}:essay_score"), "sntsum": redis.r.zcard(f"rid-{rid}:snt_cola")	}
...
@app.get('/stream/soword')
def realtime_soword(rid:str="230537", word:str='gladsome', topk:int=10):
	''' FT.SEARCH ftsnt 'gladsome @rid:{230537}' limit 0 2 return 3 snt uid borntm  '''
	q = Query(f"{word} @rid:{{{rid}}}").paging(0, topk).return_fields("trps","snt","uid","borntm", "tags")
	res = ftsnt.search(q)
	si = Counter()
	[ si.update({ tag : 1}) for doc in res.docs for tag in doc.tags.split(',') ]
	return (res, si)
...
async def test_request(request: Request):
	return request.query_params.get('rid') #http://daka.wrask.com:16379/testreq?rid=one,two => "one,two"  | .strip('"')

@app.get("/sse/pubsub")
async def sse_pubsub(request: Request) -> StreamingResponse:
	ps	= redis.r.pubsub()    
	name = request.query_params.get('name','ws').strip('"')
	ps.subscribe(name)
	async def zsub(request: Request) -> Iterator[str]:
		for item in ps.listen():   
			if item['type'] == 'message':
				yield f"data:{item['data']}\n\n"
	return StreamingResponse(zsub(request), media_type="text/event-stream", headers={"Cache-Control": "no-cache","X-Accel-Buffering": "no"})
...
async def websocket_endpoint(websocket: WebSocket):
	...
	while True:
		...
		payload = time.time() #next(measurements)
...
async def message_stream(request: Request):
	...
	async def event_generator():
		while True:
			if await request.is_disconnected():	break 			# If client closes connection, stop sending events
			if new_messages(): # Checks for new messages and return them to client if any
				yield {"time": datetime.now().strftime("%H:%M:%S"),"id": "message_id","retry": RETRY_TIMEOUT,}

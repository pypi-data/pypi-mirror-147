...
APPNAME = 'apwn'
APPAUTHOR = 'adun' # based https://github.com/WinMin/ctfpwn-env

EXIST_FLAG = '/tmp/apwn.id'
DAEMON_PID = '/tmp/apwn.daemon.pid'
...
def parse_args():
    ...
    parser = argparse.ArgumentParser(
        description="apwn's pwn environment"
    )
...
def _read_container_name():
    if not os.path.exists(EXIST_FLAG):
        raise Exception('apwn is not running, consider use apwn run first')
    ...
    if container_name == '':
        ...
        raise Exception('apwn id file is  corrupted, or unable to read saved id file. ' + \
...
def run_pwn(args):
    ...
    try:
        ...
        print('apwn unable to run docker container')

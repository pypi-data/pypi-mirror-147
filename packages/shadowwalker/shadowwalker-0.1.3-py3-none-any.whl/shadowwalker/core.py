...
class ShadowWalker:
    ...
    def __init__(self, clash_bin=os.path.join(abs_dir(__file__), "clash.{}".format("exe" if IS_WIN32 else "bin")), exclude_country=["US", "CA", "CN"]):
    ...
    def start_clash(self, quiet: bool = False):
        if not self.proxies:

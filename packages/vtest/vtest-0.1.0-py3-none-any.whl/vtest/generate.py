...
class GeneratePlugin:
    ...
    def pytest_generate_tests(self, metafunc):
        if metafunc.definition.nodeid in self.case_param:
            if len(metafunc.fixturenames) and "parametrize" in metafunc.fixturenames:
                ...
                metafunc.fixturenames.append("__vtest_repeat")
                ids = [unitcase["case_id"] for unitcase in self.case_param[metafunc.definition.nodeid]]
                metafunc.parametrize('__vtest_repeat', ids, ids=ids)
            ...
            for case in self.case_param:
                if case in metafunc.definition.nodeid:
                    for i in range(len(self.case_param[case])):
                        ...
                        metafunc.parametrize('__vtest_repeat', ids, ids=ids)
                        return
            metafunc.parametrize('__vtest_repeat', [9999], ids=[9999])
    ...
    def pytest_terminal_summary(self, terminalreporter, exitstatus, config):
        ...
        # return
        if not getattr(pytest, "vtest_stats", None):
            pytest.vtest_stats = {}

        for stats_type in KNOWN_TYPES:
            if len(terminalreporter.stats.get(stats_type, [])) and stats_type in pytest.vtest_stats:
                terminalreporter.stats[stats_type] += pytest.vtest_stats[stats_type]
            elif not len(terminalreporter.stats.get(stats_type, [])) and stats_type in pytest.vtest_stats:
                terminalreporter.stats[stats_type] = pytest.vtest_stats[stats_type]
        pytest.vtest_stats = terminalreporter.stats

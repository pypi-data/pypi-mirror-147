...
def run_testset(args):
    ...
    if args.testbed:
        ...
        with open(args.testbed) as fd:
            vtest.global_testbed = yaml.load(fd, Loader=yaml.FullLoader)
    vtest.args = args
    vtest.run()


def main():
    parser = argparse.ArgumentParser(prog="vtest", usage="vtest specific parameter list", add_help=False)

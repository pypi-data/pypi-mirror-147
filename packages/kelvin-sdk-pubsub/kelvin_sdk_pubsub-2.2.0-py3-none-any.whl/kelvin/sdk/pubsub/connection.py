...
class Connection:
    ...
    def _make_messages(self, topic: str, payload: bytes) -> List[Message]:
        ...
        # veryfing target.node_name because mypy is complaining
        node_name = target.node_name if target and target.node_name else self.config.node_name
        workload_name = (
            target.workload_name if target and target.workload_name else self.config.workload_name
        )

...
def get_box_pro(image: np.ndarray, model: int = 1, regulator = None, thresh: int = 127):
    """
    ...
    Args:
        image: 四通道矩阵图像
        model: 返回值模式
        regulator: 提供一些边缘扩张接口，输入格式为list或者int:[up, down, left, right]。
    ...
    if isinstance(regulator, int):
        regulator = [0, 0, regulator, regulator]
    elif not isinstance(regulator, list):
        raise TypeError("regulator 必须为int或者list类型!")
    # ------------ 数据格式规范完毕 -------------- #
    # 分离mask
    ...
    # ------------ 开始输出数据 -------------- #
    height, width, _ = image.shape
    y_up = y - regulator[0] if y - regulator[0] >= 0 else 0
    y_down = y + h + regulator[1] if y + h + regulator[1] < height else height - 1
    x_left = x - regulator[2] if x - regulator[2] >= 0 else 0
    x_right = x + w + regulator[3] if x + w + regulator[3] < width else width - 1
    if model == 1:

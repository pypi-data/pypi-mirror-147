...
def memoized(method_or_arg_spec=None, ignore=None):
    ...
    * http://micheles.googlecode.com/hg/decorator/documentation.html
    ...
    assert sys.version_info < (3, 2), "Use Python-3.2 'functools.lru_cache' instead of 'memoized'"
...
class _memoized(object):
    def __init__(self, func, ignore=None):
        ...
        self.argspec = getargspec(func).args
...
class MetaBunch(MutableMapping):
    '''
    ...
    def __init__(self, dct=None):
        ...
        self.__dict__['dct'] = checked_type(Mapping, dct)
    ...
    def __getitem__(self, key):
        ...
        for key in self.keypath(key):
            checked_type(Mapping, retval)
            retval = retval[key]

        if isinstance(retval, Mapping) and \
...
class TemplateBunch(MutableMapping):

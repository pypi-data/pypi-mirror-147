...
def generate(dato_trainer: DatoTrainer, datasource_id: int, sample_output_ratio: float = 1) -> int:
    dato_trainer.wait()
    response_json = dato_trainer.dato_mapper.datomizer.get_response_json(
        ...
        data=json.dumps(create_synth_request(dato_trainer, datasource_id, sample_output_ratio)))

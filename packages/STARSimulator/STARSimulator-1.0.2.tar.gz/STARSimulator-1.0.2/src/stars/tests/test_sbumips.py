...
class TestSbumips(unittest.TestCase):
    ...
    def test_no_gui(self, mock_method: mock.patch.object):
        '''
            Tests the program with the --cli option. The intended effect is the program arguments
            are processed with a function call to run_CLI.
        '''
        sbumips.main(["--cli", "testfile.asm"])
        mock_method.assert_called_once()
    ...
    def test_run_cli(self, mock_stdout: StringIO):
        '''
            Tests the program with the --cli and -i option. The intended effect is the program 
            is executed and the asssembly file output along with the instruction count is printed 
            out.
        '''
        sbumips.main(["--cli", "-i", f"{p.dirname(__file__)}/test.asm"])
    ...
    def test_run_cli_error(self, mock_stderr: StringIO):
        ...
        '''
        sbumips.main(["--cli", "-i", f"{p.dirname(__file__)}/testfile.asm"])
    ...
    def test_run_cli_assemble(self):
        ...
        '''
        arguments = ["--cli", f"{p.dirname(__file__)}/instructions.asm"]

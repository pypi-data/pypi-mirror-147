...
def deploy_from_config():
    # _, team_id, _, _ = load_function_config()
    image_id, team_id, account_id, region = load_function_config()
    ...
    for function in deploy_config['functions']: 
        ...
        if function_id is None:
            ...
            response = create_function(function_id, team_id, name, description, image_uri)
            # print(response.text)
            # update the function id 
            function['id'] = function_id
        else:
            print(f"{BCOLORS.OKBLUE}found existing id for function {name}, update function{BCOLORS.ENDC}")
            response = update_function_request(team_id, function_id, image_uri)
            # print(response.text)
    with open(CONFIG_PATH, 'w') as file:
        yaml.dump(deploy_config, file)
    print(f"{BCOLORS.OKGREEN}deployment successful, update config file{BCOLORS.ENDC}")
...
def create_function(function_id, team_id, function_name, function_desc, image_uri):
    user_id, _ = get_user_login_info()
    _, _, account_id, region = get_authorization(team_id)
    # function_id = str(uuid.uuid4())
    image_id = str(uuid.uuid4())

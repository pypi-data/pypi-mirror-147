...
class StoppingEnv(gym.Env):
  def __init__(self):    
    ...
    self.observation_space = gym.spaces.Box(low=self.min, high=self.max, dtype=np.float32)      # S in [0, inf], tao in [0, 1]
  ...
  def step(self, action):
    if action == 1: # exercise
      ...
      if self.day_step == self.periods:    # at maturity
    ...
    self.state = np.array([self.S1, tao])

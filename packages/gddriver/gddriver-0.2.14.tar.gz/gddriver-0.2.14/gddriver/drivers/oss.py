...
class OSSConnection(base.Connection):
    def __init__(self, host, port, credential, timeout=None, **kwargs):
...
class OSSStorageDriver(base.StorageDriver):
    ...
    def upload_object_via_stream(self, connection, request):
        ...
        bucket = connection.get_client(container_name)
        if not multi:
            oss_result = bucket.put_object(object_name, wrapped_stream, headers=request.headers)
        else:
            # 分片上传
            logger.debug("%s, via multipart mode", msg)
            upload_id = bucket.init_multipart_upload(object_name, headers=request.headers).upload_id
            parts = []
            part_num = 0

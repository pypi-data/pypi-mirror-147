...
def isLianjiaPod(name):
    if str(name).startswith("LJ") or str(name).startswith("lj") or str(name).startswith(
            "Lianjia") or str(name).startswith("SocialShare") or str(name).startswith("Link"):
        return True
    return False
...
def checkopts():
    ...
    with open(lockPath, "r+") as fileReader:
        lockContent = yaml.safe_load(fileReader)

...
class PrsoxrLoader:
        ...
        with h5py.File((path+hdf5_name+'.hdf5'), 'a') as file_hdf5:
            ...
            if save_images:
                ...
                for index, scan in enumerate(self.files):
                    ...
                    try:
                        image_folder.create_dataset('IMAGE_RAW', data=self.images[index], compression=compress)
                        image_folder.create_dataset('IMAGE_LOCBEAM', data=self.image_locbeam[index], compression=compress)
                        image_folder.create_dataset('IMAGE_SPOT', data=self.image_spot[index], compression=compress)
                        image_folder.create_dataset('IMAGE_DARK', data=self.image_dark[index], compression=compress)
                    except RuntimeError:
                        pass
    ...
    def _reduce_2d_images(self, h=25, w=25, tol=2):
        ...
        for i, image in enumerate(self.images):
            ...
            beamcurrent = _vars['Beam Current'] if _vars[
                                                       'Beam Current'] > 100 else 1  # Ignore current if not measured

            image_spot, image_locbeam, beamspot, _ = slice_spot(image, h, w,
                                                    mask=self.mask, edge_trim=self.edge_trim,
                                                    diz_threshold=self.diz_threshold,
                                                    diz_size=self.diz_size
                                                    )
...
def slice_spot(image, h=20, w=20, mask=None, edge_trim=(5, 5), diz_threshold=10, diz_size=3):
    ...
    image_zinged, image_locbeam = dezinger_image(image[dx:-dx, dy:-dy], threshold=diz_threshold, size=diz_size)

    mask_trim = mask[dx:-dx, dy:-dy]  # Cut the edge off the mask
    image_zinged[~mask_trim] = 0  # Use Boolean to set all 'False' pixel to 0 for the sake of finding the image maximum
...
def slice_dark(image, h=20, w=20, mask=None, edge_trim=(5, 5), beamspot=(75, 75), darkside='LHS'):
    """
    Parameters
    ----------
        image : np.ndarray 

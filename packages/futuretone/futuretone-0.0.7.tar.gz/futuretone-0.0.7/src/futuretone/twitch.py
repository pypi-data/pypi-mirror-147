...
class FutureToneIntegrationBot(commands.Bot):
    ...
    async def apply_config(self, config: Settings):
        ...
        if config.global_.verbose:
            typer.echo(f'Applying {typer.style("melody icon", fg="green")} settings...')
        await self.ft.set_melody_icon_settings(config.game.melody_icons)

        if config.global_.verbose:
            typer.echo(f'Applying {typer.style("lyric", fg="green")} settings...')
        ...
        if config.global_.verbose:
            typer.echo(f'Applying {typer.style("1.00 patch", fg="green")} settings...')
    ...
    async def help(self, ctx: commands.Context):
        ...
        await ctx.send('I do things to the streamer\'s FutureTone! '
    ...
    @commands.command(name='showcommands', aliases=['commands', 'comms'])
    async def show_commands(self, ctx: commands.Context):
        ...
        if len(ctx.view.words) > 0 and ctx.view.words[1].lower() == 'all':
            await ctx.reply(', '.join([
                ...
                for k, c in self.commands.items() if not hasattr(c, '__hidden__')
            ]))
        else:

...
# # 🥭 EntropyInstructionReporter class
#
# The `EntropyInstructionReporter` class knows a bit more about Entropy instructions.
#
class EntropyInstructionReporter(InstructionReporter):
    def __init__(self, entropy_program_address: PublicKey) -> None:
        super().__init__()
        self.entropy_program_address: PublicKey = entropy_program_address

    def matches(self, instruction: TransactionInstruction) -> bool:
        return instruction.program_id == self.entropy_program_address
...
class CompoundInstructionReporter(InstructionReporter):
    ...
    @staticmethod
    def from_addresses(
        entropy_program_address: PublicKey, serum_program_address: PublicKey
    ) -> InstructionReporter:
        base: InstructionReporter = InstructionReporter()
        serum: InstructionReporter = SerumInstructionReporter(serum_program_address)
        entropy: InstructionReporter = EntropyInstructionReporter(
            entropy_program_address
        )
        return CompoundInstructionReporter([entropy, serum, base])

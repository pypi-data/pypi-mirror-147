...
class ContextBuilder:
    ...
    def add_command_line_parameters(
        ...
        parser.add_argument(
            "--group-name", type=str, default=None, help="Entropy group name"
        )
        parser.add_argument(
            "--group-address",
            type=PublicKey,
            default=None,
            help="Entropy group address",
        )
        parser.add_argument(
            "--entropy-program-address",
            type=PublicKey,
            default=None,
            help="Entropy program address",
        )
        parser.add_argument(
    ...
    def from_command_line_parameters(args: argparse.Namespace) -> Context:
        ...
        entropy_program_address: typing.Optional[
            PublicKey
        ] = args.entropy_program_address
    ...
    def build(
        ...
        first_group_data = EntropyConstants["groups"][0]
        actual_name: str = name or os.environ.get("NAME") or "Entropy Explorer"
        actual_cluster: str = (
            cluster_name
        ...
        for group_data in EntropyConstants["groups"]:
        ...
        if actual_cluster_urls is None or len(actual_cluster_urls) == 0:
            ...
            if (
                ...
                actual_cluster_urls = [
                    ClusterUrlData(rpc=EntropyConstants["cluster_urls"][actual_cluster])
                ]
        ...
        actual_program_address: PublicKey = (
            ...
            or PublicKey(found_group_data["entropyProgramId"])
        )
        ...
        if actual_cluster == "mainnet":
            ...
            #
            # Entropy uses "ETH" with a mint 2FPyTwcZLUg1MDrwsyoP4D6s1tM7hAkHYRjkNb5w6Pxk and for a long time

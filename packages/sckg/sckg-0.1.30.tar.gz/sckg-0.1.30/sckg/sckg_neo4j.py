...
class SoftwareKGSearcher:
    def __init__(self, neo4j_url, user, password):
        ...
        self.relation_weight = self.construct_relation_weight()
        self.concepts = self.construct_concepts()
    ...
    def get_id_by_concept_local(self, concept):
    ...
    def is_exist_concept_local(self, concept):
    ...
    def get_concept_labels(self, concept):
        query = f'MATCH (n:concept{{concept_name:{repr(concept)}}}) RETURN labels(n)'
        return self.graph.run(query).evaluate()

    def get_concept_labels_local(self, concept):
        if not self.is_exist_concept_local(concept):
            return None
    ...
    def is_action_local(self, word):
        labels = self.get_concept_labels_local(word)
        if not labels:
            return False
    ...
    def is_characteristic_local(self, word):
        labels = self.get_concept_labels_local(word)
        if not labels:
            return False
        return True if 'characteristic' in labels else False
    ...
    def is_concept_local(self, word):
        labels = self.get_concept_labels_local(word)
        if not labels:
            return False
        return False if 'action' in labels or 'characteristic' in labels else True
    ...
    def find_all_concept_from_sentence(self, sentence):
        ...
        for i, word in enumerate(words):
            ...
            for j in range(i + 1, i + 5):
                ...
                if self.is_exist_concept_local(concept):
                    concepts.append(concept)
        for word in words:
            if not self.is_concept_local(word):
                continue
    ...
    def find_longest_valid_concept_from_sentence(self, sentence):
        ...
        while index < len(words):
            ...
            for j in range(index + 1, index + 6):
                if j < len(words):
                    concept = concept + " " + words[j]
                    if self.is_concept_local(concept):
                        longest_concept = concept
                        index = j
                    else:
                        break
            if self.is_concept_local(longest_concept):
    ...
    def t_find_longest_valid_concept_from_sentence(self, sentence):
        # 给一段文本，返回在KG里面出现的有效最长概念
        concepts = []
        sentence = sentence.lower()
        sentence = sentence.translate(str.maketrans('', '', string.punctuation)).strip("\n")
        ...
        # words = sentence.strip("\n").lower().split()
        index = 0
        words = ['data' if i == 'datum' else i for i in words]
        while index < len(words):
            concept = words[index]
            longest_concept = concept
            for j in range(index + 1, index + 6):
            ...
            for j in range(index + 1, index + 6):
                if j < len(words):
                    ...
                    if self.is_concept_local(concept):
                        ...
                        break
            if self.is_concept_local(longest_concept) and longest_concept not in self.stopwords:
                concepts.append(longest_concept)
            index = index + 1
        return concepts
    ...
    def cul_in_related_node(self, concept):
        ...
        for in_concept in in_re:
            ...
            if in_concept[1] not in self.constant.relation_score:
                re_score = 0.6 * 0.6 + float(self.relation_weight[in_concept[1]])
            else:
                # 计算关系得分
                re_score = self.constant.relation_score[in_concept[1]] * 0.6 + float(
                    self.relation_weight[in_concept[1]]) * 0.4
            # 计算结点得分
    ...
    def cul_out_related_node(self, concept):
        ...
        for out_concept in out_re:
            ...
            if out_concept[1] not in self.constant.relation_score:
                re_score = 0.6 * 0.6 + float(self.relation_weight[out_concept[1]])
            else:
                re_score = self.constant.relation_score[out_concept[1]] * 0.6 + float(
                    self.relation_weight[out_concept[1]]) * 0.4
            # 计算结点得分

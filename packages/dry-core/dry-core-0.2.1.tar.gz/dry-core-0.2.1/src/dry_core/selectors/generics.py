...
class BaseAPISelector(BaseSelector[SelectorInstance], Generic[SelectorInstance]):
    ...
    def get(
        ...
        with cls.client() as client:
            ...
            if response.status_code != 200:
                if raise_exception:
                    raise NotFoundError(f'Error when trying get object (HTTP status code: {response.status_code}, response: {response.text}). '
    ...
    def list_get_all(
        ...
        results = []
        for part in cls.list_get_all_g(
                query_params=query_params,
    ...
    async def alist_get_all(
        ...
        results = []
        async for part in cls.alist_get_all_g(
                query_params=query_params,
                raise_exception=raise_exception,
                **kwargs
        ):

...
class Clusterer(object):
    ...
    def _fuzzy_k_protoypes(self, data, categorical_indices, n_clusters, m=2, max_iter=1000, error=0.005):
        ...
        for it in range(0,max_iter):
            ...
            #calculate cluster centers
            centers = self._FKP_centers(um, data, categorical_indices)
        
            #calculate distances
            dist = self._FKP_distances(centers, data, categorical_indices, numerical_metric='euclidean')
            dist = np.fmax(dist, np.finfo(np.float64).eps)
    ...
    def _FKP_centers(self, um, data, categorical_indices):  
        # if data is numerical, find the mean
        mn = um.dot(data) / (np.ones((data.shape[1], 1)).dot(np.atleast_2d(um.sum(axis=1))).T)
        
        # If data is categorical find the mode        
        md = np.matrix(np.ones((np.shape(mn))) * - np.inf)
        
        for col_idx in categorical_indices:
            cats = np.unique([data[col_idx]])          #Find unique categories
            for c in cats:
                s_um[c] = max(np.sum(um[np.where(data[col_idx]==c), col_idx]))
                
            md[col_idx] =  max(s_um, key=s_um.get)
        
        # Combine the means and the modes to find the cluster centers
        centers = np.maximum(mn, md)
    
        return centers
    
    def _FKP_distances(self, centers, data, categorical_indices, numerical_metric='euclidean'):
        
        # If the data is numerical use euclidean distance
        distances = cdist(centers, data, metric='euclidean')

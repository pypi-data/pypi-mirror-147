...
class AdultDay(registry.BaseHoliday):
    def _is_holiday(self, date):
        if date.year <= 1999 and date.month == 1 and date.day == 15:
            ...
            date.year >= 2000
            and date.month == 1
            and date.day == utils._week_day(date, 2, 1).day
        ):
            return True
        return False
...
class SeaDay(registry.BaseHoliday):
    def _is_holiday(self, date):
        ...
        # 2020: 国民の祝日に関する法律の一部を改正する法律(平成30年法律第57号)
            ...
            date.year >= 2003
            and date.month == 7
            and date.day == utils._week_day(date, 3, 1).day
        ):
            return True
    ...
    def __transfer_holiday_name(self, date):
        ...
        if (
            ...
            for holiday in registry.RegistryHolder.get_registry():
                ...
                if holiday.is_holiday(
                    (date + datetime.timedelta(days=-date.isoweekday()))
                ):
                    return "{} {}".format(
                        holiday.is_holiday_name(
                            (date + datetime.timedelta(days=-date.isoweekday()))
                        ),
                        "振替休日",
                    )
        ...
        for holiday in registry.RegistryHolder.get_registry():
            ...
            if holiday.is_holiday((date + datetime.timedelta(days=-1))):
                return "{} {}".format(
                    holiday.is_holiday_name((date + datetime.timedelta(days=-1))),
                    "振替休日",
                )

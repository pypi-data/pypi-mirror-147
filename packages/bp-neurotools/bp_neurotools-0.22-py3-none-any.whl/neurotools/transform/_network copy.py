...
class GroupDifNetwork(BaseEstimator, TransformerMixin):
    ...
    def _fit(self, X, y=None, fit_index=None):
        ...
        # The different residuals are not actually symmetric, e.g., 
        # i -> i, and j -> i won't give the same residuals, so
        # we need to actually fit every combination except same to same
        self.pairs_ = [(i, j) for i in range(X.shape[1]) for j in range(X.shape[1]) if i != j]
    ...
    def _transform(self, X, fit=False):
        ...
        # Otherwise, cast X_trans to full network matrix, which
        # just means re-shaping + adding in the diagonals as 0's
    ...
    def _to_matrix(self, X, X_trans):
        ...
        # Init empty network to fill, where 0's represent same to same implicitly
        # since we init w/ all zeros, and those are the only pairs that don't appear
        # in the saved pairs

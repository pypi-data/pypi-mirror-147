...
class KiLabel(KiWidget):
    def __init__(self, Master: KiWidget = None, Text: str = "Label", Padx=0, Pady=0,
                 Background: str = "white", Foreground: str = "black", Jusitfy: str = Jusitfy_Center,
                 Relief: str = Relief_Flat,
                 Width: int = 0, Height: int = 0):
        super().__init__()
        self.Widget = Label(master=Master, padx=Padx, text=Text, pady=Pady,
                            background=Background, foreground=Foreground, justify=Jusitfy, relief=Relief,
                            width=Width, height=Height)

    def Configure(self, Text: str = Always, Background: str = Always, Padx: int = Always, Pady: int = Always, Relief: str = Always,
                  Jusitfy: str = Always, Width: int = Always, Height: int = Always):
        if Background == Always:
            self.Widget.configure(background=self.CGet("background"))
        else:
            self.Widget.configure(background=Background)
        ...
        if Jusitfy == Always:
            ...
            self.Widget.configure(justify=Jusitfy)
        if Width == Always:
            self.Widget.configure(width=self.CGet("width"))
        else:
            self.Widget.configure(width=Width)
        if Height == Always:
            self.Widget.configure(height=self.CGet("height"))
        else:
...
class KiLabelFrame(KiFrame):
    def __init__(self, Master: KiWidget = None, Label: str = "Label",Background: str = "white",
                 Padx=0, Pady=0, Relief: str = Relief_Sunken, Width=0):
        super().__init__()
        self.Widget = LabelFrame(master=Master, background=Background, width=Width, text=Label,
                                 padx=Padx, pady=Pady)

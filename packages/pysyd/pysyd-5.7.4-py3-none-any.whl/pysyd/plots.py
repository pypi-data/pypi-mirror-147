...
def _plot_parameters(star, subfilename='background_only.png', filename='global_fit.png', n_peaks=5, cmap='binary',):
    ...
    _, x, y = utils.return_max(params['region_freq'], params['region_pow'])
    ax4.axvline([x], color='white', linestyle='--', linewidth=1.5, zorder=4)
    ax4.scatter([x], [y], s=25.0, edgecolor='cyan', marker='d', facecolor='none', linewidths=1.0, zorder=5)
    _, xx, yy = utils.return_max(params['new_freq'], params['numax_fit'])
    ...
    ax5.scatter(params['peaks_f'], params['peaks_p'], s=30.0, edgecolor='red', marker='s', facecolor='none', linewidths=1.0)
    ...
    ax6.scatter(params['obs_dnu'], max(params['dnu_fit']), s=45.0, edgecolor='lime', marker='s', facecolor='none', linewidths=1.0)

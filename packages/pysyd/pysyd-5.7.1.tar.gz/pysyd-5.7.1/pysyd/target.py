...
##########################################################################################
    ...
    def load_time_series(self, save=True, stitch=False, oversampling_factor=None,):
        ...
        if os.path.exists(os.path.join(self.params['inpdir'], '%s_LC.txt' % str(self.name))):
            ...
            self.params['data'].update({'time_orig':np.copy(time),'flux_orig':np.copy(flux),})
            ...
            if self.params['stitch']:
                self.stitch_data()
            self.params['data'].update({'time_fin':np.copy(time),'flux_fin':np.copy(flux)})
...
##########################################################################################
    ...
    def initial_parameters(self, lower_bg=1.0, upper_bg=8000.0,):
        ...
        self.i = 0
        for parameter in ['numax_smooth', 'A_smooth', 'numax_gauss', 'A_gauss', 'FWHM', 'dnu']:
    ...
    def solar_scaling(self, numax=None, scaling='tau_sun_single', max_laws=3, ex_width=1.0,
        ...
        self.params['plotting'][self.module].update({'exp_numax':self.params['exp_numax'],
                                                     'nlaws_orig':len(self.params['mnu']),
                                                     'mnu_orig':np.copy(self.params['mnu']),
    ...
    def model_background(self, n_laws=None, fix_wn=False, basis='tau_sigma',):
        ...
        self.params['plotting'][self.module].update({'bin_freq':np.copy(self.bin_freq),
                                                     'bin_pow':np.copy(self.bin_pow),
                                                     'bin_pow_err':np.copy(self.bin_err),
                                                     'a_orig':np.copy(self.params['a']),
                                                     'smooth_pow':np.copy(self.smooth_pow),
        ...
        try:
            ...
            if self.i == 0:
                ...
                self.params['plotting'][self.module].update({'pssm':np.copy(self.pssm),
                                                             'obs_numax':self.params['obs_numax'],
                                                             'new_freq':new_freq,
    ...
    def frequency_spacing(self, n_peaks=10,):
        ...
        if self.i == 0:
            ...
            self.peaks_l, self.peaks_a, weights = utils.max_elements(self.lag, self.auto, npeaks=self.params['n_peaks'], distance=self.params['exp_dnu']/4., exp_dnu=self.params['exp_dnu'])
            # Pick "best" peak in ACF (i.e. closest to expected dnu)
            idx , best_lag, best_auto = utils.return_max(self.peaks_l, self.peaks_a, exp_dnu=self.params['exp_dnu'])
        ...
        # if fit converged, save appropriate results
            ...
            if self.i == 0:
                ...
                self.peaks_f, self.peaks_p, _ = utils.max_elements(self.zoom_freq, self.zoom_pow, npeaks=self.params['n_peaks'], distance=self.params['obs_dnu']/4.)
                self.params['plotting'][self.module].update({'obs_dnu':gauss[2], 'weights':weights,
                  ...
                  'peaks_f':np.copy(self.peaks_f),'peaks_p':np.copy(self.peaks_p),})
    ...
    def _acf_cutout(self, threshold=1.0,):
        ...
        self.params['plotting'][self.module].update({'zoom_freq':np.copy(self.zoom_freq),
                  ...
                  'peaks_l':np.copy(self.peaks_l),'peaks_a':np.copy(self.peaks_a),
                  'best_lag':self.params['best_lag'],'best_auto':self.params['best_auto'],})

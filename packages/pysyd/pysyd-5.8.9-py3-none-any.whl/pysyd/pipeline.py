...
def setup(args, raw='https://raw.githubusercontent.com/ashleychontos/pySYD/master/dev/'):
    ...
    if not os.path.exists(outfile1):
        subprocess.call(['curl %s > %s'%('%sinfo/todo.txt'%raw, outfile1)], shell=True)
    ...
    if not os.path.exists(outfile2):
        subprocess.call(['curl %s > %s'%('%sinfo/star_info.csv'%raw, outfile2)], shell=True)
        note+=' - saved an example for the star information file\n'
    # option to get all columns since only subset is included in example
    if args.makeall:
    ...
    if dl:
        # downloading example data will generate output in terminal, so include this statement regardless
    ...
    if args.verbose:
        if note == '':
            print("\nLooks like you've probably done this before since you\nalready have all these files!")
        else:
            print('\nOther notes:\n%s'%note)

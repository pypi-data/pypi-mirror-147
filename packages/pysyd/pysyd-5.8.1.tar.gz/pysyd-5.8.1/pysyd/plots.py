...
def _plot_parameters(star, subfilename='background_only.png', filename='global_fit.png', n_peaks=5, cmap='binary',):
    ...
    if star.params['background']:
        ...
        for r in range(params['nlaws_orig']):
            ...
            ax2.plot(params['frequency'], model, color='dodgerblue', linestyle=':', linewidth=1.5, zorder=4)
            total += model
        total += params['noise']
        ax2.plot(params['frequency'], total, color='dodgerblue', linewidth=2., zorder=5)
    ...
    if star.params['background']:
        ...
        if len(params['pars']) > 1:
            for r in range(len(params['pars'])//2):
                ...
                ax3.plot(params['frequency'], yobs, color='dodgerblue', linestyle=':', linewidth=1.5, zorder=4)
                total += yobs
        if len(params['pars'])%2 == 0:
            total += params['noise']
            ax3.axhline(params['noise'], color='dodgerblue', linestyle='dashed', linewidth=1.5, zorder=3, dashes=(5,5))
        else:
            total += params['pars'][-1]
            ax3.axhline(params['pars'][-1], color='dodgerblue', linestyle='dashed', linewidth=1.5, zorder=3, dashes=(5,5))
        ax3.plot(params['frequency'], total, color='dodgerblue', linewidth=2., zorder=5)

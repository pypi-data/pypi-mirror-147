...
def _plot_parameters(star, subfilename='background_only.png', filename='global_fit.png', n_peaks=5, cmap='binary',):
    ...
    ax2.plot(params['frequency'][params['frequency'] < star.params['ps_mask'][0]], params['smooth_pow'][params['frequency'] < star.params['ps_mask'][0]], color='blue', linestyle='-', linewidth=0.75, zorder=2)
    ax2.plot(params['frequency'][params['frequency'] > star.params['ps_mask'][1]], params['smooth_pow'][params['frequency'] > star.params['ps_mask'][1]], color='blue', linestyle='-', linewidth=0.75, zorder=2)
    if star.params['background']:
        ...
        for r in range(params['nlaws_orig']):
            ...
            ax2.plot(params['frequency'], model, color='darkorange', linestyle=':', linewidth=1.5, zorder=4)
            total += model
        total += params['noise']
        ax2.plot(params['frequency'], total, color='darkorange', linewidth=2., zorder=5)
    ...
    ax2.axvline(star.params['ps_mask'][0], color='red', linestyle='dashed', linewidth=2.0, zorder=1, dashes=(5,5))
    ax2.axvline(star.params['ps_mask'][1], color='red', linestyle='dashed', linewidth=2.0, zorder=1, dashes=(5,5))
    ax2.axhline(params['noise'], color='darkorange', linestyle='dashed', linewidth=1.5, zorder=3, dashes=(5, 5))
    ...
    ax3.plot(params['frequency'][params['frequency'] < star.params['ps_mask'][0]], params['smooth_pow'][params['frequency'] < star.params['ps_mask'][0]], color='blue', linestyle='-', linewidth=0.75, zorder=2)
    ax3.plot(params['frequency'][params['frequency'] > star.params['ps_mask'][1]], params['smooth_pow'][params['frequency'] > star.params['ps_mask'][1]], color='blue', linestyle='-', linewidth=0.75, zorder=2)
    if star.params['background']:
        total = np.zeros_like(params['frequency'])
        if len(params['pars']) > 1:
            for r in range(len(params['pars'])//2):
                ...
                ax3.plot(params['frequency'], yobs, color='magenta', linestyle=':', linewidth=1.5, zorder=4)
                total += yobs
        if len(params['pars'])%2 == 0:
            total += params['noise']
            ax3.axhline(params['noise'], color='magenta', linestyle='dashed', linewidth=1.5, zorder=3, dashes=(5,5))
        else:
            total += params['pars'][-1]
            ax3.axhline(params['pars'][-1], color='magenta', linestyle='dashed', linewidth=1.5, zorder=3, dashes=(5,5))
        ax3.plot(params['frequency'], total, color='magenta', linewidth=2., zorder=5)
        ...
        ax3.axhline(params['noise'], color='magenta', linestyle='dashed', linewidth=1.5, zorder=3, dashes=(5,5))
    ax3.axvline(star.params['ps_mask'][0], color='red', linestyle='dashed', linewidth=2.0, zorder=1, dashes=(5,5))
    ax3.axvline(star.params['ps_mask'][1], color='red', linestyle='dashed', linewidth=2.0, zorder=1, dashes=(5,5))
    ...
    ax4.scatter([f], [p], s=50.0, edgecolor='orange', marker='x', facecolor='none', linewidths=1.0, zorder=5)
...
def _plot_bgfits(star, filename='bgmodel_fits.png', highlight=True):
    ...
    if star.params['background']:
        ...
        for r in range(params['nlaws_orig']):
            model = models.background(params['frequency'], [params['b_orig'][r], params['a_orig'][r]])
            ax1.plot(params['frequency'], model, color='cyan', linestyle=':', linewidth=1.5, zorder=4)
            total += model
        total += params['noise']
        ax1.plot(params['frequency'], total, color='cyan', linewidth=2., zorder=5)
    ...
    ax1.axhline(params['noise'], color='cyan', linestyle='dashed', linewidth=1.5, zorder=3, dashes=(5, 5))

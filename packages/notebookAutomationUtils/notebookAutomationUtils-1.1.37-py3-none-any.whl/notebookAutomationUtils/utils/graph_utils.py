...
class GraphUtils:
    ...
    def set_sobjects(self, obj__:object):  
      self.sobjects = obj__
      # Add random color for object if it's not there
      for obj in self.sobjects:
        if obj not in self.objects_to_colors.keys():
          self.objects_to_colors['{obj}'] = "#"+''.join([random.choice('0123456789ABCDEF') for j in range(6)])

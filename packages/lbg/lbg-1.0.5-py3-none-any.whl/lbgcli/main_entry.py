    ...
    def check_version(self):
        ...
        try:
            ...
            if current_strict < latest_strict:
                warn = f"version is out of date. current:{current_strict} latest:{latest_strict}."
                warn += " To update run: 'pip install -U lbg'. To ignore this warning run 'lbg config version -h' for more information"
                if current_strict.major < latest_strict.major and level <= 2:
                    self.print(warn)
                    return
                if current_strict.minor < latest_strict.minor and level <= 1:
                    self.print(warn)
                    return
                if current_strict.micro < latest_strict.micro and level <= 0:
                    self.print(warn)
                    return
        except Exception as e:
            # traceback.print_exc()

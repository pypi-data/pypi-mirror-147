    ...
    try:
        for key, val in six.iteritems(new_config):
            ...
            if key == 'PYTHON_PROTOBUF_VERSION_2':
                import os
                protobuf_2_env_var = os.environ.get('PROTOBUF_VERSION_2')

                if protobuf_2_env_var:
                    logger.debug("Updating PROTOBUF_VERSION_2 value from env var to: " + str(protobuf_2_env_var in _TRUE_VALUES))
                    DumpConfig.PROTOBUF_VERSION_2 = protobuf_2_env_var in _TRUE_VALUES
                else:
                    logger.debug("Updating PROTOBUF_VERSION_2 value to: " + str(val in _TRUE_VALUES))

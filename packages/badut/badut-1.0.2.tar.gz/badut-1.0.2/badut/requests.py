...
class Badut(object):
    ...
    def __init__(self, json: bool = False, headers: dict = {}):
        ...
        if json and headers != {}:
            ...
            self.json = True
            self.headers = {"User-Agent": "Badut/1.0.1"}
        else:
            self.json = False
            self.headers = {"User-Agent": "Badut/1.0.1"}

    async def get(self, target: str, timeout: int = None):
        """Making a GET request to the target URL
    ...
    async def post(self, target: str, data: dict):
        """Making a POST request to the target URL
